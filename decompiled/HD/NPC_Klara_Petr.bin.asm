...
0x85c: @@ HasProperty(Stack[-1], Stack[-4])
0x85d: Pop(1)
...
0x887: @@ HasProperty(Stack[-1], Stack[-4])
0x888: Pop(1)
...
0x8af: Push("Can't find lsh animation : ")
...
0x8fb: @@ GetItemID(Stack[-3])
0x8fc: Pop(0)
0x8fd: Push("Category")
0x8fe: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0x8ff: Pop(1)
0x900: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7])
0x901: Pop(0)
0x902: Pop(0); Push((bool) Stack[-1] == 0)
0x903: IF (Stack[-1] == 0) GOTO 0x907; Pop(1)

0x904: @@ DropItems(Stack[-8], Stack[-7])
0x905: Pop(0)
0x906: GOTO 0x90c

0x907: PushEmpty(int, int)
0x908: Stack[-2] = Stack[-5]
0x909: Stack[-1] = Stack[-9]
0x90a: Call2 0x8ee
...
0x90f: Pop(0)
0x910: @@ SetItemName(Stack[-4])
0x911: Pop(0)
0x912: PushEmpty(object, object, int)
0x913: Stack[-3] = Stack[-8]
0x914: Stack[-2] = Stack[-4]
0x915: Stack[-1] = Stack[-6]
0x916: Call2 0x8fa
...
0x91c: Pop(0)
0x91d: Pop(0); Push((bool) Stack[-1] == 0)
...
0x956: Stack[-1] = "ui/NPC_Petr.png"
0x957: Return(); Pop(0)

0x958: Stack[-1] = "ui/NPC_Petr_b.png"
0x959: Return(); Pop(0)
...
0x964: Pop(1)
0x965: Push("k5q02PetrGotoAndrei")
0x966: Push("pt_map_andrei")
0x967: Push((int) 0)
...
0x96b: Pop(0)
0x96c: @@ AddMark(Stack[-5], Stack[-4], Stack[-3], Stack[-2], Stack[-1])
0x96d: Pop(5)
0x96e: PushEmpty()
0x96f: Call2 0x9d7
...
0x972: Stack[-2] = "quest_k5_02"
0x973: Stack[-1] = "init_kabak"
0x974: Call2 0x91a
...
0x981: PushEmpty()
0x982: Push("tvirin 5 is given")
0x983: @ Trace(Stack[-1])
0x984: Pop(1)
0x985: PushEmpty(object, string, int)
0x986: Stack[-3] = Stack[-5]
0x987: Stack[-2] = "tvirin"
0x988: Stack[-1] = (int) 5
0x989: Call2 0x90d
...
0x998: PushEmpty()
0x999: Push("tvirin is given")
0x99a: @ Trace(Stack[-1])
0x99b: Pop(1)
0x99c: PushEmpty(object, string, int)
0x99d: Stack[-3] = Stack[-5]
0x99e: Stack[-2] = "tvirin"
0x99f: Stack[-1] = (int) 1
0x9a0: Call2 0x90d
...
0x9f6: Push("Can't retrieve diary root")
0x9f7: @ Trace(Stack[-1])
0x9f8: Pop(1)
0x9f9: Stack[-3] = (bool) 0
0x9fa: Return(); Pop(2)

0x9fb: Stack[-3] = Stack[-1]
0x9fc: Return(); Pop(2)
...
0xa02: Pop(1)
0xa03: @@ Find(Stack[-7], Stack[-2])
0xa04: Pop(0)
0xa05: Pop(0); Push((bool) Stack[-2] == 0)
0xa06: IF (Stack[-1] == 0) GOTO 0xa0d; Pop(1)

0xa07: Push("Can't find diary parent with id: ")
...
0xa0d: @@ AddChild(Stack[-8])
0xa0e: Pop(0)
0xa0f: Push((int) 7)
0xa10: @ SendWorldWndMessage(Stack[-1])
0xa11: Pop(1)
0xa12: @@ GetCategory(Stack[-1])
0xa13: Pop(0)
...
0xa1c: Pop(0)
0xa1d: Pop(0); Push((bool) Stack[-2] == 0)
0xa1e: IF (Stack[-1] == 0) GOTO 0xa25; Pop(1)

0xa1f: Push("Can't find main outdoor scene")
0xa20: @ Trace(Stack[-1])
0xa21: Pop(1)
0xa22: Stack[-1] = 0
0xa23: Stack[-5] = Stack[-1]
0xa24: Return(); Pop(4)

0xa25: @@ GetMap(Stack[-1])
0xa26: Pop(0)
...
0xa30: Push("Can't find main outdoor scene")
0xa31: @ Trace(Stack[-1])
0xa32: Pop(1)
0xa33: Return(); Pop(8)

0xa34: @@ GetLocator(Stack[-10], Stack[-1], Stack[-4], Stack[-3])
0xa35: Pop(0)
0xa36: Pop(0); Push((bool) Stack[-1] == 0)
0xa37: IF (Stack[-1] == 0) GOTO 0xa3e; Pop(1)

0xa38: Push("Warning: outdoor scene locator ")
0xa39: Pop(1); Push(Stack[-1] + Stack[-11]);
0xa3a: Push(" doesnt exist")
0xa3b: Pop(2); Push(Stack[-2] + Stack[-1]);
0xa3c: @ Trace(Stack[-1])
0xa3d: Pop(1)
0xa3e: @@ GetMap(Stack[-11])
0xa3f: Pop(0)
0xa40: Pop(0); Push((bool) Stack[-11] == 0)
0xa41: IF (Stack[-1] == 0) GOTO 0xa46; Pop(1)

0xa42: Push("Can't find map")
0xa43: @ Trace(Stack[-1])
0xa44: Pop(1)
0xa45: Return(); Pop(8)
...
0xa48: @@ SetMapParams(Stack[-2], Stack[-1], Stack[-11])
0xa49: Pop(2)
0xa4a: Return(); Pop(8)
