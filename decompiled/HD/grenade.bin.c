...
Label_58:
	...
	var_130_object = GlobalVars[0]; // 0x45 PushGE
	var_131_bool = var_130_object != var_17_object; // 0x46 Neq
	if(var_131_bool == 0) goto Label_76; // 0x47 JumpB
	var_132_object = GlobalVars[0]; // 0x48 PushGE
	var_133_int = 3; // 0x49 PushI
	ReportHit(var_17_object, var_132_object, var_133_int, var_21_float, var_20_float); // 0x4a Func
	
Label_76:
	var_134_string = "Hit: "; // 0x4c PushS
	var_135_int = var_134_string + var_17_object; // 0x4d Add
	Trace(var_135_int); // 0x4e Func
	goto Label_32; // 0x50 Jump
	
Label_81:
	var_15_object = 0; // 0x51 SetNull
	SetPosition(var_2_cvector); // 0x52 Func
	var_136_string = "grenade"; // 0x54 PushS
	FindGeometry(var_136_string, var_22_object); // 0x55 Func
	var_137_bool = 0; // 0x57 PushB
	Enable(var_137_bool); // 0x58 ObjFunc
	var_138_string = "grenadefire"; // 0x5a PushS
	FindParticleSystem(var_138_string, var_23_object); // 0x5b Func
	var_139_bool = var_23_object == 0; // 0x5d NullEq
	if(var_139_bool == 0) goto Label_99; // 0x5e JumpB
	var_140_string = "Can't find fire particle system"; // 0x5f PushS
	Trace(var_140_string); // 0x60 Func
	return 20; // 0x62 Return
	
Label_99:
	var_141_cvector = CVector(0.0, 0.0, 0.0); // 0x63 PushVec
	var_142_cvector = CVector(0.0, 1.0, 0.0); // 0x64 PushVec
	var_143_int = 0; // 0x65 PushI
	AddSource(var_141_cvector, var_142_cvector, var_143_int); // 0x66 ObjFunc
	var_144_cvector = CVector(50.0, 0.0, 50.0); // 0x68 PushVec
	var_145_cvector = CVector(0.0, 1.0, 0.0); // 0x69 PushVec
	var_146_int = 0; // 0x6a PushI
	AddSource(var_144_cvector, var_145_cvector, var_146_int); // 0x6b ObjFunc
	var_147_cvector = CVector(-50.0, 0.0, 50.0); // 0x6d PushVec
	var_148_cvector = CVector(0.0, 1.0, 0.0); // 0x6e PushVec
	var_149_int = 0; // 0x6f PushI
	AddSource(var_147_cvector, var_148_cvector, var_149_int); // 0x70 ObjFunc
	var_150_cvector = CVector(-50.0, 0.0, -50.0); // 0x72 PushVec
	var_151_cvector = CVector(0.0, 1.0, 0.0); // 0x73 PushVec
	var_152_int = 0; // 0x74 PushI
	AddSource(var_150_cvector, var_151_cvector, var_152_int); // 0x75 ObjFunc
	var_153_cvector = CVector(50.0, 0.0, -50.0); // 0x77 PushVec
	var_154_cvector = CVector(0.0, 1.0, 0.0); // 0x78 PushVec
	var_155_int = 0; // 0x79 PushI
	AddSource(var_153_cvector, var_154_cvector, var_155_int); // 0x7a ObjFunc
	var_156_cvector = CVector(25.0, 0.0, 25.0); // 0x7c PushVec
	var_157_cvector = CVector(0.0, 1.0, 0.0); // 0x7d PushVec
	var_158_int = 0; // 0x7e PushI
	AddSource(var_156_cvector, var_157_cvector, var_158_int); // 0x7f ObjFunc
	var_159_cvector = CVector(-25.0, 0.0, 25.0); // 0x81 PushVec
	var_160_cvector = CVector(0.0, 1.0, 0.0); // 0x82 PushVec
	var_161_int = 0; // 0x83 PushI
	AddSource(var_159_cvector, var_160_cvector, var_161_int); // 0x84 ObjFunc
	var_162_cvector = CVector(-25.0, 0.0, -25.0); // 0x86 PushVec
	var_163_cvector = CVector(0.0, 1.0, 0.0); // 0x87 PushVec
	var_164_int = 0; // 0x88 PushI
	AddSource(var_162_cvector, var_163_cvector, var_164_int); // 0x89 ObjFunc
	var_165_cvector = CVector(25.0, 0.0, -25.0); // 0x8b PushVec
	var_166_cvector = CVector(0.0, 1.0, 0.0); // 0x8c PushVec
	var_167_int = 0; // 0x8d PushI
	AddSource(var_165_cvector, var_166_cvector, var_167_int); // 0x8e ObjFunc
	Enable(); // 0x90 ObjFunc
	var_168_bool = 0; // 0x92 PushB
	Fade(var_168_bool); // 0x93 ObjFunc
	var_169_object = GlobalVars[4]; // 0x95 PushGE
	Fade(); // 0x96 ObjFunc
	var_170_float = 10.0; // 0x98 PushF
	Sleep(var_170_float); // 0x99 Func
	Fade(); // 0x9b ObjFunc
	var_171_bool = var_14_object != 0; // 0x9d NullNeq
	if(var_171_bool == 0) goto Label_162; // 0x9e JumpB
	var_172_float = 1.0; // 0x9f PushF
	FadeOut(var_172_float); // 0xa0 ObjFunc
	
Label_162:
	var_173_float = 2.0; // 0xa2 PushF
	Sleep(var_173_float); // 0xa3 Func
	var_174_object = Obj(); // 0xa5 PushV
	func_344(var_174_object); // 0xa6 NEW_2
	RemoveActor(var_174_object); // 0xa8 Func
...
Label_290:
	var_129_string = "armor"; // 0x122 PushS
	GetProperty(var_129_string, var_70_int); // 0x123 ObjFunc
}
...
func_373(var_121_float)
{
	...
	var_124_int = 0; // 0x17a PushI
	var_125_bool = var_121_float < var_124_int; // 0x17b LT
	if(var_125_bool == 0) goto Label_385; // 0x17c JumpB
	var_126_float = 0.7; // 0x17d PushF
	var_127_int = 500; // 0x17e PushI
	RumblePlay(var_126_float, var_127_int); // 0x17f Func
	
Label_385:
	var_128_int = 15; // 0x181 PushI
	SendWorldWndMessage(var_128_int, var_123_object); // 0x182 Func
