...
Strings:
	...
	W:Category
	A:AddItem
// @pool_raw:
...
0x21: Push("object") // @poff=16
0x22: @ FindGeometry(Stack[-1], Stack[-2])
0x23: Pop(1)
0x24: Push((bool) 1)
0x25: @@ Enable(Stack[-1]); Obj=2 // @poff=30
0x26: Pop(1)
0x27: @ Hold()
0x28: Pop(0)
0x29: GOTO 0x27
...
0x56: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=61
0x57: Pop(0)
0x58: Stack[-1] = Stack[-5]
0x59: Return(); Pop(2)

0x5a: PushEmpty(bool, bool)
0x5b: @@ IsDead(Stack[-1]); Obj=3 // @poff=73
0x5c: Pop(0)
0x5d: Stack[-1] = Stack[-4]
0x5e: Return(); Pop(2)

0x5f: PushEmpty(object, object, object, object)
0x60: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x74: Pop(0)
0x75: Pop(0); PushNull((bool) Stack[-2] == 0)
0x76: IF (Stack[-1] == 0) GOTO 0x79; Pop(1)

0x77: Stack[-6] = (bool) 0
0x78: Return(); Pop(4)

0x79: @@ GetScene(Stack[-1]); Obj=5 // @poff=94
0x7a: Pop(0)
...
0x8d: Stack[-6] = Stack[-2]
0x8e: Stack[-1] = "noaccess" // @poff=103
0x8f: Call2 0x4e
...
0x95: Push("noaccess") // @poff=103
0x96: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=121
0x97: Pop(1)
0x98: Push((int) 0)
...
0xa3: Pop(0)
0xa4: @@ add(Stack[-4]); Obj=1 // @poff=133
0xa5: Pop(0)
0xa6: @@ add(Stack[-3]); Obj=1 // @poff=133
0xa7: Pop(0)
0xa8: Push((int) 3)
...
0xae: @@ GetItemID(Stack[-3]); Obj=8 // @poff=137
0xaf: Pop(0)
0xb0: Push("Category") // @poff=147
0xb1: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0xb2: Pop(1)
0xb3: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7]); Obj=9 // @poff=165
0xb4: Pop(0)
0xb5: Push(Stack[-1])
0xb6: IF (Stack[-1] == 0) GOTO 0xbc; Pop(1)

0xb7: PushEmpty(int, int)
0xb8: Stack[-5] = Stack[-2]
0xb9: Stack[-9] = Stack[-1]
0xba: Call2 0xa1

0xbb: Pop(2)
0xbc: Stack[-1] = Stack[-10]
0xbd: Return(); Pop(6)
