...
Strings:
	...
	W:Category
	A:AddItem
// @pool_raw:
...
0x39: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=89
0x3a: Pop(0)
0x3b: Stack[-1] = Stack[-5]
0x3c: Return(); Pop(2)

0x3d: PushEmpty(bool, bool)
0x3e: @@ IsDead(Stack[-1]); Obj=3 // @poff=101
0x3f: Pop(0)
0x40: Stack[-1] = Stack[-4]
0x41: Return(); Pop(2)

0x42: PushEmpty(object, object, object, object)
0x43: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x57: Pop(0)
0x58: Pop(0); PushNull((bool) Stack[-2] == 0)
0x59: IF (Stack[-1] == 0) GOTO 0x5c; Pop(1)

0x5a: Stack[-6] = (bool) 0
0x5b: Return(); Pop(4)

0x5c: @@ GetScene(Stack[-1]); Obj=5 // @poff=122
0x5d: Pop(0)
...
0x70: Stack[-6] = Stack[-2]
0x71: Stack[-1] = "noaccess" // @poff=131
0x72: Call2 0x31
...
0x78: Push("noaccess") // @poff=131
0x79: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=149
0x7a: Pop(1)
0x7b: Push((int) 0)
...
0x86: Pop(0)
0x87: @@ add(Stack[-4]); Obj=1 // @poff=161
0x88: Pop(0)
0x89: @@ add(Stack[-3]); Obj=1 // @poff=161
0x8a: Pop(0)
0x8b: Push((int) 3)
...
0x91: @@ GetItemID(Stack[-3]); Obj=8 // @poff=165
0x92: Pop(0)
0x93: Push("Category") // @poff=175
0x94: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0x95: Pop(1)
0x96: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7]); Obj=9 // @poff=193
0x97: Pop(0)
0x98: Push(Stack[-1])
0x99: IF (Stack[-1] == 0) GOTO 0x9f; Pop(1)

0x9a: PushEmpty(int, int)
0x9b: Stack[-5] = Stack[-2]
0x9c: Stack[-9] = Stack[-1]
0x9d: Call2 0x84

0x9e: Pop(2)
0x9f: Stack[-1] = Stack[-10]
0xa0: Return(); Pop(6)
