...
0xe: Push("object") // @poff=0
0xf: @ FindGeometry(Stack[-1], Stack[-2])
0x10: Pop(1)
0x11: Push((bool) 1)
0x12: @@ Enable(Stack[-1]); Obj=2 // @poff=14
0x13: Pop(1)
0x14: @ Hold()
0x15: Pop(0)
0x16: GOTO 0x14
...
0x28: Stack[-9] = Stack[-3]
0x29: Stack[-6] = Stack[-2]
0x2a: Stack[-5] = Stack[-1]
0x2b: Call2 0x9a
...
0x43: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=45
0x44: Pop(0)
0x45: Stack[-1] = Stack[-5]
0x46: Return(); Pop(2)

0x47: PushEmpty(bool, bool)
0x48: @@ IsDead(Stack[-1]); Obj=3 // @poff=57
0x49: Pop(0)
0x4a: Stack[-1] = Stack[-4]
0x4b: Return(); Pop(2)

0x4c: PushEmpty(object, object, object, object)
0x4d: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x61: Pop(0)
0x62: Pop(0); PushNull((bool) Stack[-2] == 0)
0x63: IF (Stack[-1] == 0) GOTO 0x66; Pop(1)

0x64: Stack[-6] = (bool) 0
0x65: Return(); Pop(4)

0x66: @@ GetScene(Stack[-1]); Obj=5 // @poff=78
0x67: Pop(0)
...
0x7a: Stack[-6] = Stack[-2]
0x7b: Stack[-1] = "noaccess" // @poff=87
0x7c: Call2 0x3b
...
0x82: Push("noaccess") // @poff=87
0x83: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=105
0x84: Pop(1)
0x85: Push((int) 0)
...
0x90: Pop(0)
0x91: @@ add(Stack[-4]); Obj=1 // @poff=117
0x92: Pop(0)
0x93: @@ add(Stack[-3]); Obj=1 // @poff=117
0x94: Pop(0)
0x95: Push((int) 3)
...
0x9b: @@ GetItemID(Stack[-3]); Obj=8 // @poff=121
0x9c: Pop(0)
0x9d: Push("Category") // @poff=131
0x9e: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0x9f: Pop(1)
0xa0: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7]); Obj=9 // @poff=149
0xa1: Pop(0)
0xa2: Push(Stack[-1])
...
0xa6: Stack[-9] = Stack[-1]
0xa7: Call2 0x8e

0xa8: Pop(2)
0xa9: Stack[-1] = Stack[-10]
0xaa: Return(); Pop(6)
