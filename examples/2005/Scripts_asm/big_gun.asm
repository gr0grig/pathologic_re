GlobalVarCount = 0

Strings:
	W:Gun blocking polygon: 
	A:BlockPolygons
// @pool_raw:470075006e00200062006c006f0063006b0069006e006700200070006f006c00790067006f006e003a0020000000426c6f636b506f6c79676f6e7300

Import:
	GetPFPolyID (1 args)
	Trace (1 args)
	GetMainOutdoorScene (1 args)
	Hold (0 args)
	SetVisibility (1 args)
	StopGroup0 (0 args)

RunOp = 0x0
RunTask = 0

GlobalTasks: 
	GTASK_0  Params = 0
		EVENT_5 Op = 0x15 Vars = ()


0x0: PushEmpty(int, object, int, object)
0x1: @ GetPFPolyID(Stack[-2])
0x2: Pop(0)
0x3: Push((int) -1)
0x4: Pop(1); Push((bool) Stack[-3] != Stack[-1])
0x5: IF (Stack[-1] == 0) GOTO 0x12; Pop(1)

0x6: Push("Gun blocking polygon: ") // @poff=0
0x7: Push((int) 1016)
0x8: Pop(1); Push(Stack[-4] & Stack[-1]);
0x9: Pop(2); Push(Stack[-2] + Stack[-1]);
0xa: @ Trace(Stack[-1])
0xb: Pop(1)
0xc: @ GetMainOutdoorScene(Stack[-1])
0xd: Pop(0)
0xe: Push((int) 1016)
0xf: @@ BlockPolygons(Stack[-3], Stack[-1]); Obj=2 // @poff=46
0x10: Pop(1)
0x11: Stack[-1] = 0
0x12: @ Hold()
0x13: Pop(0)
0x14: Return(); Pop(4)

0x15: Push((bool) 1)
0x16: @ SetVisibility(Stack[-1])
0x17: Pop(1)
0x18: @ StopGroup0()
0x19: Pop(0)
0x1a: Return(); Pop(0)

