GlobalVarCount = 1
	G_VAR_0 bool 

Strings:
	W:fdie
	A:GetPosition
	W:bdie
	W:GetScriptProperty
	W:Owner
	A:HasScriptProperty
	A:GetScriptProperty
	W:@GetEyesHeight
	A:GetEyesHeight
	W:head
	W:all
	W:@GetAttackDistance
	A:GetAttackDistance
	W:attack
	A:GetPFPosition
	W:attack_on
	W:attack_stay
	W:fjump
	W:attack_off
	W:attack_begin
	W:bjump
	W:attack_middle
	W:_
	W:attack_end
	W:IsAttacking
	A:IsAttacking
	W:walk
	W:run
	W:fire
	W:bullet
	W:phys
	W:HasProperty
	A:HasProperty
	W:health
	W:armor
	A:GetProperty
	W:armor_
	A:SetProperty
	A:IsDead
	W:IsDead
	A:GetScene
	W:noaccess
	W:fhit
	W:bhit
	W:hit_react
	W:1
	W:2
	A:add
	A:size
	A:get
	W:scripted
	W:blood_dir.xml
	W:battle
	W:branch
	W:GenerateMoney: iMin > iMax
	W:Money
	A:SetItemName
	W:Organ
	A:GetItemID
	W:liver
	W:kidney
	W:heart
	W:blood
	W:hook
	W:watch
	W:alpha_pills
	W:meradorm
	W:beta_pills
	W:monomicin
	W:lockpick
	W:rifle_ammo
	W:revolver_ammo
	W:samopal_ammo
	W:unholster
	W:player_shot
	A:IsWeaponHolstered
	W:heal
	W:player
	W:idle
	W:class
	W:rat
	W:rat_big
	W:dog
	W:grabitel
	W:bomber
	W:sanitar
	W:hunter
	W:soldier
// @pool_raw:

Import:
	Hold (0 args)
	GetPosition (1 args)
	GetDirection (1 args)
	RemoveRTEnvelope (0 args)
	SetDeathState (0 args)
	Stop (0 args)
	StopAsync (0 args)
	LookAsync (3 args)
	PlayAnimation (2 args)
	WaitForAnimEnd (0 args)
	UnlookAsync (1 args)
	LockAnimationEnd (2 args)
	RemoveEnvelope (0 args)
	StopSecondaryAnimation (0 args)
	SensePlayerOnly (1 args)
	KillTimer (1 args)
	ResetAAS (0 args)
	IsPlayerActor (2 args)
	SetTimer (2 args)
	rand (2 args)
	Sleep (1 args)
	GetRandomPFPointInCircle (4 args)
	RotatePath (2 args)
	FollowPath (3 args)
	GetCameraFarDistance (1 args)
	RequestClearPath (1 args)
	StopGroup0 (0 args)
	FindShiftedPathTo (2 args)
	irand (2 args)
	WaitForAnimEnd (1 args)
	Sleep (2 args)
	StopAnimation (0 args)
	PlayGlobalMusic (1 args)
	SendPlayerEnemy (2 args)
	GetPFPosition (1 args)
	CanReachByPF (2 args)
	rand (1 args)
	Face (1 args)
	SetSpeed (1 args)
	HasAnimation (3 args)
	IsExisting3DSound (2 args)
	GetAnimationOffset (3 args)
	GetVictim (2 args)
	ReportAttack (1 args)
	ReportHit (4 args)
	SetAttackState (1 args)
	IsAnimationPlaying (1 args)
	sync (0 args)
	GetScene (1 args)
	FindDirLength (3 args)
	Speak (1 args)
	CanSee (2 args)
	FindPathTo (2 args)
	FollowPath (5 args)
	IsDead (1 args)
	GetSecondaryAnimationType (1 args)
	FadeSecondaryAnimation (4 args)
	CreateVectorVector (1 args)
	GetGeometryLocator (4 args)
	AddActorByType (6 args)
	RotateAsync (2 args)
	IsLoaded (1 args)
	AddItem (3 args)
	AddItem (4 args)
	Is3DSoundLoaded (2 args)
	GetEyesHeight (1 args)
	PlayGlobalSound (2 args)
	self (1 args)
	CreateFloatVector (1 args)
	SendWorldWndMessage (2 args)
	GetGameTime (1 args)
	BroadcastMessage (3 args)
	GetVariable (2 args)
	WorkWithCorpse (1 args)
	Barter (1 args)
	Trace (1 args)
	CreateInvItem (1 args)
	ClearSubContainer (1 args)
	GetInvItemByName (2 args)
	GetProperty (2 args)
	SignalDeath (1 args)
	ReportReputationChange (3 args)
	FindActor (2 args)
	IsOverrideActive (1 args)
	ReportReputationChange (4 args)
	SetRTEnvelope (2 args)

RunOp = 0x9c
RunTask = 1

GlobalTasks: 
	GTASK_0  Params = 1
		EVENT_0 Op = 0xf Vars = (object)
		EVENT_6 Op = 0x1d Vars = ()
		EVENT_22 Op = 0x96 Vars = (object, int, float, float)
		EVENT_16 Op = 0x98 Vars = (object, string)
		EVENT_41 Op = 0x9a Vars = (object)
	GTASK_1 Vars = (bool, bool, bool) Params = 0
		EVENT_0 Op = 0xb4 Vars = (object)
		EVENT_17 Op = 0xc8 Vars = (object)
		EVENT_30 Op = 0xe0 Vars = (object, object, bool)
		EVENT_40 Op = 0xfc Vars = (object)
		EVENT_42 Op = 0x110 Vars = (object, string)
		EVENT_26 Op = 0x135 Vars = (string)
		EVENT_1 Op = 0x143 Vars = (object)
		EVENT_3 Op = 0x156 Vars = (object)
		EVENT_7 Op = 0x15c Vars = (int)
		EVENT_6 Op = 0x168 Vars = ()
		EVENT_41 Op = 0x17f Vars = (object)
		EVENT_10 Op = 0x1f0 Vars = (object)
		EVENT_28 Op = 0x1f4 Vars = ()
	GTASK_2  Params = 0
		EVENT_0 Op = 0x20d Vars = (object)
		EVENT_17 Op = 0x221 Vars = (object)
		EVENT_30 Op = 0x239 Vars = (object, object, bool)
		EVENT_40 Op = 0x255 Vars = (object)
		EVENT_42 Op = 0x269 Vars = (object, string)
		EVENT_26 Op = 0x28e Vars = (string)
		EVENT_6 Op = 0x29c Vars = ()
		EVENT_1 Op = 0x2a3 Vars = (object)
	GTASK_3 Vars = (object) Params = 1
		EVENT_0 Op = 0x33f Vars = (object)
		EVENT_17 Op = 0x353 Vars = (object)
		EVENT_30 Op = 0x36b Vars = (object, object, bool)
		EVENT_40 Op = 0x387 Vars = (object)
		EVENT_42 Op = 0x39b Vars = (object, string)
		EVENT_26 Op = 0x3c0 Vars = (string)
		EVENT_41 Op = 0x3d9 Vars = (object)
		EVENT_7 Op = 0x3e2 Vars = (int)
		EVENT_6 Op = 0x405 Vars = ()
		EVENT_1 Op = 0x40c Vars = (object)
	GTASK_4 Vars = (object, int, int, bool, float, int) Params = 1
	GTASK_5 Vars = (bool, object, bool) Params = 6
		EVENT_6 Op = 0x6be Vars = ()
		EVENT_7 Op = 0x6f6 Vars = (int)
		EVENT_1 Op = 0x711 Vars = (object)
		EVENT_2 Op = 0x720 Vars = (object)
		EVENT_10 Op = 0x7a6 Vars = (object)
		EVENT_41 Op = 0x7b1 Vars = (object)

Events:
EVENT_16 Op = 0xab3 Vars = (object, string)
EVENT_41 Op = 0xac0 Vars = (object)
EVENT_22 Op = 0xac6 Vars = (object, int, float, float)
EVENT_43 Op = 0xace Vars = (object, int, float, float, cvector, cvector)

0x0: PushEmpty()
0x1: EventDisable(0)
0x2: PushEmpty(object)
0x3: Stack[-2] = Stack[-1]
0x4: Call2 0x21

0x5: Pop(1)
0x6: PushEmpty(object)
0x7: Stack[-2] = Stack[-1]
0x8: Call2 0xc62

0x9: Pop(1)
0xa: EventEnable(0)
0xb: @ Hold()
0xc: Pop(0)
0xd: GOTO 0xb

0xe: Return(); Pop(0)

0xf: PushEmpty(int, int)
0x10: PushEmpty(int, object)
0x11: Stack[-5] = Stack[-1]
0x12: Call2 0xc12

0x13: Stack[-2] = Stack[-3]
0x14: Pop(2)
0x15: Push((int) 0)
0x16: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x17: IF (Stack[-1] == 0) GOTO 0x1c; Pop(1)

0x18: PushEmpty(object)
0x19: Stack[-4] = Stack[-1]
0x1a: Call2 0xc15

0x1b: Pop(1)
0x1c: Return(); Pop(2)

0x1d: PushEmpty()
0x1e: Call2 0xc1b

0x1f: Pop(0)
0x20: Return(); Pop(0)

0x21: PushEmpty(cvector, cvector, cvector, cvector, string, object, bool, bool, float, cvector, cvector, cvector, cvector, cvector, string, object, bool, bool, float, cvector)
0x22: Pop(0); PushNull((bool) Stack[-21] == 0)
0x23: IF (Stack[-1] == 0) GOTO 0x29; Pop(1)

0x24: PushEmpty(string)
0x25: Stack[-1] = "fdie" // @poff=0
0x26: Call2 0x7c

0x27: Pop(1)
0x28: GOTO 0x7b

0x29: @@ GetPosition(Stack[-10]); Obj=21 // @poff=10
0x2a: Pop(0)
0x2b: @ GetPosition(Stack[-9])
0x2c: Pop(0)
0x2d: @ GetDirection(Stack[-8])
0x2e: Pop(0)
0x2f: Stack[-7] = Stack[-9] - Stack[-10]; Pop(0);
0x30: Push(CvectorIndex(Stack[-7], 0))
0x31: Push(CvectorIndex(Stack[-9], 0))
0x32: Pop(2); Push(Stack[-2] * Stack[-1]);
0x33: Push(CvectorIndex(Stack[-8], 2))
0x34: Push(CvectorIndex(Stack[-10], 2))
0x35: Pop(2); Push(Stack[-2] * Stack[-1]);
0x36: Pop(2); Push(Stack[-2] + Stack[-1]);
0x37: Push((int) 0)
0x38: Pop(2); Push((bool) Stack[-2] >= Stack[-1])
0x39: IF (Stack[-1] == 0) GOTO 0x3c; Pop(1)

0x3a: Stack[-6] = "fdie" // @poff=0
0x3b: GOTO 0x3d

0x3c: Stack[-6] = "bdie" // @poff=22
0x3d: @ RemoveRTEnvelope()
0x3e: Pop(0)
0x3f: @ SetDeathState()
0x40: Pop(0)
0x41: @ Stop()
0x42: Pop(0)
0x43: @ StopAsync()
0x44: Pop(0)
0x45: Stack[-21] = Stack[-5]
0x46: Push("GetScriptProperty") // @poff=32
0x47: Push((int) 2)
0x48: Pop(2); Push(IsFuncExist(Stack[-23], Stack[-2], Stack[-1]))
0x49: IF (Stack[-1] == 0) GOTO 0x55; Pop(1)

0x4a: Push("Owner") // @poff=68
0x4b: @@ HasScriptProperty(Stack[-5], Stack[-1]); Obj=22 // @poff=80
0x4c: Pop(1)
0x4d: Push(Stack[-4])
0x4e: IF (Stack[-1] == 0) GOTO 0x55; Pop(1)

0x4f: Push("Owner") // @poff=68
0x50: @@ GetScriptProperty(Stack[-6], Stack[-1]); Obj=22 // @poff=98
0x51: Pop(1)
0x52: Pop(0); PushNull((bool) Stack[-5] == 0)
0x53: IF (Stack[-1] == 0) GOTO 0x55; Pop(1)

0x54: Stack[-21] = Stack[-5]
0x55: Push("@GetEyesHeight") // @poff=116
0x56: Push((int) 1)
0x57: Pop(2); Push(IsFuncExist(Stack[-7], Stack[-2], Stack[-1]))
0x58: IF (Stack[-1] == 0) GOTO 0x64; Pop(1)

0x59: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=146
0x5a: Pop(0)
0x5b: Stack[-1] = [0.0, 0.0, 0.0]
0x5c: Push(CvectorIndex(Stack[-1], 1))
0x5d: Stack[-3] = Stack[-1]
0x5e: CvectorIndex(Stack[-2], 1) = Stack[-1];
0x5f: Push("head") // @poff=160
0x60: @ LookAsync(Stack[-22], Stack[-1], Stack[-2])
0x61: Pop(1)
0x62: Stack[-3] = (bool) 1
0x63: GOTO 0x65

0x64: Stack[-3] = (bool) 0
0x65: PushEmpty(string)
0x66: Stack[-7] = Stack[-1]
0x67: Call2 0x930

0x68: Pop(1)
0x69: Push("all") // @poff=170
0x6a: @ PlayAnimation(Stack[-1], Stack[-7])
0x6b: Pop(1)
0x6c: @ WaitForAnimEnd()
0x6d: Pop(0)
0x6e: Push(Stack[-3])
0x6f: IF (Stack[-1] == 0) GOTO 0x75; Pop(1)

0x70: @ StopAsync()
0x71: Pop(0)
0x72: Push("head") // @poff=160
0x73: @ UnlookAsync(Stack[-1])
0x74: Pop(1)
0x75: Push("all") // @poff=170
0x76: @ LockAnimationEnd(Stack[-1], Stack[-7])
0x77: Pop(1)
0x78: @ RemoveEnvelope()
0x79: Pop(0)
0x7a: Stack[-5] = 0
0x7b: Return(); Pop(20)

0x7c: PushEmpty()
0x7d: @ RemoveRTEnvelope()
0x7e: Pop(0)
0x7f: @ SetDeathState()
0x80: Pop(0)
0x81: @ Stop()
0x82: Pop(0)
0x83: @ StopAsync()
0x84: Pop(0)
0x85: @ StopSecondaryAnimation()
0x86: Pop(0)
0x87: PushEmpty(string)
0x88: Stack[-2] = Stack[-1]
0x89: Call2 0x930

0x8a: Pop(1)
0x8b: Push("all") // @poff=170
0x8c: @ PlayAnimation(Stack[-1], Stack[-2])
0x8d: Pop(1)
0x8e: @ WaitForAnimEnd()
0x8f: Pop(0)
0x90: Push("all") // @poff=170
0x91: @ LockAnimationEnd(Stack[-1], Stack[-2])
0x92: Pop(1)
0x93: @ RemoveEnvelope()
0x94: Pop(0)
0x95: Return(); Pop(0)

0x96: PushEmpty()
0x97: Return(); Pop(0)

0x98: PushEmpty()
0x99: Return(); Pop(0)

0x9a: PushEmpty()
0x9b: Return(); Pop(0)

0x9c: Push((bool) 1)
0x9d: @ SensePlayerOnly(Stack[-1])
0x9e: Pop(1)
0x9f: PushEmpty()
0xa0: Call2 0xc40

0xa1: Pop(0)
0xa2: PushEmpty()
0xa3: Call2 0xab

0xa4: Pop(0)
0xa5: Stack[2 + Tasks[-1].StackPointer] = (bool)0
0xa6: PushEmpty()
0xa7: Call2 0x1b0

0xa8: Pop(0)
0xa9: GOTO 0xa5

0xaa: Return(); Pop(0)

0xab: PushEmpty(bool)
0xac: Call2 0x906

0xad: Pop(0)
0xae: Pop(1); Push((bool) Stack[-1] == 0)
0xaf: IF (Stack[-1] == 0) GOTO 0xb3; Pop(1)

0xb0: PushEmpty()
0xb1: Call2 0xc1b

0xb2: Pop(0)
0xb3: Return(); Pop(0)

0xb4: PushEmpty(int, int)
0xb5: PushEmpty(int, object)
0xb6: Stack[-5] = Stack[-1]
0xb7: Call2 0xc12

0xb8: Stack[-2] = Stack[-3]
0xb9: Pop(2)
0xba: Push((int) 0)
0xbb: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0xbc: IF (Stack[-1] == 0) GOTO 0xc7; Pop(1)

0xbd: Push((int) 1)
0xbe: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0xbf: IF (Stack[-1] == 0) GOTO 0xc3; Pop(1)

0xc0: PushEmpty()
0xc1: Call2 0x16f

0xc2: Pop(0)
0xc3: PushEmpty(object)
0xc4: Stack[-4] = Stack[-1]
0xc5: Call2 0xc15

0xc6: Pop(1)
0xc7: Return(); Pop(2)

0xc8: PushEmpty(int, int)
0xc9: PushEmpty(object)
0xca: Stack[-4] = Stack[-1]
0xcb: Call2 0xb2e

0xcc: Pop(1)
0xcd: PushEmpty(int, object)
0xce: Stack[-5] = Stack[-1]
0xcf: Call2 0xb81

0xd0: Stack[-2] = Stack[-3]
0xd1: Pop(2)
0xd2: Push((int) 0)
0xd3: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0xd4: IF (Stack[-1] == 0) GOTO 0xdf; Pop(1)

0xd5: Push((int) 1)
0xd6: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0xd7: IF (Stack[-1] == 0) GOTO 0xdb; Pop(1)

0xd8: PushEmpty()
0xd9: Call2 0x16f

0xda: Pop(0)
0xdb: PushEmpty(object)
0xdc: Stack[-4] = Stack[-1]
0xdd: Call2 0xb8b

0xde: Pop(1)
0xdf: Return(); Pop(2)

0xe0: PushEmpty(int, int)
0xe1: PushEmpty(bool, object, object, bool)
0xe2: Stack[-9] = Stack[-3]
0xe3: Stack[-8] = Stack[-2]
0xe4: Stack[-7] = Stack[-1]
0xe5: Call2 0xc58

0xe6: Pop(3)
0xe7: IF (Stack[-1] == 0) GOTO 0xfb; Pop(1)

0xe8: PushEmpty(int, object, bool)
0xe9: Stack[-8] = Stack[-2]
0xea: Stack[-6] = Stack[-1]
0xeb: Call2 0xbde

0xec: Stack[-3] = Stack[-4]
0xed: Pop(3)
0xee: Push((int) 0)
0xef: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0xf0: IF (Stack[-1] == 0) GOTO 0xfb; Pop(1)

0xf1: Push((int) 1)
0xf2: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0xf3: IF (Stack[-1] == 0) GOTO 0xf7; Pop(1)

0xf4: PushEmpty()
0xf5: Call2 0x16f

0xf6: Pop(0)
0xf7: PushEmpty(object)
0xf8: Stack[-6] = Stack[-1]
0xf9: Call2 0xbe5

0xfa: Pop(1)
0xfb: Return(); Pop(2)

0xfc: PushEmpty(int, int)
0xfd: PushEmpty(int, object)
0xfe: Stack[-5] = Stack[-1]
0xff: Call2 0xc08

0x100: Stack[-2] = Stack[-3]
0x101: Pop(2)
0x102: Push((int) 0)
0x103: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x104: IF (Stack[-1] == 0) GOTO 0x10f; Pop(1)

0x105: Push((int) 1)
0x106: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x107: IF (Stack[-1] == 0) GOTO 0x10b; Pop(1)

0x108: PushEmpty()
0x109: Call2 0x16f

0x10a: Pop(0)
0x10b: PushEmpty(object)
0x10c: Stack[-4] = Stack[-1]
0x10d: Call2 0xc0b

0x10e: Pop(1)
0x10f: Return(); Pop(2)

0x110: PushEmpty(int, int)
0x111: PushEmpty(bool, object, string)
0x112: Stack[-7] = Stack[-2]
0x113: Stack[-6] = Stack[-1]
0x114: Call2 0xad8

0x115: Pop(2)
0x116: IF (Stack[-1] == 0) GOTO 0x120; Pop(1)

0x117: PushEmpty()
0x118: Call2 0x16f

0x119: Pop(0)
0x11a: PushEmpty(object, string)
0x11b: Stack[-6] = Stack[-2]
0x11c: Stack[-5] = Stack[-1]
0x11d: Call2 0xaf8

0x11e: Pop(2)
0x11f: GOTO 0x134

0x120: PushEmpty(int, string, object)
0x121: Stack[-6] = Stack[-2]
0x122: Stack[-7] = Stack[-1]
0x123: Call2 0xc0d

0x124: Stack[-3] = Stack[-4]
0x125: Pop(3)
0x126: Push((int) 0)
0x127: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x128: IF (Stack[-1] == 0) GOTO 0x134; Pop(1)

0x129: Push((int) 1)
0x12a: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x12b: IF (Stack[-1] == 0) GOTO 0x12f; Pop(1)

0x12c: PushEmpty()
0x12d: Call2 0x16f

0x12e: Pop(0)
0x12f: PushEmpty(string, object)
0x130: Stack[-5] = Stack[-2]
0x131: Stack[-6] = Stack[-1]
0x132: Call2 0xc10

0x133: Pop(2)
0x134: Return(); Pop(2)

0x135: PushEmpty()
0x136: PushEmpty(bool, string)
0x137: Stack[-3] = Stack[-1]
0x138: Call2 0xb3b

0x139: Pop(1)
0x13a: IF (Stack[-1] == 0) GOTO 0x142; Pop(1)

0x13b: PushEmpty()
0x13c: Call2 0x16f

0x13d: Pop(0)
0x13e: PushEmpty(string)
0x13f: Stack[-2] = Stack[-1]
0x140: Call2 0xb4b

0x141: Pop(1)
0x142: Return(); Pop(0)

0x143: PushEmpty()
0x144: PushEmpty(bool, object)
0x145: Stack[-3] = Stack[-1]
0x146: Call2 0xb11

0x147: Pop(1)
0x148: IF (Stack[-1] == 0) GOTO 0x151; Pop(1)

0x149: PushEmpty()
0x14a: Call2 0x16f

0x14b: Pop(0)
0x14c: PushEmpty(object)
0x14d: Stack[-2] = Stack[-1]
0x14e: Call2 0xb28

0x14f: Pop(1)
0x150: GOTO 0x155

0x151: PushEmpty(object)
0x152: Stack[-2] = Stack[-1]
0x153: Call2 0x188

0x154: Pop(1)
0x155: Return(); Pop(0)

0x156: PushEmpty()
0x157: PushEmpty(object)
0x158: Stack[-2] = Stack[-1]
0x159: Call2 0x188

0x15a: Pop(1)
0x15b: Return(); Pop(0)

0x15c: PushEmpty()
0x15d: Push((int) 110)
0x15e: Pop(1); Push((bool) Stack[-2] != Stack[-1])
0x15f: IF (Stack[-1] == 0) GOTO 0x161; Pop(1)

0x160: Return(); Pop(0)

0x161: Stack[2 + Tasks[-1].StackPointer] = (bool)0
0x162: Push((int) 110)
0x163: @ KillTimer(Stack[-1])
0x164: Pop(1)
0x165: @ ResetAAS()
0x166: Pop(0)
0x167: Return(); Pop(0)

0x168: PushEmpty()
0x169: Call2 0x16f

0x16a: Pop(0)
0x16b: PushEmpty()
0x16c: Call2 0xc1b

0x16d: Pop(0)
0x16e: Return(); Pop(0)

0x16f: Push((int) 110)
0x170: @ KillTimer(Stack[-1])
0x171: Pop(1)
0x172: Stack[2 + Tasks[-1].StackPointer] = (bool)0
0x173: PushEmpty()
0x174: Call2 0x1f7

0x175: Pop(0)
0x176: Return(); Pop(0)

0x177: Push((int) 110)
0x178: @ KillTimer(Stack[-1])
0x179: Pop(1)
0x17a: Stack[2 + Tasks[-1].StackPointer] = (bool)0
0x17b: PushEmpty()
0x17c: Call2 0x1fe

0x17d: Pop(0)
0x17e: Return(); Pop(0)

0x17f: PushEmpty()
0x180: PushEmpty()
0x181: Call2 0x16f

0x182: Pop(0)
0x183: PushEmpty(object)
0x184: Stack[-2] = Stack[-1]
0x185: Call2 0xac0

0x186: Pop(1)
0x187: Return(); Pop(0)

0x188: PushEmpty(bool, int, bool, int)
0x189: PushEmpty(bool, object)
0x18a: Stack[-7] = Stack[-1]
0x18b: Call2 0x865

0x18c: Pop(1)
0x18d: Pop(1); Push((bool) Stack[-1] == 0)
0x18e: IF (Stack[-1] == 0) GOTO 0x190; Pop(1)

0x18f: Return(); Pop(4)

0x190: Push( Stack[2 + Tasks[-1].StackPointer] )
0x191: IF (Stack[-1] == 0) GOTO 0x193; Pop(1)

0x192: Return(); Pop(4)

0x193: @ IsPlayerActor(Stack[-5], Stack[-2])
0x194: Pop(0)
0x195: Pop(0); Push((bool) Stack[-2] == 0)
0x196: IF (Stack[-1] == 0) GOTO 0x198; Pop(1)

0x197: Return(); Pop(4)

0x198: PushEmpty(int, object)
0x199: Stack[-7] = Stack[-1]
0x19a: Call2 0xb70

0x19b: Stack[-2] = Stack[-3]
0x19c: Pop(2)
0x19d: Push((int) 0)
0x19e: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x19f: IF (Stack[-1] == 0) GOTO 0x1af; Pop(1)

0x1a0: Push((int) 1)
0x1a1: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x1a2: IF (Stack[-1] == 0) GOTO 0x1a6; Pop(1)

0x1a3: PushEmpty()
0x1a4: Call2 0x177

0x1a5: Pop(0)
0x1a6: PushEmpty(object)
0x1a7: Stack[-6] = Stack[-1]
0x1a8: Call2 0xb79

0x1a9: Pop(1)
0x1aa: Stack[2 + Tasks[-1].StackPointer] = (bool)1
0x1ab: Push((int) 110)
0x1ac: Push((float)10.0)
0x1ad: @ SetTimer(Stack[-2], Stack[-1])
0x1ae: Pop(2)
0x1af: Return(); Pop(4)

0x1b0: PushEmpty(float, cvector, cvector, bool, object, bool, float, cvector, cvector, bool, object, bool)
0x1b1: Stack[0 + Tasks[-1].StackPointer] = (bool)0
0x1b2: Stack[1 + Tasks[-1].StackPointer] = (bool)0
0x1b3: Push((float)0.5)
0x1b4: @ rand(Stack[-7], Stack[-1])
0x1b5: Pop(1)
0x1b6: @ Sleep(Stack[-6])
0x1b7: Pop(0)
0x1b8: Pop(0); Push((bool) Stack[0 + Tasks[-1].StackPointer] == 0)
0x1b9: IF (Stack[-1] == 0) GOTO 0x1ea; Pop(1)

0x1ba: Pop(0); Push((bool) Stack[1 + Tasks[-1].StackPointer] == 0)
0x1bb: IF (Stack[-1] == 0) GOTO 0x1cb; Pop(1)

0x1bc: @ GetPosition(Stack[-4])
0x1bd: Pop(0)
0x1be: PushEmpty(float)
0x1bf: Call2 0x1eb

0x1c0: Pop(0)
0x1c1: @ GetRandomPFPointInCircle(Stack[-6], Stack[-5], Stack[-1], Stack[-4])
0x1c2: Pop(1)
0x1c3: Push(Stack[-3])
0x1c4: IF (Stack[-1] == 0) GOTO 0x1c6; Pop(1)

0x1c5: GOTO 0x1ca

0x1c6: Push((int) 1)
0x1c7: @ Sleep(Stack[-1])
0x1c8: Pop(1)
0x1c9: GOTO 0x1bc

0x1ca: GOTO 0x1cc

0x1cb: Stack[1 + Tasks[-1].StackPointer] = (bool)0
0x1cc: PushEmpty(object, cvector)
0x1cd: Stack[-7] = Stack[-1]
0x1ce: Call2 0x207

0x1cf: Stack[-2] = Stack[-4]
0x1d0: Pop(2)
0x1d1: Pop(0); Push(( Stack[-2] != 0 )
0x1d2: IF (Stack[-1] == 0) GOTO 0x1e5; Pop(1)

0x1d3: @ RotatePath(Stack[-2], Stack[-1])
0x1d4: Pop(0)
0x1d5: Push(Stack[-1])
0x1d6: IF (Stack[-1] == 0) GOTO 0x1e4; Pop(1)

0x1d7: PushEmpty(bool)
0x1d8: Call2 0x205

0x1d9: Pop(0)
0x1da: @ FollowPath(Stack[-3], Stack[-1], Stack[-2])
0x1db: Pop(1)
0x1dc: Stack[-2] = 0
0x1dd: Push(Stack[-1])
0x1de: IF (Stack[-1] == 0) GOTO 0x1e4; Pop(1)

0x1df: PushEmpty()
0x1e0: Push(-0, 0); TaskCall(2)
0x1e1: Call2 0x2b1

0x1e2: Pop(-0, 0); TaskReturn
0x1e3: Pop(0)
0x1e4: GOTO 0x1e8

0x1e5: Push((int) 1)
0x1e6: @ Sleep(Stack[-1])
0x1e7: Pop(1)
0x1e8: Stack[-2] = 0
0x1e9: GOTO 0x1b8

0x1ea: Return(); Pop(12)

0x1eb: PushEmpty(float, float)
0x1ec: @ GetCameraFarDistance(Stack[-1])
0x1ed: Pop(0)
0x1ee: Stack[-1] = Stack[-3]
0x1ef: Return(); Pop(2)

0x1f0: PushEmpty()
0x1f1: @ RequestClearPath(Stack[-1])
0x1f2: Pop(0)
0x1f3: Return(); Pop(0)

0x1f4: @ Stop()
0x1f5: Pop(0)
0x1f6: Return(); Pop(0)

0x1f7: Stack[0 + Tasks[-1].StackPointer] = (bool)1
0x1f8: Stack[1 + Tasks[-1].StackPointer] = (bool)0
0x1f9: @ Stop()
0x1fa: Pop(0)
0x1fb: @ StopGroup0()
0x1fc: Pop(0)
0x1fd: Return(); Pop(0)

0x1fe: Stack[0 + Tasks[-1].StackPointer] = (bool)1
0x1ff: Stack[1 + Tasks[-1].StackPointer] = (bool)1
0x200: @ Stop()
0x201: Pop(0)
0x202: @ StopGroup0()
0x203: Pop(0)
0x204: Return(); Pop(0)

0x205: Stack[-1] = (bool) 0
0x206: Return(); Pop(0)

0x207: PushEmpty(object, object)
0x208: @ FindShiftedPathTo(Stack[-1], Stack[-3])
0x209: Pop(0)
0x20a: Stack[-1] = Stack[-4]
0x20b: Return(); Pop(2)

0x20c: Stack[-1] = 0
0x20d: PushEmpty(int, int)
0x20e: PushEmpty(int, object)
0x20f: Stack[-5] = Stack[-1]
0x210: Call2 0xc12

0x211: Stack[-2] = Stack[-3]
0x212: Pop(2)
0x213: Push((int) 0)
0x214: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x215: IF (Stack[-1] == 0) GOTO 0x220; Pop(1)

0x216: Push((int) 1)
0x217: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x218: IF (Stack[-1] == 0) GOTO 0x21c; Pop(1)

0x219: PushEmpty()
0x21a: Call2 0x302

0x21b: Pop(0)
0x21c: PushEmpty(object)
0x21d: Stack[-4] = Stack[-1]
0x21e: Call2 0xc15

0x21f: Pop(1)
0x220: Return(); Pop(2)

0x221: PushEmpty(int, int)
0x222: PushEmpty(object)
0x223: Stack[-4] = Stack[-1]
0x224: Call2 0xb2e

0x225: Pop(1)
0x226: PushEmpty(int, object)
0x227: Stack[-5] = Stack[-1]
0x228: Call2 0xb81

0x229: Stack[-2] = Stack[-3]
0x22a: Pop(2)
0x22b: Push((int) 0)
0x22c: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x22d: IF (Stack[-1] == 0) GOTO 0x238; Pop(1)

0x22e: Push((int) 1)
0x22f: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x230: IF (Stack[-1] == 0) GOTO 0x234; Pop(1)

0x231: PushEmpty()
0x232: Call2 0x302

0x233: Pop(0)
0x234: PushEmpty(object)
0x235: Stack[-4] = Stack[-1]
0x236: Call2 0xb8b

0x237: Pop(1)
0x238: Return(); Pop(2)

0x239: PushEmpty(int, int)
0x23a: PushEmpty(bool, object, object, bool)
0x23b: Stack[-9] = Stack[-3]
0x23c: Stack[-8] = Stack[-2]
0x23d: Stack[-7] = Stack[-1]
0x23e: Call2 0xc58

0x23f: Pop(3)
0x240: IF (Stack[-1] == 0) GOTO 0x254; Pop(1)

0x241: PushEmpty(int, object, bool)
0x242: Stack[-8] = Stack[-2]
0x243: Stack[-6] = Stack[-1]
0x244: Call2 0xbde

0x245: Stack[-3] = Stack[-4]
0x246: Pop(3)
0x247: Push((int) 0)
0x248: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x249: IF (Stack[-1] == 0) GOTO 0x254; Pop(1)

0x24a: Push((int) 1)
0x24b: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x24c: IF (Stack[-1] == 0) GOTO 0x250; Pop(1)

0x24d: PushEmpty()
0x24e: Call2 0x302

0x24f: Pop(0)
0x250: PushEmpty(object)
0x251: Stack[-6] = Stack[-1]
0x252: Call2 0xbe5

0x253: Pop(1)
0x254: Return(); Pop(2)

0x255: PushEmpty(int, int)
0x256: PushEmpty(int, object)
0x257: Stack[-5] = Stack[-1]
0x258: Call2 0xc08

0x259: Stack[-2] = Stack[-3]
0x25a: Pop(2)
0x25b: Push((int) 0)
0x25c: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x25d: IF (Stack[-1] == 0) GOTO 0x268; Pop(1)

0x25e: Push((int) 1)
0x25f: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x260: IF (Stack[-1] == 0) GOTO 0x264; Pop(1)

0x261: PushEmpty()
0x262: Call2 0x302

0x263: Pop(0)
0x264: PushEmpty(object)
0x265: Stack[-4] = Stack[-1]
0x266: Call2 0xc0b

0x267: Pop(1)
0x268: Return(); Pop(2)

0x269: PushEmpty(int, int)
0x26a: PushEmpty(bool, object, string)
0x26b: Stack[-7] = Stack[-2]
0x26c: Stack[-6] = Stack[-1]
0x26d: Call2 0xad8

0x26e: Pop(2)
0x26f: IF (Stack[-1] == 0) GOTO 0x279; Pop(1)

0x270: PushEmpty()
0x271: Call2 0x302

0x272: Pop(0)
0x273: PushEmpty(object, string)
0x274: Stack[-6] = Stack[-2]
0x275: Stack[-5] = Stack[-1]
0x276: Call2 0xaf8

0x277: Pop(2)
0x278: GOTO 0x28d

0x279: PushEmpty(int, string, object)
0x27a: Stack[-6] = Stack[-2]
0x27b: Stack[-7] = Stack[-1]
0x27c: Call2 0xc0d

0x27d: Stack[-3] = Stack[-4]
0x27e: Pop(3)
0x27f: Push((int) 0)
0x280: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x281: IF (Stack[-1] == 0) GOTO 0x28d; Pop(1)

0x282: Push((int) 1)
0x283: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x284: IF (Stack[-1] == 0) GOTO 0x288; Pop(1)

0x285: PushEmpty()
0x286: Call2 0x302

0x287: Pop(0)
0x288: PushEmpty(string, object)
0x289: Stack[-5] = Stack[-2]
0x28a: Stack[-6] = Stack[-1]
0x28b: Call2 0xc10

0x28c: Pop(2)
0x28d: Return(); Pop(2)

0x28e: PushEmpty()
0x28f: PushEmpty(bool, string)
0x290: Stack[-3] = Stack[-1]
0x291: Call2 0xb3b

0x292: Pop(1)
0x293: IF (Stack[-1] == 0) GOTO 0x29b; Pop(1)

0x294: PushEmpty()
0x295: Call2 0x302

0x296: Pop(0)
0x297: PushEmpty(string)
0x298: Stack[-2] = Stack[-1]
0x299: Call2 0xb4b

0x29a: Pop(1)
0x29b: Return(); Pop(0)

0x29c: PushEmpty()
0x29d: Call2 0x302

0x29e: Pop(0)
0x29f: PushEmpty()
0x2a0: Call2 0xc1b

0x2a1: Pop(0)
0x2a2: Return(); Pop(0)

0x2a3: PushEmpty()
0x2a4: PushEmpty(bool, object)
0x2a5: Stack[-3] = Stack[-1]
0x2a6: Call2 0xb11

0x2a7: Pop(1)
0x2a8: IF (Stack[-1] == 0) GOTO 0x2b0; Pop(1)

0x2a9: PushEmpty()
0x2aa: Call2 0x302

0x2ab: Pop(0)
0x2ac: PushEmpty(object)
0x2ad: Stack[-2] = Stack[-1]
0x2ae: Call2 0xb28

0x2af: Pop(1)
0x2b0: Return(); Pop(0)

0x2b1: PushEmpty(int, int, int, int, bool, float, bool, int, int, int, int, bool, float, bool)
0x2b2: @ WaitForAnimEnd()
0x2b3: Pop(0)
0x2b4: PushEmpty(bool)
0x2b5: Call2 0x906

0x2b6: Pop(0)
0x2b7: Pop(1); Push((bool) Stack[-1] == 0)
0x2b8: IF (Stack[-1] == 0) GOTO 0x2ba; Pop(1)

0x2b9: Return(); Pop(14)

0x2ba: PushEmpty(int)
0x2bb: Call2 0xb5f

0x2bc: Stack[-1] = Stack[-8]
0x2bd: Pop(1)
0x2be: Stack[-6] = (int) 0
0x2bf: PushEmpty(bool)
0x2c0: Stack[-1] = (bool) 0
0x2c1: Push((int) 5)
0x2c2: Pop(1); Push((bool) Stack[-8] < Stack[-1])
0x2c3: IF (Stack[-1] == 0) GOTO 0x2c9; Pop(1)

0x2c4: PushEmpty(bool)
0x2c5: Call2 0x906

0x2c6: Pop(0)
0x2c7: IF (Stack[-1] == 0) GOTO 0x2c9; Pop(1)

0x2c8: Stack[-1] = (bool) 1
0x2c9: IF (Stack[-1] == 0) GOTO 0x2fd; Pop(1)

0x2ca: Push((int) 3)
0x2cb: @ irand(Stack[-6], Stack[-1])
0x2cc: Pop(1)
0x2cd: Push((int) 0)
0x2ce: Pop(1); Push((bool) Stack[-6] == Stack[-1])
0x2cf: IF (Stack[-1] == 0) GOTO 0x2e1; Pop(1)

0x2d0: Push(Stack[-7])
0x2d1: IF (Stack[-1] == 0) GOTO 0x2e0; Pop(1)

0x2d2: @ irand(Stack[-4], Stack[-7])
0x2d3: Pop(0)
0x2d4: Push("all") // @poff=170
0x2d5: PushEmpty(string, int)
0x2d6: Stack[-7] = Stack[-1]
0x2d7: Call2 0xb58

0x2d8: Pop(1)
0x2d9: @ PlayAnimation(Stack[-2], Stack[-1])
0x2da: Pop(2)
0x2db: @ WaitForAnimEnd(Stack[-3])
0x2dc: Pop(0)
0x2dd: Pop(0); Push((bool) Stack[-3] == 0)
0x2de: IF (Stack[-1] == 0) GOTO 0x2e0; Pop(1)

0x2df: GOTO 0x2fd

0x2e0: GOTO 0x2f2

0x2e1: Push((int) 1)
0x2e2: Pop(1); Push((bool) Stack[-6] == Stack[-1])
0x2e3: IF (Stack[-1] == 0) GOTO 0x2ef; Pop(1)

0x2e4: Push((int) 4)
0x2e5: @ rand(Stack[-3], Stack[-1])
0x2e6: Pop(1)
0x2e7: Push((int) 1)
0x2e8: Pop(1); Push(Stack[-3] + Stack[-1]);
0x2e9: @ Sleep(Stack[-1], Stack[-2])
0x2ea: Pop(1)
0x2eb: Pop(0); Push((bool) Stack[-1] == 0)
0x2ec: IF (Stack[-1] == 0) GOTO 0x2ee; Pop(1)

0x2ed: GOTO 0x2fd

0x2ee: GOTO 0x2f2

0x2ef: Push(Stack[-6])
0x2f0: IF (Stack[-1] == 0) GOTO 0x2f2; Pop(1)

0x2f1: GOTO 0x2fd

0x2f2: PushEmpty(bool)
0x2f3: Call2 0x300

0x2f4: Pop(0)
0x2f5: Pop(1); Push((bool) Stack[-1] == 0)
0x2f6: IF (Stack[-1] == 0) GOTO 0x2f8; Pop(1)

0x2f7: GOTO 0x2fd

0x2f8: @ ResetAAS()
0x2f9: Pop(0)
0x2fa: Push((int) 1)
0x2fb: Stack[-7] = Stack[-7] + Stack[-1]; Pop(1);
0x2fc: GOTO 0x2bf

0x2fd: @ ResetAAS()
0x2fe: Pop(0)
0x2ff: Return(); Pop(14)

0x300: Stack[-1] = (bool) 1
0x301: Return(); Pop(0)

0x302: @ StopAnimation()
0x303: Pop(0)
0x304: @ StopGroup0()
0x305: Pop(0)
0x306: Return(); Pop(0)

0x307: PushEmpty(cvector, cvector, bool, cvector, cvector, bool)
0x308: Stack[0 + Tasks[-1].StackPointer] = Stack[-7]
0x309: PushEmpty()
0x30a: Call2 0x33a

0x30b: Pop(0)
0x30c: @ GetDirection(Stack[-3])
0x30d: Pop(0)
0x30e: PushEmpty(cvector, object)
0x30f: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x310: Call2 0x7d3

0x311: Stack[-2] = Stack[-4]
0x312: Pop(2)
0x313: PushEmpty(float, cvector, cvector)
0x314: Stack[-6] = Stack[-2]
0x315: Stack[-5] = Stack[-1]
0x316: Call2 0x993

0x317: Pop(2)
0x318: Push((int) 0)
0x319: Pop(2); Push((bool) Stack[-2] < Stack[-1])
0x31a: IF (Stack[-1] == 0) GOTO 0x321; Pop(1)

0x31b: PushEmpty(object)
0x31c: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x31d: Call2 0x8fb

0x31e: Pop(1)
0x31f: Stack[-1] = (bool) 1
0x320: GOTO 0x324

0x321: Push((float)1.5)
0x322: @ Sleep(Stack[-1], Stack[-2])
0x323: Pop(1)
0x324: Push(Stack[-1])
0x325: IF (Stack[-1] == 0) GOTO 0x334; Pop(1)

0x326: PushEmpty(object)
0x327: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x328: Call2 0x8fb

0x329: Pop(1)
0x32a: Push((int) 111)
0x32b: Push((float)0.5)
0x32c: @ SetTimer(Stack[-2], Stack[-1])
0x32d: Pop(2)
0x32e: Push((float)5.0)
0x32f: @ Sleep(Stack[-1])
0x330: Pop(1)
0x331: Push((int) 111)
0x332: @ KillTimer(Stack[-1])
0x333: Pop(1)
0x334: @ StopAsync()
0x335: Pop(0)
0x336: Push("head") // @poff=160
0x337: @ UnlookAsync(Stack[-1])
0x338: Pop(1)
0x339: Return(); Pop(6)

0x33a: PushEmpty(object)
0x33b: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x33c: Call2 0x90b

0x33d: Pop(1)
0x33e: Return(); Pop(0)

0x33f: PushEmpty(int, int)
0x340: PushEmpty(int, object)
0x341: Stack[-5] = Stack[-1]
0x342: Call2 0xc12

0x343: Stack[-2] = Stack[-3]
0x344: Pop(2)
0x345: Push((int) 0)
0x346: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x347: IF (Stack[-1] == 0) GOTO 0x352; Pop(1)

0x348: Push((int) 1)
0x349: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x34a: IF (Stack[-1] == 0) GOTO 0x34e; Pop(1)

0x34b: PushEmpty()
0x34c: Call2 0x3ce

0x34d: Pop(0)
0x34e: PushEmpty(object)
0x34f: Stack[-4] = Stack[-1]
0x350: Call2 0xc15

0x351: Pop(1)
0x352: Return(); Pop(2)

0x353: PushEmpty(int, int)
0x354: PushEmpty(object)
0x355: Stack[-4] = Stack[-1]
0x356: Call2 0xb2e

0x357: Pop(1)
0x358: PushEmpty(int, object)
0x359: Stack[-5] = Stack[-1]
0x35a: Call2 0xb81

0x35b: Stack[-2] = Stack[-3]
0x35c: Pop(2)
0x35d: Push((int) 0)
0x35e: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x35f: IF (Stack[-1] == 0) GOTO 0x36a; Pop(1)

0x360: Push((int) 1)
0x361: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x362: IF (Stack[-1] == 0) GOTO 0x366; Pop(1)

0x363: PushEmpty()
0x364: Call2 0x3ce

0x365: Pop(0)
0x366: PushEmpty(object)
0x367: Stack[-4] = Stack[-1]
0x368: Call2 0xb8b

0x369: Pop(1)
0x36a: Return(); Pop(2)

0x36b: PushEmpty(int, int)
0x36c: PushEmpty(bool, object, object, bool)
0x36d: Stack[-9] = Stack[-3]
0x36e: Stack[-8] = Stack[-2]
0x36f: Stack[-7] = Stack[-1]
0x370: Call2 0xc58

0x371: Pop(3)
0x372: IF (Stack[-1] == 0) GOTO 0x386; Pop(1)

0x373: PushEmpty(int, object, bool)
0x374: Stack[-8] = Stack[-2]
0x375: Stack[-6] = Stack[-1]
0x376: Call2 0xbde

0x377: Stack[-3] = Stack[-4]
0x378: Pop(3)
0x379: Push((int) 0)
0x37a: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x37b: IF (Stack[-1] == 0) GOTO 0x386; Pop(1)

0x37c: Push((int) 1)
0x37d: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x37e: IF (Stack[-1] == 0) GOTO 0x382; Pop(1)

0x37f: PushEmpty()
0x380: Call2 0x3ce

0x381: Pop(0)
0x382: PushEmpty(object)
0x383: Stack[-6] = Stack[-1]
0x384: Call2 0xbe5

0x385: Pop(1)
0x386: Return(); Pop(2)

0x387: PushEmpty(int, int)
0x388: PushEmpty(int, object)
0x389: Stack[-5] = Stack[-1]
0x38a: Call2 0xc08

0x38b: Stack[-2] = Stack[-3]
0x38c: Pop(2)
0x38d: Push((int) 0)
0x38e: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x38f: IF (Stack[-1] == 0) GOTO 0x39a; Pop(1)

0x390: Push((int) 1)
0x391: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x392: IF (Stack[-1] == 0) GOTO 0x396; Pop(1)

0x393: PushEmpty()
0x394: Call2 0x3ce

0x395: Pop(0)
0x396: PushEmpty(object)
0x397: Stack[-4] = Stack[-1]
0x398: Call2 0xc0b

0x399: Pop(1)
0x39a: Return(); Pop(2)

0x39b: PushEmpty(int, int)
0x39c: PushEmpty(bool, object, string)
0x39d: Stack[-7] = Stack[-2]
0x39e: Stack[-6] = Stack[-1]
0x39f: Call2 0xad8

0x3a0: Pop(2)
0x3a1: IF (Stack[-1] == 0) GOTO 0x3ab; Pop(1)

0x3a2: PushEmpty()
0x3a3: Call2 0x3ce

0x3a4: Pop(0)
0x3a5: PushEmpty(object, string)
0x3a6: Stack[-6] = Stack[-2]
0x3a7: Stack[-5] = Stack[-1]
0x3a8: Call2 0xaf8

0x3a9: Pop(2)
0x3aa: GOTO 0x3bf

0x3ab: PushEmpty(int, string, object)
0x3ac: Stack[-6] = Stack[-2]
0x3ad: Stack[-7] = Stack[-1]
0x3ae: Call2 0xc0d

0x3af: Stack[-3] = Stack[-4]
0x3b0: Pop(3)
0x3b1: Push((int) 0)
0x3b2: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x3b3: IF (Stack[-1] == 0) GOTO 0x3bf; Pop(1)

0x3b4: Push((int) 1)
0x3b5: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x3b6: IF (Stack[-1] == 0) GOTO 0x3ba; Pop(1)

0x3b7: PushEmpty()
0x3b8: Call2 0x3ce

0x3b9: Pop(0)
0x3ba: PushEmpty(string, object)
0x3bb: Stack[-5] = Stack[-2]
0x3bc: Stack[-6] = Stack[-1]
0x3bd: Call2 0xc10

0x3be: Pop(2)
0x3bf: Return(); Pop(2)

0x3c0: PushEmpty()
0x3c1: PushEmpty(bool, string)
0x3c2: Stack[-3] = Stack[-1]
0x3c3: Call2 0xb3b

0x3c4: Pop(1)
0x3c5: IF (Stack[-1] == 0) GOTO 0x3cd; Pop(1)

0x3c6: PushEmpty()
0x3c7: Call2 0x3ce

0x3c8: Pop(0)
0x3c9: PushEmpty(string)
0x3ca: Stack[-2] = Stack[-1]
0x3cb: Call2 0xb4b

0x3cc: Pop(1)
0x3cd: Return(); Pop(0)

0x3ce: @ StopGroup0()
0x3cf: Pop(0)
0x3d0: @ StopAsync()
0x3d1: Pop(0)
0x3d2: Push("head") // @poff=160
0x3d3: @ UnlookAsync(Stack[-1])
0x3d4: Pop(1)
0x3d5: Push((int) 111)
0x3d6: @ KillTimer(Stack[-1])
0x3d7: Pop(1)
0x3d8: Return(); Pop(0)

0x3d9: PushEmpty()
0x3da: PushEmpty()
0x3db: Call2 0x3ce

0x3dc: Pop(0)
0x3dd: PushEmpty(object)
0x3de: Stack[-2] = Stack[-1]
0x3df: Call2 0xac0

0x3e0: Pop(1)
0x3e1: Return(); Pop(0)

0x3e2: PushEmpty(cvector, cvector, cvector, cvector)
0x3e3: Push((int) 111)
0x3e4: Pop(1); Push((bool) Stack[-6] != Stack[-1])
0x3e5: IF (Stack[-1] == 0) GOTO 0x3e7; Pop(1)

0x3e6: Return(); Pop(4)

0x3e7: PushEmpty(bool, object)
0x3e8: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x3e9: Call2 0x865

0x3ea: Pop(1)
0x3eb: Pop(1); Push((bool) Stack[-1] == 0)
0x3ec: IF (Stack[-1] == 0) GOTO 0x3f1; Pop(1)

0x3ed: PushEmpty()
0x3ee: Call2 0x3ce

0x3ef: Pop(0)
0x3f0: Return(); Pop(4)

0x3f1: @ GetDirection(Stack[-2])
0x3f2: Pop(0)
0x3f3: PushEmpty(cvector, object)
0x3f4: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x3f5: Call2 0x7d3

0x3f6: Stack[-2] = Stack[-3]
0x3f7: Pop(2)
0x3f8: PushEmpty(float, cvector, cvector)
0x3f9: Stack[-5] = Stack[-2]
0x3fa: Stack[-4] = Stack[-1]
0x3fb: Call2 0x993

0x3fc: Pop(2)
0x3fd: Push((float)0.4999999701976776)
0x3fe: Pop(2); Push((bool) Stack[-2] < Stack[-1])
0x3ff: IF (Stack[-1] == 0) GOTO 0x404; Pop(1)

0x400: PushEmpty(object)
0x401: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x402: Call2 0x8fb

0x403: Pop(1)
0x404: Return(); Pop(4)

0x405: PushEmpty()
0x406: Call2 0x3ce

0x407: Pop(0)
0x408: PushEmpty()
0x409: Call2 0xc1b

0x40a: Pop(0)
0x40b: Return(); Pop(0)

0x40c: PushEmpty()
0x40d: PushEmpty(bool, object)
0x40e: Stack[-3] = Stack[-1]
0x40f: Call2 0xb11

0x410: Pop(1)
0x411: IF (Stack[-1] == 0) GOTO 0x419; Pop(1)

0x412: PushEmpty()
0x413: Call2 0x3ce

0x414: Pop(0)
0x415: PushEmpty(object)
0x416: Stack[-2] = Stack[-1]
0x417: Call2 0xb28

0x418: Pop(1)
0x419: Return(); Pop(0)

0x41a: PushEmpty()
0x41b: PushEmpty(object, bool, float)
0x41c: Stack[-4] = Stack[-3]
0x41d: Stack[-2] = (bool) 1
0x41e: Stack[-1] = (float) 180.0
0x41f: Call2 0x428

0x420: Pop(3)
0x421: Return(); Pop(0)

0x422: PushEmpty()
0x423: Stack[-3] = (float) 0.029999999329447746
0x424: Return(); Pop(0)

0x425: PushEmpty()
0x426: Stack[-3] = (int) 0
0x427: Return(); Pop(0)

0x428: PushEmpty(float, cvector, cvector, bool, bool, float, cvector, float, cvector, bool, float, float, cvector, cvector, bool, bool, float, cvector, float, cvector, bool, float)
0x429: PushEmpty()
0x42a: Call2 0x50d

0x42b: Pop(0)
0x42c: Stack[5 + Tasks[-1].StackPointer] = (int)0
0x42d: Push("@GetAttackDistance") // @poff=178
0x42e: Push((int) 1)
0x42f: Pop(2); Push(IsFuncExist(Stack[-27], Stack[-2], Stack[-1]))
0x430: IF (Stack[-1] == 0) GOTO 0x436; Pop(1)

0x431: @@ GetAttackDistance(Stack[-11]); Obj=25 // @poff=216
0x432: Pop(0)
0x433: Push((int) 50)
0x434: Stack[-12] = Stack[-12] + Stack[-1]; Pop(1);
0x435: GOTO 0x437

0x436: Stack[-23] = Stack[-11]
0x437: Push((int) 150)
0x438: Pop(1); Push((bool) Stack[-12] >= Stack[-1])
0x439: IF (Stack[-1] == 0) GOTO 0x43b; Pop(1)

0x43a: Stack[-11] = (int) 150
0x43b: Stack[3 + Tasks[-1].StackPointer] = (bool)0
0x43c: Stack[0 + Tasks[-1].StackPointer] = Stack[-25]
0x43d: @ IsPlayerActor(Stack[-0]T, Stack[-8])
0x43e: Pop(0)
0x43f: Push(Stack[-8])
0x440: IF (Stack[-1] == 0) GOTO 0x449; Pop(1)

0x441: Push("attack") // @poff=234
0x442: @ PlayGlobalMusic(Stack[-1])
0x443: Pop(1)
0x444: PushEmpty(object)
0x445: Call2 0x959

0x446: Pop(0)
0x447: @ SendPlayerEnemy(Stack[-26], Stack[-1])
0x448: Pop(1)
0x449: Push(Stack[-24])
0x44a: IF (Stack[-1] == 0) GOTO 0x44d; Pop(1)

0x44b: Stack[-7] = (bool) 0
0x44c: GOTO 0x44e

0x44d: Stack[-7] = (bool) 1
0x44e: Push((float)400.0)
0x44f: Stack[-7] = Stack[-1] + Stack[-12]; Pop(1);
0x450: PushEmpty(bool)
0x451: Stack[-1] = (bool) 0
0x452: PushEmpty(bool, object)
0x453: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x454: Call2 0x865

0x455: Pop(1)
0x456: IF (Stack[-1] == 0) GOTO 0x45a; Pop(1)

0x457: Pop(0); Push((bool) Stack[3 + Tasks[-1].StackPointer] == 0)
0x458: IF (Stack[-1] == 0) GOTO 0x45a; Pop(1)

0x459: Stack[-1] = (bool) 1
0x45a: IF (Stack[-1] == 0) GOTO 0x4fc; Pop(1)

0x45b: PushEmpty()
0x45c: Call2 0x6aa

0x45d: Pop(0)
0x45e: @@@ GetPFPosition(Stack[-10]); Obj=0 // @poff=248
0x45f: Pop(0)
0x460: @ GetPFPosition(Stack[-9])
0x461: Pop(0)
0x462: Stack[-5] = Stack[-10] - Stack[-9]; Pop(0);
0x463: Stack[-4] = Stack[-5] | Stack[-5]; Pop(0);
0x464: Pop(0); Push(Stack[-6] * Stack[-6]);
0x465: Pop(1); Push((bool) Stack[-5] >= Stack[-1])
0x466: IF (Stack[-1] == 0) GOTO 0x476; Pop(1)

0x467: PushEmpty(bool, object, float, float, bool, bool)
0x468: Stack[-5] = Stack[0 + Tasks[-1].StackPointer]
0x469: Stack[-17] = Stack[-4]
0x46a: Stack[-3] = (float) 3000.0
0x46b: Stack[-2] = (bool) 1
0x46c: Stack[-1] = (bool) 0
0x46d: Push(-6, 3); TaskCall(5)
0x46e: Call2 0x6c5

0x46f: Pop(-6, 3); TaskReturn
0x470: Pop(5)
0x471: Pop(1); Push((bool) Stack[-1] == 0)
0x472: IF (Stack[-1] == 0) GOTO 0x474; Pop(1)

0x473: GOTO 0x4fc

0x474: Stack[-7] = (bool) 0
0x475: GOTO 0x4fb

0x476: Pop(0); Push(Stack[-23] * Stack[-23]);
0x477: Pop(1); Push((bool) Stack[-5] >= Stack[-1])
0x478: IF (Stack[-1] == 0) GOTO 0x4f3; Pop(1)

0x479: @@@ GetPFPosition(Stack[-3]); Obj=0 // @poff=248
0x47a: Pop(0)
0x47b: @ CanReachByPF(Stack[-2], Stack[-3])
0x47c: Pop(0)
0x47d: Pop(0); Push((bool) Stack[-2] == 0)
0x47e: IF (Stack[-1] == 0) GOTO 0x48e; Pop(1)

0x47f: PushEmpty(bool, object, float, float, bool, bool)
0x480: Stack[-5] = Stack[0 + Tasks[-1].StackPointer]
0x481: Stack[-17] = Stack[-4]
0x482: Stack[-3] = (float) 3000.0
0x483: Stack[-2] = (bool) 1
0x484: Stack[-1] = (bool) 0
0x485: Push(-6, 3); TaskCall(5)
0x486: Call2 0x6c5

0x487: Pop(-6, 3); TaskReturn
0x488: Pop(5)
0x489: Pop(1); Push((bool) Stack[-1] == 0)
0x48a: IF (Stack[-1] == 0) GOTO 0x48c; Pop(1)

0x48b: GOTO 0x4fc

0x48c: Stack[-7] = (bool) 0
0x48d: GOTO 0x450

0x48e: Pop(0); Push((bool) Stack[-7] == 0)
0x48f: IF (Stack[-1] == 0) GOTO 0x4a7; Pop(1)

0x490: PushEmpty(object)
0x491: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x492: Call2 0x8fb

0x493: Pop(1)
0x494: Push("all") // @poff=170
0x495: Push("attack_on") // @poff=262
0x496: @ PlayAnimation(Stack[-2], Stack[-1])
0x497: Pop(2)
0x498: @ WaitForAnimEnd()
0x499: Pop(0)
0x49a: PushEmpty()
0x49b: Call2 0x6aa

0x49c: Pop(0)
0x49d: @ StopAsync()
0x49e: Pop(0)
0x49f: Stack[-7] = (bool) 1
0x4a0: PushEmpty(bool, object)
0x4a1: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x4a2: Call2 0x865

0x4a3: Pop(1)
0x4a4: Pop(1); Push((bool) Stack[-1] == 0)
0x4a5: IF (Stack[-1] == 0) GOTO 0x4a7; Pop(1)

0x4a6: GOTO 0x4fc

0x4a7: @ rand(Stack[-1])
0x4a8: Pop(0)
0x4a9: PushEmpty(bool)
0x4aa: Stack[-1] = (bool) 1
0x4ab: Push((float)0.25)
0x4ac: Pop(1); Push((bool) Stack[-3] < Stack[-1])
0x4ad: IF (Stack[-1] == 1) GOTO 0x4b3; Pop(1)

0x4ae: PushEmpty(bool)
0x4af: Call2 0x67f

0x4b0: Pop(0)
0x4b1: IF (Stack[-1] == 1) GOTO 0x4b3; Pop(1)

0x4b2: Stack[-1] = (bool) 0
0x4b3: IF (Stack[-1] == 0) GOTO 0x4c4; Pop(1)

0x4b4: @ Face(Stack[-0]T)
0x4b5: Pop(0)
0x4b6: PushEmpty()
0x4b7: Call2 0x6b1

0x4b8: Pop(0)
0x4b9: Push("all") // @poff=170
0x4ba: Push("attack_stay") // @poff=282
0x4bb: @ PlayAnimation(Stack[-2], Stack[-1])
0x4bc: Pop(2)
0x4bd: PushEmpty(bool, float)
0x4be: Stack[-25] = Stack[-1]
0x4bf: Call2 0x5fb

0x4c0: Pop(2)
0x4c1: @ StopAsync()
0x4c2: Pop(0)
0x4c3: GOTO 0x4f2

0x4c4: @ Face(Stack[-0]T)
0x4c5: Pop(0)
0x4c6: Push("all") // @poff=170
0x4c7: Push("fjump") // @poff=306
0x4c8: @ PlayAnimation(Stack[-2], Stack[-1])
0x4c9: Pop(2)
0x4ca: @ WaitForAnimEnd()
0x4cb: Pop(0)
0x4cc: PushEmpty()
0x4cd: Call2 0x6aa

0x4ce: Pop(0)
0x4cf: Push([0.0, 0.0, 0.0])
0x4d0: @ SetSpeed(Stack[-1])
0x4d1: Pop(1)
0x4d2: @ Stop()
0x4d3: Pop(0)
0x4d4: @ StopAsync()
0x4d5: Pop(0)
0x4d6: PushEmpty(bool)
0x4d7: Call2 0x67f

0x4d8: Pop(0)
0x4d9: Pop(1); Push((bool) Stack[-1] == 0)
0x4da: IF (Stack[-1] == 0) GOTO 0x4f2; Pop(1)

0x4db: PushEmpty(bool, object)
0x4dc: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x4dd: Call2 0x865

0x4de: Pop(1)
0x4df: Pop(1); Push((bool) Stack[-1] == 0)
0x4e0: IF (Stack[-1] == 0) GOTO 0x4e2; Pop(1)

0x4e1: GOTO 0x4fc

0x4e2: @@@ GetPFPosition(Stack[-10]); Obj=0 // @poff=248
0x4e3: Pop(0)
0x4e4: @ GetPFPosition(Stack[-9])
0x4e5: Pop(0)
0x4e6: Stack[-5] = Stack[-10] - Stack[-9]; Pop(0);
0x4e7: Stack[-4] = Stack[-5] | Stack[-5]; Pop(0);
0x4e8: Pop(0); Push(Stack[-23] * Stack[-23]);
0x4e9: Pop(1); Push((bool) Stack[-5] < Stack[-1])
0x4ea: IF (Stack[-1] == 0) GOTO 0x4f2; Pop(1)

0x4eb: PushEmpty(bool, float)
0x4ec: Stack[-25] = Stack[-1]
0x4ed: Call2 0x557

0x4ee: Pop(1)
0x4ef: Pop(1); Push((bool) Stack[-1] == 0)
0x4f0: IF (Stack[-1] == 0) GOTO 0x4f2; Pop(1)

0x4f1: GOTO 0x4fc

0x4f2: GOTO 0x4fb

0x4f3: PushEmpty(bool, float)
0x4f4: Stack[-25] = Stack[-1]
0x4f5: Call2 0x557

0x4f6: Pop(1)
0x4f7: Pop(1); Push((bool) Stack[-1] == 0)
0x4f8: IF (Stack[-1] == 0) GOTO 0x4fa; Pop(1)

0x4f9: GOTO 0x4fc

0x4fa: Stack[-7] = (bool) 1
0x4fb: GOTO 0x450

0x4fc: @ WaitForAnimEnd()
0x4fd: Pop(0)
0x4fe: Push( Stack[3 + Tasks[-1].StackPointer] )
0x4ff: IF (Stack[-1] == 0) GOTO 0x501; Pop(1)

0x500: Return(); Pop(22)

0x501: Push("all") // @poff=170
0x502: Push("attack_off") // @poff=318
0x503: @ PlayAnimation(Stack[-2], Stack[-1])
0x504: Pop(2)
0x505: @ WaitForAnimEnd()
0x506: Pop(0)
0x507: Push(Stack[-8])
0x508: IF (Stack[-1] == 0) GOTO 0x50c; Pop(1)

0x509: Push((float)2.0)
0x50a: @ Sleep(Stack[-1])
0x50b: Pop(1)
0x50c: Return(); Pop(22)

0x50d: PushEmpty(bool, bool, cvector, bool, bool, cvector)
0x50e: Stack[1 + Tasks[-1].StackPointer] = (int)0
0x50f: Push("all") // @poff=170
0x510: Push("attack_begin") // @poff=340
0x511: Push((int) 1)
0x512: Pop(1); Push(Stack[1 + Tasks[-1].StackPointer] + Stack[-1]);
0x513: Pop(2); Push(Stack[-2] + Stack[-1]);
0x514: @ HasAnimation(Stack[-5], Stack[-2], Stack[-1])
0x515: Pop(2)
0x516: Pop(0); Push((bool) Stack[-3] == 0)
0x517: IF (Stack[-1] == 0) GOTO 0x519; Pop(1)

0x518: GOTO 0x51c

0x519: Push((int) 1)
0x51a: Stack[1 + Tasks[-1].StackPointer] = Stack[1 + Tasks[-1].StackPointer] + Stack[-1]; Pop(1);
0x51b: GOTO 0x50f

0x51c: Stack[2 + Tasks[-1].StackPointer] = (int)0
0x51d: Push("attack") // @poff=234
0x51e: Push((int) 1)
0x51f: Pop(1); Push(Stack[2 + Tasks[-1].StackPointer] + Stack[-1]);
0x520: Pop(2); Push(Stack[-2] + Stack[-1]);
0x521: @ IsExisting3DSound(Stack[-3], Stack[-1])
0x522: Pop(1)
0x523: Pop(0); Push((bool) Stack[-2] == 0)
0x524: IF (Stack[-1] == 0) GOTO 0x526; Pop(1)

0x525: GOTO 0x529

0x526: Push((int) 1)
0x527: Stack[2 + Tasks[-1].StackPointer] = Stack[2 + Tasks[-1].StackPointer] + Stack[-1]; Pop(1);
0x528: GOTO 0x51d

0x529: Push("all") // @poff=170
0x52a: Push("bjump") // @poff=366
0x52b: @ GetAnimationOffset(Stack[-3], Stack[-2], Stack[-1])
0x52c: Pop(2)
0x52d: Push(CvectorIndex(Stack[-1], 2))
0x52e: Stack[4 + Tasks[-1].StackPointer] = -Stack[-1]; Pop(1);
0x52f: Return(); Pop(6)

0x530: PushEmpty(object, float, float, object, float, float)
0x531: Push((float)0.8999999761581421)
0x532: Pop(1); Push(Stack[-9] * Stack[-1]);
0x533: @ GetVictim(Stack[-1], Stack[-4])
0x534: Pop(1)
0x535: @ ReportAttack(Stack[-0]T)
0x536: Pop(0)
0x537: Pop(0); Push((bool) Stack[-3] == Stack[0 + Tasks[-1].StackPointer])
0x538: IF (Stack[-1] == 0) GOTO 0x555; Pop(1)

0x539: PushEmpty(float, object, int)
0x53a: Stack[-6] = Stack[-2]
0x53b: Stack[-10] = Stack[-1]
0x53c: Call2 0x422

0x53d: Stack[-3] = Stack[-5]
0x53e: Pop(3)
0x53f: PushEmpty(float, object, float, int)
0x540: Stack[-7] = Stack[-3]
0x541: Stack[-6] = Stack[-2]
0x542: PushEmpty(int, object, int)
0x543: Stack[-10] = Stack[-2]
0x544: Stack[-14] = Stack[-1]
0x545: Call2 0x425

0x546: Stack[-3] = Stack[-4]
0x547: Pop(3)
0x548: Call2 0x7f3

0x549: Stack[-4] = Stack[-5]
0x54a: Pop(4)
0x54b: PushEmpty(int)
0x54c: Call2 0x6af

0x54d: Pop(0)
0x54e: @ ReportHit(Stack[-0]T, Stack[-1], Stack[-2], Stack[-3])
0x54f: Pop(1)
0x550: PushEmpty(object, float)
0x551: Stack[-5] = Stack[-2]
0x552: Stack[-3] = Stack[-1]
0x553: Call2 0x6b6

0x554: Pop(2)
0x555: Return(); Pop(6)

0x556: Stack[-3] = 0
0x557: PushEmpty(int, bool, int, string, int, bool, int, string)
0x558: PushEmpty()
0x559: Call2 0x6aa

0x55a: Pop(0)
0x55b: @ irand(Stack[-4], Stack[-1]T)
0x55c: Pop(0)
0x55d: Push((int) 1)
0x55e: Stack[-5] = Stack[-5] + Stack[-1]; Pop(1);
0x55f: @ Face(Stack[-0]T)
0x560: Pop(0)
0x561: Push((bool) 1)
0x562: @ SetAttackState(Stack[-1])
0x563: Pop(1)
0x564: PushEmpty()
0x565: Call2 0x9c0

0x566: Pop(0)
0x567: Push("all") // @poff=170
0x568: Push("attack_begin") // @poff=340
0x569: Pop(1); Push(Stack[-1] + Stack[-6]);
0x56a: @ PlayAnimation(Stack[-2], Stack[-1])
0x56b: Pop(2)
0x56c: @ WaitForAnimEnd()
0x56d: Pop(0)
0x56e: PushEmpty()
0x56f: Call2 0x68a

0x570: Pop(0)
0x571: PushEmpty(bool, object)
0x572: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x573: Call2 0x865

0x574: Pop(1)
0x575: Pop(1); Push((bool) Stack[-1] == 0)
0x576: IF (Stack[-1] == 0) GOTO 0x57b; Pop(1)

0x577: @ StopAsync()
0x578: Pop(0)
0x579: Stack[-10] = (bool) 0
0x57a: Return(); Pop(8)

0x57b: PushEmpty(float, int)
0x57c: Stack[-11] = Stack[-2]
0x57d: Stack[-6] = Stack[-1]
0x57e: Call2 0x530

0x57f: Pop(2)
0x580: Push("all") // @poff=170
0x581: Push("attack_middle") // @poff=378
0x582: Pop(1); Push(Stack[-1] + Stack[-6]);
0x583: @ HasAnimation(Stack[-5], Stack[-2], Stack[-1])
0x584: Pop(2)
0x585: Push(Stack[-3])
0x586: IF (Stack[-1] == 0) GOTO 0x5cc; Pop(1)

0x587: PushEmpty()
0x588: Call2 0x9c0

0x589: Pop(0)
0x58a: Push("all") // @poff=170
0x58b: Push("attack_middle") // @poff=378
0x58c: Pop(1); Push(Stack[-1] + Stack[-6]);
0x58d: @ PlayAnimation(Stack[-2], Stack[-1])
0x58e: Pop(2)
0x58f: @ WaitForAnimEnd()
0x590: Pop(0)
0x591: PushEmpty()
0x592: Call2 0x6aa

0x593: Pop(0)
0x594: PushEmpty(bool, object)
0x595: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x596: Call2 0x865

0x597: Pop(1)
0x598: Pop(1); Push((bool) Stack[-1] == 0)
0x599: IF (Stack[-1] == 0) GOTO 0x59e; Pop(1)

0x59a: @ StopAsync()
0x59b: Pop(0)
0x59c: Stack[-10] = (bool) 0
0x59d: Return(); Pop(8)

0x59e: PushEmpty(float, int)
0x59f: Stack[-11] = Stack[-2]
0x5a0: Stack[-6] = Stack[-1]
0x5a1: Call2 0x530

0x5a2: Pop(2)
0x5a3: Stack[-2] = (int) 1
0x5a4: Push("attack_middle") // @poff=378
0x5a5: Pop(1); Push(Stack[-1] + Stack[-5]);
0x5a6: Push("_") // @poff=406
0x5a7: Pop(2); Push(Stack[-2] + Stack[-1]);
0x5a8: Stack[-2] = Stack[-1] + Stack[-3]; Pop(1);
0x5a9: Push("all") // @poff=170
0x5aa: @ HasAnimation(Stack[-4], Stack[-1], Stack[-2])
0x5ab: Pop(1)
0x5ac: Pop(0); Push((bool) Stack[-3] == 0)
0x5ad: IF (Stack[-1] == 0) GOTO 0x5af; Pop(1)

0x5ae: GOTO 0x5cc

0x5af: PushEmpty()
0x5b0: Call2 0x9c0

0x5b1: Pop(0)
0x5b2: Push("all") // @poff=170
0x5b3: @ PlayAnimation(Stack[-1], Stack[-2])
0x5b4: Pop(1)
0x5b5: @ WaitForAnimEnd()
0x5b6: Pop(0)
0x5b7: PushEmpty()
0x5b8: Call2 0x6aa

0x5b9: Pop(0)
0x5ba: PushEmpty(bool, object)
0x5bb: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x5bc: Call2 0x865

0x5bd: Pop(1)
0x5be: Pop(1); Push((bool) Stack[-1] == 0)
0x5bf: IF (Stack[-1] == 0) GOTO 0x5c4; Pop(1)

0x5c0: @ StopAsync()
0x5c1: Pop(0)
0x5c2: Stack[-10] = (bool) 0
0x5c3: Return(); Pop(8)

0x5c4: PushEmpty(float, int)
0x5c5: Stack[-11] = Stack[-2]
0x5c6: Stack[-6] = Stack[-1]
0x5c7: Call2 0x530

0x5c8: Pop(2)
0x5c9: Push((int) 1)
0x5ca: Stack[-3] = Stack[-3] + Stack[-1]; Pop(1);
0x5cb: GOTO 0x5a4

0x5cc: Push((bool) 0)
0x5cd: @ SetAttackState(Stack[-1])
0x5ce: Pop(1)
0x5cf: Push("all") // @poff=170
0x5d0: Push("attack_end") // @poff=410
0x5d1: Pop(1); Push(Stack[-1] + Stack[-6]);
0x5d2: @ PlayAnimation(Stack[-2], Stack[-1])
0x5d3: Pop(2)
0x5d4: PushEmpty(bool)
0x5d5: Call2 0x6b8

0x5d6: Pop(0)
0x5d7: IF (Stack[-1] == 0) GOTO 0x5de; Pop(1)

0x5d8: PushEmpty(bool, float)
0x5d9: Stack[-1] = (float) 0.75
0x5da: Call2 0x5e0

0x5db: Pop(2)
0x5dc: @ StopAsync()
0x5dd: Pop(0)
0x5de: Stack[-10] = (bool) 1
0x5df: Return(); Pop(8)

0x5e0: PushEmpty(float, bool, float, bool)
0x5e1: @ rand(Stack[-2])
0x5e2: Pop(0)
0x5e3: Pop(0); Push((bool) Stack[-2] < Stack[-5])
0x5e4: IF (Stack[-1] == 0) GOTO 0x5f4; Pop(1)

0x5e5: @ IsAnimationPlaying(Stack[-1])
0x5e6: Pop(0)
0x5e7: Pop(0); Push((bool) Stack[-1] == 0)
0x5e8: IF (Stack[-1] == 0) GOTO 0x5ea; Pop(1)

0x5e9: GOTO 0x5f3

0x5ea: PushEmpty(bool)
0x5eb: Call2 0x642

0x5ec: Pop(0)
0x5ed: IF (Stack[-1] == 0) GOTO 0x5f0; Pop(1)

0x5ee: Stack[-6] = (bool) 1
0x5ef: Return(); Pop(4)

0x5f0: @ sync()
0x5f1: Pop(0)
0x5f2: GOTO 0x5e5

0x5f3: GOTO 0x5f9

0x5f4: @ WaitForAnimEnd()
0x5f5: Pop(0)
0x5f6: PushEmpty()
0x5f7: Call2 0x6aa

0x5f8: Pop(0)
0x5f9: Stack[-6] = (bool) 0
0x5fa: Return(); Pop(4)

0x5fb: PushEmpty(bool, cvector, cvector, cvector, float, bool, cvector, cvector, cvector, float)
0x5fc: @ IsAnimationPlaying(Stack[-5])
0x5fd: Pop(0)
0x5fe: Pop(0); Push((bool) Stack[-5] == 0)
0x5ff: IF (Stack[-1] == 0) GOTO 0x601; Pop(1)

0x600: GOTO 0x621

0x601: PushEmpty(bool)
0x602: Call2 0x642

0x603: Pop(0)
0x604: IF (Stack[-1] == 0) GOTO 0x607; Pop(1)

0x605: Stack[-12] = (bool) 1
0x606: Return(); Pop(10)

0x607: PushEmpty(bool, object)
0x608: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x609: Call2 0x865

0x60a: Pop(1)
0x60b: Pop(1); Push((bool) Stack[-1] == 0)
0x60c: IF (Stack[-1] == 0) GOTO 0x60f; Pop(1)

0x60d: Stack[-12] = (bool) 0
0x60e: Return(); Pop(10)

0x60f: @@@ GetPFPosition(Stack[-4]); Obj=0 // @poff=248
0x610: Pop(0)
0x611: @ GetPFPosition(Stack[-3])
0x612: Pop(0)
0x613: Stack[-2] = Stack[-4] - Stack[-3]; Pop(0);
0x614: Stack[-1] = Stack[-2] | Stack[-2]; Pop(0);
0x615: Pop(0); Push(Stack[-11] * Stack[-11]);
0x616: Pop(1); Push((bool) Stack[-2] < Stack[-1])
0x617: IF (Stack[-1] == 0) GOTO 0x61e; Pop(1)

0x618: PushEmpty(bool, float)
0x619: Stack[-13] = Stack[-1]
0x61a: Call2 0x557

0x61b: Pop(2)
0x61c: Stack[-12] = (bool) 1
0x61d: Return(); Pop(10)

0x61e: @ sync()
0x61f: Pop(0)
0x620: GOTO 0x5fc

0x621: PushEmpty()
0x622: Call2 0x6aa

0x623: Pop(0)
0x624: Stack[-12] = (bool) 0
0x625: Return(); Pop(10)

0x626: PushEmpty(cvector, cvector, cvector, float, float, cvector, cvector, cvector, float, float)
0x627: PushEmpty(bool, object)
0x628: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x629: Call2 0x865

0x62a: Pop(1)
0x62b: Pop(1); Push((bool) Stack[-1] == 0)
0x62c: IF (Stack[-1] == 0) GOTO 0x62f; Pop(1)

0x62d: Stack[-11] = (bool) 0
0x62e: Return(); Pop(10)

0x62f: PushEmpty(bool)
0x630: Call2 0x67f

0x631: Pop(0)
0x632: IF (Stack[-1] == 0) GOTO 0x640; Pop(1)

0x633: @@@ GetPFPosition(Stack[-5]); Obj=0 // @poff=248
0x634: Pop(0)
0x635: @ GetPFPosition(Stack[-4])
0x636: Pop(0)
0x637: Stack[-3] = Stack[-5] - Stack[-4]; Pop(0);
0x638: Stack[-2] = Stack[-3] | Stack[-3]; Pop(0);
0x639: @@@ GetAttackDistance(Stack[-1]); Obj=0 // @poff=216
0x63a: Pop(0)
0x63b: Push((int) 50)
0x63c: Stack[-2] = Stack[-2] + Stack[-1]; Pop(1);
0x63d: Pop(0); Push(Stack[-1] * Stack[-1]);
0x63e: Stack[-12] = Stack[-3] <= Stack[-1]; Pop(1);
0x63f: Return(); Pop(10)

0x640: Stack[-11] = (bool) 0
0x641: Return(); Pop(10)

0x642: PushEmpty(bool)
0x643: Stack[-1] = (bool) 0
0x644: PushEmpty(bool)
0x645: Call2 0x626

0x646: Pop(0)
0x647: IF (Stack[-1] == 0) GOTO 0x64d; Pop(1)

0x648: PushEmpty(bool)
0x649: Call2 0x652

0x64a: Pop(0)
0x64b: IF (Stack[-1] == 0) GOTO 0x64d; Pop(1)

0x64c: Stack[-1] = (bool) 1
0x64d: IF (Stack[-1] == 0) GOTO 0x650; Pop(1)

0x64e: Stack[-1] = (bool) 1
0x64f: Return(); Pop(0)

0x650: Stack[-1] = (bool) 0
0x651: Return(); Pop(0)

0x652: PushEmpty(object, bool, float, cvector, cvector, object, bool, float, cvector, cvector)
0x653: @ GetScene(Stack[-5])
0x654: Pop(0)
0x655: Stack[-4] = (bool) 0
0x656: PushEmpty(cvector, object)
0x657: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x658: Call2 0x7d3

0x659: Pop(1)
0x65a: Pop(1); Push(( -Stack[-1])
0x65b: @ FindDirLength(Stack[-4], Stack[-1], Stack[-4]T)
0x65c: Pop(1)
0x65d: Pop(0); Push((bool) Stack[-3] < Stack[4 + Tasks[-1].StackPointer])
0x65e: IF (Stack[-1] == 0) GOTO 0x660; Pop(1)

0x65f: GOTO 0x67c

0x660: @ Face(Stack[-0]T)
0x661: Pop(0)
0x662: Push("all") // @poff=170
0x663: Push("bjump") // @poff=366
0x664: @ PlayAnimation(Stack[-2], Stack[-1])
0x665: Pop(2)
0x666: @@@ GetPFPosition(Stack[-2]); Obj=0 // @poff=248
0x667: Pop(0)
0x668: @ GetPFPosition(Stack[-1])
0x669: Pop(0)
0x66a: @ WaitForAnimEnd()
0x66b: Pop(0)
0x66c: PushEmpty()
0x66d: Call2 0x6aa

0x66e: Pop(0)
0x66f: @ StopAsync()
0x670: Pop(0)
0x671: Push([0.0, 0.0, 0.0])
0x672: @ SetSpeed(Stack[-1])
0x673: Pop(1)
0x674: Stack[-4] = (bool) 1
0x675: PushEmpty(bool)
0x676: Call2 0x626

0x677: Pop(0)
0x678: Pop(1); Push((bool) Stack[-1] == 0)
0x679: IF (Stack[-1] == 0) GOTO 0x67b; Pop(1)

0x67a: GOTO 0x67c

0x67b: GOTO 0x656

0x67c: Stack[-4] = Stack[-11]
0x67d: Return(); Pop(10)

0x67e: Stack[-5] = 0
0x67f: PushEmpty(bool, bool)
0x680: Push("IsAttacking") // @poff=432
0x681: Push((int) 1)
0x682: Pop(2); Push(IsFuncExist(Stack[0 + Tasks[-1].StackPointer], Stack[-2], Stack[-1]))
0x683: IF (Stack[-1] == 0) GOTO 0x688; Pop(1)

0x684: @@@ IsAttacking(Stack[-1]); Obj=0 // @poff=456
0x685: Pop(0)
0x686: Stack[-1] = Stack[-3]
0x687: Return(); Pop(2)

0x688: Stack[-3] = (bool) 0
0x689: Return(); Pop(2)

0x68a: PushEmpty(float, int, float, int)
0x68b: Pop(0); Push((bool) Stack[2 + Tasks[-1].StackPointer] == 0)
0x68c: IF (Stack[-1] == 0) GOTO 0x68e; Pop(1)

0x68d: Return(); Pop(4)

0x68e: Push( Stack[5 + Tasks[-1].StackPointer] )
0x68f: IF (Stack[-1] == 0) GOTO 0x696; Pop(1)

0x690: Push((int) -1)
0x691: Stack[5 + Tasks[-1].StackPointer] = Stack[5 + Tasks[-1].StackPointer] + Stack[-1]; Pop(1);
0x692: Push((int) 0)
0x693: Pop(1); Push((bool) Stack[5 + Tasks[-1].StackPointer] > Stack[-1])
0x694: IF (Stack[-1] == 0) GOTO 0x696; Pop(1)

0x695: Return(); Pop(4)

0x696: @ rand(Stack[-2])
0x697: Pop(0)
0x698: PushEmpty(float)
0x699: Call2 0x6bc

0x69a: Pop(0)
0x69b: Pop(1); Push((bool) Stack[-3] < Stack[-1])
0x69c: IF (Stack[-1] == 0) GOTO 0x6a9; Pop(1)

0x69d: @ irand(Stack[-1], Stack[-2]T)
0x69e: Pop(0)
0x69f: Push((int) 1)
0x6a0: Stack[-2] = Stack[-2] + Stack[-1]; Pop(1);
0x6a1: Push("attack") // @poff=234
0x6a2: Pop(1); Push(Stack[-1] + Stack[-2]);
0x6a3: @ Speak(Stack[-1])
0x6a4: Pop(1)
0x6a5: PushEmpty(int)
0x6a6: Call2 0x6ba

0x6a7: Stack[5 + Tasks[-1].StackPointer] = Stack[-1]
0x6a8: Pop(1)
0x6a9: Return(); Pop(4)

0x6aa: PushEmpty(object)
0x6ab: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0x6ac: Call2 0x9b7

0x6ad: Pop(1)
0x6ae: Return(); Pop(0)

0x6af: Stack[-1] = (int) 0
0x6b0: Return(); Pop(0)

0x6b1: PushEmpty(string)
0x6b2: Stack[-1] = "attack_stay" // @poff=282
0x6b3: Call2 0x930

0x6b4: Pop(1)
0x6b5: Return(); Pop(0)

0x6b6: PushEmpty()
0x6b7: Return(); Pop(0)

0x6b8: Stack[-1] = (bool) 1
0x6b9: Return(); Pop(0)

0x6ba: Stack[-1] = (int) 1
0x6bb: Return(); Pop(0)

0x6bc: Stack[-1] = (float) 0.5
0x6bd: Return(); Pop(0)

0x6be: PushEmpty()
0x6bf: Call2 0x704

0x6c0: Pop(0)
0x6c1: PushEmpty()
0x6c2: Call2 0xc1b

0x6c3: Pop(0)
0x6c4: Return(); Pop(0)

0x6c5: PushEmpty(bool, bool, bool, bool)
0x6c6: PushEmpty(object)
0x6c7: Stack[-10] = Stack[-1]
0x6c8: Call2 0x9b7

0x6c9: Pop(1)
0x6ca: Push((int) 1)
0x6cb: Push((int) 5)
0x6cc: @ SetTimer(Stack[-2], Stack[-1])
0x6cd: Pop(2)
0x6ce: @ CanSee(Stack[-2], Stack[-9])
0x6cf: Pop(0)
0x6d0: Push(Stack[-2])
0x6d1: IF (Stack[-1] == 0) GOTO 0x6d8; Pop(1)

0x6d2: Stack[2 + Tasks[-1].StackPointer] = (bool)1
0x6d3: PushEmpty(object)
0x6d4: Stack[-10] = Stack[-1]
0x6d5: Call2 0x90b

0x6d6: Pop(1)
0x6d7: GOTO 0x6d9

0x6d8: Stack[2 + Tasks[-1].StackPointer] = (bool)0
0x6d9: PushEmpty(bool, object)
0x6da: Stack[-11] = Stack[-1]
0x6db: Call2 0x7e2

0x6dc: Pop(1)
0x6dd: IF (Stack[-1] == 0) GOTO 0x6e3; Pop(1)

0x6de: PushEmpty(object)
0x6df: Call2 0x959

0x6e0: Pop(0)
0x6e1: @ SendPlayerEnemy(Stack[-10], Stack[-1])
0x6e2: Pop(1)
0x6e3: PushEmpty(bool, object, float, float, bool, bool)
0x6e4: Stack[-15] = Stack[-5]
0x6e5: Stack[-14] = Stack[-4]
0x6e6: Stack[-13] = Stack[-3]
0x6e7: Stack[-12] = Stack[-2]
0x6e8: Stack[-11] = Stack[-1]
0x6e9: Call2 0x72e

0x6ea: Stack[-6] = Stack[-7]
0x6eb: Pop(6)
0x6ec: Push( Stack[2 + Tasks[-1].StackPointer] )
0x6ed: IF (Stack[-1] == 0) GOTO 0x6f1; Pop(1)

0x6ee: Push("head") // @poff=160
0x6ef: @ UnlookAsync(Stack[-1])
0x6f0: Pop(1)
0x6f1: Push((int) 1)
0x6f2: @ KillTimer(Stack[-1])
0x6f3: Pop(1)
0x6f4: Stack[-1] = Stack[-10]
0x6f5: Return(); Pop(4)

0x6f6: PushEmpty()
0x6f7: Push((int) 1)
0x6f8: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0x6f9: IF (Stack[-1] == 0) GOTO 0x6ff; Pop(1)

0x6fa: PushEmpty(object)
0x6fb: Stack[-1] = Stack[1 + Tasks[-1].StackPointer]
0x6fc: Call2 0x9b7

0x6fd: Pop(1)
0x6fe: GOTO 0x703

0x6ff: PushEmpty(int)
0x700: Stack[-2] = Stack[-1]
0x701: Call2 0x794

0x702: Pop(1)
0x703: Return(); Pop(0)

0x704: Push((int) 1)
0x705: @ KillTimer(Stack[-1])
0x706: Pop(1)
0x707: Push( Stack[2 + Tasks[-1].StackPointer] )
0x708: IF (Stack[-1] == 0) GOTO 0x70d; Pop(1)

0x709: Stack[2 + Tasks[-1].StackPointer] = (bool)0
0x70a: Push("head") // @poff=160
0x70b: @ UnlookAsync(Stack[-1])
0x70c: Pop(1)
0x70d: PushEmpty()
0x70e: Call2 0x7aa

0x70f: Pop(0)
0x710: Return(); Pop(0)

0x711: PushEmpty()
0x712: PushEmpty(bool)
0x713: Stack[-1] = (bool) 0
0x714: Pop(0); Push((bool) Stack[1 + Tasks[-1].StackPointer] == Stack[-2])
0x715: IF (Stack[-1] == 0) GOTO 0x719; Pop(1)

0x716: Pop(0); Push((bool) Stack[2 + Tasks[-1].StackPointer] == 0)
0x717: IF (Stack[-1] == 0) GOTO 0x719; Pop(1)

0x718: Stack[-1] = (bool) 1
0x719: IF (Stack[-1] == 0) GOTO 0x71f; Pop(1)

0x71a: Stack[2 + Tasks[-1].StackPointer] = (bool)1
0x71b: PushEmpty(object)
0x71c: Stack[-2] = Stack[-1]
0x71d: Call2 0x90b

0x71e: Pop(1)
0x71f: Return(); Pop(0)

0x720: PushEmpty()
0x721: PushEmpty(bool)
0x722: Stack[-1] = (bool) 0
0x723: Pop(0); Push((bool) Stack[1 + Tasks[-1].StackPointer] == Stack[-2])
0x724: IF (Stack[-1] == 0) GOTO 0x728; Pop(1)

0x725: Push( Stack[2 + Tasks[-1].StackPointer] )
0x726: IF (Stack[-1] == 0) GOTO 0x728; Pop(1)

0x727: Stack[-1] = (bool) 1
0x728: IF (Stack[-1] == 0) GOTO 0x72d; Pop(1)

0x729: Stack[2 + Tasks[-1].StackPointer] = (bool)0
0x72a: Push("head") // @poff=160
0x72b: @ UnlookAsync(Stack[-1])
0x72c: Pop(1)
0x72d: Return(); Pop(0)

0x72e: PushEmpty(bool, bool, object, cvector, cvector, cvector, float, object, bool, bool, object, cvector, cvector, cvector, float, object)
0x72f: Stack[0 + Tasks[-1].StackPointer] = (bool)0
0x730: Stack[1 + Tasks[-1].StackPointer] = Stack[-21]
0x731: Stack[-17] = Stack[-7]
0x732: PushEmpty(bool, object)
0x733: Stack[-23] = Stack[-1]
0x734: Call2 0x7ba

0x735: Pop(1)
0x736: Pop(1); Push((bool) Stack[-1] == 0)
0x737: IF (Stack[-1] == 0) GOTO 0x73a; Pop(1)

0x738: Stack[-22] = (bool) 0
0x739: Return(); Pop(16)

0x73a: @@ GetPosition(Stack[-5]); Obj=21 // @poff=10
0x73b: Pop(0)
0x73c: @ GetPosition(Stack[-4])
0x73d: Pop(0)
0x73e: Stack[-3] = Stack[-5] - Stack[-4]; Pop(0);
0x73f: Stack[-2] = Stack[-3] | Stack[-3]; Pop(0);
0x740: PushEmpty(bool)
0x741: Stack[-1] = (bool) 0
0x742: Push((int) 0)
0x743: Pop(1); Push((bool) Stack[-21] > Stack[-1])
0x744: IF (Stack[-1] == 0) GOTO 0x749; Pop(1)

0x745: Pop(0); Push(Stack[-20] * Stack[-20]);
0x746: Pop(1); Push((bool) Stack[-4] > Stack[-1])
0x747: IF (Stack[-1] == 0) GOTO 0x749; Pop(1)

0x748: Stack[-1] = (bool) 1
0x749: IF (Stack[-1] == 0) GOTO 0x74e; Pop(1)

0x74a: @ Stop()
0x74b: Pop(0)
0x74c: Stack[-22] = (bool) 0
0x74d: Return(); Pop(16)

0x74e: Pop(0); Push(Stack[-20] * Stack[-20]);
0x74f: Pop(1); Push((bool) Stack[-3] > Stack[-1])
0x750: IF (Stack[-1] == 0) GOTO 0x78c; Pop(1)

0x751: @@ GetPFPosition(Stack[-5]); Obj=21 // @poff=248
0x752: Pop(0)
0x753: @ FindPathTo(Stack[-1], Stack[-5])
0x754: Pop(0)
0x755: Pop(0); Push(( Stack[-1] != 0 )
0x756: IF (Stack[-1] == 0) GOTO 0x759; Pop(1)

0x757: Stack[-1] = Stack[-6]
0x758: Stack[-1] = 0
0x759: Pop(0); Push(( Stack[-6] != 0 )
0x75a: IF (Stack[-1] == 0) GOTO 0x77a; Pop(1)

0x75b: Push(Stack[-7])
0x75c: IF (Stack[-1] == 0) GOTO 0x763; Pop(1)

0x75d: Stack[-7] = (bool) 0
0x75e: @ RotatePath(Stack[-6], Stack[-8])
0x75f: Pop(0)
0x760: Pop(0); Push((bool) Stack[-8] == 0)
0x761: IF (Stack[-1] == 0) GOTO 0x763; Pop(1)

0x762: GOTO 0x792

0x763: Push((int) 0)
0x764: Push((float)0.30000001192092896)
0x765: @ SetTimer(Stack[-2], Stack[-1])
0x766: Pop(2)
0x767: PushEmpty(string)
0x768: Call2 0x7c1

0x769: Pop(0)
0x76a: PushEmpty(string)
0x76b: Call2 0x7c3

0x76c: Pop(0)
0x76d: @ FollowPath(Stack[-8], Stack[-20], Stack[-10], Stack[-2], Stack[-1])
0x76e: Pop(2)
0x76f: Pop(0); Push((bool) Stack[-8] == 0)
0x770: IF (Stack[-1] == 0) GOTO 0x778; Pop(1)

0x771: Push( Stack[0 + Tasks[-1].StackPointer] )
0x772: IF (Stack[-1] == 0) GOTO 0x776; Pop(1)

0x773: Stack[-6] = 0
0x774: GOTO 0x792

0x775: GOTO 0x777

0x776: GOTO 0x791

0x777: GOTO 0x779

0x778: Stack[-6] = 0
0x779: GOTO 0x78a

0x77a: Push((int) 0)
0x77b: @ KillTimer(Stack[-1])
0x77c: Pop(1)
0x77d: Push((float)0.5)
0x77e: @ Sleep(Stack[-1], Stack[-9])
0x77f: Pop(1)
0x780: Pop(0); Push((bool) Stack[-8] == 0)
0x781: IF (Stack[-1] == 0) GOTO 0x786; Pop(1)

0x782: Push( Stack[0 + Tasks[-1].StackPointer] )
0x783: IF (Stack[-1] == 0) GOTO 0x786; Pop(1)

0x784: Stack[-6] = 0
0x785: GOTO 0x792

0x786: Push((int) 0)
0x787: Push((float)0.30000001192092896)
0x788: @ SetTimer(Stack[-2], Stack[-1])
0x789: Pop(2)
0x78a: Stack[-1] = 0
0x78b: GOTO 0x790

0x78c: Push((int) 0)
0x78d: @ KillTimer(Stack[-1])
0x78e: Pop(1)
0x78f: GOTO 0x792

0x790: Stack[-6] = 0
0x791: GOTO 0x732

0x792: Stack[-22] = !Stack[0 + Tasks[-1].StackPointer]; Pop(0);
0x793: Return(); Pop(16)

0x794: PushEmpty()
0x795: Push((int) 0)
0x796: Pop(1); Push((bool) Stack[-2] != Stack[-1])
0x797: IF (Stack[-1] == 0) GOTO 0x799; Pop(1)

0x798: Return(); Pop(0)

0x799: PushEmpty(bool, object)
0x79a: Stack[-1] = Stack[1 + Tasks[-1].StackPointer]
0x79b: Call2 0x7ba

0x79c: Pop(1)
0x79d: Pop(1); Push((bool) Stack[-1] == 0)
0x79e: IF (Stack[-1] == 0) GOTO 0x7a0; Pop(1)

0x79f: Stack[0 + Tasks[-1].StackPointer] = (bool)1
0x7a0: Push((int) 0)
0x7a1: @ KillTimer(Stack[-1])
0x7a2: Pop(1)
0x7a3: @ Stop()
0x7a4: Pop(0)
0x7a5: Return(); Pop(0)

0x7a6: PushEmpty()
0x7a7: @ RequestClearPath(Stack[-1])
0x7a8: Pop(0)
0x7a9: Return(); Pop(0)

0x7aa: Stack[0 + Tasks[-1].StackPointer] = (bool)1
0x7ab: Push((int) 0)
0x7ac: @ KillTimer(Stack[-1])
0x7ad: Pop(1)
0x7ae: @ Stop()
0x7af: Pop(0)
0x7b0: Return(); Pop(0)

0x7b1: PushEmpty()
0x7b2: PushEmpty()
0x7b3: Call2 0x704

0x7b4: Pop(0)
0x7b5: PushEmpty(object)
0x7b6: Stack[-2] = Stack[-1]
0x7b7: Call2 0xac0

0x7b8: Pop(1)
0x7b9: Return(); Pop(0)

0x7ba: PushEmpty()
0x7bb: PushEmpty(bool, object)
0x7bc: Stack[-3] = Stack[-1]
0x7bd: Call2 0x865

0x7be: Stack[-2] = Stack[-4]
0x7bf: Pop(2)
0x7c0: Return(); Pop(0)

0x7c1: Stack[-1] = "walk" // @poff=468
0x7c2: Return(); Pop(0)

0x7c3: Stack[-1] = "run" // @poff=478
0x7c4: Return(); Pop(0)

0x7c5: PushEmpty()
0x7c6: Push((int) 2)
0x7c7: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0x7c8: IF (Stack[-1] == 0) GOTO 0x7cc; Pop(1)

0x7c9: Stack[-2] = "fire" // @poff=486
0x7ca: Return(); Pop(0)

0x7cb: GOTO 0x7d1

0x7cc: Push((int) 1)
0x7cd: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0x7ce: IF (Stack[-1] == 0) GOTO 0x7d1; Pop(1)

0x7cf: Stack[-2] = "bullet" // @poff=496
0x7d0: Return(); Pop(0)

0x7d1: Stack[-2] = "phys" // @poff=510
0x7d2: Return(); Pop(0)

0x7d3: PushEmpty(cvector, cvector, cvector, cvector)
0x7d4: @ GetPosition(Stack[-2])
0x7d5: Pop(0)
0x7d6: @@ GetPosition(Stack[-1]); Obj=5 // @poff=10
0x7d7: Pop(0)
0x7d8: Stack[-6] = Stack[-1] - Stack[-2]; Pop(0);
0x7d9: Return(); Pop(4)

0x7da: PushEmpty(cvector, cvector, cvector, cvector, cvector, cvector)
0x7db: @ GetPosition(Stack[-3])
0x7dc: Pop(0)
0x7dd: @@ GetPosition(Stack[-2]); Obj=7 // @poff=10
0x7de: Pop(0)
0x7df: Stack[-1] = Stack[-2] - Stack[-3]; Pop(0);
0x7e0: Stack[-8] = Stack[-1] | Stack[-1]; Pop(0);
0x7e1: Return(); Pop(6)

0x7e2: PushEmpty(bool, bool)
0x7e3: @ IsPlayerActor(Stack[-3], Stack[-1])
0x7e4: Pop(0)
0x7e5: Stack[-1] = Stack[-4]
0x7e6: Return(); Pop(2)

0x7e7: PushEmpty(bool, bool)
0x7e8: Push("HasProperty") // @poff=520
0x7e9: Push((int) 2)
0x7ea: Pop(2); Push(IsFuncExist(Stack[-6], Stack[-2], Stack[-1]))
0x7eb: Pop(1); Push((bool) Stack[-1] == 0)
0x7ec: IF (Stack[-1] == 0) GOTO 0x7ef; Pop(1)

0x7ed: Stack[-5] = (bool) 0
0x7ee: Return(); Pop(2)

0x7ef: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=544
0x7f0: Pop(0)
0x7f1: Stack[-1] = Stack[-5]
0x7f2: Return(); Pop(2)

0x7f3: PushEmpty(int, string, int, float, float, float, int, string, int, float, float, float)
0x7f4: PushEmpty(bool, object, string)
0x7f5: Stack[-18] = Stack[-2]
0x7f6: Stack[-1] = "health" // @poff=556
0x7f7: Call2 0x7e7

0x7f8: Pop(2)
0x7f9: Pop(1); Push((bool) Stack[-1] == 0)
0x7fa: IF (Stack[-1] == 0) GOTO 0x7fd; Pop(1)

0x7fb: Stack[-16] = (float) 0.0
0x7fc: Return(); Pop(12)

0x7fd: PushEmpty(bool, object, string)
0x7fe: Stack[-18] = Stack[-2]
0x7ff: Stack[-1] = "armor" // @poff=570
0x800: Call2 0x7e7

0x801: Pop(2)
0x802: Pop(1); Push((bool) Stack[-1] == 0)
0x803: IF (Stack[-1] == 0) GOTO 0x806; Pop(1)

0x804: Stack[-6] = (int) 0
0x805: GOTO 0x809

0x806: Push("armor") // @poff=570
0x807: @@ GetProperty(Stack[-1], Stack[-7]); Obj=16 // @poff=582
0x808: Pop(1)
0x809: Push("armor_") // @poff=594
0x80a: PushEmpty(string, int)
0x80b: Stack[-16] = Stack[-1]
0x80c: Call2 0x7c5

0x80d: Pop(1)
0x80e: Stack[-7] = Stack[-2] + Stack[-1]; Pop(2);
0x80f: PushEmpty(bool, object, string)
0x810: Stack[-18] = Stack[-2]
0x811: Stack[-8] = Stack[-1]
0x812: Call2 0x7e7

0x813: Pop(2)
0x814: Pop(1); Push((bool) Stack[-1] == 0)
0x815: IF (Stack[-1] == 0) GOTO 0x818; Pop(1)

0x816: Stack[-4] = (int) 0
0x817: GOTO 0x81a

0x818: @@ GetProperty(Stack[-5], Stack[-4]); Obj=15 // @poff=582
0x819: Pop(0)
0x81a: PushEmpty(float, float, float)
0x81b: Pop(0); Push(Stack[-9] + Stack[-7]);
0x81c: Push((float)100.0)
0x81d: Stack[-4] = Stack[-2] / Stack[-1]; Pop(2);
0x81e: Stack[-1] = (int) 1
0x81f: Call2 0x969

0x820: Stack[-3] = Stack[-6]
0x821: Pop(3)
0x822: Push("health") // @poff=556
0x823: @@ GetProperty(Stack[-1], Stack[-3]); Obj=16 // @poff=582
0x824: Pop(1)
0x825: Push((int) 1)
0x826: Pop(1); Push(Stack[-1] - Stack[-4]);
0x827: Stack[-2] = Stack[-15] * Stack[-1]; Pop(1);
0x828: Push("health") // @poff=556
0x829: PushEmpty(float, float, float, float)
0x82a: Stack[-3] = Stack[-7] - Stack[-6]; Pop(0);
0x82b: Stack[-2] = (int) 0
0x82c: Stack[-1] = (int) 1
0x82d: Call2 0x970

0x82e: Pop(3)
0x82f: @@ SetProperty(Stack[-2], Stack[-1]); Obj=17 // @poff=608
0x830: Pop(2)
0x831: PushEmpty(bool, object)
0x832: Stack[-17] = Stack[-1]
0x833: Call2 0x7e2

0x834: Pop(1)
0x835: IF (Stack[-1] == 0) GOTO 0x83a; Pop(1)

0x836: PushEmpty(float)
0x837: Stack[-1] = -Stack[-2]; Pop(0);
0x838: Call2 0x9a4

0x839: Pop(1)
0x83a: Stack[-1] = Stack[-16]
0x83b: Return(); Pop(12)

0x83c: PushEmpty(bool, bool)
0x83d: @@ IsDead(Stack[-1]); Obj=3 // @poff=620
0x83e: Pop(0)
0x83f: Stack[-1] = Stack[-4]
0x840: Return(); Pop(2)

0x841: PushEmpty(object, object, object, object)
0x842: Pop(0); PushNull((bool) Stack[-5] == 0)
0x843: IF (Stack[-1] == 0) GOTO 0x846; Pop(1)

0x844: Stack[-6] = (bool) 0
0x845: Return(); Pop(4)

0x846: PushEmpty(bool)
0x847: Stack[-1] = (bool) 0
0x848: Push("IsDead") // @poff=627
0x849: Push((int) 1)
0x84a: Pop(2); Push(IsFuncExist(Stack[-8], Stack[-2], Stack[-1]))
0x84b: IF (Stack[-1] == 0) GOTO 0x852; Pop(1)

0x84c: PushEmpty(bool, object)
0x84d: Stack[-8] = Stack[-1]
0x84e: Call2 0x83c

0x84f: Pop(1)
0x850: IF (Stack[-1] == 0) GOTO 0x852; Pop(1)

0x851: Stack[-1] = (bool) 1
0x852: IF (Stack[-1] == 0) GOTO 0x855; Pop(1)

0x853: Stack[-6] = (bool) 0
0x854: Return(); Pop(4)

0x855: @ GetScene(Stack[-2])
0x856: Pop(0)
0x857: Pop(0); PushNull((bool) Stack[-2] == 0)
0x858: IF (Stack[-1] == 0) GOTO 0x85b; Pop(1)

0x859: Stack[-6] = (bool) 0
0x85a: Return(); Pop(4)

0x85b: @@ GetScene(Stack[-1]); Obj=5 // @poff=641
0x85c: Pop(0)
0x85d: Pop(0); Push((bool) Stack[-2] != Stack[-1])
0x85e: IF (Stack[-1] == 0) GOTO 0x861; Pop(1)

0x85f: Stack[-6] = (bool) 0
0x860: Return(); Pop(4)

0x861: Stack[-6] = (bool) 1
0x862: Return(); Pop(4)

0x863: Stack[-1] = 0
0x864: Stack[-2] = 0
0x865: PushEmpty(int, int)
0x866: PushEmpty(bool, object)
0x867: Stack[-5] = Stack[-1]
0x868: Call2 0x841

0x869: Pop(1)
0x86a: Pop(1); Push((bool) Stack[-1] == 0)
0x86b: IF (Stack[-1] == 0) GOTO 0x86e; Pop(1)

0x86c: Stack[-4] = (bool) 0
0x86d: Return(); Pop(2)

0x86e: PushEmpty(bool, object, string)
0x86f: Stack[-6] = Stack[-2]
0x870: Stack[-1] = "noaccess" // @poff=650
0x871: Call2 0x7e7

0x872: Pop(2)
0x873: Pop(1); Push((bool) Stack[-1] == 0)
0x874: IF (Stack[-1] == 0) GOTO 0x877; Pop(1)

0x875: Stack[-4] = (bool) 1
0x876: Return(); Pop(2)

0x877: Push("noaccess") // @poff=650
0x878: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=582
0x879: Pop(1)
0x87a: Push((int) 0)
0x87b: Stack[-5] = Stack[-2] == Stack[-1]; Pop(1);
0x87c: Return(); Pop(2)

0x87d: PushEmpty(bool, int, cvector, cvector, cvector, cvector, string, bool, int, cvector, cvector, cvector, cvector, string)
0x87e: Pop(0); PushNull((bool) Stack[-15] == 0)
0x87f: IF (Stack[-1] == 0) GOTO 0x881; Pop(1)

0x880: Return(); Pop(14)

0x881: @ IsDead(Stack[-7])
0x882: Pop(0)
0x883: Push(Stack[-7])
0x884: IF (Stack[-1] == 0) GOTO 0x886; Pop(1)

0x885: Return(); Pop(14)

0x886: @ GetSecondaryAnimationType(Stack[-6])
0x887: Pop(0)
0x888: Push((int) 0)
0x889: Pop(1); Push((bool) Stack[-7] < Stack[-1])
0x88a: IF (Stack[-1] == 0) GOTO 0x88c; Pop(1)

0x88b: Return(); Pop(14)

0x88c: @@ GetPosition(Stack[-5]); Obj=15 // @poff=10
0x88d: Pop(0)
0x88e: @ GetPosition(Stack[-4])
0x88f: Pop(0)
0x890: @ GetDirection(Stack[-3])
0x891: Pop(0)
0x892: Stack[-2] = Stack[-4] - Stack[-5]; Pop(0);
0x893: Push(CvectorIndex(Stack[-2], 0))
0x894: Push(CvectorIndex(Stack[-4], 0))
0x895: Pop(2); Push(Stack[-2] * Stack[-1]);
0x896: Push(CvectorIndex(Stack[-3], 2))
0x897: Push(CvectorIndex(Stack[-5], 2))
0x898: Pop(2); Push(Stack[-2] * Stack[-1]);
0x899: Pop(2); Push(Stack[-2] + Stack[-1]);
0x89a: Push((int) 0)
0x89b: Pop(2); Push((bool) Stack[-2] >= Stack[-1])
0x89c: IF (Stack[-1] == 0) GOTO 0x89f; Pop(1)

0x89d: Stack[-1] = "fhit" // @poff=668
0x89e: GOTO 0x8a0

0x89f: Stack[-1] = "bhit" // @poff=678
0x8a0: Push("hit_react") // @poff=688
0x8a1: Push("1") // @poff=708
0x8a2: Pop(1); Push(Stack[-3] + Stack[-1]);
0x8a3: Push("2") // @poff=712
0x8a4: Pop(1); Push(Stack[-4] + Stack[-1]);
0x8a5: Push((int) -10)
0x8a6: @ FadeSecondaryAnimation(Stack[-4], Stack[-3], Stack[-2], Stack[-1])
0x8a7: Pop(4)
0x8a8: Return(); Pop(14)

0x8a9: PushEmpty(cvector, object, int, bool, cvector, cvector, int, int, cvector, cvector, object, int, bool, cvector, cvector, int, int, cvector)
0x8aa: PushEmpty(bool)
0x8ab: Stack[-1] = (bool) 0
0x8ac: PushEmpty(bool)
0x8ad: Stack[-1] = (bool) 0
0x8ae: Push(Stack[-23])
0x8af: IF (Stack[-1] == 0) GOTO 0x8b4; Pop(1)

0x8b0: Push((int) 4)
0x8b1: Pop(1); Push((bool) Stack[-23] != Stack[-1])
0x8b2: IF (Stack[-1] == 0) GOTO 0x8b4; Pop(1)

0x8b3: Stack[-1] = (bool) 1
0x8b4: IF (Stack[-1] == 0) GOTO 0x8b9; Pop(1)

0x8b5: Push((int) 5)
0x8b6: Pop(1); Push((bool) Stack[-22] != Stack[-1])
0x8b7: IF (Stack[-1] == 0) GOTO 0x8b9; Pop(1)

0x8b8: Stack[-1] = (bool) 1
0x8b9: IF (Stack[-1] == 0) GOTO 0x8e8; Pop(1)

0x8ba: PushEmpty(cvector, cvector)
0x8bb: PushEmpty(cvector, object)
0x8bc: Stack[-25] = Stack[-1]
0x8bd: Call2 0x7d3

0x8be: Stack[-2] = Stack[-3]
0x8bf: Pop(2)
0x8c0: Call2 0x95f

0x8c1: Stack[-2] = Stack[-11]
0x8c2: Pop(2)
0x8c3: @ CreateVectorVector(Stack[-8])
0x8c4: Pop(0)
0x8c5: Stack[-7] = (int) 1
0x8c6: Push("hit") // @poff=670
0x8c7: Pop(1); Push(Stack[-1] + Stack[-8]);
0x8c8: @ GetGeometryLocator(Stack[-1], Stack[-7], Stack[-6], Stack[-5])
0x8c9: Pop(1)
0x8ca: Pop(0); Push((bool) Stack[-6] == 0)
0x8cb: IF (Stack[-1] == 0) GOTO 0x8cd; Pop(1)

0x8cc: GOTO 0x8d6

0x8cd: Pop(0); Push(Stack[-4] | Stack[-9]);
0x8ce: Push((float)0.7071067690849304)
0x8cf: Pop(2); Push((bool) Stack[-2] >= Stack[-1])
0x8d0: IF (Stack[-1] == 0) GOTO 0x8d3; Pop(1)

0x8d1: @@ add(Stack[-5]); Obj=8 // @poff=716
0x8d2: Pop(0)
0x8d3: Push((int) 1)
0x8d4: Stack[-8] = Stack[-8] + Stack[-1]; Pop(1);
0x8d5: GOTO 0x8c6

0x8d6: @@ size(Stack[-3]); Obj=8 // @poff=720
0x8d7: Pop(0)
0x8d8: Push(Stack[-3])
0x8d9: IF (Stack[-1] == 0) GOTO 0x8e7; Pop(1)

0x8da: @ irand(Stack[-2], Stack[-3])
0x8db: Pop(0)
0x8dc: @@ get(Stack[-1], Stack[-2]); Obj=8 // @poff=725
0x8dd: Pop(0)
0x8de: PushEmpty(object, int, float, cvector, cvector)
0x8df: Stack[-26] = Stack[-5]
0x8e0: Stack[-25] = Stack[-4]
0x8e1: Stack[-24] = Stack[-3]
0x8e2: Stack[-6] = Stack[-2]
0x8e3: Stack[-1] = -Stack[-14]; Pop(0);
0x8e4: Call2 0x8ed

0x8e5: Pop(5)
0x8e6: Return(); Pop(18)

0x8e7: Stack[-8] = 0
0x8e8: PushEmpty(object)
0x8e9: Stack[-22] = Stack[-1]
0x8ea: Call2 0x87d

0x8eb: Pop(1)
0x8ec: Return(); Pop(18)

0x8ed: PushEmpty(object, object, object, object)
0x8ee: @ GetScene(Stack[-2])
0x8ef: Pop(0)
0x8f0: Push("scripted") // @poff=729
0x8f1: Push("blood_dir.xml") // @poff=747
0x8f2: @ AddActorByType(Stack[-3], Stack[-2], Stack[-4], Stack[-8], Stack[-7], Stack[-1])
0x8f3: Pop(2)
0x8f4: PushEmpty(object)
0x8f5: Stack[-10] = Stack[-1]
0x8f6: Call2 0x87d

0x8f7: Pop(1)
0x8f8: Return(); Pop(4)

0x8f9: Stack[-1] = 0
0x8fa: Stack[-2] = 0
0x8fb: PushEmpty(cvector, cvector, cvector, cvector, cvector, cvector)
0x8fc: @@ GetPosition(Stack[-3]); Obj=7 // @poff=10
0x8fd: Pop(0)
0x8fe: @ GetPosition(Stack[-2])
0x8ff: Pop(0)
0x900: Stack[-1] = Stack[-3] - Stack[-2]; Pop(0);
0x901: Push(CvectorIndex(Stack[-1], 0))
0x902: Push(CvectorIndex(Stack[-2], 2))
0x903: @ RotateAsync(Stack[-2], Stack[-1])
0x904: Pop(2)
0x905: Return(); Pop(6)

0x906: PushEmpty(bool, bool)
0x907: @ IsLoaded(Stack[-1])
0x908: Pop(0)
0x909: Stack[-1] = Stack[-3]
0x90a: Return(); Pop(2)

0x90b: PushEmpty(float, cvector, float, cvector)
0x90c: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=146
0x90d: Pop(0)
0x90e: Stack[-1] = [0.0, 0.0, 0.0]
0x90f: Push(CvectorIndex(Stack[-1], 1))
0x910: Stack[-3] = Stack[-1]
0x911: CvectorIndex(Stack[-2], 1) = Stack[-1];
0x912: Push("head") // @poff=160
0x913: @ LookAsync(Stack[-6], Stack[-1], Stack[-2])
0x914: Pop(1)
0x915: Return(); Pop(4)

0x916: PushEmpty(bool, bool)
0x917: PushEmpty(bool, int, int)
0x918: Stack[-7] = Stack[-2]
0x919: Stack[-6] = Stack[-1]
0x91a: Call2 0x97b

0x91b: Pop(2)
0x91c: IF (Stack[-1] == 0) GOTO 0x920; Pop(1)

0x91d: Push((int) 0)
0x91e: @ AddItem(Stack[-2], Stack[-6], Stack[-1])
0x91f: Pop(1)
0x920: Return(); Pop(2)

0x921: PushEmpty(int, bool, int, bool)
0x922: PushEmpty(bool, int, int)
0x923: Stack[-10] = Stack[-2]
0x924: Stack[-9] = Stack[-1]
0x925: Call2 0x97b

0x926: Pop(2)
0x927: IF (Stack[-1] == 0) GOTO 0x92f; Pop(1)

0x928: @ irand(Stack[-2], Stack[-5])
0x929: Pop(0)
0x92a: Push((int) 0)
0x92b: Push((int) 1)
0x92c: Pop(1); Push(Stack[-4] + Stack[-1]);
0x92d: @ AddItem(Stack[-3], Stack[-10], Stack[-2], Stack[-1])
0x92e: Pop(2)
0x92f: Return(); Pop(4)

0x930: PushEmpty(bool, int, bool, int, bool, float, cvector, cvector, bool, int, bool, int, bool, float, cvector, cvector)
0x931: @ IsExisting3DSound(Stack[-8], Stack[-17])
0x932: Pop(0)
0x933: Pop(0); Push((bool) Stack[-8] == 0)
0x934: IF (Stack[-1] == 0) GOTO 0x949; Pop(1)

0x935: Stack[-7] = (int) 0
0x936: Push((int) 1)
0x937: Pop(1); Push(Stack[-8] + Stack[-1]);
0x938: Pop(1); Push(Stack[-18] + Stack[-1]);
0x939: @ IsExisting3DSound(Stack[-7], Stack[-1])
0x93a: Pop(1)
0x93b: Pop(0); Push((bool) Stack[-6] == 0)
0x93c: IF (Stack[-1] == 0) GOTO 0x93e; Pop(1)

0x93d: GOTO 0x941

0x93e: Push((int) 1)
0x93f: Stack[-8] = Stack[-8] + Stack[-1]; Pop(1);
0x940: GOTO 0x936

0x941: Pop(0); Push((bool) Stack[-7] == 0)
0x942: IF (Stack[-1] == 0) GOTO 0x944; Pop(1)

0x943: Return(); Pop(16)

0x944: @ irand(Stack[-5], Stack[-7])
0x945: Pop(0)
0x946: Push((int) 1)
0x947: Pop(1); Push(Stack[-6] + Stack[-1]);
0x948: Stack[-18] = Stack[-18] + Stack[-1]; Pop(1);
0x949: @ Is3DSoundLoaded(Stack[-4], Stack[-17])
0x94a: Pop(0)
0x94b: Push(Stack[-4])
0x94c: IF (Stack[-1] == 0) GOTO 0x958; Pop(1)

0x94d: @ GetEyesHeight(Stack[-3])
0x94e: Pop(0)
0x94f: @ GetDirection(Stack[-2])
0x950: Pop(0)
0x951: Push((int) 50)
0x952: Stack[-2] = Stack[-3] * Stack[-1]; Pop(1);
0x953: Push(CvectorIndex(Stack[-1], 1))
0x954: Stack[-1] = Stack[-1] + Stack[-4]; Pop(0);
0x955: CvectorIndex(Stack[-2], 1) = Stack[-1];
0x956: @ PlayGlobalSound(Stack[-17], Stack[-1])
0x957: Pop(0)
0x958: Return(); Pop(16)

0x959: PushEmpty(object, object)
0x95a: @ self(Stack[-1])
0x95b: Pop(0)
0x95c: Stack[-1] = Stack[-3]
0x95d: Return(); Pop(2)

0x95e: Stack[-1] = 0
0x95f: PushEmpty(float, float)
0x960: Pop(0); Push(Stack[-3] | Stack[-3]);
0x961: Stack[-2] = Sqrt(Stack[-1]); Pop(1);
0x962: Push((float)9.999999974752427e-07)
0x963: Pop(1); Push((bool) Stack[-2] < Stack[-1])
0x964: IF (Stack[-1] == 0) GOTO 0x967; Pop(1)

0x965: Stack[-4] = [0.0, 0.0, 0.0]
0x966: Return(); Pop(2)

0x967: Stack[-4] = Stack[-3] / Stack[-1]; Pop(0);
0x968: Return(); Pop(2)

0x969: PushEmpty()
0x96a: Pop(0); Push((bool) Stack[-2] < Stack[-1])
0x96b: IF (Stack[-1] == 0) GOTO 0x96e; Pop(1)

0x96c: Stack[-2] = Stack[-3]
0x96d: GOTO 0x96f

0x96e: Stack[-1] = Stack[-3]
0x96f: Return(); Pop(0)

0x970: PushEmpty()
0x971: Pop(0); Push((bool) Stack[-3] < Stack[-2])
0x972: IF (Stack[-1] == 0) GOTO 0x975; Pop(1)

0x973: Stack[-2] = Stack[-4]
0x974: Return(); Pop(0)

0x975: Pop(0); Push((bool) Stack[-3] > Stack[-1])
0x976: IF (Stack[-1] == 0) GOTO 0x979; Pop(1)

0x977: Stack[-1] = Stack[-4]
0x978: Return(); Pop(0)

0x979: Stack[-3] = Stack[-4]
0x97a: Return(); Pop(0)

0x97b: PushEmpty(int, int)
0x97c: @ irand(Stack[-1], Stack[-3])
0x97d: Pop(0)
0x97e: Stack[-5] = Stack[-1] < Stack[-4]; Pop(0);
0x97f: Return(); Pop(2)

0x980: PushEmpty()
0x981: Push(CvectorIndex(Stack[-2], 0))
0x982: Push(CvectorIndex(Stack[-2], 0))
0x983: Pop(2); Push(Stack[-2] * Stack[-1]);
0x984: Push(CvectorIndex(Stack[-3], 2))
0x985: Push(CvectorIndex(Stack[-3], 2))
0x986: Pop(2); Push(Stack[-2] * Stack[-1]);
0x987: Stack[-5] = Stack[-2] + Stack[-1]; Pop(2);
0x988: Return(); Pop(0)

0x989: PushEmpty()
0x98a: Push(CvectorIndex(Stack[-1], 0))
0x98b: Push(CvectorIndex(Stack[-2], 0))
0x98c: Pop(2); Push(Stack[-2] * Stack[-1]);
0x98d: Push(CvectorIndex(Stack[-2], 2))
0x98e: Push(CvectorIndex(Stack[-3], 2))
0x98f: Pop(2); Push(Stack[-2] * Stack[-1]);
0x990: Pop(2); Push(Stack[-2] + Stack[-1]);
0x991: Stack[-3] = Sqrt(Stack[-1]); Pop(1);
0x992: Return(); Pop(0)

0x993: PushEmpty()
0x994: PushEmpty(float, cvector, cvector)
0x995: Stack[-5] = Stack[-2]
0x996: Stack[-4] = Stack[-1]
0x997: Call2 0x980

0x998: Pop(2)
0x999: PushEmpty(float, cvector)
0x99a: Stack[-5] = Stack[-1]
0x99b: Call2 0x989

0x99c: Pop(1)
0x99d: PushEmpty(float, cvector)
0x99e: Stack[-5] = Stack[-1]
0x99f: Call2 0x989

0x9a0: Pop(1)
0x9a1: Pop(2); Push(Stack[-2] * Stack[-1]);
0x9a2: Stack[-5] = Stack[-2] / Stack[-1]; Pop(2);
0x9a3: Return(); Pop(0)

0x9a4: PushEmpty(object, object)
0x9a5: @ CreateFloatVector(Stack[-1])
0x9a6: Pop(0)
0x9a7: @@ add(Stack[-3]); Obj=1 // @poff=716
0x9a8: Pop(0)
0x9a9: Push((int) 15)
0x9aa: @ SendWorldWndMessage(Stack[-1], Stack[-2])
0x9ab: Pop(1)
0x9ac: Return(); Pop(2)

0x9ad: Stack[-1] = 0
0x9ae: PushEmpty(float, float)
0x9af: @ GetGameTime(Stack[-1])
0x9b0: Pop(0)
0x9b1: Push((int) 1)
0x9b2: PushEmpty(int)
0x9b3: Push((int) 24)
0x9b4: Stack[-2] = Stack[-4] / Stack[-1]; Pop(1);
0x9b5: Stack[-5] = Stack[-2] + Stack[-1]; Pop(2);
0x9b6: Return(); Pop(2)

0x9b7: PushEmpty(bool, bool)
0x9b8: @ IsPlayerActor(Stack[-3], Stack[-1])
0x9b9: Pop(0)
0x9ba: Push(Stack[-1])
0x9bb: IF (Stack[-1] == 0) GOTO 0x9bf; Pop(1)

0x9bc: Push("attack") // @poff=234
0x9bd: @ PlayGlobalMusic(Stack[-1])
0x9be: Pop(1)
0x9bf: Return(); Pop(2)

0x9c0: PushEmpty(object, object)
0x9c1: @ GetScene(Stack[-1])
0x9c2: Pop(0)
0x9c3: Push("battle") // @poff=775
0x9c4: PushEmpty(object)
0x9c5: Call2 0x959

0x9c6: Pop(0)
0x9c7: @ BroadcastMessage(Stack[-2], Stack[-1], Stack[-3])
0x9c8: Pop(2)
0x9c9: Return(); Pop(2)

0x9ca: Stack[-1] = 0
0x9cb: PushEmpty(int, int)
0x9cc: Push("branch") // @poff=789
0x9cd: @ GetVariable(Stack[-1], Stack[-2])
0x9ce: Pop(1)
0x9cf: Stack[-1] = Stack[-3]
0x9d0: Return(); Pop(2)

0x9d1: PushEmpty()
0x9d2: PushEmpty(int)
0x9d3: Call2 0x9cb

0x9d4: Pop(0)
0x9d5: Push((int) 1)
0x9d6: Pop(2); Push((bool) Stack[-2] == Stack[-1])
0x9d7: IF (Stack[-1] == 0) GOTO 0x9db; Pop(1)

0x9d8: @ WorkWithCorpse(Stack[-1])
0x9d9: Pop(0)
0x9da: GOTO 0x9dd

0x9db: @ Barter(Stack[-1])
0x9dc: Pop(0)
0x9dd: Return(); Pop(0)

0x9de: PushEmpty(int, bool, int, bool)
0x9df: Pop(0); Push((bool) Stack[-6] > Stack[-5])
0x9e0: IF (Stack[-1] == 0) GOTO 0x9e5; Pop(1)

0x9e1: Push("GenerateMoney: iMin > iMax") // @poff=803
0x9e2: @ Trace(Stack[-1])
0x9e3: Pop(1)
0x9e4: Return(); Pop(4)

0x9e5: Stack[-2] = (int) 0
0x9e6: Pop(0); Push((bool) Stack[-6] != Stack[-5])
0x9e7: IF (Stack[-1] == 0) GOTO 0x9ec; Pop(1)

0x9e8: Pop(0); Push(Stack[-5] - Stack[-6]);
0x9e9: @ irand(Stack[-3], Stack[-1])
0x9ea: Pop(1)
0x9eb: GOTO 0x9f0

0x9ec: Push((int) 0)
0x9ed: Pop(1); Push((bool) Stack[-7] == Stack[-1])
0x9ee: IF (Stack[-1] == 0) GOTO 0x9f0; Pop(1)

0x9ef: Return(); Pop(4)

0x9f0: Stack[-2] = Stack[-2] + Stack[-6]; Pop(0);
0x9f1: Push((int) 0)
0x9f2: Pop(1); Push((bool) Stack[-3] == Stack[-1])
0x9f3: IF (Stack[-1] == 0) GOTO 0x9f5; Pop(1)

0x9f4: Return(); Pop(4)

0x9f5: PushEmpty(int, string)
0x9f6: Stack[-1] = "Money" // @poff=857
0x9f7: Call2 0xaa6

0x9f8: Pop(1)
0x9f9: Push((int) 0)
0x9fa: @ AddItem(Stack[-3], Stack[-2], Stack[-1], Stack[-4])
0x9fb: Pop(2)
0x9fc: Return(); Pop(4)

0x9fd: PushEmpty(object, int, bool, object, int, bool)
0x9fe: @ CreateInvItem(Stack[-3])
0x9ff: Pop(0)
0xa00: @@ SetItemName(Stack[-7]); Obj=3 // @poff=869
0xa01: Pop(0)
0xa02: Push("Organ") // @poff=881
0xa03: Push((int) 1)
0xa04: @@ SetProperty(Stack[-2], Stack[-1]); Obj=5 // @poff=608
0xa05: Pop(2)
0xa06: @@ GetItemID(Stack[-2]); Obj=3 // @poff=893
0xa07: Pop(0)
0xa08: Push((int) 0)
0xa09: Push((int) 1)
0xa0a: @ AddItem(Stack[-3], Stack[-5], Stack[-2], Stack[-1])
0xa0b: Pop(2)
0xa0c: Return(); Pop(6)

0xa0d: Stack[-3] = 0
0xa0e: PushEmpty(int)
0xa0f: Call2 0x9cb

0xa10: Pop(0)
0xa11: Push((int) 1)
0xa12: Pop(2); Push((bool) Stack[-2] != Stack[-1])
0xa13: IF (Stack[-1] == 0) GOTO 0xa15; Pop(1)

0xa14: Return(); Pop(0)

0xa15: PushEmpty(string)
0xa16: Stack[-1] = "liver" // @poff=903
0xa17: Call2 0x9fd

0xa18: Pop(1)
0xa19: PushEmpty(string)
0xa1a: Stack[-1] = "kidney" // @poff=915
0xa1b: Call2 0x9fd

0xa1c: Pop(1)
0xa1d: PushEmpty(string)
0xa1e: Stack[-1] = "heart" // @poff=929
0xa1f: Call2 0x9fd

0xa20: Pop(1)
0xa21: PushEmpty(string)
0xa22: Stack[-1] = "blood" // @poff=941
0xa23: Call2 0x9fd

0xa24: Pop(1)
0xa25: Return(); Pop(0)

0xa26: PushEmpty(bool, bool)
0xa27: Push((int) 0)
0xa28: @ ClearSubContainer(Stack[-1])
0xa29: Pop(1)
0xa2a: PushEmpty(int, int)
0xa2b: Stack[-2] = (int) 0
0xa2c: Push((int) 10)
0xa2d: PushEmpty(int)
0xa2e: Call2 0x9ae

0xa2f: Pop(0)
0xa30: Push((int) 10)
0xa31: Pop(2); Push(Stack[-2] * Stack[-1]);
0xa32: Stack[-3] = Stack[-2] + Stack[-1]; Pop(2);
0xa33: Call2 0x9de

0xa34: Pop(2)
0xa35: PushEmpty(string, int, int)
0xa36: Stack[-3] = "hook" // @poff=953
0xa37: Stack[-2] = (int) 1
0xa38: Stack[-1] = (int) 4
0xa39: Call2 0x916

0xa3a: Pop(3)
0xa3b: PushEmpty(string, int, int)
0xa3c: Stack[-3] = "watch" // @poff=963
0xa3d: Stack[-2] = (int) 1
0xa3e: Stack[-1] = (int) 10
0xa3f: Call2 0x916

0xa40: Pop(3)
0xa41: Return(); Pop(2)

0xa42: PushEmpty(int, bool, int, int, bool, int)
0xa43: Push((int) 0)
0xa44: @ ClearSubContainer(Stack[-1])
0xa45: Pop(1)
0xa46: PushEmpty(int)
0xa47: Call2 0x9ae

0xa48: Stack[-1] = Stack[-2]
0xa49: Pop(1)
0xa4a: Push(Stack[-7])
0xa4b: IF (Stack[-1] == 0) GOTO 0xa78; Pop(1)

0xa4c: PushEmpty(string, int, int, int)
0xa4d: Stack[-4] = "alpha_pills" // @poff=975
0xa4e: Stack[-3] = (int) 1
0xa4f: Stack[-2] = (int) 2
0xa50: Stack[-1] = (int) 4
0xa51: Call2 0x921

0xa52: Pop(4)
0xa53: PushEmpty(string, int, int, int)
0xa54: Stack[-4] = "meradorm" // @poff=999
0xa55: Stack[-3] = (int) 2
0xa56: Stack[-2] = (int) 3
0xa57: Stack[-1] = (int) 3
0xa58: Call2 0x921

0xa59: Pop(4)
0xa5a: Push((int) 3)
0xa5b: Pop(1); Push((bool) Stack[-2] >= Stack[-1])
0xa5c: IF (Stack[-1] == 0) GOTO 0xa63; Pop(1)

0xa5d: PushEmpty(string, int, int)
0xa5e: Stack[-3] = "beta_pills" // @poff=1017
0xa5f: Stack[-2] = (int) 1
0xa60: Stack[-1] = (int) 4
0xa61: Call2 0x916

0xa62: Pop(3)
0xa63: Push((int) 8)
0xa64: Pop(1); Push((bool) Stack[-2] >= Stack[-1])
0xa65: IF (Stack[-1] == 0) GOTO 0xa6e; Pop(1)

0xa66: PushEmpty(string, int, int, int)
0xa67: Stack[-4] = "monomicin" // @poff=1039
0xa68: Stack[-3] = (int) 1
0xa69: Stack[-2] = (int) 2
0xa6a: Stack[-1] = (int) 2
0xa6b: Call2 0x921

0xa6c: Pop(4)
0xa6d: GOTO 0xa77

0xa6e: Push((int) 4)
0xa6f: Pop(1); Push((bool) Stack[-2] >= Stack[-1])
0xa70: IF (Stack[-1] == 0) GOTO 0xa77; Pop(1)

0xa71: PushEmpty(string, int, int)
0xa72: Stack[-3] = "monomicin" // @poff=1039
0xa73: Stack[-2] = (int) 1
0xa74: Stack[-1] = (int) 2
0xa75: Call2 0x916

0xa76: Pop(3)
0xa77: GOTO 0xaa5

0xa78: PushEmpty(string, int, int)
0xa79: Stack[-3] = "lockpick" // @poff=1059
0xa7a: Stack[-2] = (int) 1
0xa7b: Stack[-1] = (int) 4
0xa7c: Call2 0x916

0xa7d: Pop(3)
0xa7e: PushEmpty(string, int, int)
0xa7f: Stack[-3] = "rifle_ammo" // @poff=1077
0xa80: Stack[-2] = (int) 1
0xa81: Stack[-1] = (int) 2
0xa82: Call2 0x916

0xa83: Pop(3)
0xa84: PushEmpty(string, int, int)
0xa85: Stack[-3] = "revolver_ammo" // @poff=1099
0xa86: Stack[-2] = (int) 1
0xa87: Stack[-1] = (int) 2
0xa88: Call2 0x916

0xa89: Pop(3)
0xa8a: PushEmpty(string, int, int, int)
0xa8b: Stack[-4] = "samopal_ammo" // @poff=1127
0xa8c: Stack[-3] = (int) 1
0xa8d: Stack[-2] = (int) 2
0xa8e: Stack[-1] = (int) 2
0xa8f: Call2 0x921

0xa90: Pop(4)
0xa91: Push((int) 8)
0xa92: Pop(1); Push((bool) Stack[-2] >= Stack[-1])
0xa93: IF (Stack[-1] == 0) GOTO 0xa9c; Pop(1)

0xa94: PushEmpty(string, int, int, int)
0xa95: Stack[-4] = "monomicin" // @poff=1039
0xa96: Stack[-3] = (int) 1
0xa97: Stack[-2] = (int) 2
0xa98: Stack[-1] = (int) 2
0xa99: Call2 0x921

0xa9a: Pop(4)
0xa9b: GOTO 0xaa5

0xa9c: Push((int) 4)
0xa9d: Pop(1); Push((bool) Stack[-2] >= Stack[-1])
0xa9e: IF (Stack[-1] == 0) GOTO 0xaa5; Pop(1)

0xa9f: PushEmpty(string, int, int)
0xaa0: Stack[-3] = "monomicin" // @poff=1039
0xaa1: Stack[-2] = (int) 1
0xaa2: Stack[-1] = (int) 2
0xaa3: Call2 0x916

0xaa4: Pop(3)
0xaa5: Return(); Pop(6)

0xaa6: PushEmpty(int, int)
0xaa7: @ GetInvItemByName(Stack[-1], Stack[-3])
0xaa8: Pop(0)
0xaa9: Stack[-1] = Stack[-4]
0xaaa: Return(); Pop(2)

0xaab: PushEmpty()
0xaac: PushEmpty(object)
0xaad: Stack[-2] = Stack[-1]
0xaae: Push(-1, 0); TaskCall(0)
0xaaf: Call2 0x0

0xab0: Pop(-1, 0); TaskReturn
0xab1: Pop(1)
0xab2: Return(); Pop(0)

0xab3: PushEmpty(float, float)
0xab4: Push("health") // @poff=556
0xab5: Pop(1); Push((bool) Stack[-4] == Stack[-1])
0xab6: IF (Stack[-1] == 0) GOTO 0xabf; Pop(1)

0xab7: Push("health") // @poff=556
0xab8: @ GetProperty(Stack[-1], Stack[-2])
0xab9: Pop(1)
0xaba: Push((int) 0)
0xabb: Pop(1); Push((bool) Stack[-2] <= Stack[-1])
0xabc: IF (Stack[-1] == 0) GOTO 0xabf; Pop(1)

0xabd: @ SignalDeath(Stack[-4])
0xabe: Pop(0)
0xabf: Return(); Pop(2)

0xac0: PushEmpty()
0xac1: PushEmpty(object)
0xac2: Stack[-2] = Stack[-1]
0xac3: Call2 0xaab

0xac4: Pop(1)
0xac5: Return(); Pop(0)

0xac6: PushEmpty()
0xac7: PushEmpty(object, int, float)
0xac8: Stack[-7] = Stack[-3]
0xac9: Stack[-6] = Stack[-2]
0xaca: Stack[-5] = Stack[-1]
0xacb: Call2 0x8a9

0xacc: Pop(3)
0xacd: Return(); Pop(0)

0xace: PushEmpty()
0xacf: PushEmpty(object, int, float, cvector, cvector)
0xad0: Stack[-11] = Stack[-5]
0xad1: Stack[-10] = Stack[-4]
0xad2: Stack[-9] = Stack[-3]
0xad3: Stack[-7] = Stack[-2]
0xad4: Stack[-6] = Stack[-1]
0xad5: Call2 0x8ed

0xad6: Pop(5)
0xad7: Return(); Pop(0)

0xad8: PushEmpty()
0xad9: Push("unholster") // @poff=1153
0xada: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xadb: IF (Stack[-1] == 0) GOTO 0xae3; Pop(1)

0xadc: PushEmpty(bool, object)
0xadd: Stack[-4] = Stack[-1]
0xade: Call2 0xc1c

0xadf: Stack[-2] = Stack[-5]
0xae0: Pop(2)
0xae1: Return(); Pop(0)

0xae2: GOTO 0xaf6

0xae3: Push("player_shot") // @poff=1173
0xae4: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xae5: IF (Stack[-1] == 0) GOTO 0xaed; Pop(1)

0xae6: PushEmpty(bool, object)
0xae7: Stack[-4] = Stack[-1]
0xae8: Call2 0xc21

0xae9: Stack[-2] = Stack[-5]
0xaea: Pop(2)
0xaeb: Return(); Pop(0)

0xaec: GOTO 0xaf6

0xaed: Push("battle") // @poff=775
0xaee: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xaef: IF (Stack[-1] == 0) GOTO 0xaf6; Pop(1)

0xaf0: PushEmpty(bool, object)
0xaf1: Stack[-4] = Stack[-1]
0xaf2: Call2 0xc36

0xaf3: Stack[-2] = Stack[-5]
0xaf4: Pop(2)
0xaf5: Return(); Pop(0)

0xaf6: Stack[-3] = (bool) 0
0xaf7: Return(); Pop(0)

0xaf8: PushEmpty()
0xaf9: Push("unholster") // @poff=1153
0xafa: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xafb: IF (Stack[-1] == 0) GOTO 0xb01; Pop(1)

0xafc: PushEmpty(object)
0xafd: Stack[-3] = Stack[-1]
0xafe: Call2 0xc1f

0xaff: Pop(1)
0xb00: GOTO 0xb10

0xb01: Push("player_shot") // @poff=1173
0xb02: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xb03: IF (Stack[-1] == 0) GOTO 0xb09; Pop(1)

0xb04: PushEmpty(object)
0xb05: Stack[-3] = Stack[-1]
0xb06: Call2 0xc30

0xb07: Pop(1)
0xb08: GOTO 0xb10

0xb09: Push("battle") // @poff=775
0xb0a: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xb0b: IF (Stack[-1] == 0) GOTO 0xb10; Pop(1)

0xb0c: PushEmpty(object)
0xb0d: Stack[-3] = Stack[-1]
0xb0e: Call2 0xc39

0xb0f: Pop(1)
0xb10: Return(); Pop(0)

0xb11: PushEmpty(bool, bool)
0xb12: PushEmpty(bool)
0xb13: Stack[-1] = (bool) 0
0xb14: PushEmpty(bool, object)
0xb15: Stack[-6] = Stack[-1]
0xb16: Call2 0xc1c

0xb17: Pop(1)
0xb18: IF (Stack[-1] == 0) GOTO 0xb1f; Pop(1)

0xb19: PushEmpty(bool, object)
0xb1a: Stack[-6] = Stack[-1]
0xb1b: Call2 0x7e2

0xb1c: Pop(1)
0xb1d: IF (Stack[-1] == 0) GOTO 0xb1f; Pop(1)

0xb1e: Stack[-1] = (bool) 1
0xb1f: IF (Stack[-1] == 0) GOTO 0xb26; Pop(1)

0xb20: @@ IsWeaponHolstered(Stack[-1]); Obj=3 // @poff=1197
0xb21: Pop(0)
0xb22: Pop(0); Push((bool) Stack[-1] == 0)
0xb23: IF (Stack[-1] == 0) GOTO 0xb26; Pop(1)

0xb24: Stack[-4] = (bool) 1
0xb25: Return(); Pop(2)

0xb26: Stack[-4] = (bool) 0
0xb27: Return(); Pop(2)

0xb28: PushEmpty()
0xb29: PushEmpty(object)
0xb2a: Stack[-2] = Stack[-1]
0xb2b: Call2 0xc1f

0xb2c: Pop(1)
0xb2d: Return(); Pop(0)

0xb2e: PushEmpty()
0xb2f: PushEmpty(bool, object)
0xb30: Stack[-3] = Stack[-1]
0xb31: Call2 0x7e2

0xb32: Pop(1)
0xb33: IF (Stack[-1] == 0) GOTO 0xb3a; Pop(1)

0xb34: PushEmpty(object)
0xb35: Call2 0x959

0xb36: Pop(0)
0xb37: Push((float)-0.05000000074505806)
0xb38: @ ReportReputationChange(Stack[-3], Stack[-2], Stack[-1])
0xb39: Pop(2)
0xb3a: Return(); Pop(0)

0xb3b: PushEmpty(object, object)
0xb3c: Push("heal") // @poff=1215
0xb3d: Pop(1); Push((bool) Stack[-4] == Stack[-1])
0xb3e: IF (Stack[-1] == 0) GOTO 0xb49; Pop(1)

0xb3f: Push("player") // @poff=1225
0xb40: @ FindActor(Stack[-2], Stack[-1])
0xb41: Pop(1)
0xb42: PushEmpty(bool, object)
0xb43: Stack[-3] = Stack[-1]
0xb44: Call2 0xc3b

0xb45: Stack[-2] = Stack[-6]
0xb46: Pop(2)
0xb47: Return(); Pop(2)

0xb48: Stack[-1] = 0
0xb49: Stack[-4] = (bool) 0
0xb4a: Return(); Pop(2)

0xb4b: PushEmpty(object, object)
0xb4c: Push("heal") // @poff=1215
0xb4d: Pop(1); Push((bool) Stack[-4] == Stack[-1])
0xb4e: IF (Stack[-1] == 0) GOTO 0xb57; Pop(1)

0xb4f: Push("player") // @poff=1225
0xb50: @ FindActor(Stack[-2], Stack[-1])
0xb51: Pop(1)
0xb52: PushEmpty(object)
0xb53: Stack[-2] = Stack[-1]
0xb54: Call2 0xc3e

0xb55: Pop(1)
0xb56: Stack[-1] = 0
0xb57: Return(); Pop(2)

0xb58: PushEmpty(string, string)
0xb59: Stack[-1] = "idle" // @poff=1239
0xb5a: Push(Stack[-3])
0xb5b: IF (Stack[-1] == 0) GOTO 0xb5d; Pop(1)

0xb5c: Stack[-1] = Stack[-1] + Stack[-3]; Pop(0);
0xb5d: Stack[-1] = Stack[-4]
0xb5e: Return(); Pop(2)

0xb5f: PushEmpty(int, bool, int, bool)
0xb60: Stack[-2] = (int) 0
0xb61: Push("all") // @poff=170
0xb62: PushEmpty(string, int)
0xb63: Stack[-5] = Stack[-1]
0xb64: Call2 0xb58

0xb65: Pop(1)
0xb66: @ HasAnimation(Stack[-3], Stack[-2], Stack[-1])
0xb67: Pop(2)
0xb68: Pop(0); Push((bool) Stack[-1] == 0)
0xb69: IF (Stack[-1] == 0) GOTO 0xb6b; Pop(1)

0xb6a: GOTO 0xb6e

0xb6b: Push((int) 1)
0xb6c: Stack[-3] = Stack[-3] + Stack[-1]; Pop(1);
0xb6d: GOTO 0xb61

0xb6e: Stack[-2] = Stack[-5]
0xb6f: Return(); Pop(4)

0xb70: PushEmpty()
0xb71: PushEmpty(bool)
0xb72: Call2 0xc56

0xb73: Pop(0)
0xb74: IF (Stack[-1] == 0) GOTO 0xb77; Pop(1)

0xb75: Stack[-2] = (int) 2
0xb76: GOTO 0xb78

0xb77: Stack[-2] = (int) 0
0xb78: Return(); Pop(0)

0xb79: PushEmpty()
0xb7a: PushEmpty(object)
0xb7b: Stack[-2] = Stack[-1]
0xb7c: Push(-1, 1); TaskCall(3)
0xb7d: Call2 0x307

0xb7e: Pop(-1, 1); TaskReturn
0xb7f: Pop(1)
0xb80: Return(); Pop(0)

0xb81: PushEmpty()
0xb82: PushEmpty(bool, object)
0xb83: Stack[-3] = Stack[-1]
0xb84: Call2 0x865

0xb85: Pop(1)
0xb86: IF (Stack[-1] == 0) GOTO 0xb89; Pop(1)

0xb87: Stack[-2] = (int) 2
0xb88: GOTO 0xb8a

0xb89: Stack[-2] = (int) 0
0xb8a: Return(); Pop(0)

0xb8b: PushEmpty()
0xb8c: PushEmpty(object)
0xb8d: Stack[-2] = Stack[-1]
0xb8e: Push(-1, 6); TaskCall(4)
0xb8f: Call2 0x41a

0xb90: Pop(-1, 6); TaskReturn
0xb91: Pop(1)
0xb92: Return(); Pop(0)

0xb93: PushEmpty(string, string, string, string)
0xb94: PushEmpty(bool, object, string)
0xb95: Stack[-9] = Stack[-2]
0xb96: Stack[-1] = "class" // @poff=1249
0xb97: Call2 0x7e7

0xb98: Pop(2)
0xb99: Pop(1); Push((bool) Stack[-1] == 0)
0xb9a: IF (Stack[-1] == 0) GOTO 0xb9d; Pop(1)

0xb9b: Stack[-7] = (bool) 0
0xb9c: Return(); Pop(4)

0xb9d: Push("class") // @poff=1249
0xb9e: @ GetProperty(Stack[-1], Stack[-3])
0xb9f: Pop(1)
0xba0: Push("class") // @poff=1249
0xba1: @@ GetProperty(Stack[-1], Stack[-2]); Obj=7 // @poff=582
0xba2: Pop(1)
0xba3: PushEmpty(bool)
0xba4: Stack[-1] = (bool) 0
0xba5: Pop(0); Push((bool) Stack[-6] == 0)
0xba6: IF (Stack[-1] == 0) GOTO 0xbaa; Pop(1)

0xba7: Pop(0); Push((bool) Stack[-3] == Stack[-2])
0xba8: IF (Stack[-1] == 0) GOTO 0xbaa; Pop(1)

0xba9: Stack[-1] = (bool) 1
0xbaa: IF (Stack[-1] == 0) GOTO 0xbad; Pop(1)

0xbab: Stack[-7] = (bool) 1
0xbac: Return(); Pop(4)

0xbad: Push("rat") // @poff=1261
0xbae: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xbaf: IF (Stack[-1] == 0) GOTO 0xbb3; Pop(1)

0xbb0: Stack[-7] = (bool) 0
0xbb1: Return(); Pop(4)

0xbb2: GOTO 0xbdc

0xbb3: Push("rat_big") // @poff=1269
0xbb4: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xbb5: IF (Stack[-1] == 0) GOTO 0xbb9; Pop(1)

0xbb6: Stack[-7] = (bool) 0
0xbb7: Return(); Pop(4)

0xbb8: GOTO 0xbdc

0xbb9: Push("dog") // @poff=1285
0xbba: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xbbb: IF (Stack[-1] == 0) GOTO 0xbbf; Pop(1)

0xbbc: Stack[-7] = (bool) 0
0xbbd: Return(); Pop(4)

0xbbe: GOTO 0xbdc

0xbbf: Push("grabitel") // @poff=1293
0xbc0: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xbc1: IF (Stack[-1] == 0) GOTO 0xbc5; Pop(1)

0xbc2: Stack[-7] = (bool) 0
0xbc3: Return(); Pop(4)

0xbc4: GOTO 0xbdc

0xbc5: Push("bomber") // @poff=1311
0xbc6: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xbc7: IF (Stack[-1] == 0) GOTO 0xbcb; Pop(1)

0xbc8: Stack[-7] = (bool) 0
0xbc9: Return(); Pop(4)

0xbca: GOTO 0xbdc

0xbcb: Push("sanitar") // @poff=1325
0xbcc: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xbcd: IF (Stack[-1] == 0) GOTO 0xbd1; Pop(1)

0xbce: Stack[-7] = (bool) 0
0xbcf: Return(); Pop(4)

0xbd0: GOTO 0xbdc

0xbd1: Push("hunter") // @poff=1341
0xbd2: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xbd3: IF (Stack[-1] == 0) GOTO 0xbd7; Pop(1)

0xbd4: Stack[-7] = (bool) 0
0xbd5: Return(); Pop(4)

0xbd6: GOTO 0xbdc

0xbd7: Push("soldier") // @poff=1355
0xbd8: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xbd9: IF (Stack[-1] == 0) GOTO 0xbdc; Pop(1)

0xbda: Stack[-7] = (bool) 0
0xbdb: Return(); Pop(4)

0xbdc: Stack[-7] = (bool) 1
0xbdd: Return(); Pop(4)

0xbde: PushEmpty()
0xbdf: PushEmpty(int, object)
0xbe0: Stack[-4] = Stack[-1]
0xbe1: Call2 0xb81

0xbe2: Stack[-2] = Stack[-5]
0xbe3: Pop(2)
0xbe4: Return(); Pop(0)

0xbe5: PushEmpty()
0xbe6: PushEmpty(object)
0xbe7: Stack[-2] = Stack[-1]
0xbe8: Call2 0xb8b

0xbe9: Pop(1)
0xbea: Return(); Pop(0)

0xbeb: PushEmpty(bool, bool)
0xbec: PushEmpty(bool, object, bool)
0xbed: Stack[-8] = Stack[-2]
0xbee: Stack[-1] = !Stack[-6]; Pop(0);
0xbef: Call2 0xb93

0xbf0: Pop(2)
0xbf1: Pop(1); Push((bool) Stack[-1] == 0)
0xbf2: IF (Stack[-1] == 0) GOTO 0xbf5; Pop(1)

0xbf3: Stack[-7] = (bool) 0
0xbf4: Return(); Pop(2)

0xbf5: @ CanSee(Stack[-1], Stack[-6])
0xbf6: Pop(0)
0xbf7: PushEmpty(bool)
0xbf8: Stack[-1] = (bool) 1
0xbf9: Push(Stack[-2])
0xbfa: IF (Stack[-1] == 1) GOTO 0xc03; Pop(1)

0xbfb: PushEmpty(float, object)
0xbfc: Stack[-9] = Stack[-1]
0xbfd: Call2 0x7da

0xbfe: Pop(1)
0xbff: Pop(0); Push(Stack[-6] * Stack[-6]);
0xc00: Pop(2); Push((bool) Stack[-2] <= Stack[-1])
0xc01: IF (Stack[-1] == 1) GOTO 0xc03; Pop(1)

0xc02: Stack[-1] = (bool) 0
0xc03: IF (Stack[-1] == 0) GOTO 0xc06; Pop(1)

0xc04: Stack[-7] = (bool) 1
0xc05: Return(); Pop(2)

0xc06: Stack[-7] = (bool) 0
0xc07: Return(); Pop(2)

0xc08: PushEmpty()
0xc09: Stack[-2] = (int) 0
0xc0a: Return(); Pop(0)

0xc0b: PushEmpty()
0xc0c: Return(); Pop(0)

0xc0d: PushEmpty()
0xc0e: Stack[-3] = (int) 0
0xc0f: Return(); Pop(0)

0xc10: PushEmpty()
0xc11: Return(); Pop(0)

0xc12: PushEmpty()
0xc13: Stack[-2] = (int) 2
0xc14: Return(); Pop(0)

0xc15: PushEmpty()
0xc16: PushEmpty(object)
0xc17: Stack[-2] = Stack[-1]
0xc18: Call2 0xc48

0xc19: Pop(1)
0xc1a: Return(); Pop(0)

0xc1b: Return(); Pop(0)

0xc1c: PushEmpty()
0xc1d: Stack[-2] = (bool) 0
0xc1e: Return(); Pop(0)

0xc1f: PushEmpty()
0xc20: Return(); Pop(0)

0xc21: PushEmpty(bool, bool)
0xc22: @ CanSee(Stack[-1], Stack[-3])
0xc23: Pop(0)
0xc24: Stack[-4] = (bool) 1
0xc25: Push(Stack[-1])
0xc26: IF (Stack[-1] == 1) GOTO 0xc2f; Pop(1)

0xc27: PushEmpty(float, object)
0xc28: Stack[-5] = Stack[-1]
0xc29: Call2 0x7da

0xc2a: Pop(1)
0xc2b: Push((int) 2250000)
0xc2c: Pop(2); Push((bool) Stack[-2] <= Stack[-1])
0xc2d: IF (Stack[-1] == 1) GOTO 0xc2f; Pop(1)

0xc2e: Stack[-4] = (bool) 0
0xc2f: Return(); Pop(2)

0xc30: PushEmpty()
0xc31: PushEmpty(object)
0xc32: Stack[-2] = Stack[-1]
0xc33: Call2 0xb79

0xc34: Pop(1)
0xc35: Return(); Pop(0)

0xc36: PushEmpty()
0xc37: Stack[-2] = (bool) 0
0xc38: Return(); Pop(0)

0xc39: PushEmpty()
0xc3a: Return(); Pop(0)

0xc3b: PushEmpty()
0xc3c: Stack[-2] = (bool) 0
0xc3d: Return(); Pop(0)

0xc3e: PushEmpty()
0xc3f: Return(); Pop(0)

0xc40: Push(GlobalVars[0])
0xc41: Stack[-1] = (bool) 0
0xc42: GlobalVars[0] = Stack[-1]; Pop(1)
0xc43: PushEmpty(bool)
0xc44: Stack[-1] = (bool) 1
0xc45: Call2 0xa42

0xc46: Pop(1)
0xc47: Return(); Pop(0)

0xc48: PushEmpty(bool, bool)
0xc49: Push(GlobalVars[0])
0xc4a: IF (Stack[-1] == 0) GOTO 0xc55; Pop(1)

0xc4b: @ IsOverrideActive(Stack[-1])
0xc4c: Pop(0)
0xc4d: Pop(0); Push((bool) Stack[-1] == 0)
0xc4e: IF (Stack[-1] == 0) GOTO 0xc53; Pop(1)

0xc4f: PushEmpty(object)
0xc50: Stack[-4] = Stack[-1]
0xc51: Call2 0x9d1

0xc52: Pop(1)
0xc53: Return(); Pop(2)

0xc54: GOTO 0xc55

0xc55: Return(); Pop(2)

0xc56: Stack[-1] = (bool) 0
0xc57: Return(); Pop(0)

0xc58: PushEmpty()
0xc59: PushEmpty(bool, object, object, float, bool)
0xc5a: Stack[-8] = Stack[-4]
0xc5b: Stack[-7] = Stack[-3]
0xc5c: Stack[-2] = (float) 700.0
0xc5d: Stack[-6] = Stack[-1]
0xc5e: Call2 0xbeb

0xc5f: Stack[-5] = Stack[-9]
0xc60: Pop(5)
0xc61: Return(); Pop(0)

0xc62: PushEmpty()
0xc63: PushEmpty(bool, object)
0xc64: Stack[-3] = Stack[-1]
0xc65: Call2 0x7e2

0xc66: Pop(1)
0xc67: IF (Stack[-1] == 0) GOTO 0xc73; Pop(1)

0xc68: PushEmpty(object)
0xc69: Call2 0x959

0xc6a: Pop(0)
0xc6b: Push((float)-0.10000000149011612)
0xc6c: Push((bool) 1)
0xc6d: @ ReportReputationChange(Stack[-4], Stack[-3], Stack[-2], Stack[-1])
0xc6e: Pop(3)
0xc6f: PushEmpty(bool)
0xc70: Stack[-1] = (bool) 1
0xc71: Call2 0xa26

0xc72: Pop(1)
0xc73: PushEmpty()
0xc74: Call2 0xa0e

0xc75: Pop(0)
0xc76: Push(GlobalVars[0])
0xc77: Stack[-1] = (bool) 1
0xc78: GlobalVars[0] = Stack[-1]; Pop(1)
0xc79: Push((int) 50)
0xc7a: Push((int) 40)
0xc7b: @ SetRTEnvelope(Stack[-2], Stack[-1])
0xc7c: Pop(2)
0xc7d: Return(); Pop(0)

