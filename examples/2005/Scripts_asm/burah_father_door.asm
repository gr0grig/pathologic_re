...
Strings:
	...
	A:GetItemID
	A:RemoveItem
// @pool_raw:
...
0x6: Stack[-5] = Stack[-3]
0x7: Stack[-2] = "burah_father_key" // @poff=0
0x8: Stack[-1] = (bool) 1
0x9: Call2 0x7f
...
0x21: Push("door_closed") // @poff=34
0x22: @ PlaySound(Stack[-1])
0x23: Pop(1)
0x24: Push("Door is locked") // @poff=58
0x25: @ Trace(Stack[-1])
0x26: Pop(1)
0x27: Return(); Pop(16)
...
0x2c: Stack[-1] = "@" // @poff=88
0x2d: Call2 0x64

0x2e: Stack[-3] = Stack[-10]
0x2f: Pop(3)
0x30: Push((int) -1)
0x31: Pop(1); Push((bool) Stack[-8] == Stack[-1])
0x32: IF (Stack[-1] == 0) GOTO 0x37; Pop(1)

0x33: Push("Wrong door name") // @poff=92
0x34: @ Trace(Stack[-1])
0x35: Pop(1)
0x36: Return(); Pop(16)

0x37: PushEmpty(string, string, int, int)
0x38: Stack[-12] = Stack[-3]
0x39: Stack[-2] = (int) 0
0x3a: Stack[-11] = Stack[-1]
0x3b: Call2 0x6e

0x3c: Stack[-4] = Stack[-10]
0x3d: Pop(4)
0x3e: Push("pt_") // @poff=124
0x3f: PushEmpty(string, string, int)
0x40: Stack[-12] = Stack[-2]
0x41: Push((int) 1)
...
0x47: Pop(0)
0x48: Pop(0); PushNull((bool) Stack[-4] == 0)
0x49: IF (Stack[-1] == 0) GOTO 0x51; Pop(1)

0x4a: Push("Destination scene '") // @poff=132
0x4b: Pop(1); Push(Stack[-1] + Stack[-7]);
0x4c: Push("' not found") // @poff=172
0x4d: Pop(2); Push(Stack[-2] + Stack[-1]);
0x4e: @ Trace(Stack[-1])
0x4f: Pop(1)
0x50: Return(); Pop(16)

0x51: @@ GetLocator(Stack[-5], Stack[-3], Stack[-2], Stack[-1]); Obj=4 // @poff=196
0x52: Pop(0)
0x53: Push(Stack[-3])
0x54: IF (Stack[-1] == 0) GOTO 0x5b; Pop(1)

0x55: Push("door_open") // @poff=207
...
0x5b: Push("Locator ") // @poff=227
0x5c: Pop(1); Push(Stack[-1] + Stack[-6]);
0x5d: Push(" not found in destination scene ") // @poff=245
...
0x81: Pop(0)
0x82: @@ GetSubContainerCount(Stack[-6]); Obj=17 // @poff=311
0x83: Pop(0)
0x84: Stack[-5] = (int) 0
0x85: Pop(0); Push((bool) Stack[-5] < Stack[-6])
0x86: IF (Stack[-1] == 0) GOTO 0xa0; Pop(1)

0x87: @@ GetItemCount(Stack[-4], Stack[-5]); Obj=17 // @poff=332
0x88: Pop(0)
0x89: Stack[-3] = (int) 0
0x8a: Pop(0); Push((bool) Stack[-3] < Stack[-4])
0x8b: IF (Stack[-1] == 0) GOTO 0x9d; Pop(1)

0x8c: @@ GetItem(Stack[-2], Stack[-3], Stack[-5]); Obj=17 // @poff=345
0x8d: Pop(0)
0x8e: @@ GetItemID(Stack[-1]); Obj=2 // @poff=353
0x8f: Pop(0)
...
0x94: Push((int) 1)
0x95: @@ RemoveItem(Stack[-4], Stack[-1], Stack[-6]); Obj=18 // @poff=363
0x96: Pop(1)
