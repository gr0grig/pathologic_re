...
Strings:
	...
	A:get
	W:.isc
	W:City manager: can't replace indoor scene "
	W:" because it doesn't exist
	W:horror
	W:Replaced
	W:Invalid region index
	W:night time
	W:day time
// @pool_raw:
...
0x5b: Push("scenes/") // @poff=0
0x5c: Pop(1); Push(Stack[-1] + Stack[-3]);
0x5d: Push(".mot") // @poff=16
...
0x75: PushEmpty()
0x76: Push("playing: ") // @poff=36
0x77: Pop(1); Push(Stack[-1] + Stack[-2]);
0x78: @ Trace(Stack[-1])
0x79: Pop(1)
0x7a: Push("space") // @poff=56
0x7b: @ RegisterKeyCallback(Stack[-1])
0x7c: Pop(1)
0x7d: Push("escape") // @poff=68
...
0x84: Pop(0)
0x85: Push("escape") // @poff=68
0x86: @ UnregisterKeyCallback(Stack[-1])
0x87: Pop(1)
0x88: Push("space") // @poff=56
...
0xbd: Push("City manager: can't replace indoor scene "") // @poff=113
0xbe: Pop(1); Push(Stack[-1] + Stack[-4]);
0xbf: Push("" because it doesn't exist") // @poff=199
...
0xd2: @@ size(Stack[-3]); Obj=1 // @poff=82
0xd3: Pop(1)
0xd4: PushEmpty(bool)
0xd5: Stack[-1] = (bool) 0
0xd6: Push((int) 0)
...
0xde: @@ get(Stack[-2], Stack[-4]); Obj=1 // @poff=99
0xdf: Pop(1)
0xe0: PushEmpty(object, int)
0xe1: Stack[-3] = Stack[-2]
0xe2: Stack[-1] = (bool) 0
0xe3: Call2 0xa5

0xe4: Pop(2)
0xe5: Push("Replaced") // @poff=267
0xe6: @ Trace(Stack[-1])
0xe7: Pop(1)
0xe8: Stack[-1] = 0
0xe9: GOTO 0xed

0xea: Push("Invalid region index") // @poff=285
0xeb: @ Trace(Stack[-1])
0xec: Pop(1)
0xed: Return(); Pop(6)
