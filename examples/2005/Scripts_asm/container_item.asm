GlobalVarCount = 0

Strings:
	W:HasProperty
	A:HasProperty
	A:IsDead
	W:IsDead
	A:GetScene
	W:noaccess
	A:GetProperty
	W:nouse_container
	A:add
	A:GetItemID
	W:Category
	A:AddItem
// @pool_raw:

Import:
	SetVisibility (1 args)
	Hold (0 args)
	GetItem (2 args)
	GetItemAmount (2 args)
	RemoveActor (1 args)
	GetScene (1 args)
	self (1 args)
	GetVariable (2 args)
	CreateIntVector (1 args)
	SendWorldWndMessage (2 args)
	GetInvItemProperty (3 args)

RunOp = 0x0
RunTask = 0

GlobalTasks: 
	GTASK_0  Params = 0
		EVENT_0 Op = 0x7 Vars = (object)


0x0: Push((bool) 1)
0x1: @ SetVisibility(Stack[-1])
0x2: Pop(1)
0x3: @ Hold()
0x4: Pop(0)
0x5: GOTO 0x3

0x6: Return(); Pop(0)

0x7: PushEmpty(object, int, object, int)
0x8: PushEmpty(bool)
0x9: Stack[-1] = (bool) 1
0xa: PushEmpty(bool, object)
0xb: Stack[-8] = Stack[-1]
0xc: Call2 0x61

0xd: Pop(1)
0xe: Pop(1); Push((bool) Stack[-1] == 0)
0xf: IF (Stack[-1] == 1) GOTO 0x16; Pop(1)

0x10: PushEmpty(bool)
0x11: Call2 0x7f

0x12: Pop(0)
0x13: Pop(1); Push((bool) Stack[-1] == 0)
0x14: IF (Stack[-1] == 1) GOTO 0x16; Pop(1)

0x15: Stack[-1] = (bool) 0
0x16: IF (Stack[-1] == 0) GOTO 0x18; Pop(1)

0x17: Return(); Pop(4)

0x18: Push((int) 0)
0x19: @ GetItem(Stack[-3], Stack[-1])
0x1a: Pop(1)
0x1b: Push((int) 0)
0x1c: @ GetItemAmount(Stack[-2], Stack[-1])
0x1d: Pop(1)
0x1e: PushEmpty(bool, object, object, int)
0x1f: Stack[-9] = Stack[-3]
0x20: Stack[-6] = Stack[-2]
0x21: Stack[-5] = Stack[-1]
0x22: Call2 0x91

0x23: Pop(3)
0x24: IF (Stack[-1] == 0) GOTO 0x2a; Pop(1)

0x25: PushEmpty(object)
0x26: Call2 0x79

0x27: Pop(0)
0x28: @ RemoveActor(Stack[-1])
0x29: Pop(1)
0x2a: Return(); Pop(4)

0x2b: Stack[-2] = 0
0x2c: PushEmpty(bool, bool)
0x2d: Push("HasProperty") // @poff=0
0x2e: Push((int) 2)
0x2f: Pop(2); Push(IsFuncExist(Stack[-6], Stack[-2], Stack[-1]))
0x30: Pop(1); Push((bool) Stack[-1] == 0)
0x31: IF (Stack[-1] == 0) GOTO 0x34; Pop(1)

0x32: Stack[-5] = (bool) 0
0x33: Return(); Pop(2)

0x34: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=24
0x35: Pop(0)
0x36: Stack[-1] = Stack[-5]
0x37: Return(); Pop(2)

0x38: PushEmpty(bool, bool)
0x39: @@ IsDead(Stack[-1]); Obj=3 // @poff=36
0x3a: Pop(0)
0x3b: Stack[-1] = Stack[-4]
0x3c: Return(); Pop(2)

0x3d: PushEmpty(object, object, object, object)
0x3e: Pop(0); PushNull((bool) Stack[-5] == 0)
0x3f: IF (Stack[-1] == 0) GOTO 0x42; Pop(1)

0x40: Stack[-6] = (bool) 0
0x41: Return(); Pop(4)

0x42: PushEmpty(bool)
0x43: Stack[-1] = (bool) 0
0x44: Push("IsDead") // @poff=43
0x45: Push((int) 1)
0x46: Pop(2); Push(IsFuncExist(Stack[-8], Stack[-2], Stack[-1]))
0x47: IF (Stack[-1] == 0) GOTO 0x4e; Pop(1)

0x48: PushEmpty(bool, object)
0x49: Stack[-8] = Stack[-1]
0x4a: Call2 0x38

0x4b: Pop(1)
0x4c: IF (Stack[-1] == 0) GOTO 0x4e; Pop(1)

0x4d: Stack[-1] = (bool) 1
0x4e: IF (Stack[-1] == 0) GOTO 0x51; Pop(1)

0x4f: Stack[-6] = (bool) 0
0x50: Return(); Pop(4)

0x51: @ GetScene(Stack[-2])
0x52: Pop(0)
0x53: Pop(0); PushNull((bool) Stack[-2] == 0)
0x54: IF (Stack[-1] == 0) GOTO 0x57; Pop(1)

0x55: Stack[-6] = (bool) 0
0x56: Return(); Pop(4)

0x57: @@ GetScene(Stack[-1]); Obj=5 // @poff=57
0x58: Pop(0)
0x59: Pop(0); Push((bool) Stack[-2] != Stack[-1])
0x5a: IF (Stack[-1] == 0) GOTO 0x5d; Pop(1)

0x5b: Stack[-6] = (bool) 0
0x5c: Return(); Pop(4)

0x5d: Stack[-6] = (bool) 1
0x5e: Return(); Pop(4)

0x5f: Stack[-1] = 0
0x60: Stack[-2] = 0
0x61: PushEmpty(int, int)
0x62: PushEmpty(bool, object)
0x63: Stack[-5] = Stack[-1]
0x64: Call2 0x3d

0x65: Pop(1)
0x66: Pop(1); Push((bool) Stack[-1] == 0)
0x67: IF (Stack[-1] == 0) GOTO 0x6a; Pop(1)

0x68: Stack[-4] = (bool) 0
0x69: Return(); Pop(2)

0x6a: PushEmpty(bool, object, string)
0x6b: Stack[-6] = Stack[-2]
0x6c: Stack[-1] = "noaccess" // @poff=66
0x6d: Call2 0x2c

0x6e: Pop(2)
0x6f: Pop(1); Push((bool) Stack[-1] == 0)
0x70: IF (Stack[-1] == 0) GOTO 0x73; Pop(1)

0x71: Stack[-4] = (bool) 1
0x72: Return(); Pop(2)

0x73: Push("noaccess") // @poff=66
0x74: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=84
0x75: Pop(1)
0x76: Push((int) 0)
0x77: Stack[-5] = Stack[-2] == Stack[-1]; Pop(1);
0x78: Return(); Pop(2)

0x79: PushEmpty(object, object)
0x7a: @ self(Stack[-1])
0x7b: Pop(0)
0x7c: Stack[-1] = Stack[-3]
0x7d: Return(); Pop(2)

0x7e: Stack[-1] = 0
0x7f: PushEmpty(int, int)
0x80: Push("nouse_container") // @poff=96
0x81: @ GetVariable(Stack[-1], Stack[-2])
0x82: Pop(1)
0x83: Stack[-3] = !Stack[-1]; Pop(0);
0x84: Return(); Pop(2)

0x85: PushEmpty(object, object)
0x86: @ CreateIntVector(Stack[-1])
0x87: Pop(0)
0x88: @@ add(Stack[-4]); Obj=1 // @poff=128
0x89: Pop(0)
0x8a: @@ add(Stack[-3]); Obj=1 // @poff=128
0x8b: Pop(0)
0x8c: Push((int) 3)
0x8d: @ SendWorldWndMessage(Stack[-1], Stack[-2])
0x8e: Pop(1)
0x8f: Return(); Pop(2)

0x90: Stack[-1] = 0
0x91: PushEmpty(int, int, bool, int, int, bool)
0x92: @@ GetItemID(Stack[-3]); Obj=8 // @poff=132
0x93: Pop(0)
0x94: Push("Category") // @poff=142
0x95: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0x96: Pop(1)
0x97: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7]); Obj=9 // @poff=160
0x98: Pop(0)
0x99: Push(Stack[-1])
0x9a: IF (Stack[-1] == 0) GOTO 0xa0; Pop(1)

0x9b: PushEmpty(int, int)
0x9c: Stack[-5] = Stack[-2]
0x9d: Stack[-9] = Stack[-1]
0x9e: Call2 0x85

0x9f: Pop(2)
0xa0: Stack[-1] = Stack[-10]
0xa1: Return(); Pop(6)

