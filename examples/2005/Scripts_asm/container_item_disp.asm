...
Strings:
	...
	W:Category
	A:AddItem
// @pool_raw:
...
0x62: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=56
0x63: Pop(0)
0x64: Stack[-1] = Stack[-5]
0x65: Return(); Pop(2)

0x66: PushEmpty(bool, bool)
0x67: @@ IsDead(Stack[-1]); Obj=3 // @poff=68
0x68: Pop(0)
0x69: Stack[-1] = Stack[-4]
0x6a: Return(); Pop(2)

0x6b: PushEmpty(object, object, object, object)
0x6c: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x80: Pop(0)
0x81: Pop(0); PushNull((bool) Stack[-2] == 0)
0x82: IF (Stack[-1] == 0) GOTO 0x85; Pop(1)

0x83: Stack[-6] = (bool) 0
0x84: Return(); Pop(4)

0x85: @@ GetScene(Stack[-1]); Obj=5 // @poff=89
0x86: Pop(0)
...
0x99: Stack[-6] = Stack[-2]
0x9a: Stack[-1] = "noaccess" // @poff=98
0x9b: Call2 0x5a
...
0xa1: Push("noaccess") // @poff=98
0xa2: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=116
0xa3: Pop(1)
0xa4: Push((int) 0)
...
0xae: Push("nouse_container") // @poff=128
...
0xb5: Pop(0)
0xb6: @@ add(Stack[-4]); Obj=1 // @poff=160
0xb7: Pop(0)
0xb8: @@ add(Stack[-3]); Obj=1 // @poff=160
0xb9: Pop(0)
0xba: Push((int) 3)
...
0xc0: @@ GetItemID(Stack[-3]); Obj=8 // @poff=164
0xc1: Pop(0)
0xc2: Push("Category") // @poff=174
0xc3: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0xc4: Pop(1)
0xc5: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7]); Obj=9 // @poff=192
0xc6: Pop(0)
0xc7: Push(Stack[-1])
