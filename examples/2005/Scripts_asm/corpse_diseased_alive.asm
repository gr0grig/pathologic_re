...
Strings:
	...
	W:Region
	W:State
// @pool_raw:
...
0x1d: Stack[-3] = "blooddiseasedalive" // @poff=39
0x1e: Stack[-2] = (int) 1
0x1f: Stack[-1] = (int) 0
0x20: Call2 0x87
...
0x24: Push("Can't add corpse item") // @poff=77
0x25: @ Trace(Stack[-1])
0x26: Pop(1)
0x27: Return(); Pop(0)
...
0x3f: Pop(1)
0x40: Push("I am in region ") // @poff=121
0x41: Pop(1); Push(Stack[-1] + Stack[-2]);
0x42: Push(" and visible") // @poff=153
...
0x49: Pop(1)
0x4a: Push("I am in region ") // @poff=121
0x4b: Pop(1); Push(Stack[-1] + Stack[-2]);
0x4c: Push(" and not visible") // @poff=179
...
0x5b: Pop(0)
0x5c: @@ GetRegionByPt(Stack[-2], Stack[-1]); Obj=3 // @poff=213
0x5d: Pop(1)
...
0x70: PushEmpty()
0x71: Push("Region") // @poff=227
0x72: Pop(1); Push(Stack[-1] + Stack[-2]);
0x73: Push("State") // @poff=241
...
0x7f: Push((int) 1)
0x80: Stack[-6] = Stack[-2] == Stack[-1]; Pop(2);
0x81: Return(); Pop(2)

0x82: PushEmpty(int, int)
0x83: @ GetInvItemByName(Stack[-1], Stack[-3])
0x84: Pop(0)
0x85: Stack[-1] = Stack[-4]
0x86: Return(); Pop(2)

0x87: PushEmpty(int, bool, int, bool)
0x88: PushEmpty(int, string)
0x89: Stack[-9] = Stack[-1]
0x8a: Call2 0x82

0x8b: Stack[-2] = Stack[-4]
0x8c: Pop(2)
0x8d: Push((int) -1)
...
0x92: Push((int) 0)
0x93: Pop(1); Push((bool) Stack[-7] == Stack[-1])
0x94: IF (Stack[-1] == 0) GOTO 0x97; Pop(1)

0x95: Stack[-8] = (bool) 1
0x96: Return(); Pop(4)

0x97: @ AddItem(Stack[-1], Stack[-2], Stack[-5], Stack[-6])
0x98: Pop(0)
