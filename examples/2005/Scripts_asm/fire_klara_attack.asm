...
Strings:
	...
	W:noaccess
	A:GetProperty
// @pool_raw:
...
0xf: IF (Stack[-1] == 1) GOTO 0x19; Pop(1)
...
0x17: IF (Stack[-1] == 1) GOTO 0x19; Pop(1)
...
0x21: @ Trace(Stack[-1])
0x22: Pop(1)
0x23: Push("fire") // @poff=12
0x24: @ FindParticleSystem(Stack[-1], Stack[-9])
0x25: Pop(1)
0x26: Pop(0); PushNull((bool) Stack[-8] == 0)
0x27: IF (Stack[-1] == 0) GOTO 0x2c; Pop(1)

0x28: Push("Can't find fire particle system") // @poff=22
0x29: @ Trace(Stack[-1])
0x2a: Pop(1)
0x2b: Return(); Pop(16)
...
0x42: Push([0.0, 1.0, 0.0])
0x43: Push((float)0.5)
0x44: @@ AddSource(Stack[-3], Stack[-2], Stack[-1]); Obj=11 // @poff=120
0x45: Pop(3)
0x46: @@ Enable(); Obj=8 // @poff=130
0x47: Pop(0)
0x48: Push((bool) 0)
0x49: @@ Fade(Stack[-1]); Obj=9 // @poff=137
0x4a: Pop(1)
0x4b: Push("5") // @poff=142
0x4c: @ Trace(Stack[-1])
0x4d: Pop(1)
0x4e: Stack[-7] = Stack[-5]
0x4f: Stack[-4] = (int) 2
0x50: Stack[-3] = (int) 0
0x51: Push((int) 600)
...
0x60: @@ GetPosition(Stack[-8]); Obj=1 // @poff=90
0x61: Pop(1)
...
0x6c: Push([0.0, 1.0, 0.0])
0x6d: Push((float)0.5)
0x6e: @@ AddSource(Stack[-3], Stack[-2], Stack[-1]); Obj=11 // @poff=120
0x6f: Pop(3)
0x70: Push((int) 1)
...
0x75: Stack[-7] = Stack[-5]
0x76: Push((int) 1)
0x77: Stack[-4] = Stack[-4] + Stack[-1]; Pop(1);
0x78: GOTO 0x51

0x79: Push("6") // @poff=146
0x7a: @ Trace(Stack[-1])
0x7b: Pop(1)
0x7c: @@ Fade(); Obj=8 // @poff=137
0x7d: Pop(0)
0x7e: Push((int) 3)
0x7f: @ Sleep(Stack[-1])
0x80: Pop(1)
...
0x8f: Push("HasProperty") // @poff=150
0x90: Push((int) 2)
...
0x96: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=174
0x97: Pop(0)
0x98: Stack[-1] = Stack[-5]
0x99: Return(); Pop(2)

0x9a: PushEmpty(object, object, int, object, object, int)
0x9b: Pop(0); PushNull((bool) Stack[-7] == 0)
...
0xa0: Pop(0)
0xa1: Pop(0); PushNull((bool) Stack[-3] == 0)
0xa2: IF (Stack[-1] == 0) GOTO 0xa5; Pop(1)

0xa3: Stack[-8] = (bool) 0
0xa4: Return(); Pop(6)

0xa5: @@ GetScene(Stack[-2]); Obj=7 // @poff=186
0xa6: Pop(0)
...
0xac: Stack[-10] = Stack[-2]
0xad: Stack[-1] = "noaccess" // @poff=195
0xae: Call2 0x8e
...
0xb4: Push("noaccess") // @poff=195
0xb5: @@ GetProperty(Stack[-1], Stack[-2]); Obj=8 // @poff=213
0xb6: Pop(1)
0xb7: Push((int) 0)
