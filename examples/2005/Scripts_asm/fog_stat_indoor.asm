...
Strings:
	...
	W:cleanup
	W:restore
// @pool_raw:
...
Events:
EVENT_26 Op = 0x101 Vars = (string)
EVENT_6 Op = 0x119 Vars = ()
...
0x32: Stack[-1] = (float) 0.10000000149011612
0x33: Call2 0x87

0x34: Stack[-3] = Stack[-4]
0x35: Pop(3)
0x36: Push((int) 5)
0x37: Push((float)0.10000000149011612)
...
0x48: Pop(2)
0x49: Push((int) 1)
0x4a: @@ FadeOut(Stack[-1]); Obj=3 // @poff=14
0x4b: Pop(1)
0x4c: Push((int) 1)
0x4d: @ Sleep(Stack[-1])
0x4e: Pop(1)
...
0x53: Pop(0)
0x54: @@ GetPosition(Stack[-2]); Obj=7 // @poff=22
0x55: Pop(0)
...
0x6d: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=72
0x6e: Pop(0)
...
0x73: Stack[-10] = Stack[-2]
0x74: Stack[-9] = Stack[-1]
0x75: Call2 0x65
...
0x7b: @@ GetProperty(Stack[-6], Stack[-1]); Obj=7 // @poff=84
0x7c: Pop(0)
0x7d: PushEmpty(float, float, float, float)
0x7e: Stack[-3] = Stack[-5] + Stack[-9]; Pop(0);
0x7f: Stack[-8] = Stack[-2]
0x80: Stack[-7] = Stack[-1]
0x81: Call2 0xd7

0x82: Pop(3)
0x83: @@ SetProperty(Stack[-7], Stack[-1]); Obj=8 // @poff=96
0x84: Pop(1)
0x85: Stack[-8] = (bool) 1
0x86: Return(); Pop(2)
...
0x89: Stack[-9] = Stack[-2]
0x8a: Stack[-1] = "disease" // @poff=108
0x8b: Call2 0x65
...
0x93: Stack[-9] = Stack[-2]
0x94: Stack[-1] = "armor_disease" // @poff=124
0x95: Call2 0x65

0x96: Pop(2)
0x97: IF (Stack[-1] == 0) GOTO 0x9d; Pop(1)

0x98: Push("armor_disease") // @poff=124
0x99: @@ GetProperty(Stack[-1], Stack[-3]); Obj=7 // @poff=84
0x9a: Pop(1)
0x9b: Push((int) 100)
0x9c: Stack[-3] = Stack[-3] / Stack[-1]; Pop(1);
0x9d: PushEmpty(bool, object, string)
0x9e: Stack[-9] = Stack[-2]
0x9f: Stack[-1] = "immunity" // @poff=152
0xa0: Call2 0x65

0xa1: Pop(2)
0xa2: IF (Stack[-1] == 0) GOTO 0xaf; Pop(1)

0xa3: Push("immunity") // @poff=152
0xa4: @@ GetProperty(Stack[-1], Stack[-2]); Obj=7 // @poff=84
0xa5: Pop(1)
0xa6: Stack[-2] = Stack[-2] + Stack[-1]; Pop(0);
0xa7: PushEmpty(bool, object, string, float, float, float)
0xa8: Stack[-12] = Stack[-5]
0xa9: Stack[-4] = "immunity" // @poff=152
...
0xba: Stack[-12] = Stack[-5]
0xbb: Stack[-4] = "disease" // @poff=108
0xbc: Stack[-11] = Stack[-3]
0xbd: Stack[-2] = (int) 0
0xbe: Stack[-1] = (int) 1
0xbf: Call2 0x71
...
0xc7: Stack[-6] = Stack[-1]
0xc8: Call2 0xe2

0xc9: Pop(1)
0xca: Stack[-5] = Stack[-7]
0xcb: Return(); Pop(4)
...
0x102: Push("cleanup") // @poff=174
0x103: Pop(1); Push((bool) Stack[-4] == Stack[-1])
0x104: IF (Stack[-1] == 0) GOTO 0x112; Pop(1)

0x105: Push(GlobalVars[0])
0x106: Stack[-1] = (bool) 1
0x107: GlobalVars[0] = Stack[-1]; Pop(1)
0x108: @ IsLoaded(Stack[-1])
0x109: Pop(0)
...
0x112: Push("restore") // @poff=190
0x113: Pop(1); Push((bool) Stack[-4] == Stack[-1])
0x114: IF (Stack[-1] == 0) GOTO 0x118; Pop(1)

0x115: Push(GlobalVars[0])
0x116: Stack[-1] = (bool) 0
0x117: GlobalVars[0] = Stack[-1]; Pop(1)
0x118: Return(); Pop(2)

0x119: Push(GlobalVars[0])
