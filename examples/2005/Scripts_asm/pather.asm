...
Strings:
	...
	W:noaccess
	A:GetProperty
// @pool_raw:
...
0xa: Stack[-4] = Stack[-2]
0xb: Stack[-1] = "pt_b4_boy_dest" // @poff=14
...
0x21: Pop(0)
0x22: @@ GetLocator(Stack[-9], Stack[-3], Stack[-2], Stack[-1]); Obj=4 // @poff=44
0x23: Pop(0)
0x24: Pop(0); Push((bool) Stack[-3] == 0)
0x25: IF (Stack[-1] == 0) GOTO 0x2b; Pop(1)

0x26: Push("Unknown destination point") // @poff=55
0x27: @ Trace(Stack[-1])
0x28: Pop(1)
...
0x2c: Stack[-16] = Stack[-5]
0x2d: Stack[-8] = Stack[-4]
...
0x3b: Stack[-17] = Stack[-7]
0x3c: PushEmpty(bool, object)
0x3d: Stack[-23] = Stack[-1]
0x3e: Call2 0xd2
...
0x44: @@ GetPosition(Stack[-5]); Obj=21 // @poff=107
0x45: Pop(0)
...
0x63: Push("1") // @poff=141
0x64: @ Trace(Stack[-1])
0x65: Pop(1)
0x66: Push(Stack[-7])
0x67: IF (Stack[-1] == 0) GOTO 0x71; Pop(1)

0x68: Push("2") // @poff=145
0x69: @ Trace(Stack[-1])
0x6a: Pop(1)
0x6b: Stack[-7] = (bool) 0
...
0x8f: Push("6") // @poff=161
0x90: @ Trace(Stack[-1])
0x91: Pop(1)
0x92: Stack[-6] = 0
0x93: GOTO 0xa7

0x94: Push("7") // @poff=165
0x95: @ Trace(Stack[-1])
0x96: Pop(1)
0x97: Push((int) 0)
...
0xa3: Push((int) 0)
0xa4: Push((float)0.30000001192092896)
...
0xd6: Stack[-2] = Stack[-4]
0xd7: Pop(2)
0xd8: Return(); Pop(0)

0xd9: Stack[-1] = "walk" // @poff=169
0xda: Return(); Pop(0)

0xdb: Stack[-1] = "run" // @poff=179
0xdc: Return(); Pop(0)

0xdd: Push("Waiting for your orders!") // @poff=187
0xde: @ Trace(Stack[-1])
0xdf: Pop(1)
0xe0: @ Hold()
0xe1: Pop(0)
0xe2: Push("Following you!") // @poff=237
0xe3: @ Trace(Stack[-1])
0xe4: Pop(1)
0xe5: Return(); Pop(0)

0xe6: PushEmpty(bool, bool)
0xe7: Push("HasProperty") // @poff=267
0xe8: Push((int) 2)
...
0xee: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=291
0xef: Pop(0)
0xf0: Stack[-1] = Stack[-5]
0xf1: Return(); Pop(2)

0xf2: PushEmpty(bool, bool)
0xf3: @@ IsDead(Stack[-1]); Obj=3 // @poff=303
0xf4: Pop(0)
0xf5: Stack[-1] = Stack[-4]
0xf6: Return(); Pop(2)

0xf7: PushEmpty(object, object, object, object)
0xf8: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x10c: Pop(0)
0x10d: Pop(0); PushNull((bool) Stack[-2] == 0)
0x10e: IF (Stack[-1] == 0) GOTO 0x111; Pop(1)

0x10f: Stack[-6] = (bool) 0
0x110: Return(); Pop(4)

0x111: @@ GetScene(Stack[-1]); Obj=5 // @poff=324
0x112: Pop(0)
...
0x125: Stack[-6] = Stack[-2]
0x126: Stack[-1] = "noaccess" // @poff=333
0x127: Call2 0xe6
...
0x12d: Push("noaccess") // @poff=333
0x12e: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=351
0x12f: Pop(1)
0x130: Push((int) 0)
