...
Strings:
	W:disease
	W:Disease effect started
	W:Disease effect stopped
	W:sick
	W:sleeping
// @pool_raw:
...
0x19: Push("Disease effect started") // @poff=16
0x1a: @ Trace(Stack[-1])
0x1b: Pop(1)
0x1c: Push("disease") // @poff=0
...
0x5a: Stack[-1] = (float) 0.10000000149011612
0x5b: Call2 0x76
...
0x72: Push("Disease effect stopped") // @poff=62
0x73: @ Trace(Stack[-1])
0x74: Pop(1)
0x75: Return(); Pop(6)
...
0x86: Pop(1)
0x87: Push((float)0.6283185482025146)
0x88: Push((float)0.5)
...
0xb4: Stack[-5] = Stack[-2]
0xb5: Stack[-9] = Stack[-1]
0xb6: Call2 0x12f
...
0xbb: Stack[-5] = Stack[-2]
0xbc: Stack[-8] = Stack[-1]
0xbd: Call2 0x143

0xbe: Pop(2)
0xbf: Push((int) 0)
0xc0: Pop(1); Push((bool) Stack[-4] > Stack[-1])
0xc1: IF (Stack[-1] == 1) GOTO 0xab; Pop(1)
...
0xd8: Pop(1)
0xd9: Push((float)0.6283185482025146)
0xda: Push((float)0.5)
...
0xfd: Stack[-5] = Stack[-2]
0xfe: Stack[-8] = Stack[-1]
0xff: Call2 0x143
...
0x104: IF (Stack[-1] == 1) GOTO 0xef; Pop(1)
...
0x10f: Stack[-5] = Stack[-2]
0x110: Stack[-9] = Stack[-1]
0x111: Call2 0x12f
...
0x116: Stack[-5] = Stack[-2]
0x117: Stack[-8] = Stack[-1]
0x118: Call2 0x143
...
0x11e: IF (Stack[-1] == 1) GOTO 0x106; Pop(1)
...
0x14e: Push("disease") // @poff=0
0x14f: @ GetProperty(Stack[-1], Stack[-0]T)
0x150: Pop(1)
...
0x16f: Stack[-4] = Stack[-5]
0x170: Pop(4)
0x171: Push((float)0.0010000000474974513)
0x172: Pop(1); Push(Stack[0 + Tasks[-1].StackPointer] + Stack[-1]);
...
0x178: Push((float)0.0010000000474974513)
...
0x1a2: Stack[-3] = Stack[-4]
0x1a3: Return(); Pop(0)

0x1a4: PushEmpty(float, float)
0x1a5: Push("disease") // @poff=0
0x1a6: @ GetProperty(Stack[-1], Stack[-2])
0x1a7: Pop(1)
0x1a8: Push((float)0.0010000000474974513)
