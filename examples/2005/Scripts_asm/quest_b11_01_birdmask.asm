...
Strings:
	...
	W:oob11Birdmask1
	W:branch
// @pool_raw:
// @pool_encoding:utf8
...
0x1fe: Push("head") // @poff=198
0x1ff: @ HasAnimationTrack(Stack[-2], Stack[-1])
0x200: Pop(1)
0x201: Push(Stack[-1])
0x202: IF (Stack[-1] == 0) GOTO 0x206; Pop(1)

0x203: Push("head") // @poff=198
0x204: @ UnlookAsync(Stack[-1])
0x205: Pop(1)
0x206: Return(); Pop(2)

0x207: PushEmpty(float, float, float, float)
0x208: @ lshGetAnimTimes(Stack[-5], Stack[-2], Stack[-1])
0x209: Pop(0)
0x20a: Push((bool) 0)
0x20b: @ lshPlayAnimation(Stack[-3], Stack[-2], Stack[-1])
0x20c: Pop(1)
0x20d: Return(); Pop(4)

0x20e: PushEmpty(float, float, float, float)
0x20f: @ lshGetAnimTimes(Stack[-6], Stack[-2], Stack[-1])
0x210: Pop(0)
0x211: @ lshPlayAnimation(Stack[-2], Stack[-1], Stack[-5])
0x212: Pop(0)
0x213: Return(); Pop(4)
...
0x220: Stack[-1] = 0
0x221: PushEmpty(float, float)
0x222: Pop(0); Push(Stack[-3] | Stack[-3]);
0x223: Stack[-2] = Sqrt(Stack[-1]); Pop(1);
0x224: Push((float)9.999999974752427e-07)
0x225: Pop(1); Push((bool) Stack[-2] < Stack[-1])
0x226: IF (Stack[-1] == 0) GOTO 0x229; Pop(1)

0x227: Stack[-4] = [0.0, 0.0, 0.0]
0x228: Return(); Pop(2)

0x229: Stack[-4] = Stack[-3] / Stack[-1]; Pop(0);
0x22a: Return(); Pop(2)

0x22b: PushEmpty(int, int)
0x22c: @ GetVariable(Stack[-3], Stack[-1])
0x22d: Pop(0)
0x22e: Stack[-1] = Stack[-4]
0x22f: Return(); Pop(2)

0x230: PushEmpty(string, string)
0x231: Stack[-1] = "idle" // @poff=146
0x232: Push(Stack[-3])
...
0x246: Stack[-2] = Stack[-5]
0x247: Return(); Pop(4)

0x248: Stack[-1] = (int) 515571
0x249: Return(); Pop(0)

0x24a: Stack[-1] = (int) 504029
0x24b: Return(); Pop(0)

0x24c: Stack[-1] = "ui/NPC_bmask.png" // @poff=208
0x24d: Return(); Pop(0)

0x24e: Stack[-1] = "ui/NPC_bmask_b.png" // @poff=242
0x24f: Return(); Pop(0)
...
0x252: PushEmpty()
0x253: Push("oob11Birdmask1") // @poff=280
0x254: Push((int) 1)
0x255: @ SetVariable(Stack[-2], Stack[-1])
0x256: Pop(2)
0x257: Return(); Pop(0)

0x258: PushEmpty()
0x259: PushEmpty(int, string)
0x25a: Stack[-1] = "oob11Birdmask1" // @poff=280
0x25b: Call2 0x22b
