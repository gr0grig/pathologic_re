...
Strings:
	...
	A:GetCategory
	A:Remove
// @pool_raw:
...
0x14: Stack[-6] = Stack[-2]
0x15: Stack[-1] = "b2q01_key" // @poff=14
0x16: Call2 0x69

0x17: Pop(3)
0x18: Push("locked") // @poff=0
0x19: Push((int) 0)
0x1a: @ SetProperty(Stack[-2], Stack[-1])
0x1b: Pop(2)
0x1c: Push("unlock") // @poff=34
...
0x3a: IF (Stack[-1] == 1) GOTO 0x41; Pop(1)
...
0x3f: IF (Stack[-1] == 1) GOTO 0x41; Pop(1)
...
0x43: @@ IsOnGround(Stack[-2]); Obj=5 // @poff=48
0x44: Pop(0)
0x45: Push(Stack[-2])
...
0x51: Push("nouse_container") // @poff=59
...
0x58: Pop(0)
0x59: @@ add(Stack[-4]); Obj=1 // @poff=91
0x5a: Pop(0)
0x5b: @@ add(Stack[-3]); Obj=1 // @poff=91
0x5c: Pop(0)
0x5d: Push((int) 4)
...
0x64: Pop(0)
0x65: @@ HasItem(Stack[-2], Stack[-1]); Obj=6 // @poff=95
0x66: Pop(0)
...
0x6b: Pop(0)
0x6c: Push("Category") // @poff=103
0x6d: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0x6e: Pop(1)
0x6f: @@ RemoveItemByType(Stack[-1], Stack[-3], Stack[-2]); Obj=8 // @poff=121
0x70: Pop(0)
0x71: Push(Stack[-1])
0x72: IF (Stack[-1] == 0) GOTO 0x78; Pop(1)

0x73: PushEmpty(int, int)
0x74: Stack[-5] = Stack[-2]
0x75: Stack[-1] = (int) 1
0x76: Call2 0x56
...
0x9e: Push("Can't retrieve diary root") // @poff=138
0x9f: @ Trace(Stack[-1])
0xa0: Pop(1)
0xa1: Stack[-3] = (bool) 0
0xa2: Return(); Pop(2)
...
0xa9: Stack[-1] = Stack[-4]
0xaa: Pop(1)
0xab: @@ Find(Stack[-7], Stack[-2]); Obj=3 // @poff=190
0xac: Pop(0)
0xad: Pop(0); Push((bool) Stack[-2] == 0)
0xae: IF (Stack[-1] == 0) GOTO 0xb5; Pop(1)

0xaf: Push("Can't find diary parent with id: ") // @poff=195
...
0xb5: @@ AddChild(Stack[-8]); Obj=2 // @poff=263
0xb6: Pop(0)
0xb7: Push((int) 7)
0xb8: @ SendWorldWndMessage(Stack[-1])
0xb9: Pop(1)
0xba: @@ GetCategory(Stack[-1]); Obj=8 // @poff=272
0xbb: Pop(0)
...
0xc6: Pop(1)
0xc7: @@ Find(Stack[-5], Stack[-1]); Obj=2 // @poff=190
0xc8: Pop(0)
