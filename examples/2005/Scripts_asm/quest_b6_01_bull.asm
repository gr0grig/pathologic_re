...
Strings:
	W:b6q01_bull_envelope
	W:all
	W:idle
	W:cleanup
	W:restore
// @pool_raw:620036007100300031005f00620075006c006c005f0065006e00760065006c006f0070006500000061006c006c000000690064006c006500000063006c00650061006e0075007000000072006500730074006f00720065000000
...
0x16: Pop(0)
0x17: Push("b6q01_bull_envelope") // @poff=0
0x18: Push([0.0, 0.0, 0.0])
0x19: Push([0.0, 0.0, 1.0])
...
0x53: Stack[-2] = Stack[-3]
0x54: Pop(2)
0x55: Push("all") // @poff=40
...
0x5f: Push("all") // @poff=40
0x60: Push("idle") // @poff=48
...
0x9e: Stack[-1] = "idle" // @poff=48
0x9f: Push(Stack[-3])
0xa0: IF (Stack[-1] == 0) GOTO 0xa2; Pop(1)

0xa1: Stack[-1] = Stack[-1] + Stack[-3]; Pop(0);
0xa2: Stack[-1] = Stack[-4]
0xa3: Return(); Pop(2)

0xa4: PushEmpty()
0xa5: Push("idle") // @poff=48
0xa6: Push((int) 1)
...
0xba: Stack[-2] = Stack[-5]
0xbb: Return(); Pop(4)

0xbc: PushEmpty(bool, bool)
0xbd: Push("cleanup") // @poff=58
