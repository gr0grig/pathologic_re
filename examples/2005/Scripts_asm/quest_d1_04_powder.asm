...
Strings:
	...
	A:AddChild
	A:GetCategory
// @pool_raw:
...
0x2a: Push("object") // @poff=16
0x2b: @ FindGeometry(Stack[-1], Stack[-2])
0x2c: Pop(1)
0x2d: Push((bool) 1)
0x2e: @@ Enable(Stack[-1]); Obj=2 // @poff=30
0x2f: Pop(1)
0x30: @ Hold()
0x31: Pop(0)
0x32: GOTO 0x30
...
0x44: Stack[-9] = Stack[-3]
0x45: Stack[-6] = Stack[-2]
...
0x5f: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=61
0x60: Pop(0)
0x61: Stack[-1] = Stack[-5]
0x62: Return(); Pop(2)

0x63: PushEmpty(bool, bool)
0x64: @@ IsDead(Stack[-1]); Obj=3 // @poff=73
0x65: Pop(0)
0x66: Stack[-1] = Stack[-4]
0x67: Return(); Pop(2)

0x68: PushEmpty(object, object, object, object)
0x69: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x7d: Pop(0)
0x7e: Pop(0); PushNull((bool) Stack[-2] == 0)
0x7f: IF (Stack[-1] == 0) GOTO 0x82; Pop(1)

0x80: Stack[-6] = (bool) 0
0x81: Return(); Pop(4)

0x82: @@ GetScene(Stack[-1]); Obj=5 // @poff=94
0x83: Pop(0)
...
0x96: Stack[-6] = Stack[-2]
0x97: Stack[-1] = "noaccess" // @poff=103
0x98: Call2 0x57
...
0x9e: Push("noaccess") // @poff=103
0x9f: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=121
0xa0: Pop(1)
0xa1: Push((int) 0)
...
0xac: Pop(0)
0xad: @@ add(Stack[-4]); Obj=1 // @poff=133
0xae: Pop(0)
0xaf: @@ add(Stack[-3]); Obj=1 // @poff=133
0xb0: Pop(0)
0xb1: Push((int) 3)
...
0xb7: @@ GetItemID(Stack[-3]); Obj=8 // @poff=137
0xb8: Pop(0)
0xb9: Push("Category") // @poff=147
0xba: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0xbb: Pop(1)
0xbc: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7]); Obj=9 // @poff=165
0xbd: Pop(0)
0xbe: Push(Stack[-1])
0xbf: IF (Stack[-1] == 0) GOTO 0xc5; Pop(1)

0xc0: PushEmpty(int, int)
0xc1: Stack[-5] = Stack[-2]
0xc2: Stack[-9] = Stack[-1]
0xc3: Call2 0xaa

0xc4: Pop(2)
0xc5: Stack[-1] = Stack[-10]
0xc6: Return(); Pop(6)
...
0xd9: Push("Can't retrieve diary root") // @poff=173
0xda: @ Trace(Stack[-1])
0xdb: Pop(1)
0xdc: Stack[-3] = (bool) 0
0xdd: Return(); Pop(2)

0xde: Stack[-1] = Stack[-3]
0xdf: Return(); Pop(2)
...
0xe5: Pop(1)
0xe6: @@ Find(Stack[-7], Stack[-2]); Obj=3 // @poff=225
0xe7: Pop(0)
0xe8: Pop(0); Push((bool) Stack[-2] == 0)
0xe9: IF (Stack[-1] == 0) GOTO 0xf0; Pop(1)

0xea: Push("Can't find diary parent with id: ") // @poff=230
...
0xf0: @@ AddChild(Stack[-8]); Obj=2 // @poff=298
0xf1: Pop(0)
0xf2: Push((int) 7)
0xf3: @ SendWorldWndMessage(Stack[-1])
0xf4: Pop(1)
0xf5: @@ GetCategory(Stack[-1]); Obj=8 // @poff=307
0xf6: Pop(0)
