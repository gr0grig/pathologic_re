...
Strings:
	...
	W:ood6Woman1
	W:branch
// @pool_raw:
// @pool_encoding:utf8
...
0x122: Push("player") // @poff=156
0x123: @ FindActor(Stack[-4]T, Stack[-1])
0x124: Pop(1)
...
0x137: Pop(0); PushNull((bool) Stack[4 + Tasks[-1].StackPointer] == 0)
...
0x1c2: Pop(0)
0x1c3: @@ GetPosition(Stack[-2]); Obj=7 // @poff=180
0x1c4: Pop(0)
...
0x1ce: @@ GetPosition(Stack[-8]); Obj=20 // @poff=180
0x1cf: Pop(0)
0x1d0: @@ GetEyesHeight(Stack[-9]); Obj=20 // @poff=192
0x1d1: Pop(0)
...
0x1f6: @ StopWorld()
0x1f7: Pop(0)
0x1f8: @ CameraTransit(Stack[-3], Stack[-5])
0x1f9: Pop(0)
...
0x222: PushEmpty(float, float, float, float)
0x223: @ lshGetAnimTimes(Stack[-5], Stack[-2], Stack[-1])
0x224: Pop(0)
0x225: Push((bool) 0)
0x226: @ lshPlayAnimation(Stack[-3], Stack[-2], Stack[-1])
0x227: Pop(1)
0x228: Return(); Pop(4)

0x229: PushEmpty(float, float, float, float)
0x22a: @ lshGetAnimTimes(Stack[-6], Stack[-2], Stack[-1])
0x22b: Pop(0)
0x22c: @ lshPlayAnimation(Stack[-2], Stack[-1], Stack[-5])
0x22d: Pop(0)
0x22e: Return(); Pop(4)

0x22f: PushEmpty(float, cvector, float, cvector)
0x230: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=192
0x231: Pop(0)
0x232: Stack[-1] = [0.0, 0.0, 0.0]
0x233: Push(CvectorIndex(Stack[-1], 1))
0x234: Stack[-3] = Stack[-1]
0x235: CvectorIndex(Stack[-2], 1) = Stack[-1];
0x236: Push("head") // @poff=170
...
0x251: Stack[-1] = "idle" // @poff=146
0x252: Push(Stack[-3])
0x253: IF (Stack[-1] == 0) GOTO 0x255; Pop(1)

0x254: Stack[-1] = Stack[-1] + Stack[-3]; Pop(0);
0x255: Stack[-1] = Stack[-4]
0x256: Return(); Pop(2)

0x257: PushEmpty(int, bool, int, bool)
0x258: Stack[-2] = (int) 0
0x259: Push("all") // @poff=138
0x25a: PushEmpty(string, int)
0x25b: Stack[-5] = Stack[-1]
0x25c: Call2 0x250

0x25d: Pop(1)
0x25e: @ HasAnimation(Stack[-3], Stack[-2], Stack[-1])
0x25f: Pop(2)
...
0x263: Push((int) 1)
0x264: Stack[-3] = Stack[-3] + Stack[-1]; Pop(1);
0x265: GOTO 0x259

0x266: Stack[-2] = Stack[-5]
0x267: Return(); Pop(4)

0x268: Stack[-1] = (int) 515567
0x269: Return(); Pop(0)

0x26a: Stack[-1] = (int) 503352
0x26b: Return(); Pop(0)

0x26c: Stack[-1] = "ui/NPC_Citizen3.png" // @poff=206
0x26d: Return(); Pop(0)

0x26e: Stack[-1] = "ui/NPC_Citizen3_b.png" // @poff=246
0x26f: Return(); Pop(0)
...
0x272: PushEmpty()
0x273: Push("ood6Woman1") // @poff=290
0x274: Push((int) 1)
0x275: @ SetVariable(Stack[-2], Stack[-1])
0x276: Pop(2)
0x277: Return(); Pop(0)

0x278: PushEmpty()
0x279: PushEmpty(int, string)
0x27a: Stack[-1] = "ood6Woman1" // @poff=290
0x27b: Call2 0x24b
...
0x280: Stack[-2] = (bool) 1
0x281: Return(); Pop(0)

0x282: Stack[-2] = (bool) 0
0x283: Return(); Pop(0)

0x284: PushEmpty(int, int)
0x285: Push("branch") // @poff=312
0x286: @ GetVariable(Stack[-1], Stack[-2])
0x287: Pop(1)
0x288: Push((int) 0)
