...
Strings:
	...
	W:ood6Worker1
	W:branch
// @pool_raw:
// @pool_encoding:utf8
...
0x139: Push("player") // @poff=156
0x13a: @ FindActor(Stack[-4]T, Stack[-1])
0x13b: Pop(1)
...
0x14e: Pop(0); PushNull((bool) Stack[4 + Tasks[-1].StackPointer] == 0)
...
0x1d9: Pop(0)
0x1da: @@ GetPosition(Stack[-2]); Obj=7 // @poff=180
0x1db: Pop(0)
...
0x1e5: @@ GetPosition(Stack[-8]); Obj=20 // @poff=180
0x1e6: Pop(0)
0x1e7: @@ GetEyesHeight(Stack[-9]); Obj=20 // @poff=192
0x1e8: Pop(0)
...
0x20d: @ StopWorld()
0x20e: Pop(0)
0x20f: @ CameraTransit(Stack[-3], Stack[-5])
0x210: Pop(0)
0x211: Push(CvectorIndex(Stack[-4], 0))
0x212: Push(CvectorIndex(Stack[-5], 2))
...
0x220: @ LookAsyncCamera(Stack[-1])
0x221: Pop(1)
0x222: @ CameraWaitForPlayFinish()
0x223: Pop(0)
0x224: @ ResumeWorld()
0x225: Pop(0)
0x226: Stack[-21] = (bool) 1
0x227: Return(); Pop(18)

0x228: PushEmpty(bool, bool)
0x229: @ CameraSwitchToNormal()
0x22a: Pop(0)
...
0x230: Push("head") // @poff=170
0x231: @ HasAnimationTrack(Stack[-2], Stack[-1])
0x232: Pop(1)
0x233: Push(Stack[-1])
0x234: IF (Stack[-1] == 0) GOTO 0x238; Pop(1)

0x235: Push("head") // @poff=170
0x236: @ UnlookAsync(Stack[-1])
0x237: Pop(1)
0x238: Return(); Pop(2)

0x239: PushEmpty(float, float, float, float)
0x23a: @ lshGetAnimTimes(Stack[-5], Stack[-2], Stack[-1])
0x23b: Pop(0)
0x23c: Push((bool) 0)
0x23d: @ lshPlayAnimation(Stack[-3], Stack[-2], Stack[-1])
0x23e: Pop(1)
0x23f: Return(); Pop(4)

0x240: PushEmpty(float, float, float, float)
0x241: @ lshGetAnimTimes(Stack[-6], Stack[-2], Stack[-1])
0x242: Pop(0)
0x243: @ lshPlayAnimation(Stack[-2], Stack[-1], Stack[-5])
0x244: Pop(0)
0x245: Return(); Pop(4)

0x246: PushEmpty(float, cvector, float, cvector)
0x247: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=192
0x248: Pop(0)
0x249: Stack[-1] = [0.0, 0.0, 0.0]
0x24a: Push(CvectorIndex(Stack[-1], 1))
0x24b: Stack[-3] = Stack[-1]
0x24c: CvectorIndex(Stack[-2], 1) = Stack[-1];
0x24d: Push("head") // @poff=170
0x24e: @ LookAsync(Stack[-6], Stack[-1], Stack[-2])
0x24f: Pop(1)
0x250: Return(); Pop(4)
...
0x258: PushEmpty(float, float)
0x259: Pop(0); Push(Stack[-3] | Stack[-3]);
0x25a: Stack[-2] = Sqrt(Stack[-1]); Pop(1);
0x25b: Push((float)9.999999974752427e-07)
0x25c: Pop(1); Push((bool) Stack[-2] < Stack[-1])
0x25d: IF (Stack[-1] == 0) GOTO 0x260; Pop(1)

0x25e: Stack[-4] = [0.0, 0.0, 0.0]
0x25f: Return(); Pop(2)

0x260: Stack[-4] = Stack[-3] / Stack[-1]; Pop(0);
0x261: Return(); Pop(2)

0x262: PushEmpty(int, int)
0x263: @ GetVariable(Stack[-3], Stack[-1])
0x264: Pop(0)
0x265: Stack[-1] = Stack[-4]
0x266: Return(); Pop(2)

0x267: PushEmpty(string, string)
0x268: Stack[-1] = "idle" // @poff=146
0x269: Push(Stack[-3])
0x26a: IF (Stack[-1] == 0) GOTO 0x26c; Pop(1)

0x26b: Stack[-1] = Stack[-1] + Stack[-3]; Pop(0);
0x26c: Stack[-1] = Stack[-4]
0x26d: Return(); Pop(2)

0x26e: PushEmpty(int, bool, int, bool)
0x26f: Stack[-2] = (int) 0
0x270: Push("all") // @poff=138
0x271: PushEmpty(string, int)
0x272: Stack[-5] = Stack[-1]
0x273: Call2 0x267

0x274: Pop(1)
0x275: @ HasAnimation(Stack[-3], Stack[-2], Stack[-1])
0x276: Pop(2)
...
0x27f: Stack[-1] = (int) 515568
0x280: Return(); Pop(0)

0x281: Stack[-1] = (int) 503353
0x282: Return(); Pop(0)

0x283: Stack[-1] = "ui/NPC_Citizen1.png" // @poff=206
0x284: Return(); Pop(0)

0x285: Stack[-1] = "ui/NPC_Citizen1_b.png" // @poff=246
0x286: Return(); Pop(0)
...
0x289: PushEmpty()
0x28a: Push("ood6Worker1") // @poff=290
0x28b: Push((int) 1)
...
0x291: Stack[-1] = "ood6Worker1" // @poff=290
0x292: Call2 0x262
