...
Strings:
	...
	W:teleport_to_han
	W:branch
// @pool_raw:
// @pool_encoding:utf8
...
0x22d: PushEmpty(float, float, float, float)
0x22e: @ lshGetAnimTimes(Stack[-5], Stack[-2], Stack[-1])
0x22f: Pop(0)
0x230: Push((bool) 0)
0x231: @ lshPlayAnimation(Stack[-3], Stack[-2], Stack[-1])
0x232: Pop(1)
0x233: Return(); Pop(4)

0x234: PushEmpty(float, float, float, float)
0x235: @ lshGetAnimTimes(Stack[-6], Stack[-2], Stack[-1])
0x236: Pop(0)
0x237: @ lshPlayAnimation(Stack[-2], Stack[-1], Stack[-5])
0x238: Pop(0)
0x239: Return(); Pop(4)

0x23a: PushEmpty(float, cvector, float, cvector)
0x23b: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=192
0x23c: Pop(0)
0x23d: Stack[-1] = [0.0, 0.0, 0.0]
0x23e: Push(CvectorIndex(Stack[-1], 1))
0x23f: Stack[-3] = Stack[-1]
0x240: CvectorIndex(Stack[-2], 1) = Stack[-1];
0x241: Push("head") // @poff=170
...
0x258: Pop(0)
0x259: Pop(0); PushNull((bool) Stack[-1] == 0)
0x25a: IF (Stack[-1] == 0) GOTO 0x25d; Pop(1)

0x25b: Stack[-5] = (bool) 0
0x25c: Return(); Pop(2)

0x25d: @ Trigger(Stack[-1], Stack[-3])
0x25e: Pop(0)
0x25f: Stack[-5] = (bool) 1
0x260: Return(); Pop(2)

0x261: Stack[-1] = 0
0x262: PushEmpty(string, string)
0x263: Stack[-1] = "idle" // @poff=146
0x264: Push(Stack[-3])
0x265: IF (Stack[-1] == 0) GOTO 0x267; Pop(1)

0x266: Stack[-1] = Stack[-1] + Stack[-3]; Pop(0);
0x267: Stack[-1] = Stack[-4]
0x268: Return(); Pop(2)

0x269: PushEmpty(int, bool, int, bool)
0x26a: Stack[-2] = (int) 0
0x26b: Push("all") // @poff=138
0x26c: PushEmpty(string, int)
0x26d: Stack[-5] = Stack[-1]
0x26e: Call2 0x262

0x26f: Pop(1)
0x270: @ HasAnimation(Stack[-3], Stack[-2], Stack[-1])
0x271: Pop(2)
...
0x275: Push((int) 1)
0x276: Stack[-3] = Stack[-3] + Stack[-1]; Pop(1);
0x277: GOTO 0x26b

0x278: Stack[-2] = Stack[-5]
0x279: Return(); Pop(4)

0x27a: Stack[-1] = (int) 518097
0x27b: Return(); Pop(0)

0x27c: Stack[-1] = (int) 518096
0x27d: Return(); Pop(0)

0x27e: Stack[-1] = "ui/NPC_Citizen2.png" // @poff=206
0x27f: Return(); Pop(0)

0x280: Stack[-1] = "ui/NPC_Citizen2_b.png" // @poff=246
0x281: Return(); Pop(0)
...
0x284: PushEmpty()
0x285: PushEmpty(bool, string, string)
0x286: Stack[-2] = "quest_d9_01" // @poff=290
0x287: Stack[-1] = "teleport_to_han" // @poff=314
0x288: Call2 0x256
...
0x28c: Push("branch") // @poff=346
0x28d: @ GetVariable(Stack[-1], Stack[-2])
0x28e: Pop(1)
0x28f: Push((int) 0)
