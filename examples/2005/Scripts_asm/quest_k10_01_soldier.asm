...
Strings:
	...
	W:k10q01
	W:branch
// @pool_raw:
// @pool_encoding:utf8
...
0x10e: Push("player") // @poff=156
0x10f: @ FindActor(Stack[-4]T, Stack[-1])
0x110: Pop(1)
...
0x123: Pop(0); PushNull((bool) Stack[4 + Tasks[-1].StackPointer] == 0)
...
0x1ae: Pop(0)
0x1af: @@ GetPosition(Stack[-2]); Obj=7 // @poff=180
0x1b0: Pop(0)
...
0x1ba: @@ GetPosition(Stack[-8]); Obj=20 // @poff=180
0x1bb: Pop(0)
0x1bc: @@ GetEyesHeight(Stack[-9]); Obj=20 // @poff=192
0x1bd: Pop(0)
...
0x20a: Push("head") // @poff=170
0x20b: @ UnlookAsync(Stack[-1])
0x20c: Pop(1)
0x20d: Return(); Pop(2)

0x20e: PushEmpty(float, float, float, float)
0x20f: @ lshGetAnimTimes(Stack[-5], Stack[-2], Stack[-1])
0x210: Pop(0)
0x211: Push((bool) 0)
0x212: @ lshPlayAnimation(Stack[-3], Stack[-2], Stack[-1])
0x213: Pop(1)
0x214: Return(); Pop(4)

0x215: PushEmpty(float, float, float, float)
0x216: @ lshGetAnimTimes(Stack[-6], Stack[-2], Stack[-1])
0x217: Pop(0)
0x218: @ lshPlayAnimation(Stack[-2], Stack[-1], Stack[-5])
0x219: Pop(0)
0x21a: Return(); Pop(4)

0x21b: PushEmpty(float, cvector, float, cvector)
0x21c: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=192
0x21d: Pop(0)
0x21e: Stack[-1] = [0.0, 0.0, 0.0]
0x21f: Push(CvectorIndex(Stack[-1], 1))
0x220: Stack[-3] = Stack[-1]
0x221: CvectorIndex(Stack[-2], 1) = Stack[-1];
0x222: Push("head") // @poff=170
0x223: @ LookAsync(Stack[-6], Stack[-1], Stack[-2])
0x224: Pop(1)
0x225: Return(); Pop(4)
...
0x23c: Stack[-1] = (int) 527721
0x23d: Return(); Pop(0)

0x23e: Stack[-1] = (int) 527720
0x23f: Return(); Pop(0)

0x240: Stack[-1] = "ui/NPC_Citizen1.png" // @poff=206
0x241: Return(); Pop(0)

0x242: Stack[-1] = "ui/NPC_Citizen1_b.png" // @poff=246
0x243: Return(); Pop(0)

0x244: Stack[-1] = (bool) 0
0x245: Return(); Pop(0)

0x246: PushEmpty(string, string)
0x247: Stack[-1] = "idle" // @poff=146
0x248: Push(Stack[-3])
0x249: IF (Stack[-1] == 0) GOTO 0x24b; Pop(1)

0x24a: Stack[-1] = Stack[-1] + Stack[-3]; Pop(0);
0x24b: Stack[-1] = Stack[-4]
0x24c: Return(); Pop(2)

0x24d: PushEmpty(int, bool, int, bool)
0x24e: Stack[-2] = (int) 0
0x24f: Push("all") // @poff=138
0x250: PushEmpty(string, int)
0x251: Stack[-5] = Stack[-1]
0x252: Call2 0x246

0x253: Pop(1)
0x254: @ HasAnimation(Stack[-3], Stack[-2], Stack[-1])
0x255: Pop(2)
...
0x259: Push((int) 1)
0x25a: Stack[-3] = Stack[-3] + Stack[-1]; Pop(1);
0x25b: GOTO 0x24f

0x25c: Stack[-2] = Stack[-5]
0x25d: Return(); Pop(4)

0x25e: PushEmpty()
0x25f: PushEmpty(int, string)
0x260: Stack[-1] = "k10q01" // @poff=290
0x261: Call2 0x237
...
0x266: Stack[-2] = (bool) 1
0x267: Return(); Pop(0)

0x268: Stack[-2] = (bool) 0
0x269: Return(); Pop(0)

0x26a: PushEmpty(int, int)
0x26b: Push("branch") // @poff=304
0x26c: @ GetVariable(Stack[-1], Stack[-2])
0x26d: Pop(1)
0x26e: Push((int) 0)
