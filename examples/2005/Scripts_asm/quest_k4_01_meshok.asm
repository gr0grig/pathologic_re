...
Strings:
	...
	W:Category
	A:AddItem
// @pool_raw:
...
0x2: Stack[-2] = "quest_k4_01" // @poff=0
0x3: Stack[-1] = "meshok_taken" // @poff=24
0x4: Call2 0xc3
...
0x2d: Push("object") // @poff=66
0x2e: @ FindGeometry(Stack[-1], Stack[-2])
0x2f: Pop(1)
0x30: Push((bool) 1)
0x31: @@ Enable(Stack[-1]); Obj=2 // @poff=80
0x32: Pop(1)
0x33: @ Hold()
0x34: Pop(0)
0x35: GOTO 0x33
...
0x48: Stack[-6] = Stack[-2]
0x49: Stack[-5] = Stack[-1]
0x4a: Call2 0xb2
...
0x5b: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=111
0x5c: Pop(0)
0x5d: Stack[-1] = Stack[-5]
0x5e: Return(); Pop(2)

0x5f: PushEmpty(bool, bool)
0x60: @@ IsDead(Stack[-1]); Obj=3 // @poff=123
0x61: Pop(0)
0x62: Stack[-1] = Stack[-4]
0x63: Return(); Pop(2)

0x64: PushEmpty(object, object, object, object)
0x65: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x79: Pop(0)
0x7a: Pop(0); PushNull((bool) Stack[-2] == 0)
0x7b: IF (Stack[-1] == 0) GOTO 0x7e; Pop(1)

0x7c: Stack[-6] = (bool) 0
0x7d: Return(); Pop(4)

0x7e: @@ GetScene(Stack[-1]); Obj=5 // @poff=144
0x7f: Pop(0)
...
0x93: Stack[-1] = "noaccess" // @poff=153
0x94: Call2 0x53
...
0x9a: Push("noaccess") // @poff=153
0x9b: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=171
0x9c: Pop(1)
0x9d: Push((int) 0)
...
0xa8: Pop(0)
0xa9: @@ add(Stack[-4]); Obj=1 // @poff=183
0xaa: Pop(0)
0xab: @@ add(Stack[-3]); Obj=1 // @poff=183
0xac: Pop(0)
0xad: Push((int) 3)
...
0xb3: @@ GetItemID(Stack[-3]); Obj=8 // @poff=187
0xb4: Pop(0)
0xb5: Push("Category") // @poff=197
0xb6: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0xb7: Pop(1)
0xb8: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7]); Obj=9 // @poff=215
0xb9: Pop(0)
0xba: Push(Stack[-1])
0xbb: IF (Stack[-1] == 0) GOTO 0xc1; Pop(1)

0xbc: PushEmpty(int, int)
0xbd: Stack[-5] = Stack[-2]
0xbe: Stack[-9] = Stack[-1]
0xbf: Call2 0xa6
...
0xc5: Pop(0)
0xc6: Pop(0); PushNull((bool) Stack[-1] == 0)
