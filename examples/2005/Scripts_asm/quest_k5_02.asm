...
Strings:
	...
	W:pt_k5q02_patrol2
	W:init_kabak
	W:pt_k5q02_kabatchik
	W:pers_dohodyaga
	W:k5q02_kabatchik.xml
	W:place_andrei
	W:cleanup
	W:completed
	A:GetLocator
	W:Locator 
	W: doesn't exist
// @pool_raw:
...
0x1: Push("k5q02") // @poff=0
0x2: Push((int) 1)
0x3: @ SetVariable(Stack[-2], Stack[-1])
0x4: Pop(2)
0x5: Push("shouse1_kabak") // @poff=12
0x6: @ GetSceneByName(Stack[-2], Stack[-1])
0x7: Pop(1)
0x8: Push("noandrei") // @poff=40
0x9: @ Trigger(Stack[-2], Stack[-1])
0xa: Pop(1)
0xb: PushEmpty(object, object, string, string, string)
0xc: Stack[-6] = Stack[-4]
0xd: Stack[-3] = "pt_k5q02_patrol1" // @poff=58
0xe: Stack[-2] = "pers_patrool" // @poff=92
0xf: Stack[-1] = "k5q02_patrol.xml" // @poff=118
0x10: Call2 0xac
...
0x15: Stack[-3] = "pt_k5q02_patrol2" // @poff=152
0x16: Stack[-2] = "pers_patrool" // @poff=92
0x17: Stack[-1] = "k5q02_patrol.xml" // @poff=118
0x18: Call2 0xac
...
0x22: @ Trace(Stack[-5])
0x23: Pop(0)
0x24: Push("init_kabak") // @poff=186
0x25: Pop(1); Push((bool) Stack[-6] == Stack[-1])
0x26: IF (Stack[-1] == 0) GOTO 0x34; Pop(1)

0x27: Push("shouse1_kabak") // @poff=12
0x28: @ GetSceneByName(Stack[-3], Stack[-1])
0x29: Pop(1)
0x2a: PushEmpty(object, object, string, string, string)
0x2b: Stack[-7] = Stack[-4]
0x2c: Stack[-3] = "pt_k5q02_kabatchik" // @poff=208
0x2d: Stack[-2] = "pers_dohodyaga" // @poff=246
0x2e: Stack[-1] = "k5q02_kabatchik.xml" // @poff=276
0x2f: Call2 0xac
...
0xad: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=378
0xae: Pop(0)
0xaf: Pop(0); Push((bool) Stack[-4] == 0)
0xb0: IF (Stack[-1] == 0) GOTO 0xb9; Pop(1)

0xb1: Push("Locator ") // @poff=389
0xb2: Pop(1); Push(Stack[-1] + Stack[-12]);
0xb3: Push(" doesn't exist") // @poff=407
