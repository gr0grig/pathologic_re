...
// @pool_raw:
// @pool_encoding:utf8
...
0xe0: Push("player") // @poff=156
0xe1: @ FindActor(Stack[-4]T, Stack[-1])
0xe2: Pop(1)
...
0xf5: Pop(0); PushNull((bool) Stack[4 + Tasks[-1].StackPointer] == 0)
...
0x180: Pop(0)
0x181: @@ GetPosition(Stack[-2]); Obj=7 // @poff=180
0x182: Pop(0)
...
0x18c: @@ GetPosition(Stack[-8]); Obj=20 // @poff=180
0x18d: Pop(0)
0x18e: @@ GetEyesHeight(Stack[-9]); Obj=20 // @poff=192
0x18f: Pop(0)
...
0x1b4: @ StopWorld()
0x1b5: Pop(0)
0x1b6: @ CameraTransit(Stack[-3], Stack[-5])
0x1b7: Pop(0)
0x1b8: Push(CvectorIndex(Stack[-4], 0))
0x1b9: Push(CvectorIndex(Stack[-5], 2))
...
0x1c1: Push("head") // @poff=170
0x1c2: @ HasAnimationTrack(Stack[-2], Stack[-1])
0x1c3: Pop(1)
0x1c4: Push(Stack[-1])
0x1c5: IF (Stack[-1] == 0) GOTO 0x1c9; Pop(1)

0x1c6: Push("head") // @poff=170
0x1c7: @ LookAsyncCamera(Stack[-1])
0x1c8: Pop(1)
0x1c9: @ CameraWaitForPlayFinish()
0x1ca: Pop(0)
0x1cb: @ ResumeWorld()
0x1cc: Pop(0)
0x1cd: Stack[-21] = (bool) 1
0x1ce: Return(); Pop(18)

0x1cf: PushEmpty(bool, bool)
0x1d0: @ CameraSwitchToNormal()
0x1d1: Pop(0)
...
0x1d8: @ HasAnimationTrack(Stack[-2], Stack[-1])
0x1d9: Pop(1)
0x1da: Push(Stack[-1])
...
0x1e0: PushEmpty(float, float, float, float)
0x1e1: @ lshGetAnimTimes(Stack[-5], Stack[-2], Stack[-1])
0x1e2: Pop(0)
0x1e3: Push((bool) 0)
0x1e4: @ lshPlayAnimation(Stack[-3], Stack[-2], Stack[-1])
0x1e5: Pop(1)
0x1e6: Return(); Pop(4)

0x1e7: PushEmpty(float, float, float, float)
0x1e8: @ lshGetAnimTimes(Stack[-6], Stack[-2], Stack[-1])
0x1e9: Pop(0)
0x1ea: @ lshPlayAnimation(Stack[-2], Stack[-1], Stack[-5])
0x1eb: Pop(0)
0x1ec: Return(); Pop(4)

0x1ed: PushEmpty(float, cvector, float, cvector)
0x1ee: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=192
0x1ef: Pop(0)
0x1f0: Stack[-1] = [0.0, 0.0, 0.0]
0x1f1: Push(CvectorIndex(Stack[-1], 1))
0x1f2: Stack[-3] = Stack[-1]
0x1f3: CvectorIndex(Stack[-2], 1) = Stack[-1];
0x1f4: Push("head") // @poff=170
0x1f5: @ LookAsync(Stack[-6], Stack[-1], Stack[-2])
0x1f6: Pop(1)
0x1f7: Return(); Pop(4)
...
0x200: Pop(0); Push(Stack[-3] | Stack[-3]);
0x201: Stack[-2] = Sqrt(Stack[-1]); Pop(1);
0x202: Push((float)9.999999974752427e-07)
0x203: Pop(1); Push((bool) Stack[-2] < Stack[-1])
0x204: IF (Stack[-1] == 0) GOTO 0x207; Pop(1)

0x205: Stack[-4] = [0.0, 0.0, 0.0]
0x206: Return(); Pop(2)

0x207: Stack[-4] = Stack[-3] / Stack[-1]; Pop(0);
0x208: Return(); Pop(2)

0x209: PushEmpty(string, string)
0x20a: Stack[-1] = "idle" // @poff=146
0x20b: Push(Stack[-3])
0x20c: IF (Stack[-1] == 0) GOTO 0x20e; Pop(1)

0x20d: Stack[-1] = Stack[-1] + Stack[-3]; Pop(0);
0x20e: Stack[-1] = Stack[-4]
0x20f: Return(); Pop(2)

0x210: PushEmpty(int, bool, int, bool)
0x211: Stack[-2] = (int) 0
0x212: Push("all") // @poff=138
0x213: PushEmpty(string, int)
0x214: Stack[-5] = Stack[-1]
0x215: Call2 0x209

0x216: Pop(1)
0x217: @ HasAnimation(Stack[-3], Stack[-2], Stack[-1])
0x218: Pop(2)
...
0x21c: Push((int) 1)
0x21d: Stack[-3] = Stack[-3] + Stack[-1]; Pop(1);
0x21e: GOTO 0x212
...
0x232: Stack[-1] = (int) 515568
0x233: Return(); Pop(0)

0x234: Stack[-1] = (int) 503353
0x235: Return(); Pop(0)

0x236: Stack[-1] = "ui/NPC_Citizen1.png" // @poff=220
0x237: Return(); Pop(0)

0x238: Stack[-1] = "ui/NPC_Citizen1_b.png" // @poff=260
0x239: Return(); Pop(0)
