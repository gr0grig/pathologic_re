...
Strings:
	...
	A:AddSource
	A:Fade
// @pool_raw:72006900630068006f0063006800650074000000456e61626c6500416464536f75726365004661646500
...
0xc: Push("richochet") // @poff=0
0xd: @ FindParticleSystem(Stack[-1], Stack[-2])
0xe: Pop(1)
0xf: @@ Enable(); Obj=1 // @poff=20
0x10: Pop(0)
0x11: Push([0.0, 0.0, 0.0])
0x12: Push([0.0, 400.0, 0.0])
0x13: Push((float)1.5)
0x14: @@ AddSource(Stack[-3], Stack[-2], Stack[-1]); Obj=4 // @poff=27
0x15: Pop(3)
0x16: Push((float)0.5)
0x17: @ Sleep(Stack[-1])
0x18: Pop(1)
0x19: Push((bool) 1)
0x1a: @@ Fade(Stack[-1]); Obj=2 // @poff=37
0x1b: Pop(1)
0x1c: Push((float)0.5)
0x1d: @ Sleep(Stack[-1])
0x1e: Pop(1)
