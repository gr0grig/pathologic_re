...
Strings:
	...
	A:size
	A:get
// @pool_raw:700074005f00660069007200650070006c006100630065000000730063007200690070007400650064000000660069007200650070006c006100630065002e0078006d006c00000061646400636c6561720073697a650067657400
...
0x17: Push("scripted") // @poff=26
0x18: PushEmpty(object)
0x19: Call2 0x47

0x1a: Pop(0)
0x1b: Push([0.0, 0.0, 1.0])
0x1c: Push("fireplace.xml") // @poff=44
0x1d: @ AddActorByType(Stack[-5], Stack[-4], Stack[-3], Stack[-6], Stack[-2], Stack[-1])
0x1e: Pop(4)
0x1f: @@@ add(Stack[-1]); Obj=0 // @poff=72
0x20: Pop(0)
0x21: Stack[-1] = 0
0x22: Push((int) 1)
...
0x50: @@ size(Stack[-3]); Obj=7 // @poff=82
0x51: Pop(0)
0x52: Stack[-2] = (int) 0
0x53: Pop(0); Push((bool) Stack[-2] < Stack[-3])
0x54: IF (Stack[-1] == 0) GOTO 0x5f; Pop(1)

0x55: @@ get(Stack[-1], Stack[-2]); Obj=7 // @poff=87
0x56: Pop(0)
0x57: Push(Stack[-1])
