...
Strings:
	W:pt_petr
	W:NPC_Petr
	W:NPC_Burah_Petr.xml
	W:nopetr
	A:GetLocator
	W:Locator 
	W: doesn't exist
// @pool_raw:
...
0xa: Stack[-1] = Stack[-5]
0xb: Pop(1)
0xc: Stack[-3] = "pt_petr" // @poff=0
0xd: Stack[-2] = "NPC_Petr" // @poff=16
0xe: Stack[-1] = "NPC_Burah_Petr.xml" // @poff=34
0xf: Call2 0x29
...
0x2a: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=86
0x2b: Pop(0)
0x2c: Pop(0); Push((bool) Stack[-4] == 0)
0x2d: IF (Stack[-1] == 0) GOTO 0x36; Pop(1)

0x2e: Push("Locator ") // @poff=97
0x2f: Pop(1); Push(Stack[-1] + Stack[-12]);
0x30: Push(" doesn't exist") // @poff=115
