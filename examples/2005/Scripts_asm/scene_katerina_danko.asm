...
Strings:
	...
	W:Locator 
	W: doesn't exist
// @pool_raw:700074005f006b006c0061007200610000004e00500043005f004b006c006100720061000000640033005f006b006c006100720061002e0078006d006c0000004765744c6f6361746f72004c006f006300610074006f00720020000000200064006f00650073006e00270074002000650078006900730074000000
...
0xc: Stack[-1] = Stack[-5]
0xd: Pop(1)
0xe: Stack[-3] = "pt_klara" // @poff=0
0xf: Stack[-2] = "NPC_Klara" // @poff=18
0x10: Stack[-1] = "d3_klara.xml" // @poff=38
0x11: Call2 0x20
...
0x21: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=64
0x22: Pop(0)
0x23: Pop(0); Push((bool) Stack[-4] == 0)
0x24: IF (Stack[-1] == 0) GOTO 0x2d; Pop(1)

0x25: Push("Locator ") // @poff=75
0x26: Pop(1); Push(Stack[-1] + Stack[-12]);
0x27: Push(" doesn't exist") // @poff=93
