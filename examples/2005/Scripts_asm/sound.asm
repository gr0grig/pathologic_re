...
Strings:
	...
	W:started
	W:envsound
	W:Sound was not found
	A:FadeIn
	A:GetPosition
	W:HasProperty
	A:HasProperty
// @pool_raw:
...
0x24: Push("distance") // @poff=0
0x25: @ GetProperty(Stack[-1], Stack[-1]T)
0x26: Pop(1)
...
0x2c: Pop(1)
0x2d: Stack[-1] = "mindistance" // @poff=18
0x2e: Call2 0xa2
...
0x34: Push("mindistance") // @poff=18
0x35: @ GetProperty(Stack[-1], Stack[-2]T)
0x36: Pop(1)
0x37: Push((int) 101)
0x38: Push((float)1.0)
...
0x43: Pop(1)
0x44: Pop(0); PushNull((bool) Stack[-5] == 0)
0x45: IF (Stack[-1] == 0) GOTO 0x4a; Pop(1)

0x46: Push("Null player") // @poff=42
0x47: @ Trace(Stack[-1])
0x48: Pop(1)
0x49: Return(); Pop(10)
...
0x5d: Push("stopped") // @poff=66
0x5e: @ Trace(Stack[-1])
0x5f: Pop(1)
0x60: @@@ Stop(); Obj=0 // @poff=82
0x61: Pop(0)
...
0x64: PushEmpty(bool)
0x65: Stack[-1] = (bool) 0
0x66: Pop(0); PushNull((bool) Stack[0 + Tasks[-1].StackPointer] == 0)
...
0x6c: EventDisable(7)
0x6d: Push("started") // @poff=87
0x6e: @ Trace(Stack[-1])
0x6f: Pop(1)
0x70: Push("envsound") // @poff=103
0x71: @ WaitFor3DSoundToLoad(Stack[-1])
0x72: Pop(1)
0x73: Push("envsound") // @poff=103
0x74: Push([0.0, 0.0, 0.0])
0x75: Push((float)0.0)
0x76: @ PlayLoopedGlobalSound(Stack[-0]T, Stack[-3], Stack[-2], Stack[-1], Stack[-2]T)
0x77: Pop(3)
0x78: Pop(0); PushNull((bool) Stack[0 + Tasks[-1].StackPointer] == 0)
0x79: IF (Stack[-1] == 0) GOTO 0x7e; Pop(1)

0x7a: Push("Sound was not found") // @poff=121
0x7b: @ Trace(Stack[-1])
0x7c: Pop(1)
0x7d: GOTO 0x81

0x7e: Push((float)0.5)
0x7f: @@@ FadeIn(Stack[-1]); Obj=0 // @poff=161
0x80: Pop(1)
0x81: EventEnable(7)
0x82: Return(); Pop(10)
...
0x97: @@ GetPosition(Stack[-1]); Obj=3 // @poff=168
0x98: Pop(0)
0x99: Stack[-1] = Stack[-4]
0x9a: Return(); Pop(2)

0x9b: PushEmpty(object, object)
0x9c: Push("player") // @poff=52
...
0xa3: Push("HasProperty") // @poff=180
0xa4: Push((int) 2)
...
0xaa: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=204
0xab: Pop(0)
0xac: Stack[-1] = Stack[-5]
0xad: Return(); Pop(2)
