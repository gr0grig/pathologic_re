...
Strings:
	A:BlockPolygons
	A:UnblockPolygons
// @pool_raw:426c6f636b506f6c79676f6e7300556e626c6f636b506f6c79676f6e7300
// @pool_encoding:utf8
...
0x11: Pop(0)
0x12: IF (Stack[-1] == 1) GOTO 0xd; Pop(1)
...
0x21: Push((int) 4)
0x22: Pop(1); Push(Stack[0 + Tasks[-1].StackPointer] / Stack[-1]);
0x23: Push((int) 256)
...
0x34: Pop(0)
0x35: Push((int) 255)
0x36: @@ BlockPolygons(Stack[-0]T, Stack[-1]); Obj=2 // @poff=0
0x37: Pop(1)
0x38: Push((bool) 1)
...
0x45: Pop(1)
0x46: Push((int) 255)
0x47: @@ UnblockPolygons(Stack[-0]T, Stack[-1]); Obj=2 // @poff=14
0x48: Pop(1)
0x49: Stack[-1] = 0
0x4a: GOTO 0x50
