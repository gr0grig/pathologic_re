...
Strings:
	...
	W:cleanup
	W:restore
// @pool_raw:47657445796573486569676874004c004f004f004b0020004100530059004e004300000063006c00650061006e0075007000000072006500730074006f00720065000000
// @pool_encoding:utf8
...
0x4: PushEmpty(float, cvector, float, cvector)
0x5: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=0
0x6: Pop(0)
0x7: Stack[-1] = [0.0, 0.0, 0.0]
0x8: Push(CvectorIndex(Stack[-1], 1))
0x9: Stack[-3] = Stack[-1]
0xa: CvectorIndex(Stack[-2], 1) = Stack[-1];
0xb: @ lshLookAsync(Stack[-5], Stack[-1])
0xc: Pop(0)
0xd: Push("LOOK ASYNC") // @poff=14
0xe: @ Trace(Stack[-1])
0xf: Pop(1)
0x10: Return(); Pop(4)
