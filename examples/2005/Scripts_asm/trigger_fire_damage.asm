...
Strings:
	...
	W:armor_
	A:SetProperty
// @pool_raw:
// @pool_encoding:utf8
...
0x11: @@ get(Stack[-3], Stack[-4]); Obj=1 // @poff=5
0x12: Pop(1)
0x13: Pop(0); Push((bool) Stack[-2] == 0)
0x14: IF (Stack[-1] == 0) GOTO 0x19; Pop(1)

0x15: Push(GlobalVars[0])
0x16: @@ remove(Stack[-4]); Obj=1 // @poff=9
0x17: Pop(1)
0x18: GOTO 0x2d

0x19: Push("Damaging: ") // @poff=16
0x1a: Pop(1); Push(Stack[-1] + Stack[-3]);
0x1b: @ Trace(Stack[-1])
0x1c: Pop(1)
0x1d: PushEmpty(float, object, float, int)
0x1e: Stack[-6] = Stack[-3]
0x1f: Stack[-2] = (float) 0.800000011920929
0x20: Stack[-1] = (int) 2
0x21: Call2 0xa9

0x22: Stack[-4] = Stack[-5]
0x23: Pop(4)
...
0x26: Pop(0)
0x27: Push((int) 4)
0x28: Push((float)0.800000011920929)
...
0x57: Push("Removed damage target: ") // @poff=96
...
0x94: Stack[-2] = "bullet" // @poff=186
0x95: Return(); Pop(0)

0x96: Stack[-2] = "phys" // @poff=200
0x97: Return(); Pop(0)
...
0x9e: Push("HasProperty") // @poff=210
0x9f: Push((int) 2)
...
0xa5: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=234
0xa6: Pop(0)
0xa7: Stack[-1] = Stack[-5]
0xa8: Return(); Pop(2)
...
0xb4: Stack[-18] = Stack[-2]
0xb5: Stack[-1] = "armor" // @poff=246
0xb6: Call2 0x9d
...
0xbc: Push("armor") // @poff=246
0xbd: @@ GetProperty(Stack[-1], Stack[-7]); Obj=16 // @poff=258
0xbe: Pop(1)
0xbf: Push("armor_") // @poff=270
0xc0: PushEmpty(string, int)
0xc1: Stack[-16] = Stack[-1]
0xc2: Call2 0x8a

0xc3: Pop(1)
0xc4: Stack[-7] = Stack[-2] + Stack[-1]; Pop(2);
0xc5: PushEmpty(bool, object, string)
0xc6: Stack[-18] = Stack[-2]
...
0xce: @@ GetProperty(Stack[-5], Stack[-4]); Obj=15 // @poff=258
0xcf: Pop(0)
...
0xd6: Stack[-3] = Stack[-6]
0xd7: Pop(3)
0xd8: Push("health") // @poff=38
0xd9: @@ GetProperty(Stack[-1], Stack[-3]); Obj=16 // @poff=258
0xda: Pop(1)
0xdb: Push((int) 1)
0xdc: Pop(1); Push(Stack[-1] - Stack[-4]);
0xdd: Stack[-2] = Stack[-15] * Stack[-1]; Pop(1);
0xde: Push("health") // @poff=38
...
0xe4: Pop(3)
0xe5: @@ SetProperty(Stack[-2], Stack[-1]); Obj=17 // @poff=284
0xe6: Pop(2)
0xe7: PushEmpty(bool, object)
0xe8: Stack[-17] = Stack[-1]
0xe9: Call2 0x98
...
0xef: Pop(1)
0xf0: Stack[-1] = Stack[-16]
0xf1: Return(); Pop(12)

0xf2: PushEmpty(object, object)
0xf3: @ self(Stack[-1])
0xf4: Pop(0)
0xf5: Stack[-1] = Stack[-3]
0xf6: Return(); Pop(2)
...
0x112: Pop(0)
0x113: @@ add(Stack[-3]); Obj=1 // @poff=92
0x114: Pop(0)
0x115: Push((int) 15)
0x116: @ SendWorldWndMessage(Stack[-1], Stack[-2])
0x117: Pop(1)
0x118: Return(); Pop(2)
