...
Strings:
	...
	A:GetLocator
	W:locked
// @pool_raw:40000000570072006f006e006700200064006f006f00720020006e0061006d00650000004e006f0020006d00610069006e0020006f007500740064006f006f00720020007300630065006e00650000004765744e616d6500700074005f0000004765744c6f6361746f72006c006f0063006b00650064000000
...
0x12: Stack[-12] = Stack[-2]
0x13: Stack[-1] = "@" // @poff=0
0x14: Call2 0x40

0x15: Stack[-3] = Stack[-11]
0x16: Pop(3)
0x17: Push((int) -1)
0x18: Pop(1); Push((bool) Stack[-9] == Stack[-1])
0x19: IF (Stack[-1] == 0) GOTO 0x1e; Pop(1)

0x1a: Push("Wrong door name") // @poff=4
0x1b: @ Trace(Stack[-1])
0x1c: Pop(1)
0x1d: Return(); Pop(20)
...
0x1f: Pop(0)
0x20: Pop(0); PushNull((bool) Stack[-7] == 0)
0x21: IF (Stack[-1] == 0) GOTO 0x26; Pop(1)

0x22: Push("No main outdoor scene") // @poff=36
0x23: @ Trace(Stack[-1])
0x24: Pop(1)
0x25: Return(); Pop(20)
...
0x35: @ Trace(Stack[-4])
0x36: Pop(0)
0x37: @@ GetLocator(Stack[-4], Stack[-3], Stack[-2], Stack[-1]); Obj=7 // @poff=96
0x38: Pop(0)
0x39: Push(Stack[-3])
