...
Strings:
	...
	W:branch
// @pool_raw:
// @pool_encoding:utf8
...
0x1a0: Stack[-1] = "cleanup" // @poff=156
0x1a1: Call2 0x1a4
...
0x1b2: Push("tutorial_light_2") // @poff=172
0x1b3: @ FindActor(Stack[-2], Stack[-1])
0x1b4: Pop(1)
0x1b5: @@ Switch(Stack[-3]); Obj=1 // @poff=206
0x1b6: Pop(0)
0x1b7: Return(); Pop(2)
...
0x25d: @@ GetPosition(Stack[-8]); Obj=20 // @poff=213
0x25e: Pop(0)
0x25f: @@ GetEyesHeight(Stack[-9]); Obj=20 // @poff=225
0x260: Pop(0)
...
0x285: @ StopWorld()
0x286: Pop(0)
0x287: @ CameraTransit(Stack[-3], Stack[-5])
0x288: Pop(0)
0x289: Push(CvectorIndex(Stack[-4], 0))
0x28a: Push(CvectorIndex(Stack[-5], 2))
0x28b: @ Rotate(Stack[-2], Stack[-1])
0x28c: Pop(2)
...
0x292: Push("head") // @poff=239
0x293: @ HasAnimationTrack(Stack[-2], Stack[-1])
0x294: Pop(1)
0x295: Push(Stack[-1])
0x296: IF (Stack[-1] == 0) GOTO 0x29a; Pop(1)

0x297: Push("head") // @poff=239
0x298: @ LookAsyncCamera(Stack[-1])
0x299: Pop(1)
0x29a: @ CameraWaitForPlayFinish()
0x29b: Pop(0)
0x29c: @ ResumeWorld()
0x29d: Pop(0)
0x29e: Stack[-21] = (bool) 1
0x29f: Return(); Pop(18)

0x2a0: PushEmpty(bool, bool)
0x2a1: @ CameraSwitchToNormal()
0x2a2: Pop(0)
...
0x2a8: Push("head") // @poff=239
0x2a9: @ HasAnimationTrack(Stack[-2], Stack[-1])
0x2aa: Pop(1)
0x2ab: Push(Stack[-1])
0x2ac: IF (Stack[-1] == 0) GOTO 0x2b0; Pop(1)

0x2ad: Push("head") // @poff=239
0x2ae: @ UnlookAsync(Stack[-1])
0x2af: Pop(1)
0x2b0: Return(); Pop(2)

0x2b1: PushEmpty(float, float, float, float)
0x2b2: @ lshGetAnimTimes(Stack[-5], Stack[-2], Stack[-1])
0x2b3: Pop(0)
0x2b4: Push((bool) 0)
...
0x2b8: PushEmpty(float, float, float, float)
0x2b9: @ lshGetAnimTimes(Stack[-6], Stack[-2], Stack[-1])
0x2ba: Pop(0)
0x2bb: @ lshPlayAnimation(Stack[-2], Stack[-1], Stack[-5])
0x2bc: Pop(0)
0x2bd: Return(); Pop(4)
...
0x2ce: Push((float)9.999999974752427e-07)
0x2cf: Pop(1); Push((bool) Stack[-2] < Stack[-1])
0x2d0: IF (Stack[-1] == 0) GOTO 0x2d3; Pop(1)

0x2d1: Stack[-4] = [0.0, 0.0, 0.0]
0x2d2: Return(); Pop(2)

0x2d3: Stack[-4] = Stack[-3] / Stack[-1]; Pop(0);
0x2d4: Return(); Pop(2)

0x2d5: PushEmpty(string, string)
0x2d6: Stack[-1] = "idle" // @poff=146
0x2d7: Push(Stack[-3])
0x2d8: IF (Stack[-1] == 0) GOTO 0x2da; Pop(1)

0x2d9: Stack[-1] = Stack[-1] + Stack[-3]; Pop(0);
0x2da: Stack[-1] = Stack[-4]
0x2db: Return(); Pop(2)

0x2dc: PushEmpty(int, bool, int, bool)
0x2dd: Stack[-2] = (int) 0
0x2de: Push("all") // @poff=138
0x2df: PushEmpty(string, int)
0x2e0: Stack[-5] = Stack[-1]
0x2e1: Call2 0x2d5

0x2e2: Pop(1)
0x2e3: @ HasAnimation(Stack[-3], Stack[-2], Stack[-1])
0x2e4: Pop(2)
...
0x2eb: Stack[-2] = Stack[-5]
0x2ec: Return(); Pop(4)

0x2ed: Stack[-1] = (int) 515569
0x2ee: Return(); Pop(0)

0x2ef: Stack[-1] = (int) 503354
0x2f0: Return(); Pop(0)

0x2f1: Stack[-1] = "ui/NPC_wmask.png" // @poff=249
0x2f2: Return(); Pop(0)

0x2f3: Stack[-1] = "ui/NPC_wmask_b.png" // @poff=283
0x2f4: Return(); Pop(0)
