...
Strings:
	...
	W:button_plus
	W:button_minus
// @pool_raw:
...
0x6: Pop(0)
0x7: Push((int) 100)
0x8: Pop(1); Push(Stack[-1] * Stack[2 + Tasks[-1].StackPointer]);
0x9: Push((float)6.0)
0xa: Pop(2); Push(Stack[-2] / Stack[-1]);
0xb: Push("dream_progress") // @poff=0
0xc: @ SendMessage(Stack[-2], Stack[-1])
0xd: Pop(2)
0xe: Push((int) 701)
0xf: Push("health_text") // @poff=30
0x10: @ SendMessage(Stack[-2], Stack[-1])
0x11: Pop(2)
0x12: Push((int) 702)
0x13: Push("immunity_text") // @poff=54
0x14: @ SendMessage(Stack[-2], Stack[-1])
0x15: Pop(2)
0x16: Push((int) 703)
0x17: Push("hunger_text") // @poff=82
0x18: @ SendMessage(Stack[-2], Stack[-1])
0x19: Pop(2)
0x1a: Push((int) 704)
0x1b: Push("tiredness_text") // @poff=106
0x1c: @ SendMessage(Stack[-2], Stack[-1])
0x1d: Pop(2)
0x1e: Push((int) 705)
0x1f: Push("infection_text") // @poff=136
0x20: @ SendMessage(Stack[-2], Stack[-1])
0x21: Pop(2)
0x22: @ ShowCursor()
0x23: Pop(0)
0x24: Push("default") // @poff=166
...
0x34: Pop(0)
0x35: Push("sleeping") // @poff=182
0x36: Push((bool) 0)
0x37: @@ SetProperty(Stack[-2], Stack[-1]); Obj=4 // @poff=200
0x38: Pop(2)
0x39: @ CreateFloatVector(Stack[-1])
0x3a: Pop(0)
0x3b: Push((int) 0)
0x3c: @@ add(Stack[-1]); Obj=2 // @poff=212
0x3d: Pop(1)
0x3e: Push((int) 11)
...
0x55: Pop(0)
0x56: Push("sleeping") // @poff=182
0x57: Push((bool) 1)
0x58: @@ SetProperty(Stack[-2], Stack[-1]); Obj=3 // @poff=200
0x59: Pop(2)
...
0x9e: Pop(1); Push(Stack[-1] * Stack[1 + Tasks[-1].StackPointer]);
...
0xa9: Pop(1); Push(Stack[-1] * Stack[1 + Tasks[-1].StackPointer]);
0xaa: Pop(1); Push(Stack[-1] * Stack[-4]);
0xab: Pop(1); Push(Stack[2 + Tasks[-1].StackPointer] - Stack[-1]);
0xac: Pop(2); Push(Stack[-2] * Stack[-1]);
0xad: Push((float)6.0)
0xae: Stack[-3] = Stack[-2] / Stack[-1]; Pop(2);
0xaf: Push("dream_progress") // @poff=0
...
0xbd: Push("dream_button") // @poff=302
...
0xcd: Push("button_plus") // @poff=328
...
0xde: Push((int) 100)
0xdf: Pop(1); Push(Stack[-1] * Stack[2 + Tasks[-1].StackPointer]);
0xe0: Push((float)6.0)
0xe1: Pop(2); Push(Stack[-2] / Stack[-1]);
0xe2: Push("dream_progress") // @poff=0
0xe3: @ SendMessage(Stack[-2], Stack[-1])
0xe4: Pop(2)
0xe5: GOTO 0xfe

0xe6: Push("button_minus") // @poff=352
...
0xf7: Push((int) 100)
0xf8: Pop(1); Push(Stack[-1] * Stack[2 + Tasks[-1].StackPointer]);
0xf9: Push((float)6.0)
0xfa: Pop(2); Push(Stack[-2] / Stack[-1]);
0xfb: Push("dream_progress") // @poff=0
