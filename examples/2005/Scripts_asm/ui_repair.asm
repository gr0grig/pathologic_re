...
Strings:
	...
	W:money
	A:GetProperty
// @pool_raw:
...
0xd: Pop(1)
0xe: @ GetChooseItems(Stack[-0]T)
0xf: Pop(0)
0x10: @ GetAdditionalData(Stack[-1]T)
0x11: Pop(0)
0x12: @ GetReturnValue(Stack[-2]T)
0x13: Pop(0)
0x14: PushEmpty()
0x15: Call2 0x66
...
0x1e: @@@ size(Stack[-3]); Obj=2 // @poff=16
0x1f: Pop(0)
0x20: Stack[-2] = (int) 0
0x21: Pop(0); Push((bool) Stack[-2] < Stack[-3])
0x22: IF (Stack[-1] == 0) GOTO 0x2c; Pop(1)

0x23: @@@ get(Stack[-1], Stack[-2]); Obj=2 // @poff=21
0x24: Pop(0)
...
0x35: @@@ get(Stack[-1], Stack[-3]); Obj=1 // @poff=21
0x36: Pop(0)
...
0x67: @@@ size(Stack[-2]); Obj=0 // @poff=16
0x68: Pop(0)
0x69: Push((int) 4)
...
0x86: Pop(0)
0x87: Push("money_slot") // @poff=62
0x88: @ SendMessage(Stack[-2], Stack[-1])
0x89: Pop(2)
0x8a: Push("sel_money_slot") // @poff=84
0x8b: @ SendMessage(Stack[-4]T, Stack[-1])
0x8c: Pop(1)
0x8d: @@@ size(Stack[-4]); Obj=0 // @poff=16
0x8e: Pop(0)
0x8f: Stack[-3] = Stack[5 + Tasks[-1].StackPointer]
0x90: Push((int) 4)
0x91: Pop(1); Push(Stack[5 + Tasks[-1].StackPointer] + Stack[-1]);
...
0x96: @@@ get(Stack[-2], Stack[-3]); Obj=0 // @poff=21
0x97: Pop(0)
0x98: @@@ get(Stack[-1], Stack[-3]); Obj=1 // @poff=21
0x99: Pop(0)
0x9a: PushEmpty(bool, int)
0x9b: Stack[-5] = Stack[-1]
0x9c: Call2 0x1d
...
0xca: Push("scrollbar") // @poff=42
0xcb: Pop(1); Push((bool) Stack[-7] == Stack[-1])
0xcc: IF (Stack[-1] == 0) GOTO 0xe1; Pop(1)

0xcd: @@@ size(Stack[-2]); Obj=0 // @poff=16
0xce: Pop(0)
0xcf: Push((float)0.009999999776482582)
0xd0: Push((int) 4)
...
0xe2: Stack[-10] = Stack[-2]
0xe3: Stack[-9] = Stack[-1]
0xe4: Call2 0x12a

0xe5: Stack[-3] = Stack[-4]
0xe6: Pop(3)
0xe7: Push((int) -1)
...
0x13d: Push("player") // @poff=146
0x13e: @ FindActor(Stack[-3], Stack[-1])
0x13f: Pop(1)
0x140: Push("money") // @poff=160
0x141: @@ GetProperty(Stack[-1], Stack[-2]); Obj=3 // @poff=172
0x142: Pop(1)
0x143: Stack[-1] = Stack[-5]
0x144: Return(); Pop(4)
