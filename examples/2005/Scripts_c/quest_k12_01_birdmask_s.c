// @GLOBALS: 0:object:

task task_0
{
}


task task_1
{
	// @pe
	void event_11(object var_0_object, object var_1_object, object var_2_object, string var_3_string, bool var_4_bool, int var_5_int, int var_6_int, int var_7_int, int var_8_int, bool var_9_bool, bool var_10_bool, object var_11_object)
	{
		if(1 != 0) {
			func_643();
			if(var_11_object == 41167) {
				object var_16_object; object var_17_object;
				var_16_object = var_1_object;
				var_17_object = var_0_object;
				func_749();
				object var_28_object; object var_29_object;
				var_28_object = var_1_object;
				var_29_object = var_0_object;
				func_736();
			}
			if(var_10_bool == 41164) {
				bool var_38_bool;
				func_756(var_38_bool, var_1_object);
				if(!var_38_bool) { //@nz
					func_194(var_11_object, "Neutral");
					var_0_object->SetMessage(539221); //@t
					var_0_object->ClearReplies(); //@t
					var_0_object->AddReply(539222, -1, 41165); //@t
					return 0;
				}
				bool var_103_bool;
				func_766(var_103_bool, var_1_object);
				if(!var_103_bool) { //@nz
					func_194(var_11_object, "Neutral");
					var_0_object->SetMessage(539219); //@t
					var_0_object->ClearReplies(); //@t
					var_0_object->AddReply(539220, -1, 41163); //@t
					return 0;
				}
				bool var_120_bool;
				func_776(var_1_object);
				if(var_120_bool != 0) {
					object var_128_object; object var_129_object;
					var_128_object = var_1_object;
					var_129_object = var_0_object;
					func_743();
					func_194(var_11_object, "Neutral");
					var_0_object->SetMessage(539223); //@t
					var_0_object->ClearReplies(); //@t
					var_0_object->AddReply(539224, -1, 41167); //@t
					return 0;
				}
				func_194(var_11_object, "Neutral");
				var_0_object->SetMessage(539225); //@t
				var_0_object->ClearReplies(); //@t
				var_0_object->AddReply(539226, -1, 41169); //@t
				return 0;
			}
			var_3_string = true;
			bool var_142_bool;
			func_734(var_142_bool);
			if(var_142_bool != 0)
				@lshStopAnimation();
			else
				@StopAnimation();
			return 0;
		}
		return 0;
	EMIT "GOTO 0xd9";
	
	}

}


maintask task_2
{
	void init(object var_0_object, object var_1_object, object var_2_object, string var_3_string, bool var_4_bool, int var_5_int, int var_6_int, bool var_7_bool, bool var_8_bool, object var_9_object)
	{
	
		while(true != 0) {
			func_356(var_5_int, var_6_int, var_7_bool, var_8_bool, var_9_object, (float)300, (float)100);
			@Sleep(1);
		}
	
	}

	// @pe
	void OnUse(object actor, object var_1_object, object var_2_object, string var_3_string, bool var_4_bool, int var_5_int, int var_6_int, bool var_7_bool, bool var_8_bool, object var_9_object, object var_10_object)
	{
		func_421(var_9_object, var_10_object);
		int var_14_int; object var_15_object;
		var_10_object = var_15_object;
		TaskCall(0);
		func_0(var_16_object, var_14_int, var_15_object);
		TaskReturn();
	}

	// @pe
	void OnTimer(object var_0_object, object var_1_object, object var_2_object, string var_3_string, bool var_4_bool, int var_5_int, int var_6_int, bool var_7_bool, bool var_8_bool, object var_9_object, int var_10_int)
	{
		if(var_10_int == 10) {
			bool var_13_bool;
			func_383(var_8_bool, var_9_object, var_10_int, var_13_bool);
			if(var_13_bool != 0) {
				if(!var_2_object) { //@nz
					func_632(var_4_bool);
					var_2_object = true;
				}
			} else if(var_2_object != 0) {
				@UnlookAsync("head");
				var_2_object = false;
			}
		}
	
	}

}


void func_0(object var_0_object, int var_14_int, object var_15_object)
{
	var_0_object = var_15_object;
	bool var_25_bool; object var_26_object;
	var_15_object = var_26_object;
	func_534(var_25_bool, var_26_object, 130.0);
	if(!var_25_bool) { //@nz
		var_14_int = -2;
		return 8;
	}
	object var_21_object;
	@CreateDialog(var_21_object);
	int var_72_int;
	func_728(var_72_int);
	var_21_object->SetNPCName(var_72_int);
	int var_73_int;
	func_726(var_73_int);
	var_21_object->SetNPCDescription(var_73_int);
	string var_74_string;
	func_730(var_74_string);
	var_21_object->SetPhoto(var_74_string);
	string var_75_string;
	func_732(var_75_string);
	var_21_object->SetPhoto2(var_75_string);
	int var_76_int;
	func_805(var_76_int);
	var_21_object->SetPlayerName(var_76_int);
	bool var_22_bool;
	@IsOverrideActive(var_22_bool);
	if(var_22_bool != 0) {
		var_14_int = -2;
		return 8;
	}
	@DoDialog(var_21_object);
	object var_85_object; object var_86_object;
	var_15_object = var_85_object;
	var_21_object = var_86_object;
	TaskCall(1);
	func_74(var_87_object, var_88_object, var_89_string, var_90_bool, var_85_object, var_86_object);
	TaskReturn();
	bool var_24_bool;
	var_21_object->IsDialogEnd(var_24_bool);
	
	for(;;) {
		var_209_bool = !var_24_bool; //@nz
		if(var_209_bool == 0) goto Label_63;
		@sync();
		var_21_object->IsDialogEnd(var_24_bool);
	}
	
Label_63:
	object var_210_object;
	var_15_object = var_210_object;
	func_602();
	@StopDialog(var_21_object);
	var_21_object->GetReturnValue(-1);
	int var_23_int = var_14_int;
}
EMIT "Stack[-4] = 0";


void func_514(bool var_61_bool)
{
	var_61_bool = true;
}


void func_643(void)
{
	bool var_13_bool;
	func_734(var_13_bool);
	if(var_13_bool != 0)
		@lshStopSpeech();
}


void func_516(void)
{
	@StopAnimation();
	@StopGroup0();
}


// @pe
void func_776(bool var_187_bool)
{
	int var_189_int;
	func_660(var_189_int, "ook12BirdmaskS1");
	if(var_189_int == 0) {
		var_187_bool = true;
		return 0;
	}
	var_187_bool = false;
}


void func_521(float var_17_float, object var_18_object)
{
	cvector var_22_cvector;
	@GetPosition(var_22_cvector);
	cvector var_23_cvector;
	var_18_object->GetPosition(var_23_cvector);
	var_17_float = (var_23_cvector - var_22_cvector) | (var_23_cvector - var_22_cvector);
}


void func_650(cvector var_52_cvector, cvector var_53_cvector)
{
	float var_56_float = sqrt(var_53_cvector | var_53_cvector);
	if(var_56_float < 0.000001)
		var_52_cvector = [0.0, 0.0, 0.0];
	var_52_cvector = var_53_cvector / var_56_float;
}


void func_529(bool var_13_bool)
{
	bool var_15_bool;
	@IsLoaded(var_15_bool);
	var_15_bool = var_13_bool;
}


// @pe
void func_788(bool var_94_bool)
{
	bool var_96_bool;
	func_874(var_96_bool);
	var_96_bool = var_94_bool;
}


void func_660(int var_189_int, string var_190_string)
{
	int var_192_int;
	@GetVariable(var_190_string, var_192_int);
	var_192_int = var_189_int;
}


void func_534(bool var_25_bool, object var_26_object, float var_27_float)
{
	cvector var_38_cvector; bool var_45_bool;
	var_26_object->GetPosition(var_38_cvector);
	float var_37_float;
	var_26_object->GetEyesHeight(var_37_float);
	var_46_float = GetByIndex(var_38_cvector, 1);
	SetByIndex(var_38_cvector, 1) = (var_46_float + var_37_float);
	cvector var_39_cvector;
	@GetPosition(var_39_cvector);
	@GetEyesHeight(var_37_float);
	var_47_float = GetByIndex(var_39_cvector, 1);
	SetByIndex(var_39_cvector, 1) = (var_47_float + var_37_float);
	cvector var_40_cvector = var_38_cvector - var_39_cvector;
	var_48_float = GetByIndex(var_40_cvector, 1);
	SetByIndex(var_40_cvector, 1) = (float)0;
	var_50_float = sqrt(var_40_cvector | var_40_cvector);
	var_40_cvector /= var_50_float;
	cvector var_41_cvector = -var_40_cvector;
	cvector var_52_cvector;
	func_650(var_52_cvector, (var_41_cvector ^ [0.0, 1.0, 0.0]));
	cvector var_42_cvector = ((var_40_cvector * var_27_float) + (var_52_cvector * 25)) - [0.0, 10.0, 0.0];
	bool var_44_bool;
	@IsOverrideActive(var_44_bool);
	if(var_44_bool != 0)
		var_25_bool = false;
	@StopWorld();
	@CameraTransit((var_39_cvector + var_42_cvector), var_41_cvector);
	var_65_float = GetByIndex(var_42_cvector, 0);
	var_66_float = GetByIndex(var_42_cvector, 2);
	@Rotate(var_65_float, var_66_float);
	bool var_67_bool;
	func_734(var_67_bool);
	if(var_67_bool != 0) {
	} else {
		@HasAnimationTrack(var_45_bool, "head");
		if(var_45_bool == 0) goto Label_596;
		@LookAsyncCamera("head");
	}
Label_596:
	@CameraWaitForPlayFinish();
	@ResumeWorld();
	var_25_bool = true;
	
}


void func_665(string var_18_string, bool var_19_bool)
{
	object var_21_object;
	@FindActor(var_21_object, var_18_string);
	if(!var_21_object) //@nz
		@Trace(("Door " + var_18_string) + " not found");
	else
		var_21_object->SetProperty("locked", var_19_bool);
	
}
EMIT "Stack[-1] = 0";


// @pe
void func_794(bool var_172_bool)
{
	int var_174_int;
	func_694(var_174_int);
	if(var_174_int >= 19)
		var_172_bool = true;
	var_172_bool = false;
}


void func_805(int var_76_int)
{
	int var_78_int;
	@GetVariable("branch", var_78_int);
	if(var_78_int == 0) {
		var_76_int = 1;
		return 2;
	EMIT "GOTO 0x334";
	}
	if(var_78_int == 1) {
		var_76_int = 2;
		return 2;
	}
	var_76_int = 3;
}


// @pe
void func_421(object var_2_object, string var_3_string)
{
	func_516();
	@KillTimer(10);
	if(var_2_object != 0) {
		@UnlookAsync("head");
		var_2_object = false;
	}
	var_3_string = true;
}


void func_682(bool var_30_bool, string var_31_string, string var_32_string)
{
	object var_34_object;
	@FindActor(var_34_object, var_31_string);
	if(var_34_object == null)
		var_30_bool = false;
	@Trigger(var_34_object, var_32_string);
	var_30_bool = true;
}
EMIT "Stack[-1] = 0";


void func_435(void)
{
	int var_29_int; int var_30_int; bool var_31_bool; float var_32_float; bool var_33_bool;
	@WaitForAnimEnd();
	bool var_34_bool;
	func_529(var_34_bool);
	if(!var_34_bool) //@nz
		return 14;
	int var_36_int;
	func_709(var_36_int);
	int var_27_int;
	var_36_int = var_27_int;
	int var_28_int = 0;
	
	for(;;) {
		bool var_49_bool = false;
		if(var_28_int < 5) {
			bool var_52_bool;
			func_529(var_52_bool);
			if(var_52_bool != 0)
				var_49_bool = true;
		}
		if(var_49_bool != 0) {
			@irand(var_29_int, 3);
			if(var_29_int == 0) {
				if(var_27_int == 0) goto Label_482;
				@irand(var_30_int, var_27_int);
				string var_58_string; int var_59_int;
				var_30_int = var_59_int;
				func_702(var_58_string, var_59_int);
				@PlayAnimation("all", var_58_string);
				@WaitForAnimEnd(var_31_bool);
				if(!var_31_bool) { //@nz
				} else {
			} else {
			if(var_29_int == 1) {
				@rand(var_32_float, 4);
				@Sleep((var_32_float + 1), var_33_bool);
				if(!var_33_bool) { //@nz
					goto Label_511;
				}
			} else if(var_28_int != 0) {
				goto Label_511;
			}
			}
					bool var_61_bool;
					func_514(var_61_bool);
					var_62_bool = !var_61_bool; //@nz
					if(var_62_bool == 0) goto Label_506;
			}
		}
	Label_511:
		for(;;) {
			@ResetAAS();
			return 14;

		}

	Label_506:
		@ResetAAS();
		var_28_int += 1;
	}
	
}


void func_694(int var_174_int)
{
	float var_176_float;
	@GetGameTime(var_176_float);
	int var_177_int;
	var_176_float = var_177_int;
	var_174_int = var_177_int % 24;
}


// @pe
void func_822(object var_105_object, bool var_106_bool)
{
	var_105_object->add(7);
	var_105_object->add(23);
	var_105_object->add(21);
	var_105_object->add(9);
	var_105_object->add(3);
	var_105_object->add(1);
	var_105_object->add(11);
	var_105_object->add(13);
	if(var_106_bool == false)
		var_105_object->add(25);
}


void func_702(string var_42_string, int var_43_int)
{
	string var_45_string = "idle";
	if(var_43_int != 0)
		var_45_string += var_43_int;
	var_45_string = var_42_string;
}


// @pe
void func_194(object var_2_object, string var_141_string)
{
	bool var_142_bool;
	func_734(var_142_bool);
	if(!var_142_bool) //@nz
		return 0;
	if(var_141_string == var_2_object)
		return 0;
	string var_145_string; bool var_146_bool;
	var_141_string = var_145_string;
	if(var_141_string == "")
		var_146_bool = false;
	else
		var_146_bool = true;
	func_626(var_145_string, var_146_bool);
	var_2_object = var_141_string;
	
}


void func_709(int var_36_int)
{
	int var_39_int; bool var_40_bool;
	var_39_int = 0;
	
	for(;;) {
		string var_42_string; int var_43_int;
		var_39_int = var_43_int;
		func_702(var_42_string, var_43_int);
		@HasAnimation(var_40_bool, "all", var_42_string);
		if(!var_40_bool) //@nz
			break;
		var_39_int += 1;
	}
	var_39_int = var_36_int;
}


// @pe
void func_74(object var_0_object, object var_1_object, object var_2_object, string var_3_string, object var_85_object, object var_86_object)
{
	var_0_object = var_86_object;
	var_1_object = var_85_object;
	var_3_string = false;
	if(1 != 0) {
		bool var_92_bool;
		func_756(var_92_bool, var_1_object);
		if(!var_92_bool) { //@nz
			func_194(var_86_object, "Neutral");
			var_0_object->SetMessage(539221); //@t
			var_0_object->ClearReplies(); //@t
			var_0_object->AddReply(539222, -1, 41165); //@t
		} else {
					bool var_170_bool;
					func_766(var_170_bool, var_1_object);
					var_181_bool = !var_170_bool; //@nz
					if(var_181_bool == 0) goto Label_122;
					func_194(var_86_object, "Neutral");
					var_0_object->SetMessage(539219); //@t
					var_0_object->ClearReplies(); //@t
					var_0_object->AddReply(539220, -1, 41163); //@t
		}
	}
Label_164:
	for(;;) {
		bool var_157_bool;
		func_734(var_157_bool);
		if(var_157_bool != 0) {

			for(;;) {
				@lshWaitForAnimEnd();
				if(var_3_string != 0) {
				} else {
					func_619(var_2_object);
				}
		} else {
			@PlayAnimation("all", "idle");

			for(;;) {
				@WaitForAnimEnd();
				if(var_3_string != 0) {
					goto Label_193;
				}
				@PlayAnimation("all", "idle");
			}

		}
		Label_193:
			}
			bool var_187_bool;
			func_776(var_1_object);
			if(var_187_bool != 0) {
			object var_195_object; object var_196_object;
			var_195_object = var_1_object;
			var_196_object = var_0_object;
			func_743();
			func_194(var_86_object, "Neutral");
			var_0_object->SetMessage(539223); //@t
			var_0_object->ClearReplies(); //@t
			var_0_object->AddReply(539224, -1, 41167); //@t
			goto Label_164;
		}
		return 0;
	}
	func_194(var_86_object, "Neutral");
	var_0_object->SetMessage(539225); //@t
	var_0_object->ClearReplies(); //@t
	var_0_object->AddReply(539226, -1, 41169); //@t
	goto Label_164;
}
EMIT "Return(); Pop(0)";
EMIT "GOTO 0x4e";


void func_726(int var_73_int)
{
	var_73_int = 515571;
}


void func_854(bool var_130_bool, int var_131_int)
{
	int var_133_int;
	@GetVariable(("vol_" + var_131_int), var_133_int);
	var_130_bool = (var_133_int & 4) != 0;
}


void func_728(int var_72_int)
{
	var_72_int = 504029;
}


void func_602(void)
{
	bool var_212_bool;
	@CameraSwitchToNormal();
	bool var_213_bool;
	func_734(var_213_bool);
	if(var_213_bool != 0) {
	} else {
		@HasAnimationTrack(var_212_bool, "head");
		if(var_212_bool == 0) goto Label_618;
		@UnlookAsync("head");
	}
Label_618:
	
}


void func_730(string var_74_string)
{
	var_74_string = "ui/NPC_bmask.png";
}


void func_732(string var_75_string)
{
	var_75_string = "ui/NPC_bmask_b.png";
}


void func_734(bool var_67_bool)
{
	var_67_bool = false;
}


// @pe
void func_736(void)
{
	bool var_30_bool;
	func_682(var_30_bool, "quest_k12_01", "init_sobor");
}


void func_864(bool var_121_bool, int var_122_int)
{
	int var_124_int;
	@GetVariable(("vol_" + var_122_int), var_124_int);
	var_121_bool = (var_124_int & 16) != 0;
}


// @pe
void func_356(object var_0_object, object var_1_object, object var_2_object, string var_3_string, bool var_4_bool, float var_11_float, float var_12_float)
{
	bool var_13_bool;
	func_529(var_13_bool);
	if(!var_13_bool) //@nz
		return 0;
	@FindActor(var_4_bool, "player");
	var_2_object = false;
	var_0_object = var_11_float;
	var_1_object = var_12_float;
	@SetTimer(10, 1.0);
	func_435();
	if(!false) //@nz
		@KillTimer(10);
}


// @pe
void func_743(void)
{
	@SetVariable("ook12BirdmaskS1", 1);
}


void func_874(bool var_96_bool)
{
	object var_101_object; int var_104_int;
	@CreateIntVector(var_101_object);
	object var_105_object;
	var_101_object = var_105_object;
	func_822(false, -1);
	int var_102_int;
	var_101_object->size(var_102_int);
	int var_103_int = 0;
	
	while(var_103_int < var_102_int) {
		var_101_object->get(var_104_int, var_103_int);
		bool var_120_bool = true;
		bool var_121_bool; int var_122_int;
		var_104_int = var_122_int;
		func_864(var_121_bool, var_122_int);
		if(var_121_bool != 1) {
			bool var_130_bool; int var_131_int;
			var_104_int = var_131_int;
			func_854(var_130_bool, var_131_int);
			if(var_130_bool != 1)
				var_120_bool = false;
		}
		if(var_120_bool != 0)
			var_96_bool = false;
		var_103_int += 1;
	}
	
	var_96_bool = true;
}
EMIT "Stack[-4] = 0";


void func_619(string var_159_string)
{
	float var_162_float; float var_163_float;
	@lshGetAnimTimes(var_159_string, var_162_float, var_163_float);
	@lshPlayAnimation(var_162_float, var_163_float, false);
}


// @pe
void func_749(void)
{
	func_665("sobor@door1", false);
}


void func_626(string var_145_string, bool var_146_bool)
{
	float var_151_float; float var_152_float;
	@lshGetAnimTimes(var_145_string, var_151_float, var_152_float);
	@lshPlayAnimation(var_151_float, var_152_float, var_146_bool);
}


// @pe
void func_756(bool var_92_bool, object var_93_object)
{
	object var_95_object;
	var_93_object = var_95_object;
	func_788(var_95_object);
	bool var_94_bool;
	if(var_94_bool != 0) {
		var_92_bool = true;
		return 0;
	}
	var_92_bool = false;
}


void func_632(object var_27_object)
{
	float var_30_float;
	var_27_object->GetEyesHeight(var_30_float);
	cvector var_31_cvector = [0.0, 0.0, 0.0];
	var_32_float = GetByIndex(var_31_cvector, 1);
	var_30_float = var_32_float;
	SetByIndex(var_31_cvector, 1) = var_32_float;
	@LookAsync(var_27_object, "head", var_31_cvector);
}


// @pe
void func_766(bool var_170_bool, object var_171_object)
{
	object var_173_object;
	var_171_object = var_173_object;
	func_794(var_173_object);
	bool var_172_bool;
	if(var_172_bool != 0) {
		var_170_bool = true;
		return 0;
	}
	var_170_bool = false;
}


void func_383(object var_0_object, object var_1_object, bool var_4_bool, bool var_13_bool)
{
	if(var_4_bool == null)
		var_13_bool = false;
	float var_17_float;
	func_521(var_17_float, var_4_bool);
	float var_15_float = sqrt(var_17_float);
	if(var_2_object != 0)
		var_15_float -= var_1_object;
	var_13_bool = var_15_float < var_0_object;
}


