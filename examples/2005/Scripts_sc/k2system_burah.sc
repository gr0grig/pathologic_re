event OnLoad 5;
event evt_11 11;
event OnTrigger 26;
event OnUnload 6;
event OnTimer 7;
event OnLSHAnimationEnd 45;
event OnUse 0;

var object g0;

task t0
{
	void init(void)
	{
		do {
			@Hold();
		} while (!f_439_a0_b());
	}

	void OnLoad(void)
	{
		@StopGroup0();
		@sync();
	}
}

task t1
{
	var object tv0;

	int init(object a0)
	{
		object L0;
		bool L1;
		int L2;
		bool L3;
		tv0 = a0;
		f_493_a1_b(f_540_a0_o());
		if (!f_43e_a2_b(a0, 70.0)) {
			return -2;
		}
		@CreateDialog(L0);
		L0->SetNPCName(f_57f_a0_i());
		L0->SetNPCDescription(f_57d_a0_i());
		L0->SetPhoto(f_581_a0_s());
		L0->SetPhoto2(f_583_a0_s());
		L0->SetPlayerName(f_625_a0_i());
		L2 = -1;
		@IsOverrideActive(L1);
		if (L1) {
			return -2;
		}
		@DoDialog(L0);
		t2{a0, L0};
		L0->IsDialogEnd(L3);
		while (!L3) {
			@sync();
			L0->IsDialogEnd(L3);
		}
		f_482_a1_v(a0);
		@StopDialog(L0);
		L0->GetReturnValue(L2);
		return L2;
	}
}

task t2
{
	var object tv0;
	var object tv1;
	var string tv2;
	var bool tv3;

	void init(object a0, object a1)
	{
		tv0 = a1;
		tv1 = a0;
		tv3 = false;
		while (1) {
			f_98_a1_v("Neutral");
			tv0->SetMessage(539060);
			tv0->ClearReplies();
			tv0->AddReply(539061, 43176, 41009);
			tv0->AddReply(541070, -1, 43175);
			break;
			return;
		}
		if (f_585_a0_b()) {
			for (; ; ) {
				@lshWaitForAnimEnd();
				if (tv3) {
					break;
				}
				f_51d_a1_v(tv2);
			}
		} else {
			@PlayAnimation("all", "idle");
			for (; ; ) {
				@WaitForAnimEnd();
				if (tv3) {
					break;
				}
				@PlayAnimation("all", "idle");
			}
		}
	}

	void f_98_a1_v(string a0)
	{
		if (!f_585_a0_b()) {
			return;
		}
		if (a0 == tv2) {
			return;
		}
		f_524_a2_v(a0, a0 == "" ? false : true);
		tv2 = a0;
	}

	void evt_11(int a0, int a1)
	{
		while (1) {
			f_539_a0_v();
			if (a0 == 41008) {
				f_98_a1_v("Neutral");
				tv0->SetMessage(539060);
				tv0->ClearReplies();
				tv0->AddReply(539061, 43176, 41009);
				tv0->AddReply(541070, -1, 43175);
				return;
			}
			if (a0 == 43176) {
				f_98_a1_v("Neutral");
				tv0->SetMessage(541071);
				tv0->ClearReplies();
				tv0->AddReply(541072, -1, 43177);
				tv0->AddReply(541073, -1, 43178);
				return;
			}
			tv3 = true;
			if (f_585_a0_b()) {
				@lshStopAnimation();
			} else {
				@StopAnimation();
			}
			return;
		}
	}
}

task t3
{
	var object tv0;

	int init(object a0)
	{
		object L0;
		bool L1;
		int L2;
		bool L3;
		tv0 = a0;
		f_493_a1_b(f_540_a0_o());
		if (!f_43e_a2_b(a0, 70.0)) {
			return -2;
		}
		@CreateDialog(L0);
		L0->SetNPCName(f_57f_a0_i());
		L0->SetNPCDescription(f_57d_a0_i());
		L0->SetPhoto(f_581_a0_s());
		L0->SetPhoto2(f_583_a0_s());
		L0->SetPlayerName(f_625_a0_i());
		L2 = -1;
		@IsOverrideActive(L1);
		if (L1) {
			return -2;
		}
		@DoDialog(L0);
		t4{a0, L0};
		L0->IsDialogEnd(L3);
		while (!L3) {
			@sync();
			L0->IsDialogEnd(L3);
		}
		f_482_a1_v(a0);
		@StopDialog(L0);
		L0->GetReturnValue(L2);
		return L2;
	}
}

task t4
{
	var object tv0;
	var object tv1;
	var string tv2;
	var bool tv3;

	void init(object a0, object a1)
	{
		tv0 = a1;
		tv1 = a0;
		tv3 = false;
		while (1) {
			f_1a2_a1_v("Neutral");
			tv0->SetMessage(539063);
			tv0->ClearReplies();
			if (f_5a5_a1_b(tv1)) {
				tv0->AddReply(539064, 41700, 41012);
			}
			if (f_5a5_a1_b(tv1)) {
				tv0->AddReply(539750, 41703, 41702);
			}
			if (f_5af_a1_b(tv1)) {
				tv0->AddReply(539753, 41706, 41705);
			}
			if (f_5b9_a1_b(tv1)) {
				tv0->AddReply(539756, 41709, 41708);
			}
			tv0->AddReply(539759, -1, 41711);
			tv0->AddReply(541075, -1, 43180);
			break;
			return;
		}
		if (f_585_a0_b()) {
			for (; ; ) {
				@lshWaitForAnimEnd();
				if (tv3) {
					break;
				}
				f_51d_a1_v(tv2);
			}
		} else {
			@PlayAnimation("all", "idle");
			for (; ; ) {
				@WaitForAnimEnd();
				if (tv3) {
					break;
				}
				@PlayAnimation("all", "idle");
			}
		}
	}

	void f_1a2_a1_v(string a0)
	{
		if (!f_585_a0_b()) {
			return;
		}
		if (a0 == tv2) {
			return;
		}
		f_524_a2_v(a0, a0 == "" ? false : true);
		tv2 = a0;
	}

	void evt_11(int a0, int a1)
	{
		while (1) {
			f_539_a0_v();
			if (a1 == 41701) {
				f_587_a2_v(tv1, tv0);
				f_59b_a2_v(tv1, tv0);
			}
			if (a1 == 41704) {
				f_58c_a2_v(tv1, tv0);
			}
			if (a1 == 41707) {
				f_591_a2_v(tv1, tv0);
				f_59b_a2_v(tv1, tv0);
				f_5a0_a2_v(tv1, tv0);
			}
			if (a1 == 41710) {
				f_596_a2_v(tv1, tv0);
			}
			if (a0 == 41011) {
				f_1a2_a1_v("Neutral");
				tv0->SetMessage(539063);
				tv0->ClearReplies();
				if (f_5a5_a1_b(tv1)) {
					tv0->AddReply(539064, 41700, 41012);
				}
				if (f_5a5_a1_b(tv1)) {
					tv0->AddReply(539750, 41703, 41702);
				}
				if (f_5af_a1_b(tv1)) {
					tv0->AddReply(539753, 41706, 41705);
				}
				if (f_5b9_a1_b(tv1)) {
					tv0->AddReply(539756, 41709, 41708);
				}
				tv0->AddReply(539759, -1, 41711);
				tv0->AddReply(541075, -1, 43180);
				return;
			}
			if (a0 == 41709) {
				f_1a2_a1_v("Neutral");
				tv0->SetMessage(539757);
				tv0->ClearReplies();
				tv0->AddReply(539758, -1, 41710);
				return;
			}
			if (a0 == 41706) {
				f_1a2_a1_v("Neutral");
				tv0->SetMessage(539754);
				tv0->ClearReplies();
				tv0->AddReply(541088, 43197, 43196);
				tv0->AddReply(541090, 43197, 43198);
				return;
			}
			if (a0 == 43197) {
				f_1a2_a1_v("Neutral");
				tv0->SetMessage(541089);
				tv0->ClearReplies();
				tv0->AddReply(539755, -1, 41707);
				return;
			}
			if (a0 == 41703) {
				f_1a2_a1_v("Neutral");
				tv0->SetMessage(539751);
				tv0->ClearReplies();
				tv0->AddReply(541077, 43183, 43182);
				return;
			}
			if (a0 == 43183) {
				f_1a2_a1_v("Neutral");
				tv0->SetMessage(541078);
				tv0->ClearReplies();
				tv0->AddReply(541085, 43194, 43192);
				tv0->AddReply(541086, 43194, 43193);
				return;
			}
			if (a0 == 43194) {
				f_1a2_a1_v("Neutral");
				tv0->SetMessage(541087);
				tv0->ClearReplies();
				tv0->AddReply(539752, -1, 41704);
				return;
			}
			if (a0 == 41700) {
				f_1a2_a1_v("Neutral");
				tv0->SetMessage(539748);
				tv0->ClearReplies();
				tv0->AddReply(541079, 43186, 43184);
				tv0->AddReply(541080, 43186, 43185);
				return;
			}
			if (a0 == 43186) {
				f_1a2_a1_v("Neutral");
				tv0->SetMessage(541081);
				tv0->ClearReplies();
				tv0->AddReply(541082, 43189, 43187);
				tv0->AddReply(541083, 43189, 43188);
				return;
			}
			if (a0 == 43189) {
				f_1a2_a1_v("Neutral");
				tv0->SetMessage(541084);
				tv0->ClearReplies();
				tv0->AddReply(539749, -1, 41701);
				return;
			}
			tv3 = true;
			if (f_585_a0_b()) {
				@lshStopAnimation();
			} else {
				@StopAnimation();
			}
			return;
		}
	}
}

maintask t5
{
	var Vector tv0;
	var bool tv1;

	void init(void)
	{
		f_2f3_a0_v();
	}

	void f_2f3_a0_v(void)
	{
		f_31c_a0_v();
	}

	void OnTrigger(string a0)
	{
		if (a0 == "cleanup") {
			bool L0;
			tv1 = true;
			@IsLoaded(L0);
			if (!L0) {
				@RemoveActor(f_540_a0_o());
			}
		} else {
			if (a0 == "restore") {
				tv1 = false;
			}
		}
	}

	void OnUnload(void)
	{
		if (tv1) {
			@RemoveActor(f_540_a0_o());
			@Hold();
		}
		f_38d_a0_v();
	}

	void OnLoad(void)
	{
		f_39c_a0_v();
	}

	void f_31c_a0_v(void)
	{
		if (!f_439_a0_b()) {
			t0{};
		}
		OnLoad();
		@GetDirection(tv0);
		for (; ; ) {
			f_3d2_a0_v();
		}
	}

	bool f_330_a0_b(void)
	{
		object L0;
		@FindActor(L0, "player");
		if (!L0) {
			return false;
		}
		return f_430_a1_b(L0);
	}

	void f_33f_a0_v(void)
	{
		@RotateAsync(tv0.x, tv0.z);
	}

	bool f_344_a0_b(void)
	{
		object L0;
		bool L1;
		@FindActor(L0, "player");
		if (!L0) {
			return false;
		}
		if (f_41e_a1_f(L0) > 90000.0) {
			return false;
		}
		@CanSee(L1, L0);
		return L1;
	}

	void f_35a_a0_v(void)
	{
		float L0;
		@rand(L0, 8, 16);
		@SetTimer(10, L0);
	}

	void f_363_a0_v(void)
	{
		@KillTimer(10);
	}

	void OnTimer(int a0)
	{
		if (a0 == 10) {
			f_363_a0_v();
			if (f_439_a0_b() && f_344_a0_b()) {
				if (f_330_a0_b()) {
					f_4cd_a1_b(f_540_a0_o());
				}
			} else {
				f_33f_a0_v();
				f_35a_a0_v();
			}
		}
	}

	void f_38d_a0_v(void)
	{
		f_419_a0_v();
		f_363_a0_v();
		@lshStopSpeech();
		@lshStopAnimation();
		@StopAsync();
		@Hold();
	}

	void f_39c_a0_v(void)
	{
		@StopGroup0();
		f_363_a0_v();
		f_51d_a1_v("Neutral");
		f_35a_a0_v();
	}

	void OnLSHAnimationEnd(bool a0)
	{
		if (a0) {
			f_35a_a0_v();
		} else {
			f_51d_a1_v("Neutral");
		}
	}

	void OnUse(object a0)
	{
		bool L0;
		@IsOverrideActive(L0);
		if (!L0) {
			disable OnUse;
			f_419_a0_v();
			f_430_a1_b(a0);
			enable OnUse;
			f_6c2_a1_v(a0);
			f_51d_a1_v("Neutral");
			f_363_a0_v();
			f_35a_a0_v();
		}
	}
}

void f_3d2_a0_v(void)
{
	int L0;
	int L1;
	@WaitForAnimEnd();
	if (!f_439_a0_b()) {
		return;
	}
	L0 = f_56c_a0_i();
	for (L1 = 0; L1 < 5 && f_439_a0_b(); L1 = L1 + 1) {
		if (!L0) {
			bool L2;
			@Sleep(3, L2);
			if (!L2) {
				break;
			}
		} else {
			int L3;
			int L4;
			bool L5;
			@irand(L3, L0);
			@irand(L4, 5);
			if (L4 != 0) {
				L3 = 0;
			}
			@PlayAnimation("all", f_565_a1_s(L3));
			@WaitForAnimEnd(L5);
			if (!L5) {
				break;
			}
		}
		if (!f_417_a0_b()) {
			break;
		}
		@ResetAAS();
	}
	@ResetAAS();
}

bool f_417_a0_b(void)
{
	return true;
}

void f_419_a0_v(void)
{
	@StopAnimation();
	@StopGroup0();
}

float f_41e_a1_f(object a0)
{
	Vector L0;
	Vector L1;
	Vector L2;
	@GetPosition(L0);
	a0->GetPosition(L1);
	L2 = L1 - L0;
	return L2 | L2;
}

bool f_426_a1_b(Vector a0)
{
	Vector L0;
	Vector L1;
	bool L2;
	@GetPosition(L0);
	L1 = a0 - L0;
	@Rotate(L1.x, L1.z, L2);
	return L2;
}

bool f_430_a1_b(object a0)
{
	Vector L0;
	a0->GetPosition(L0);
	return f_426_a1_b(L0);
}

bool f_439_a0_b(void)
{
	bool L0;
	@IsLoaded(L0);
	return L0;
}

bool f_43e_a2_b(object a0, float a1)
{
	float L0;
	Vector L1;
	Vector L2;
	Vector L3;
	Vector L4;
	Vector L5;
	Vector L6;
	bool L7;
	a0->GetPosition(L1);
	a0->GetEyesHeight(L0);
	L1.y += L0;
	@GetPosition(L2);
	@GetEyesHeight(L0);
	L2.y += L0;
	L3 = L1 - L2;
	L3.y = 0;
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * a1 + f_546_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
	}
	@StopWorld();
	@CameraTransit(L6, L4);
	@Rotate(L5.x, L5.z);
	if (f_585_a0_b()) {
	} else {
		bool L8;
		@HasAnimationTrack(L8, "head");
		if (L8) {
			@LookAsyncCamera("head");
		}
	}
	@CameraWaitForPlayFinish();
	@ResumeWorld();
	return true;
}

void f_482_a1_v(object a0)
{
	@CameraSwitchToNormal();
	if (f_585_a0_b()) {
	} else {
		bool L0;
		@HasAnimationTrack(L0, "head");
		if (L0) {
			@UnlookAsync("head");
		}
	}
}

bool f_493_a1_b(object a0)
{
	int L0;
	@GetVariable("voice_common", L0);
	if (L0) {
		int L1;
		if (!f_4cd_a1_b(a0)) {
			if (!f_4f2_a1_b(a0)) {
				return false;
			}
		}
		@irand(L1, 2);
		if (L1) {
			@SetVariable("voice_common", (L0 + 1) % 3);
		} else {
			@SetVariable("voice_common", 0);
		}
	} else {
		if (!f_4f2_a1_b(a0)) {
			if (!f_4cd_a1_b(a0)) {
				return false;
			}
		}
		@SetVariable("voice_common", 1);
	}
	return true;
}

bool f_4cd_a1_b(object a0)
{
	string L0;
	L0 = "c";
	int L1;
	L1 = 0;
	while (1) {
		bool L2;
		a0->HasProperty(L0 + (L1 + 1), L2);
		if (!L2) {
			break;
		}
		L1 = L1 + 1;
	}
	if (!L1) {
		return false;
	}
	int L3;
	L3 = 0;
	if (L1 > 1) {
		@irand(L3, L1);
	}
	string L4;
	a0->GetProperty(L0 + (L3 + 1), L4);
	return f_52a_a1_b(L4);
}

bool f_4f2_a1_b(object a0)
{
	string L0;
	L0 = "d" + f_55c_a0_i() + "m";
	int L1;
	L1 = 0;
	while (1) {
		bool L2;
		a0->HasProperty(L0 + (L1 + 1), L2);
		if (!L2) {
			break;
		}
		L1 = L1 + 1;
	}
	if (!L1) {
		return false;
	}
	int L3;
	L3 = 0;
	if (L1 > 1) {
		@irand(L3, L1);
	}
	string L4;
	a0->GetProperty(L0 + (L3 + 1), L4);
	return f_52a_a1_b(L4);
}

void f_51d_a1_v(string a0)
{
	float L0;
	float L1;
	@lshGetAnimTimes(a0, L0, L1);
	@lshPlayAnimation(L0, L1, false);
}

void f_524_a2_v(string a0, bool a1)
{
	float L0;
	float L1;
	@lshGetAnimTimes(a0, L0, L1);
	@lshPlayAnimation(L0, L1, a1);
}

bool f_52a_a1_b(string a0)
{
	if (f_585_a0_b()) {
		bool L0;
		@lshHasSpeech(L0, a0);
		if (L0) {
			@lshPlaySpeech(a0);
			return true;
		}
	}
	return false;
}

void f_539_a0_v(void)
{
	if (f_585_a0_b()) {
		@lshStopSpeech();
	}
}

object f_540_a0_o(void)
{
	object L0;
	@self(L0);
	return L0;
}

Vector f_546_a1_V(Vector a0)
{
	float L0;
	L0 = sqrt(a0 | a0);
	if (L0 < 9.999999974752427e-07) {
		return [0.0, 0.0, 0.0];
	}
	return a0 / L0;
}

bool f_550_a2_b(string a0, string a1)
{
	object L0;
	@FindActor(L0, a0);
	if (L0 == null) {
		return false;
	}
	@Trigger(L0, a1);
	return true;
}

int f_55c_a0_i(void)
{
	float L0;
	@GetGameTime(L0);
	return 1 + (int)(L0 / 24);
}

string f_565_a1_s(int a0)
{
	string L0;
	L0 = "idle";
	if (a0) {
		L0 = L0 + a0;
	}
	return L0;
}

int f_56c_a0_i(void)
{
	int L0;
	for (L0 = 0; ; L0 = L0 + 1) {
		bool L1;
		@HasAnimation(L1, "all", f_565_a1_s(L0));
		if (!L1) {
			break;
		}
	}
	return L0;
}

int f_57d_a0_i(void)
{
	return 515592;
}

int f_57f_a0_i(void)
{
	return 511961;
}

string f_581_a0_s(void)
{
	return "ui/NPC_Burah.png";
}

string f_583_a0_s(void)
{
	return "ui/NPC_Burah_b.png";
}

bool f_585_a0_b(void)
{
	return true;
}

void f_587_a2_v(object a0, object a1)
{
	f_6a1_a0_v();
}

void f_58c_a2_v(object a0, object a1)
{
	f_6a9_a0_v();
}

void f_591_a2_v(object a0, object a1)
{
	f_693_a0_v();
}

void f_596_a2_v(object a0, object a1)
{
	f_6bd_a0_v();
}

void f_59b_a2_v(object a0, object a1)
{
	f_63c_a0_v();
}

void f_5a0_a2_v(object a0, object a1)
{
	f_636_a0_v();
}

bool f_5a5_a1_b(object a0)
{
	if (f_5c3_a1_b(a0)) {
		return true;
	}
	return false;
}

bool f_5af_a1_b(object a0)
{
	if (f_5c9_a1_b(a0)) {
		return true;
	}
	return false;
}

bool f_5b9_a1_b(object a0)
{
	if (f_5cf_a1_b(a0)) {
		return true;
	}
	return false;
}

bool f_5c3_a1_b(object a0)
{
	return f_6b1_a0_b();
}

bool f_5c9_a1_b(object a0)
{
	return f_69b_a0_b();
}

bool f_5cf_a1_b(object a0)
{
	return f_6b7_a0_b();
}

void f_5d5_a0_v(void)
{
	object L0;
	@CreateDiaryEntry(L0, 753, 2, 539738);
	f_609_a2_b(L0, 751);
}

void f_5e2_a0_v(void)
{
	object L0;
	@CreateDiaryEntry(L0, 754, 2, 539739);
	f_609_a2_b(L0, 751);
}

void f_5ef_a0_v(void)
{
	object L0;
	@CreateDiaryEntry(L0, 761, 2, 539746);
	f_609_a2_b(L0, 757);
}

object f_5fc_a0_o(void)
{
	object L0;
	@GetDiaryRoot(L0);
	if (!L0) {
		@Trace("Can't retrieve diary root");
		return false;
	}
	return L0;
}

bool f_609_a2_b(object a0, int a1)
{
	object L0;
	object L1;
	int L2;
	L0 = f_5fc_a0_o();
	L0->Find(a1, L1);
	if (!L1) {
		@Trace("Can't find diary parent with id: " + a1);
		return false;
	}
	L1->AddChild(a0);
	@SendWorldWndMessage(7);
	a0->GetCategory(L2);
	@SetDiarySection(L2);
	return false;
}

int f_625_a0_i(void)
{
	int L0;
	@GetVariable("branch", L0);
	if (L0 == 0) {
		return 1;
	} else {
		if (L0 == 1) {
			return 2;
		}
	}
	return 3;
}

void f_636_a0_v(void)
{
	f_550_a2_b("klara2_npc_positioner", "remove_danko");
}

void f_63c_a0_v(void)
{
	f_550_a2_b("klara2_npc_positioner", "remove_burah");
}

int f_642_a0_i(void)
{
	int L0;
	@GetVariable("k2system_klara_day", L0);
	return L0;
}

int f_648_a0_i(void)
{
	int L0;
	@GetVariable("k2system_burah_day", L0);
	return L0;
}

int f_64e_a0_i(void)
{
	int L0;
	@GetVariable("k2system_danko_day", L0);
	return L0;
}

int f_654_a0_i(void)
{
	int L0;
	int L1;
	L0 = f_648_a0_i();
	@GetVariable("k2system_burah_state" + L0, L1);
	return L1;
}

void f_65f_a1_v(int a0)
{
	int L0;
	L0 = f_648_a0_i();
	@SetVariable("k2system_burah_state" + L0, a0);
}

int f_669_a0_i(void)
{
	int L0;
	int L1;
	L0 = f_64e_a0_i();
	@GetVariable("k2system_danko_state" + L0, L1);
	return L1;
}

void f_674_a1_v(int a0)
{
	int L0;
	L0 = f_64e_a0_i();
	@SetVariable("k2system_danko_state" + L0, a0);
}

int f_67e_a0_i(void)
{
	int L0;
	int L1;
	L0 = f_642_a0_i();
	@GetVariable("k2system_klara_state" + L0, L1);
	return L1;
}

void f_689_a1_v(int a0)
{
	int L0;
	L0 = f_642_a0_i();
	@SetVariable("k2system_klara_state" + L0, a0);
}

void f_693_a0_v(void)
{
	f_5ef_a0_v();
	f_674_a1_v(3);
}

bool f_69b_a0_b(void)
{
	return f_669_a0_i() == 1;
}

void f_6a1_a0_v(void)
{
	f_5d5_a0_v();
	f_65f_a1_v(2);
}

void f_6a9_a0_v(void)
{
	f_5e2_a0_v();
	f_65f_a1_v(1);
}

bool f_6b1_a0_b(void)
{
	return f_654_a0_i() == 0;
}

bool f_6b7_a0_b(void)
{
	return f_67e_a0_i() == 1;
}

void f_6bd_a0_v(void)
{
	f_689_a1_v(3);
}

void f_6c2_a1_v(object a0)
{
	int L0;
	@GetProperty("day", L0);
	if (L0 == f_55c_a0_i()) {
		t3{a0};
	} else {
		t1{a0};
	}
}

