		...
		for (L1 = 0; L1 < 5 && f_726_a0_b(); L1++) {
			...
			if (L2 == 0) {
				if (L0) {
					...
					bool L4;
					@irand(L3, L0);
					@PlayAnimation("all", f_7d4_a1_s(L3));
					@WaitForAnimEnd(L4);
					if (!L4) {
						break;
					}
...
int f_7db_a0_i(void)
{
	...
	for (L0 = 0; ; L0++) {
		bool L1;
		@HasAnimation(L1, "all", f_7d4_a1_s(L0));
		if (!L1) {
			break;
		}
	}
