...
task t0
{
	...
	int init(object a0)
	{
		...
		@CreateDialog(L0);
		L0->SetNPCName(f_9f5_a0_i());
		L0->SetNPCDescription(f_9f3_a0_i());
		L0->SetPhoto(f_9f7_a0_s());
		L0->SetPhoto2(f_9f9_a0_s());
		L0->SetPlayerName(f_9cf_a0_i());
		L2 = -1;
		@IsOverrideActive(L1);
		if (L1) {
			return -2;
...
maintask t2
{
	...
	void OnUse(object a0)
	{
		...
		L0 = t0{a0};
		if (L0 == 0) {
			f_952_a2_b(a0, 0.05000000074505806);
			t3{a0};
		} else {
			if (L0 == 1) {
				t4{a0, true};
		...
		for (L1 = 0; L1 < 5 && f_886_a0_b(); L1++) {
			...
			if (L2 == 0) {
				if (L0) {
					...
					bool L4;
					@irand(L3, L0);
					@PlayAnimation("all", f_9a9_a1_s(L3));
					@WaitForAnimEnd(L4);
					if (!L4) {
						break;
					}
...
task t3
{
	...
	Vector f_228_a1_V(float a0)
	{
		...
		tv0->GetPosition(L1);
		@GetDirection(L2);
		L3 = f_92e_a1_V(f_92e_a1_V(L0 - L1) + L2 * 0.75);
	...
	bool f_2d3_a0_b(void)
	{
		...
		@GetDirection(L0);
		L1 = f_745_a1_V(tv0);
		return f_94a_a2_f(L0, L1) >= -0.3420201241970062;
	}
}
...
task t4
{
	...
	void init(object a0, bool a1)
	{
		object L0;
		@GetScene(L0);
		L0->RemoveStationaryActor(f_928_a0_o());
		for (; ; ) {
...
bool f_75d_a5_b(object a0, string a1, float a2, float a3, float a4)
{
	...
	a0->SetProperty(a1, f_93f_a3_f(L0 + a2, a3, a4));
	return true;
}
...
float f_773_a3_f(object a0, float a1, int a2)
{
	...
	L3 = f_938_a2_f((L0 + L2) / 100.0, 1);
	a0->GetProperty("health", L4);
	L5 = a1 * (1 - L3);
	a0->SetProperty("health", f_93f_a3_f(L4 - L5, 0, 1));
	if (f_74c_a1_b(a0)) {
		f_975_a1_v(-L5);
	}
	return L5;
...
bool f_88b_a2_b(object a0, float a1)
{
	...
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * a1 + f_92e_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
...
void f_8e7_a2_v(string a0, bool a1)
{
	float L0;
	float L1;
	@lshGetAnimTimes(a0, L0, L1);
...
float f_938_a2_f(float a0, float a1)
{
	return a0 < a1 ? a0 : a1;
}

float f_93f_a3_f(float a0, float a1, float a2)
{
	if (a0 < a1) {
		return a1;
	}
...
float f_94a_a2_f(Vector a0, Vector a1)
{
	return (a0 | a1) / sqrt((a0 | a0) * (a1 | a1));
}

bool f_952_a2_b(object a0, float a1)
{
	if (!a0) {
		return false;
	}
...
bool f_989_a2_b(string a0, string a1)
{
	object L0;
	@FindActor(L0, a0);
	if (L0 == null) {
...
void f_99e_a0_v(void)
{
	object L0;
	@GetScene(L0);
	@BroadcastMessage("battle", f_928_a0_o(), L0);
}

string f_9a9_a1_s(int a0)
{
	string L0;
	L0 = "idle";
	if (a0) {
...
int f_9b0_a0_i(void)
{
	...
	for (L0 = 0; ; L0++) {
		bool L1;
		@HasAnimation(L1, "all", f_9a9_a1_s(L0));
		if (!L1) {
			break;
		}
	}
...
string f_9f7_a0_s(void)
{
	return "ui/NPC_Citizen2.png";
}

string f_9f9_a0_s(void)
{
	return "ui/NPC_Citizen2_b.png";
}

bool f_9fb_a0_b(void)
{
	return false;
}
...
void f_a07_a1_v(object a0)
{
	if (f_74c_a1_b(a0)) {
		f_952_a2_b(a0, -0.10000000149011612);
		f_989_a2_b("quest_b1_03", "doberman_dead");
	}
	f_9fd_a0_v();
	t6{a0};
}
