...
void f_28c_a0_v(void)
{
	...
	L0 = f_41d_a0_i();
	for (L1 = 0; L1 < 5 && f_2e2_a0_b(); L1 = L1 + 1) {
		int L2;
		@irand(L2, 3);
		if (L2 == 0) {
			if (L0) {
				...
				bool L4;
				@irand(L3, L0);
				@PlayAnimation("all", f_416_a1_s(L3));
				@WaitForAnimEnd(L4);
				if (!L4) {
					break;
				}
...
void f_3cd_a2_v(string a0, bool a1)
{
	float L0;
	float L1;
	@lshGetAnimTimes(a0, L0, L1);
...
int f_41d_a0_i(void)
{
	...
	for (L0 = 0; ; L0 = L0 + 1) {
		bool L1;
		@HasAnimation(L1, "all", f_416_a1_s(L0));
		if (!L1) {
			break;
		}
	}
...
void f_42e_a2_v(object a0, object a1)
{
	object L0;
	@SetVariable("b3q01", 3);
	L0 = f_499_a0_o();
	L0->AddMark("b3q01ButcherGotoBigVlad", "pt_map_bigvlad", 1, 519641, f_3fe_a0_f());
	f_463_a0_v();
}
