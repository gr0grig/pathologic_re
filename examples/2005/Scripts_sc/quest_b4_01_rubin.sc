...
task t0
{
	...
	int init(object a0)
	{
		...
		bool L3;
		tv0 = a0;
		f_3ae_a1_b(f_45b_a0_o());
		if (!f_359_a2_b(a0, 70.0)) {
			return -2;
		}
		@CreateDialog(L0);
...
void f_2fe_a0_v(void)
{
	...
	L0 = f_48a_a0_i();
	for (L1 = 0; L1 < 5 && f_354_a0_b(); L1 = L1 + 1) {
		int L2;
		@irand(L2, 3);
		if (L2 == 0) {
			if (L0) {
				...
				bool L4;
				@irand(L3, L0);
				@PlayAnimation("all", f_483_a1_s(L3));
				@WaitForAnimEnd(L4);
				if (!L4) {
					break;
				}
...
bool f_359_a2_b(object a0, float a1)
{
	float L0;
	Vector L1;
	Vector L2;
	...
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * a1 + f_461_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
...
void f_43f_a2_v(string a0, bool a1)
{
	float L0;
	float L1;
	@lshGetAnimTimes(a0, L0, L1);
...
int f_48a_a0_i(void)
{
	...
	for (L0 = 0; ; L0 = L0 + 1) {
		bool L1;
		@HasAnimation(L1, "all", f_483_a1_s(L0));
		if (!L1) {
			break;
		}
	}
...
void f_4a1_a2_v(object a0, object a1)
{
...
bool f_4e2_a2_b(object a0, int a1)
{
	object L0;
	object L1;
	int L2;
