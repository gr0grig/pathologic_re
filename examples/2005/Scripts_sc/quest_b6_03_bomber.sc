		...
		for (L1 = 0; L1 < 5 && f_5b5_a0_b(); L1++) {
			...
			if (L2 == 0) {
				if (L0) {
					...
					bool L4;
					@irand(L3, L0);
					@PlayAnimation("all", f_6bb_a1_s(L3));
					@WaitForAnimEnd(L4);
					if (!L4) {
						break;
					}
...
task t4
{
	...
	void init(object a0)
	{
		...
		while (f_50c_a1_b(tv0)) {
			...
			L7->SetScriptProperty("Owner", f_65f_a0_o());
			if (L3 == 1) {
...
bool f_4cd_a5_b(object a0, string a1, float a2, float a3, float a4)
{
	...
	a0->SetProperty(a1, f_66f_a3_f(L0 + a2, a3, a4));
	return true;
}
...
float f_66f_a3_f(float a0, float a1, float a2)
{
	if (a0 < a1) {
		return a1;
	}
...
bool f_685_a2_b(object a0, float a1)
{
	if (!a0) {
		return false;
	}
...
void f_6e6_a2_v(int a0, int a1)
{
	...
	@AddItem(L1, f_753_a1_i("Money"), 0, L0);
}

void f_705_a1_v(string a0)
{
	object L0;
	int L1;
	bool L2;
...
void f_72e_a0_v(void)
{
	...
	@AddItem(L1, f_753_a1_i("bomber_mark"), 0, 1);
}

int f_753_a1_i(string a0)
{
	int L0;
...
float f_76a_a3_f(Vector a0, Vector a1, Vector a2)
{
	float L0;
	float L1;
	float L2;
...
float f_7ac_a2_f(int a0, float a1)
{
	if (a1 < 10) {
		return 10;
	} else {
...
void f_7fa_a1_v(object a0)
{
	f_888_a1_v(a0);
	if (f_4bc_a1_b(a0)) {
		f_685_a2_b(a0, 0.05000000074505806);
	}
	t1{a0};
}
...
bool f_823_a2_b(object a0, object a1)
{
	if (f_758_a1_b(a1)) {
		bool L0;
		@CanSee(L0, a0);
