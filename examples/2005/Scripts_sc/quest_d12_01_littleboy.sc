...
task t0
{
	...
	int init(object a0)
	{
		...
		bool L3;
		tv0 = a0;
		if (!f_396_a2_b(a0, 70.0)) {
			return -2;
		}
		@CreateDialog(L0);
		L0->SetNPCName(f_4bd_a0_i());
		L0->SetNPCDescription(f_4bb_a0_i());
		L0->SetPhoto(f_4bf_a0_s());
		L0->SetPhoto2(f_4c1_a0_s());
		L0->SetPlayerName(f_4aa_a0_i());
		L2 = -1;
		@IsOverrideActive(L1);
		if (L1) {
			return -2;
...
maintask t2
{
	...
	void f_2e4_a2_v(float a0, float a1)
	{
		if (!f_391_a0_b()) {
			return;
		}
...
void f_333_a0_v(void)
{
	...
	L0 = f_4cc_a0_i();
	for (L1 = 0; L1 < 5 && f_391_a0_b(); L1 = L1 + 1) {
		int L2;
		@irand(L2, 3);
		if (L2 == 0) {
			if (L0) {
				...
				bool L4;
				@irand(L3, L0);
				@PlayAnimation("all", f_4c5_a1_s(L3));
				@WaitForAnimEnd(L4);
				if (!L4) {
					break;
				}
...
bool f_396_a2_b(object a0, float a1)
{
	float L0;
	Vector L1;
	Vector L2;
	...
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * a1 + f_410_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
...
void f_3f2_a2_v(string a0, bool a1)
{
	float L0;
	float L1;
	@lshGetAnimTimes(a0, L0, L1);
...
bool f_44b_a1_b(object a0)
{
	if (f_41a_a1_i("ood12Littleboy1") == 0) {
		return true;
	}
	return false;
}
...
{
	if (f_41a_a1_i("ood12Littleboy2") == 0) {
		return true;
	}
	return false;
}
...
bool f_47d_a2_b(object a0, int a1)
{
	object L0;
	object L1;
	int L2;
...
int f_4cc_a0_i(void)
{
	...
	for (L0 = 0; ; L0 = L0 + 1) {
		bool L1;
		@HasAnimation(L1, "all", f_4c5_a1_s(L0));
		if (!L1) {
			break;
		}
	}
