...
task t0
{
	...
	int init(object a0)
	{
		...
		@CreateDialog(L0);
		L0->SetNPCName(f_269_a0_i());
		L0->SetNPCDescription(f_267_a0_i());
		L0->SetPhoto(f_26b_a0_s());
		L0->SetPhoto2(f_26d_a0_s());
		L0->SetPlayerName(f_271_a0_i());
		L2 = -1;
		@IsOverrideActive(L1);
		if (L1) {
			return -2;
...
maintask t2
{
	...
	void OnUse(object a0)
	{
		if (!tv0) {
			...
			if (L0) {
				f_243_a2_b("quest_d1_01", "remove_masks");
			}
		}
	}
}
...
void f_175_a0_v(void)
{
	...
	L0 = f_256_a0_i();
	for (L1 = 0; L1 < 5 && f_1cb_a0_b(); L1 = L1 + 1) {
		int L2;
		@irand(L2, 3);
		if (L2 == 0) {
			if (L0) {
				...
				bool L4;
				@irand(L3, L0);
				@PlayAnimation("all", f_24f_a1_s(L3));
				@WaitForAnimEnd(L4);
				if (!L4) {
					break;
				}
...
bool f_1d0_a2_b(object a0, float a1)
{
	...
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * a1 + f_239_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
...
void f_22c_a2_v(string a0, bool a1)
{
	float L0;
	float L1;
	@lshGetAnimTimes(a0, L0, L1);
...
bool f_243_a2_b(string a0, string a1)
{
	object L0;
	@FindActor(L0, a0);
	if (L0 == null) {
...
int f_256_a0_i(void)
{
	...
	for (L0 = 0; ; L0 = L0 + 1) {
		bool L1;
		@HasAnimation(L1, "all", f_24f_a1_s(L0));
		if (!L1) {
			break;
		}
	}
