...
task t0
{
	...
	int init(object a0)
	{
		...
		@CreateDialog(L0);
		L0->SetNPCName(f_45b_a0_i());
		L0->SetNPCDescription(f_459_a0_i());
		L0->SetPhoto(f_45d_a0_s());
		L0->SetPhoto2(f_45f_a0_s());
		L0->SetPlayerName(f_52a_a0_i());
		L2 = -1;
		@IsOverrideActive(L1);
		if (L1) {
			return -2;
...
maintask t2
{
	...
	void OnUse(object a0)
	{
		...
		if (L0 == 0) {
			object L1;
			@GetScene(L1);
			L1->RemoveStationaryActor(f_413_a0_o());
			t3{a0};
		}
	}
		...
		for (L1 = 0; L1 < 5 && f_39a_a0_b(); L1++) {
			...
			if (L2 == 0) {
				if (L0) {
					...
					bool L4;
					@irand(L3, L0);
					@PlayAnimation("all", f_441_a1_s(L3));
					@WaitForAnimEnd(L4);
					if (!L4) {
						break;
					}
...
task t3
{
	...
	Vector f_24d_a1_V(float a0)
	{
		...
		tv0->GetPosition(L1);
		@GetDirection(L2);
		L3 = f_419_a1_V(f_419_a1_V(L0 - L1) + L2 * 0.75);
	...
	bool f_2f8_a0_b(void)
	{
		...
		@GetDirection(L0);
		L1 = f_30d_a1_V(tv0);
		return f_423_a2_f(L0, L1) >= -0.3420201241970062;
	}
}
...
bool f_39f_a2_b(object a0, float a1)
{
	...
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * a1 + f_419_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
...
float f_423_a2_f(Vector a0, Vector a1)
{
...
bool f_430_a2_b(string a0, string a1)
{
	object L0;
	@FindActor(L0, a0);
	if (L0 == null) {
...
int f_448_a0_i(void)
{
	...
	for (L0 = 0; ; L0++) {
		bool L1;
		@HasAnimation(L1, "all", f_441_a1_s(L0));
		if (!L1) {
			break;
		}
	}
...
string f_45d_a0_s(void)
{
	return "ui/NPC_Citizen3.png";
}

string f_45f_a0_s(void)
{
...
void f_46c_a2_v(object a0, object a1)
{
	...
	L0 = f_4f8_a0_o();
	L0->AddMark("k10q01WastedGotoBurah", "pt_gmap_factory", 1, 530196, f_43c_a0_f());
	f_4c2_a0_v();
	f_430_a2_b("quest_k10_01", "init_factory");
}

void f_488_a2_v(object a0, object a1)
{
	f_509_a3_v(f_4f8_a0_o(), "pt_gmap_factory", 2);
	a1->ShowMap(f_4f8_a0_o());
}
...
void f_509_a3_v(object a0, string a1, float a2)
{
	Vector L0;
	Vector L1;
	object L2;
