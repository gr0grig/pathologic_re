...
task t0
{
	...
	int init(object a0)
	{
		...
		bool L3;
		tv0 = a0;
		if (!f_392_a2_b(a0, 70.0)) {
			return -2;
		}
		@CreateDialog(L0);
		L0->SetNPCName(f_4a6_a0_i());
		L0->SetNPCDescription(f_4a4_a0_i());
		L0->SetPhoto(f_4a8_a0_s());
		L0->SetPhoto2(f_4aa_a0_s());
		L0->SetPlayerName(f_480_a0_i());
		L2 = -1;
		@IsOverrideActive(L1);
		if (L1) {
			return -2;
...
task t1
{
	...
	void init(object a0, object a1)
	{
		...
		tv3 = false;
		while (1) {
			if (f_468_a1_b(tv1) && f_474_a1_b(tv1)) {
				f_462_a2_v(tv1, tv0);
				f_a6_a1_v("Neutral");
...
maintask t2
{
	...
	void f_1bc_a2_v(float a0, float a1)
	{
		if (!f_38d_a0_b()) {
			return;
		}
		...
		for (L1 = 0; L1 < 5 && f_38d_a0_b(); L1 = L1 + 1) {
			...
			if (L2 == 0) {
				if (L0) {
					...
					bool L4;
					@irand(L3, L0);
					@PlayAnimation("all", f_44a_a1_s(L3));
					@WaitForAnimEnd(L4);
					if (!L4) {
						break;
					}
...
void f_33b_a3_v(object a0, int a1, float a2)
{
	if (a0 && a1 != 4 && a1 != 5) {
		Vector L0;
		object L1;
...
void f_37f_a5_v(object a0, int a1, float a2, Vector a3, Vector a4)
{
	object L0;
	object L1;
	@GetScene(L0);
...
bool f_392_a2_b(object a0, float a1)
{
	float L0;
	Vector L1;
	Vector L2;
	...
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * a1 + f_42f_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
...
void f_3ee_a2_v(string a0, bool a1)
{
	float L0;
	float L1;
	@lshGetAnimTimes(a0, L0, L1);
...
bool f_43e_a2_b(string a0, string a1)
{
	object L0;
	@FindActor(L0, a0);
	if (L0 == null) {
...
void f_462_a2_v(object a0, object a1)
{
...
void OnDeath(object a0)
{
	...
	@IsPlayerActor(a0, L0);
	if (L0) {
		f_43e_a2_b("quest_k1_01", "doberman_dead");
	}
	f_4ae_a1_v(a0);
}
