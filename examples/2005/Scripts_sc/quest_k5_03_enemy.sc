							...
							if (2 == g2) {
								...
								if (3 == g2) {
									...
									for (L3 = 0; L3 < g1; L3++) {
										...
										L0->GetLocator(f_df3_a0_s() + (L7 + 1), L4, L5, L6);
										t5{L5, L6, false};
										if (3 != g2) {
											break;
										}
										...
										if (4 == g2) {
											...
											if (4 == g2) {
												...
												if (4 == g2) {
													...
													@BroadcastMessage(f_df7_a0_s(), f_bf7_a0_o(), L10);
													t5{L8, L8, true};
													if (4 == g2) {
														t7{3};
														if (4 == g2) {
...
task t1
{
	...
	void OnTimer(int a0)
	{
		if (a0 == 1) {
			object L0;
			@GetMainOutdoorScene(L0);
			@BroadcastMessage(f_df5_a0_s(), tv7, L0);
			@Trace("helpme timer");
		}
	}
...
task t2
{
	...
	void OnTimer(int a0)
	{
		if (a0 == 2) {
			object L0;
			@GetMainOutdoorScene(L0);
			@BroadcastMessage(f_df5_a0_s(), tv3, L0);
			@Trace("helpme timer");
		} else {
			f_520_a1_v(a0);
		}
		...
		for (L1 = 0; L1 < 5 && f_baf_a0_b(); L1++) {
			...
			if (L2 == 0) {
				if (L0) {
					...
					bool L4;
					@irand(L3, L0);
					@PlayAnimation("all", f_c36_a1_s(L3));
					@WaitForAnimEnd(L4);
					if (!L4) {
						break;
					}
...
task t6
{
	...
	bool init(void)
	{
		...
		L1->GetLocator(f_df9_a0_s(), L0, L2, L3);
		if (!L0) {
			return false;
		}
...
int f_c3d_a0_i(void)
{
	...
	for (L0 = 0; ; L0++) {
		bool L1;
		@HasAnimation(L1, "all", f_c36_a1_s(L0));
		if (!L1) {
			break;
		}
	}
...
bool f_c69_a2_b(object a0, string a1)
{
	if (a0 == f_bf7_a0_o()) {
		return false;
	}
...
bool f_cb3_a1_b(object a0)
{
	if (a0->FuncExist("IsDead", 1) && f_ae5_a1_b(a0)) {
		if (f_a90_a2_b(a0, f_df7_a0_s())) {
			return false;
		}
		a0->SetProperty(f_df7_a0_s(), 1);
		return f_d47_a1_b(a0);
	} else {
		if (f_a8b_a1_b(a0)) {
...
bool f_ce6_a1_b(object a0)
{
	if (f_a8b_a1_b(a0)) {
		return f_d6b_a1_b(a0);
	}
	return false;
}

bool f_cf4_a0_b(void)
{
	object L0;
	Vector L1;
	Vector L2;
	...
	g1 = 0;
	while (true) {
		L0->GetLocator(f_df3_a0_s() + (g1 + 1), L4, L2, L3);
		if (!L4) {
			break;
		}
		g1++;
...
bool f_d8f_a0_b(void)
{
	int L0;
	@GetVariable(f_df9_a0_s() + "_locked", L0);
	return L0 != 0;
}

void f_d9a_a1_v(bool a0)
{
	@SetVariable(f_df9_a0_s() + "_locked", a0 ? 1 : 0);
}

bool f_da8_a0_b(void)
{
	int L0;
	@GetVariable(f_df3_a0_s() + "_locked", L0);
	return L0 >= f_dfb_a0_i();
}

void f_db5_a1_v(bool a0)
{
	int L0;
	@GetVariable(f_df3_a0_s() + "_locked", L0);
	if (a0) {
	...
	@SetVariable(f_df3_a0_s() + "_locked", L0);
}
...
string f_df3_a0_s(void)
{
	return "pt_k5q03_patrol";
}

string f_df5_a0_s(void)
{
	return "k5q03_helpme";
}

string f_df7_a0_s(void)
{
	return "k5q03_corpse";
}
