...
maintask t2
{
	...
	void f_1c4_a2_v(float a0, float a1)
	{
		if (!f_271_a0_b()) {
			return;
		}
...
void f_213_a0_v(void)
{
	...
	L0 = f_37d_a0_i();
	for (L1 = 0; L1 < 5 && f_271_a0_b(); L1 = L1 + 1) {
		int L2;
		@irand(L2, 3);
		if (L2 == 0) {
			if (L0) {
...
bool f_276_a2_b(object a0, float a1)
{
	float L0;
	Vector L1;
	Vector L2;
	...
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * a1 + f_2f0_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
...
void f_2d2_a2_v(string a0, bool a1)
{
	float L0;
	float L1;
	@lshGetAnimTimes(a0, L0, L1);
...
{
	if (f_2fa_a1_i("k9q01ChildTalk") == 0) {
...
{
	if (f_2fa_a1_i("ook9Littleboy1") == 0) {
		return true;
	}
	return false;
}
