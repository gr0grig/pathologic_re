...
maintask t0
{
	...
	{
		return "grass_combination" == f_3f4_a1_s(a0);
	}
	...
	{
		@SendMessage(f_338_a0_i(), "money");
	}

	void f_34a_a1_v(int a0)
	{
		if (a0 != 0) {
	...
	void f_388_a1_v(int a0)
	{
		...
		for (; L2 < 12; L2++) {
			if (L2 < L1) {
				...
				if (L5) {
					if (L6) {
						@SendMessage(0, f_3df_a1_s(L2), L3);
					} else {
						@SendMessage(16384, f_3df_a1_s(L2), L3);
					}
				} else {
					if (L6) {
						@SendMessage(131072, f_3df_a1_s(L2), L3);
					} else {
						@SendMessage(0, f_3df_a1_s(L2), L3);
					}
				}
				@SendMessage(L4 | 65536, f_3df_a1_s(L2));
			} else {
				@SendMessage(32768, f_3df_a1_s(L2));
			}
		}
	}
}

string f_3df_a1_s(int a0)
{
	if (a0 + 1 < 10) {
		return "slot0" + (a0 + 1);
	}
