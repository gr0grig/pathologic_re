...
// external
// string GetPatrolPathPrefix(void) // "pt_d7q04_patrol"
// string GetHelpMessage(void) // d7q04_helpme
// string GetCorpseMessage(void) // d7q04_corpse
// string GetPatrolCenter(void) // pt_d7q04_bonfire
// int GetSimultaneousPatrolCount(void) // 3
...
bool MessageFilter(object actor, string strMessage)
{
	...
	if (strMessage == GetCorpseMessage()) {
		if (ChangeFSMStateToSearchEnemy(actor))
			return true;
	}
	else
	if (strMessage == GetHelpMessage()) {
...
bool SeeFilter(object actor) {
	if (actor->FuncExist("IsDead", 1) && IsDead(actor)) {
		if (HasProperty(actor, GetCorpseMessage()))
			return false;
			
		actor->SetProperty(GetCorpseMessage(), 1);
...
bool InitFSM(void) {
	...
	while (true) {
		scene->GetLocator(GetPatrolPathPrefix() + (g_iPatrolPointsCount + 1), bFound, vPos, vDir);
		if (!bFound)
			break;
	...
	@Trace("Pantrol point count: "  + g_iPatrolPointsCount);
	return true;
}
...
bool IsHandHeatLocked() {
	int iLocked;
	@GetVariable(GetPatrolCenter() + "_locked", iLocked);
	return (iLocked != 0);
}

void LockHandHeat(bool bLock) {
	@SetVariable(GetPatrolCenter() + "_locked", bLock ? 1 : 0);
}

bool IsPatrolLocked() {
	int iLocked;
	@GetVariable(GetPatrolPathPrefix() + "_locked", iLocked);
	return (iLocked >= GetSimultaneousPatrolCount());
}
...
void LockPatrol(bool bLock) {
	...
	@SetVariable(GetPatrolPathPrefix() + "_locked", iLocked);
}
		...
		while (true) {
			...
			if (c_iStatePatrol == g_iState) {
				...
				for (int i = 0; i < g_iPatrolPointsCount; ++i) {
					...
					scene->GetLocator(GetPatrolPathPrefix() + (iIndex + 1), bFound, vPos, vDir);
				...
				if (c_iStateAware == g_iState) {
					...
					if (c_iStateAware == g_iState) {
						...
						if (c_iStateAware == g_iState) {
							object scene;
							@GetMainOutdoorScene(scene);
							@BroadcastMessage(GetCorpseMessage(), self(), scene);
...
task TAttack : TAttackMeleeBase {
	...
	void OnTimer(int id) { 
		if (id == c_iAttackTimer) {
			object scene;
			@GetMainOutdoorScene(scene);
			@BroadcastMessage(GetHelpMessage(), m_Actor, scene);
			@Trace("helpme timer");
		}
	}
...
task TGotoBonfire : TGotoPoint {
	bool init(void) {
		...
		@GetMainOutdoorScene(scene);
		Vector vPos, vDir;
		scene->GetLocator(GetPatrolCenter(), bFound, vPos, vDir);
		if (!bFound)
			return false;
