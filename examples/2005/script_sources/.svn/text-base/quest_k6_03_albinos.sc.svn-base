...
task TAlbinosIdle : TIdleBaseLookAsync
{
	...
	void OnUnload(void) {
		TriggerActor("quest_k6_03", "abinos_unload");
		@Hold();
	}	
}
...
maintask TRun : TPointsFollower
{
	...
	void init(void) {
		...
		@GetVariable("pt_k6q03_albinos_path", m_iStartPoint);
		if (m_iStartPoint != -1)
			super.init("pt_k6q03_albinos_path", m_iStartPoint, true);
		TAlbinosIdle{};
	}
...
task TRetreat : TRetreatBase
{
	...
	void OnUnload(void) {
		TriggerActor("quest_k6_03", "albinos_retreated");
		@Hold();
	}
