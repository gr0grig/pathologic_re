include rat_base.sci

const float c_fDamageAmount = 0.02;
const float c_fDiseaseAmount = 0.05;
const float c_fRetreatStopDist = 1500;
const float c_fDeathRepChange = 0.01;

void RatDamage(object victim) {
	float fDamage = Damage(victim, c_fDamageAmount, c_iDamageTypePhysical);
	@ReportHit(victim, c_iHitTypeRat, fDamage, c_fDamageAmount);
	fDamage = Disease(victim, c_fDiseaseAmount);
	@ReportHit(victim, c_iHitTypeDisease, fDamage, c_fDiseaseAmount);
}

void HandleUse(object actor)
{
	object item;
	@CreateInvItem(item);
	item->SetItemName("rat_big");
	float fQuality;
	@rand(fQuality, 0.5, 1);
	item->SetProperty("quality", fQuality);
	PlayerPutItem(actor, item, 1);

	@RemoveActor(self());	
}
