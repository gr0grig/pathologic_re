...
0x11: Pop(1)
0x12: @@ GetItemCount(Stack[-7], Stack[-8]); Obj=19 // @poff=44
0x13: Pop(0)
0x14: Stack[-6] = (int) 0
0x15: Stack[-5] = (int) 0
...
0x18: @@ GetItem(Stack[-4], Stack[-5], Stack[-8]); Obj=19 // @poff=57
0x19: Pop(0)
0x1a: @@ GetItemID(Stack[-3]); Obj=4 // @poff=65
0x1b: Pop(0)
0x1c: Pop(0); Push((bool) Stack[-3] == Stack[-9])
0x1d: IF (Stack[-1] == 0) GOTO 0x26; Pop(1)

0x1e: @@ GetItemAmount(Stack[-2], Stack[-5], Stack[-8]); Obj=19 // @poff=75
0x1f: Pop(0)
0x20: Stack[-6] = Stack[-6] + Stack[-2]; Pop(0);
0x21: @@ RemoveItem(Stack[-5], Stack[-2], Stack[-8]); Obj=19 // @poff=89
0x22: Pop(0)
0x23: Push((int) -1)
...
0x2e: Pop(0)
0x2f: Push("bottle_water") // @poff=100
0x30: @@ SetItemName(Stack[-1]); Obj=2 // @poff=126
0x31: Pop(1)
0x32: PushEmpty(object, object, int)
0x33: Stack[-22] = Stack[-3]
0x34: Stack[-4] = Stack[-2]
0x35: Stack[-9] = Stack[-1]
0x36: Call2 0x5f

0x37: Pop(3)
0x38: Push("fill") // @poff=138
...
0x4b: Pop(0)
0x4c: @@ add(Stack[-3]); Obj=1 // @poff=148
0x4d: Pop(0)
0x4e: Push((int) 200)
...
0x55: Pop(0)
0x56: @@ add(Stack[-4]); Obj=1 // @poff=148
0x57: Pop(0)
0x58: @@ add(Stack[-3]); Obj=1 // @poff=148
0x59: Pop(0)
0x5a: Push((int) 3)
...
0x60: @@ GetItemID(Stack[-3]); Obj=8 // @poff=65
0x61: Pop(0)
0x62: Push("Category") // @poff=26
0x63: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0x64: Pop(1)
0x65: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7]); Obj=9 // @poff=152
0x66: Pop(0)
0x67: Pop(0); Push((bool) Stack[-1] == 0)
0x68: IF (Stack[-1] == 0) GOTO 0x6c; Pop(1)

0x69: @@ DropItems(Stack[-8], Stack[-7]); Obj=9 // @poff=160
0x6a: Pop(0)
0x6b: GOTO 0x71

0x6c: PushEmpty(int, int)
0x6d: Stack[-5] = Stack[-2]
