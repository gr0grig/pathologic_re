...
Strings:
	...
	A:IsOnGround
	W:nouse_container
// @pool_raw:73697a650067657400636c6561720065006e00610062006c0065006400000061006c007000680061005f00700069006c006c00730000005365744974656d4e616d65006164640049734f6e47726f756e64006e006f007500730065005f0063006f006e007400610069006e00650072000000
// @pool_encoding:utf8
...
0x49: Pop(0)
0x4a: Push("alpha_pills") // @poff=31
0x4b: @@ SetItemName(Stack[-1]); Obj=2 // @poff=55
0x4c: Pop(1)
0x4d: @@@ add(Stack[-1]); Obj=0 // @poff=67
0x4e: Pop(0)
0x4f: Stack[-1] = 0
0x50: Return(); Pop(6)
...
0x56: PushEmpty(bool)
0x57: Stack[-1] = (bool) 1
0x58: Pop(0); Push((bool) Stack[-6] == 0)
0x59: IF (Stack[-1] == 1) GOTO 0x60; Pop(1)
...
0x5e: IF (Stack[-1] == 1) GOTO 0x60; Pop(1)
...
0x62: @@ IsOnGround(Stack[-2]); Obj=5 // @poff=71
0x63: Pop(0)
0x64: Push(Stack[-2])
0x65: IF (Stack[-1] == 0) GOTO 0x6e; Pop(1)

0x66: EventDisable(0)
0x67: @ IsOverrideActive(Stack[-1])
0x68: Pop(0)
0x69: Pop(0); Push((bool) Stack[-1] == 0)
...
0x70: @ CreateObjectVector(Stack[-1])
0x71: Pop(0)
...
0x76: Push("nouse_container") // @poff=82
0x77: @ GetVariable(Stack[-1], Stack[-2])
0x78: Pop(1)
0x79: Stack[-3] = !Stack[-1]; Pop(0);
0x7a: Return(); Pop(2)
