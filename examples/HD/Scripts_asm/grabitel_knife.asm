...
Strings:
	...
	A:add
	W:god_mode
// @pool_raw:
// @pool_encoding:utf8
...
0x7: Pop(5)
0x8: Push((bool) 1)
0x9: @@ Enable(Stack[-1]); Obj=2 // @poff=0
0xa: Pop(1)
...
0xe: @ Attach(Stack[-1])
0xf: Pop(0)
0x10: Push("knife") // @poff=7
0x11: @ FindGeometry(Stack[-1], Stack[-2])
0x12: Pop(1)
0x13: Push(GlobalVars[1])
0x14: @@ SetRotation(Stack[-1]); Obj=2 // @poff=19
0x15: Pop(1)
0x16: Push((bool) 1)
0x17: @@ Enable(Stack[-1]); Obj=2 // @poff=0
0x18: Pop(1)
0x19: @ Hold()
0x1a: Pop(0)
0x1b: GOTO 0x19
...
0x28: Push((bool) 0)
0x29: @@ Enable(Stack[-1]); Obj=2 // @poff=0
0x2a: Pop(2)
...
0x35: IF (Stack[-1] == 1) GOTO 0x3c; Pop(1)

0x36: Push("GetScene") // @poff=31
0x37: Push((int) 1)
0x38: Pop(2); Push(IsFuncExist(Stack[-9], Stack[-2], Stack[-1]))
0x39: Pop(1); Push((bool) Stack[-1] == 0)
0x3a: IF (Stack[-1] == 1) GOTO 0x3c; Pop(1)

0x3b: Stack[-1] = (bool) 0
0x3c: IF (Stack[-1] == 1) GOTO 0x44; Pop(1)

0x3d: PushEmpty(bool, object)
0x3e: Stack[-8] = Stack[-1]
0x3f: Call2 0x11f

0x40: Pop(1)
0x41: Pop(1); Push((bool) Stack[-1] == 0)
0x42: IF (Stack[-1] == 1) GOTO 0x44; Pop(1)
...
0x47: Stack[-9] = Stack[-3]
0x48: Stack[-2] = (float) 0.8999999761581421
0x49: Stack[-1] = (int) 0
0x4a: Call2 0x8e

0x4b: Stack[-4] = Stack[-5]
0x4c: Pop(4)
...
0x5a: Push((int) 1)
0x5b: Push((float)0.8999999761581421)
...
0x79: Stack[-2] = "bullet" // @poff=69
0x7a: Return(); Pop(0)

0x7b: Stack[-2] = "phys" // @poff=83
0x7c: Return(); Pop(0)
...
0x8a: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=117
0x8b: Pop(0)
...
0x90: Stack[-18] = Stack[-2]
0x91: Stack[-1] = "health" // @poff=129
0x92: Call2 0x82
...
0x99: Stack[-18] = Stack[-2]
0x9a: Stack[-1] = "armor" // @poff=143
0x9b: Call2 0x82
...
0xa1: Push("armor") // @poff=143
0xa2: @@ GetProperty(Stack[-1], Stack[-7]); Obj=16 // @poff=155
0xa3: Pop(1)
0xa4: Push("armor_") // @poff=167
0xa5: PushEmpty(string, int)
0xa6: Stack[-16] = Stack[-1]
0xa7: Call2 0x6f

0xa8: Pop(1)
0xa9: Stack[-7] = Stack[-2] + Stack[-1]; Pop(2);
0xaa: PushEmpty(bool, object, string)
0xab: Stack[-18] = Stack[-2]
0xac: Stack[-8] = Stack[-1]
0xad: Call2 0x82
...
0xb3: @@ GetProperty(Stack[-5], Stack[-4]); Obj=15 // @poff=155
0xb4: Pop(0)
...
0xbb: Stack[-3] = Stack[-6]
0xbc: Pop(3)
0xbd: Push("health") // @poff=129
0xbe: @@ GetProperty(Stack[-1], Stack[-3]); Obj=16 // @poff=155
0xbf: Pop(1)
0xc0: Push((int) 1)
0xc1: Pop(1); Push(Stack[-1] - Stack[-4]);
0xc2: Stack[-2] = Stack[-15] * Stack[-1]; Pop(1);
0xc3: Push("health") // @poff=129
...
0xc9: Pop(3)
0xca: @@ SetProperty(Stack[-2], Stack[-1]); Obj=17 // @poff=181
0xcb: Pop(2)
0xcc: PushEmpty(bool, object)
0xcd: Stack[-17] = Stack[-1]
0xce: Call2 0x7d
...
0xd4: Pop(1)
0xd5: Stack[-1] = Stack[-16]
0xd6: Return(); Pop(12)

0xd7: PushEmpty(float, float)
0xd8: PushEmpty(bool, object, string)
0xd9: Stack[-6] = Stack[-2]
0xda: Stack[-1] = "health" // @poff=129
0xdb: Call2 0x82
...
0xf0: Push("health") // @poff=129
0xf1: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=155
0xf2: Pop(1)
0xf3: Push((float)0.0)
...
0xf7: @@ IsDead(Stack[-1]); Obj=3 // @poff=193
0xf8: Pop(0)
0xf9: Stack[-1] = Stack[-4]
0xfa: Return(); Pop(2)

0xfb: PushEmpty(object, object, object, object)
0xfc: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x110: Pop(0)
0x111: Pop(0); PushNull((bool) Stack[-2] == 0)
0x112: IF (Stack[-1] == 0) GOTO 0x115; Pop(1)

0x113: Stack[-6] = (bool) 0
0x114: Return(); Pop(4)

0x115: @@ GetScene(Stack[-1]); Obj=5 // @poff=214
0x116: Pop(0)
...
0x12a: Stack[-1] = "noaccess" // @poff=223
0x12b: Call2 0x82
...
0x131: Push("noaccess") // @poff=223
0x132: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=155
0x133: Pop(1)
0x134: Push((int) 0)
...
0x140: Stack[-2] = Stack[-3]
0x141: GOTO 0x143

0x142: Stack[-1] = Stack[-3]
0x143: Return(); Pop(0)
...
0x151: Pop(0)
0x152: @@ add(Stack[-3]); Obj=1 // @poff=241
0x153: Pop(0)
0x154: Push((int) 0)
0x155: Pop(1); Push((bool) Stack[-4] < Stack[-1])
0x156: IF (Stack[-1] == 0) GOTO 0x15b; Pop(1)

0x157: Push((float)0.699999988079071)
0x158: Push((int) 500)
0x159: @ RumblePlay(Stack[-2], Stack[-1])
0x15a: Pop(2)
0x15b: Push((int) 15)
0x15c: @ SendWorldWndMessage(Stack[-1], Stack[-2])
0x15d: Pop(1)
0x15e: Return(); Pop(2)

0x15f: Stack[-1] = 0
0x160: PushEmpty(bool, bool)
0x161: Push("god_mode") // @poff=245
0x162: @ GetVariable(Stack[-1], Stack[-2])
0x163: Pop(1)
