...
0x45: Push("remove_klara") // @poff=24
...
0x58: Push("cleanup") // @poff=72
0x59: @ Trigger(Stack[-1]T, Stack[-1])
0x5a: Pop(1)
0x5b: Push((int) 1)
0x5c: Pop(1); Push(Stack[0 + Tasks[-1].StackPointer] - Stack[-1]);
0x5d: Push((bool) 0)
0x5e: @@@ set(Stack[-2], Stack[-1]); Obj=3 // @poff=88
0x5f: Pop(2)
0x60: PushEmpty()
0x61: Call2 0xa8
...
0x67: @ RemoveActor(Stack[-1]T)
0x68: Pop(0)
0x69: Push((int) 1)
0x6a: Pop(1); Push(Stack[-8] - Stack[-1]);
0x6b: @@@ get(Stack[-4], Stack[-1]); Obj=3 // @poff=92
0x6c: Pop(1)
...
0x71: Stack[-9] = Stack[-1]
0x72: Call2 0x17f

0x73: Stack[-2] = Stack[-3]
0x74: Pop(2)
0x75: Push("Klara scene: ") // @poff=96
...
0x7e: Stack[-7] = Stack[-4]
0x7f: Push("pt_klara") // @poff=124
0x80: Stack[-4] = Stack[-1] + Stack[-13]; Pop(1);
0x81: Stack[-2] = "NPC_Klara" // @poff=142
0x82: Stack[-1] = "k2system_klara2.xml" // @poff=162
0x83: Call2 0xb9

0x84: Stack[1 + Tasks[-1].StackPointer] = Stack[-5]
0x85: Pop(5)
0x86: Push("day") // @poff=202
0x87: @@@ SetProperty(Stack[-1], Stack[-8]); Obj=1 // @poff=210
0x88: Pop(1)
0x89: PushEmpty(int)
0x8a: Stack[-8] = Stack[-1]
0x8b: Call2 0x1fa
...
0x8f: Push("ERROR: Klara2 scene was not found: ") // @poff=222
...
0x98: @@@ get(Stack[-2], Stack[-1]); Obj=2 // @poff=92
0x99: Pop(1)
0x9a: Push(Stack[-1])
...
0x9f: Push((bool) 1)
0xa0: @@@ set(Stack[-2], Stack[-1]); Obj=2 // @poff=88
0xa1: Pop(2)
0xa2: PushEmpty(int, float)
0xa3: Stack[-6] = Stack[-2]
0xa4: Stack[-5] = Stack[-1]
0xa5: Call2 0x1a5
...
0xad: @@@ get(Stack[-1], Stack[-2]); Obj=3 // @poff=92
0xae: Pop(0)
...
0xba: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=294
0xbb: Pop(0)
0xbc: Pop(0); Push((bool) Stack[-4] == 0)
0xbd: IF (Stack[-1] == 0) GOTO 0xc6; Pop(1)

0xbe: Push("Locator ") // @poff=305
0xbf: Pop(1); Push(Stack[-1] + Stack[-12]);
0xc0: Push(" doesn't exist") // @poff=323
...
0x10a: Stack[-5] = Stack[-1]
0x10b: Call2 0x16c

0x10c: Stack[-4] = Stack[-6]
0x10d: Pop(4)
0x10e: Return(); Pop(0)
...
0x148: Push("Can't retrieve diary root") // @poff=353
0x149: @ Trace(Stack[-1])
0x14a: Pop(1)
...
0x153: Stack[-1] = Stack[-4]
0x154: Pop(1)
0x155: @@ Find(Stack[-7], Stack[-2]); Obj=3 // @poff=405
0x156: Pop(0)
0x157: Pop(0); Push((bool) Stack[-2] == 0)
0x158: IF (Stack[-1] == 0) GOTO 0x15f; Pop(1)

0x159: Push("Can't find diary parent with id: ") // @poff=410
...
0x15f: @@ AddChild(Stack[-8]); Obj=2 // @poff=478
0x160: Pop(0)
0x161: Push((int) 7)
0x162: @ SendWorldWndMessage(Stack[-1])
0x163: Pop(1)
0x164: @@ GetCategory(Stack[-1]); Obj=8 // @poff=487
0x165: Pop(0)
...
0x171: Pop(1)
0x172: Stack[-1] = Stack[-6]
0x173: Return(); Pop(2)

0x174: PushEmpty()
0x175: Push("K2System") // @poff=499
...
0x183: Stack[-2] = "lc_House6_04" // @poff=517
0x184: Return(); Pop(0)
...
0x189: Stack[-2] = "r2_house3_03" // @poff=543
0x18a: Return(); Pop(0)
...
0x18f: Stack[-2] = "house1_se_04r" // @poff=569
0x190: Return(); Pop(0)
...
0x195: Stack[-2] = "r7_house3_02_i2" // @poff=597
0x196: Return(); Pop(0)
...
0x19b: Stack[-2] = "house5_15" // @poff=629
0x19c: Return(); Pop(0)
...
0x1a1: Stack[-2] = "r5_house2_01" // @poff=649
0x1a2: Return(); Pop(0)

0x1a3: Stack[-2] = "" // @poff=21
0x1a4: Return(); Pop(0)
...
0x1fa: PushEmpty()
0x1fb: Push("k2system_klara_day") // @poff=675
