...
Strings:
	...
	W:State
	W:agressive
// @pool_raw:
...
0x17: Push("Music time limit reached") // @poff=0
0x18: @ Trace(Stack[-1])
0x19: Pop(1)
0x1a: Push("") // @poff=47
...
0x57: IF (Stack[-1] == 1) GOTO 0x5c; Pop(1)

0x58: Push((int) 2)
0x59: Pop(1); Push((bool) Stack[-4] == Stack[-1])
0x5a: IF (Stack[-1] == 1) GOTO 0x5c; Pop(1)

0x5b: Stack[-1] = (bool) 0
0x5c: IF (Stack[-1] == 0) GOTO 0x60; Pop(1)

0x5d: Stack[-11] = "horror" // @poff=50
0x5e: Return(); Pop(10)
...
0x6e: Stack[-11] = "day" // @poff=70
0x6f: GOTO 0x71

0x70: Stack[-11] = "night" // @poff=78
0x71: Return(); Pop(10)

0x72: Stack[-11] = "general" // @poff=90
0x73: Return(); Pop(10)
...
0x78: Pop(1)
0x79: Push("music") // @poff=106
0x7a: @ HasMusicValue(Stack[-3], Stack[-0]T, Stack[-1])
0x7b: Pop(1)
0x7c: Push(Stack[-2])
0x7d: IF (Stack[-1] == 0) GOTO 0x89; Pop(1)

0x7e: Push("music") // @poff=106
0x7f: @ GetMusicValue(Stack[-2], Stack[-0]T, Stack[-1])
0x80: Pop(1)
0x81: PushEmpty(string, string, bool)
0x82: Stack[-3] = Stack[0 + Tasks[-1].StackPointer]
0x83: Stack[-4] = Stack[-2]
0x84: Stack[-8] = Stack[-1]
0x85: Call2 0x91
...
0x8b: Stack[-2] = "" // @poff=47
0x8c: Stack[-8] = Stack[-1]
0x8d: Call2 0x91
...
0x96: Push("Playing: '") // @poff=118
0x97: Pop(1); Push(Stack[-1] + Stack[-3]);
0x98: Push("' from category: ") // @poff=140
...
0xa8: Pop(2)
0xa9: Push("Chosen agressive: ") // @poff=176
0xaa: Pop(1); Push(Stack[-1] + Stack[2 + Tasks[-1].StackPointer]);
0xab: @ Trace(Stack[-1])
0xac: Pop(1)
0xad: Return(); Pop(0)

0xae: PushEmpty(bool, string, bool, string)
0xaf: Push("music") // @poff=106
0xb0: @ HasMusicValue(Stack[-3], Stack[-6], Stack[-1])
0xb1: Pop(1)
0xb2: Push(Stack[-2])
0xb3: IF (Stack[-1] == 0) GOTO 0xb9; Pop(1)

0xb4: Push("music") // @poff=106
