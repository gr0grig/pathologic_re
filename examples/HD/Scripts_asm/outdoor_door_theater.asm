...
Strings:
	...
	W:NightMasks
	W:.wmv
// @pool_raw:
...
0x1f: Push("door_closed") // @poff=0
0x20: @ PlaySound(Stack[-1])
0x21: Pop(1)
0x22: Push("Door is locked") // @poff=24
0x23: @ Trace(Stack[-1])
0x24: Pop(1)
0x25: Return(); Pop(16)
...
0x29: Stack[-11] = Stack[-2]
0x2a: Stack[-1] = "@" // @poff=54
0x2b: Call2 0x62

0x2c: Stack[-3] = Stack[-10]
0x2d: Pop(3)
0x2e: Push((int) -1)
0x2f: Pop(1); Push((bool) Stack[-8] == Stack[-1])
0x30: IF (Stack[-1] == 0) GOTO 0x35; Pop(1)

0x31: Push("Wrong door name") // @poff=58
0x32: @ Trace(Stack[-1])
0x33: Pop(1)
0x34: Return(); Pop(16)

0x35: PushEmpty(string, string, int, int)
0x36: Stack[-12] = Stack[-3]
0x37: Stack[-2] = (int) 0
0x38: Stack[-11] = Stack[-1]
0x39: Call2 0x6c

0x3a: Stack[-4] = Stack[-10]
0x3b: Pop(4)
0x3c: Push("pt_") // @poff=90
0x3d: PushEmpty(string, string, int)
0x3e: Stack[-12] = Stack[-2]
0x3f: Push((int) 1)
...
0x45: Pop(0)
0x46: Pop(0); PushNull((bool) Stack[-4] == 0)
0x47: IF (Stack[-1] == 0) GOTO 0x4f; Pop(1)

0x48: Push("Destination scene '") // @poff=98
0x49: Pop(1); Push(Stack[-1] + Stack[-7]);
0x4a: Push("' not found") // @poff=138
0x4b: Pop(2); Push(Stack[-2] + Stack[-1]);
0x4c: @ Trace(Stack[-1])
0x4d: Pop(1)
0x4e: Return(); Pop(16)

0x4f: @@ GetLocator(Stack[-5], Stack[-3], Stack[-2], Stack[-1]); Obj=4 // @poff=162
0x50: Pop(0)
0x51: Push(Stack[-3])
0x52: IF (Stack[-1] == 0) GOTO 0x59; Pop(1)

0x53: Push("door_open") // @poff=173
...
0x59: Push("Locator ") // @poff=193
0x5a: Pop(1); Push(Stack[-1] + Stack[-6]);
0x5b: Push(" not found in destination scene ") // @poff=211
...
0x98: Push("Performance") // @poff=277
