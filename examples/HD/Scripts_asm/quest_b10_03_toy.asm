...
Strings:
	...
	A:AddChild
	A:GetCategory
// @pool_raw:
...
0x26: Pop(0)
0x27: @@ RemoveStationaryActor(Stack[-1]); Obj=2 // @poff=43
0x28: Pop(1)
0x29: PushEmpty()
0x2a: Call2 0xa4
...
0x3c: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=89
0x3d: Pop(0)
0x3e: Stack[-1] = Stack[-5]
0x3f: Return(); Pop(2)

0x40: PushEmpty(bool, bool)
0x41: @@ IsDead(Stack[-1]); Obj=3 // @poff=101
0x42: Pop(0)
0x43: Stack[-1] = Stack[-4]
0x44: Return(); Pop(2)

0x45: PushEmpty(object, object, object, object)
0x46: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x5a: Pop(0)
0x5b: Pop(0); PushNull((bool) Stack[-2] == 0)
0x5c: IF (Stack[-1] == 0) GOTO 0x5f; Pop(1)

0x5d: Stack[-6] = (bool) 0
0x5e: Return(); Pop(4)

0x5f: @@ GetScene(Stack[-1]); Obj=5 // @poff=122
0x60: Pop(0)
...
0x74: Stack[-1] = "noaccess" // @poff=131
0x75: Call2 0x34
...
0x7b: Push("noaccess") // @poff=131
0x7c: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=149
0x7d: Pop(1)
0x7e: Push((int) 0)
...
0x89: Pop(0)
0x8a: @@ add(Stack[-4]); Obj=1 // @poff=161
0x8b: Pop(0)
0x8c: @@ add(Stack[-3]); Obj=1 // @poff=161
0x8d: Pop(0)
0x8e: Push((int) 3)
...
0x94: @@ GetItemID(Stack[-3]); Obj=8 // @poff=165
0x95: Pop(0)
0x96: Push("Category") // @poff=175
0x97: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0x98: Pop(1)
0x99: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7]); Obj=9 // @poff=193
0x9a: Pop(0)
0x9b: Push(Stack[-1])
0x9c: IF (Stack[-1] == 0) GOTO 0xa2; Pop(1)

0x9d: PushEmpty(int, int)
0x9e: Stack[-5] = Stack[-2]
0x9f: Stack[-9] = Stack[-1]
0xa0: Call2 0x87

0xa1: Pop(2)
0xa2: Stack[-1] = Stack[-10]
0xa3: Return(); Pop(6)
...
0xb6: Push("Can't retrieve diary root") // @poff=201
0xb7: @ Trace(Stack[-1])
0xb8: Pop(1)
0xb9: Stack[-3] = (bool) 0
0xba: Return(); Pop(2)
...
0xc2: Pop(1)
0xc3: @@ Find(Stack[-7], Stack[-2]); Obj=3 // @poff=253
0xc4: Pop(0)
0xc5: Pop(0); Push((bool) Stack[-2] == 0)
0xc6: IF (Stack[-1] == 0) GOTO 0xcd; Pop(1)

0xc7: Push("Can't find diary parent with id: ") // @poff=258
...
0xcd: @@ AddChild(Stack[-8]); Obj=2 // @poff=326
0xce: Pop(0)
0xcf: Push((int) 7)
0xd0: @ SendWorldWndMessage(Stack[-1])
0xd1: Pop(1)
0xd2: @@ GetCategory(Stack[-1]); Obj=8 // @poff=335
0xd3: Pop(0)
