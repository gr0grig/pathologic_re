...
Strings:
	...
	A:GetCategory
	W:branch
// @pool_raw:
// @pool_encoding:utf8
...
0x26c: Push("Can't find lsh animation : ") // @poff=206
...
0x27b: Push("Can't find lsh animation : ") // @poff=206
...
0x281: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=192
0x282: Pop(0)
...
0x295: Push((float)9.999999974752427e-07)
...
0x2a3: Pop(0)
0x2a4: Pop(0); PushNull((bool) Stack[-1] == 0)
...
0x2c9: Stack[-1] = "ui/NPC_Citizen2.png" // @poff=262
0x2ca: Return(); Pop(0)

0x2cb: Stack[-1] = "ui/NPC_Citizen2_b.png" // @poff=302
0x2cc: Return(); Pop(0)
...
0x2cf: PushEmpty()
0x2d0: Push("b11q02") // @poff=346
0x2d1: Push((int) 3)
0x2d2: @ SetVariable(Stack[-2], Stack[-1])
0x2d3: Pop(2)
0x2d4: PushEmpty(bool, string, string)
0x2d5: Stack[-2] = "quest_b11_02" // @poff=360
0x2d6: Stack[-1] = "teleport" // @poff=386
0x2d7: Call2 0x2a1
...
0x2df: Stack[-2] = "quest_b11_02" // @poff=360
0x2e0: Stack[-1] = "fail" // @poff=404
0x2e1: Call2 0x2a1
...
0x302: Push("Can't retrieve diary root") // @poff=414
0x303: @ Trace(Stack[-1])
0x304: Pop(1)
...
0x30e: Pop(1)
0x30f: @@ Find(Stack[-7], Stack[-2]); Obj=3 // @poff=466
0x310: Pop(0)
0x311: Pop(0); Push((bool) Stack[-2] == 0)
0x312: IF (Stack[-1] == 0) GOTO 0x319; Pop(1)

0x313: Push("Can't find diary parent with id: ") // @poff=471
...
0x319: @@ AddChild(Stack[-8]); Obj=2 // @poff=539
0x31a: Pop(0)
0x31b: Push((int) 7)
0x31c: @ SendWorldWndMessage(Stack[-1])
0x31d: Pop(1)
0x31e: @@ GetCategory(Stack[-1]); Obj=8 // @poff=548
0x31f: Pop(0)
