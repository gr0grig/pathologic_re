...
Strings:
	...
	A:SetMapParams
	W:branch
// @pool_raw:
// @pool_encoding:utf8
...
0x390: Push("Can't find lsh animation : ") // @poff=206
...
0x39f: Push("Can't find lsh animation : ") // @poff=206
...
0x3b9: Push((float)9.999999974752427e-07)
...
0x3d5: Stack[-1] = "ui/NPC_bmask.png" // @poff=262
0x3d6: Return(); Pop(0)

0x3d7: Stack[-1] = "ui/NPC_bmask_b.png" // @poff=296
0x3d8: Return(); Pop(0)
...
0x3f1: Stack[-2] = Stack[-5]
0x3f2: Return(); Pop(4)

0x3f3: PushEmpty()
0x3f4: Push("oob1Birdmask1_1") // @poff=334
0x3f5: Push((int) 1)
0x3f6: @ SetVariable(Stack[-2], Stack[-1])
0x3f7: Pop(2)
0x3f8: PushEmpty(bool, string, string)
0x3f9: Stack[-2] = "quest_b1_01" // @poff=366
0x3fa: Stack[-1] = "remove1" // @poff=390
0x3fb: Call2 0x3c5
...
0x403: Pop(1)
0x404: Stack[-2] = "pt_map_bigvlad" // @poff=406
0x405: Stack[-1] = (int) 2
0x406: Call2 0x42b
...
0x40a: Pop(0)
0x40b: @@ ShowMap(Stack[-1]); Obj=2 // @poff=436
0x40c: Pop(1)
0x40d: Return(); Pop(0)

0x40e: PushEmpty()
0x40f: PushEmpty(int, string)
0x410: Stack[-1] = "oob1Birdmask1_1" // @poff=334
0x411: Call2 0x3c0
...
0x41c: Pop(0)
0x41d: Pop(0); PushNull((bool) Stack[-2] == 0)
0x41e: IF (Stack[-1] == 0) GOTO 0x425; Pop(1)

0x41f: Push("Can't find main outdoor scene") // @poff=444
0x420: @ Trace(Stack[-1])
0x421: Pop(1)
0x422: Stack[-1] = 0
0x423: Stack[-1] = Stack[-5]
0x424: Return(); Pop(4)

0x425: @@ GetMap(Stack[-1]); Obj=2 // @poff=504
0x426: Pop(0)
...
0x42d: Pop(0)
0x42e: Pop(0); PushNull((bool) Stack[-2] == 0)
0x42f: IF (Stack[-1] == 0) GOTO 0x434; Pop(1)

0x430: Push("Can't find main outdoor scene") // @poff=444
0x431: @ Trace(Stack[-1])
0x432: Pop(1)
0x433: Return(); Pop(8)

0x434: @@ GetLocator(Stack[-10], Stack[-1], Stack[-4], Stack[-3]); Obj=2 // @poff=511
0x435: Pop(0)
0x436: Pop(0); Push((bool) Stack[-1] == 0)
0x437: IF (Stack[-1] == 0) GOTO 0x43e; Pop(1)

0x438: Push("Warning: outdoor scene locator ") // @poff=522
0x439: Pop(1); Push(Stack[-1] + Stack[-11]);
0x43a: Push(" doesnt exist") // @poff=586
0x43b: Pop(2); Push(Stack[-2] + Stack[-1]);
0x43c: @ Trace(Stack[-1])
0x43d: Pop(1)
0x43e: @@ GetMap(Stack[-11]); Obj=2 // @poff=504
0x43f: Pop(0)
0x440: Pop(0); PushNull((bool) Stack[-11] == 0)
0x441: IF (Stack[-1] == 0) GOTO 0x446; Pop(1)

0x442: Push("Can't find map") // @poff=614
0x443: @ Trace(Stack[-1])
0x444: Pop(1)
0x445: Return(); Pop(8)
...
0x448: @@ SetMapParams(Stack[-2], Stack[-1], Stack[-11]); Obj=13 // @poff=644
0x449: Pop(2)
0x44a: Return(); Pop(8)

0x44b: Stack[-2] = 0
0x44c: PushEmpty(int, int)
0x44d: Push("branch") // @poff=657
