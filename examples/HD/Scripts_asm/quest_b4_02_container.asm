...
Strings:
	...
	W:Category
	A:AddItem
// @pool_raw:
...
0x2: Stack[-2] = "quest_b4_02" // @poff=0
0x3: Stack[-1] = "ammo_or_samopal_taken" // @poff=24
0x4: Call2 0xc9
...
0x2c: Push("object") // @poff=84
0x2d: @ FindGeometry(Stack[-1], Stack[-2])
0x2e: Pop(1)
0x2f: Push((bool) 1)
0x30: @@ Enable(Stack[-1]); Obj=2 // @poff=98
0x31: Pop(1)
0x32: @ Hold()
0x33: Pop(0)
0x34: GOTO 0x32
...
0x5a: Push("HasProperty") // @poff=105
0x5b: Push((int) 2)
...
0x61: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=129
0x62: Pop(0)
0x63: Stack[-1] = Stack[-5]
0x64: Return(); Pop(2)

0x65: PushEmpty(bool, bool)
0x66: @@ IsDead(Stack[-1]); Obj=3 // @poff=141
0x67: Pop(0)
0x68: Stack[-1] = Stack[-4]
0x69: Return(); Pop(2)

0x6a: PushEmpty(object, object, object, object)
0x6b: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x7f: Pop(0)
0x80: Pop(0); PushNull((bool) Stack[-2] == 0)
0x81: IF (Stack[-1] == 0) GOTO 0x84; Pop(1)

0x82: Stack[-6] = (bool) 0
0x83: Return(); Pop(4)

0x84: @@ GetScene(Stack[-1]); Obj=5 // @poff=162
0x85: Pop(0)
...
0x99: Stack[-1] = "noaccess" // @poff=171
0x9a: Call2 0x59
...
0xa0: Push("noaccess") // @poff=171
0xa1: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=189
0xa2: Pop(1)
0xa3: Push((int) 0)
...
0xae: Pop(0)
0xaf: @@ add(Stack[-4]); Obj=1 // @poff=201
0xb0: Pop(0)
0xb1: @@ add(Stack[-3]); Obj=1 // @poff=201
0xb2: Pop(0)
0xb3: Push((int) 3)
...
0xb9: @@ GetItemID(Stack[-3]); Obj=8 // @poff=205
0xba: Pop(0)
0xbb: Push("Category") // @poff=215
0xbc: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0xbd: Pop(1)
0xbe: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7]); Obj=9 // @poff=233
0xbf: Pop(0)
0xc0: Push(Stack[-1])
0xc1: IF (Stack[-1] == 0) GOTO 0xc7; Pop(1)

0xc2: PushEmpty(int, int)
0xc3: Stack[-5] = Stack[-2]
0xc4: Stack[-9] = Stack[-1]
0xc5: Call2 0xac

0xc6: Pop(2)
0xc7: Stack[-1] = Stack[-10]
0xc8: Return(); Pop(6)

0xc9: PushEmpty(object, object)
0xca: @ FindActor(Stack[-1], Stack[-4])
0xcb: Pop(0)
0xcc: Pop(0); PushNull((bool) Stack[-1] == 0)
