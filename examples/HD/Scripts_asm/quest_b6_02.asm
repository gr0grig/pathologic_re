...
Strings:
	...
	A:size
	A:get
// @pool_raw:
...
0xa: @ Trace(Stack[-15])
0xb: Pop(0)
0xc: Push("place_klara") // @poff=0
0xd: Pop(1); Push((bool) Stack[-16] == Stack[-1])
0xe: IF (Stack[-1] == 0) GOTO 0x1c; Pop(1)

0xf: Push("cot_anna") // @poff=24
0x10: @ GetSceneByName(Stack[-8], Stack[-1])
0x11: Pop(1)
0x12: PushEmpty(object, object, string, string, string)
0x13: Stack[-12] = Stack[-4]
0x14: Stack[-3] = "pt_b6q02_klara" // @poff=42
0x15: Stack[-2] = "NPC_Klara" // @poff=72
0x16: Stack[-1] = "b6q02_klara.xml" // @poff=92
0x17: Call2 0xbf
...
0x1c: Push("init_maze") // @poff=124
0x1d: Pop(1); Push((bool) Stack[-16] == Stack[-1])
0x1e: IF (Stack[-1] == 0) GOTO 0x43; Pop(1)

0x1f: PushEmpty(string, bool)
0x20: Stack[-2] = "house_vlad@door2" // @poff=144
0x21: Stack[-1] = (bool) 0
0x22: Call2 0xd1

0x23: Pop(2)
0x24: Push("house_vlad") // @poff=178
0x25: @ GetSceneByName(Stack[-7], Stack[-1])
0x26: Pop(1)
0x27: PushEmpty(object, object, string, string, string)
0x28: Stack[-11] = Stack[-4]
0x29: Stack[-3] = "pt_prorok" // @poff=200
0x2a: Stack[-2] = "pers_krysa" // @poff=220
0x2b: Stack[-1] = "b6q02_prorok.xml" // @poff=242
0x2c: Call2 0xbf

0x2d: Stack[1 + Tasks[-1].StackPointer] = Stack[-5]
0x2e: Pop(5)
0x2f: Stack[-5] = (int) 1
0x30: Push("pt_fog") // @poff=276
0x31: Pop(1); Push(Stack[-1] + Stack[-6]);
0x32: @@ GetLocator(Stack[-1], Stack[-5], Stack[-4], Stack[-3]); Obj=7 // @poff=290
0x33: Pop(1)
...
0x37: Push("fog") // @poff=282
0x38: Push("fog_stat_indoor.xml") // @poff=301
0x39: @ AddActorByType(Stack[-3], Stack[-2], Stack[-8], Stack[-5], Stack[-4], Stack[-1])
0x3a: Pop(2)
0x3b: @@@ add(Stack[-1]); Obj=2 // @poff=341
0x3c: Pop(0)
0x3d: Stack[-1] = 0
0x3e: Push((int) 1)
...
0x4a: Pop(1)
0x4b: Stack[-2] = "house_vlad" // @poff=178
0x4c: Stack[-1] = "pt_maze_start" // @poff=367
0x4d: Call2 0xe2

0x4e: Pop(3)
0x4f: GOTO 0x5d

0x50: Push("fail") // @poff=395
...
0x7c: Push("cleanup") // @poff=437
0x7d: @ Trigger(Stack[-0]T, Stack[-1])
0x7e: Pop(1)
0x7f: Push( Stack[1 + Tasks[-1].StackPointer] )
0x80: IF (Stack[-1] == 0) GOTO 0x84; Pop(1)

0x81: Push("cleanup") // @poff=437
0x82: @ Trigger(Stack[-1]T, Stack[-1])
0x83: Pop(1)
0x84: PushEmpty(object, string)
0x85: Stack[-2] = Stack[2 + Tasks[-1].StackPointer]
0x86: Stack[-1] = "cleanup" // @poff=437
0x87: Call2 0xfa

0x88: Pop(2)
0x89: PushEmpty(string, bool)
0x8a: Stack[-2] = "house_vlad@door2" // @poff=144
0x8b: Stack[-1] = (bool) 1
0x8c: Call2 0xd1
...
0xc0: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=290
0xc1: Pop(0)
0xc2: Pop(0); Push((bool) Stack[-4] == 0)
0xc3: IF (Stack[-1] == 0) GOTO 0xcc; Pop(1)

0xc4: Push("Locator ") // @poff=467
0xc5: Pop(1); Push(Stack[-1] + Stack[-12]);
0xc6: Push(" doesn't exist") // @poff=485
...
0xd6: Push("Door ") // @poff=515
0xd7: Pop(1); Push(Stack[-1] + Stack[-5]);
0xd8: Push(" not found") // @poff=527
0xd9: Pop(2); Push(Stack[-2] + Stack[-1]);
0xda: @ Trace(Stack[-1])
0xdb: Pop(1)
0xdc: GOTO 0xe0

0xdd: Push("locked") // @poff=549
0xde: @@ SetProperty(Stack[-1], Stack[-4]); Obj=2 // @poff=563
0xdf: Pop(1)
0xe0: Return(); Pop(2)
...
0xe7: Pop(0)
0xe8: @@ GetLocator(Stack[-9], Stack[-3], Stack[-2], Stack[-1]); Obj=4 // @poff=290
0xe9: Pop(0)
0xea: Pop(0); Push((bool) Stack[-3] == 0)
0xeb: IF (Stack[-1] == 0) GOTO 0xf6; Pop(1)

0xec: Push("Teleport location '") // @poff=575
0xed: Pop(1); Push(Stack[-1] + Stack[-10]);
0xee: Push("' not found in scene '") // @poff=615
...
0xfd: @@ size(Stack[-3]); Obj=8 // @poff=661
0xfe: Pop(0)
0xff: Stack[-2] = (int) 0
0x100: Pop(0); Push((bool) Stack[-2] < Stack[-3])
0x101: IF (Stack[-1] == 0) GOTO 0x10c; Pop(1)

0x102: @@ get(Stack[-1], Stack[-2]); Obj=8 // @poff=666
0x103: Pop(0)
0x104: Push(Stack[-1])
