...
0x2: Pop(0)
0x3: Push("trigger_d10q01_fire") // @poff=0
0x4: Push("trigger_fire_damage.bin") // @poff=40
0x5: Push(CVector(0.0, 0.0, 0.0))
0x6: @ AddScriptedActor(Stack[-1]T, Stack[-3], Stack[-2], Stack[-9], Stack[-1])
0x7: Pop(3)
0x8: PushEmpty(bool)
0x9: Call2 0x81
...
0xd: @ Hold()
0xe: Pop(0)
0xf: Push("fire") // @poff=30
0x10: @ FindParticleSystem(Stack[-1], Stack[-6])
0x11: Pop(1)
0x12: Pop(0); PushNull((bool) Stack[-5] == 0)
0x13: IF (Stack[-1] == 0) GOTO 0x18; Pop(1)

0x14: Push("Can't find fire particle system") // @poff=88
0x15: @ Trace(Stack[-1])
0x16: Pop(1)
0x17: Return(); Pop(12)
...
0x30: Push((float)0.0)
0x31: @@ AddSource(Stack[-3], Stack[-2], Stack[-1]); Obj=7 // @poff=152
0x32: Pop(2)
0x33: Push((int) 10)
...
0x50: @ RemoveActor(Stack[-1]T)
0x51: Pop(0)
0x52: Return(); Pop(0)

0x53: PushEmpty(bool, bool)
0x54: Push("cleanup") // @poff=169
