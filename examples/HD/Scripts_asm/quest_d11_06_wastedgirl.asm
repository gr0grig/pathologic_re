...
Strings:
	...
	A:GetCategory
	W:branch
// @pool_raw:
// @pool_encoding:utf8
...
0x11b: Push("player") // @poff=156
0x11c: @ FindActor(Stack[-4]T, Stack[-1])
0x11d: Pop(1)
...
0x130: Pop(0); PushNull((bool) Stack[4 + Tasks[-1].StackPointer] == 0)
...
0x1bb: Pop(0)
0x1bc: @@ GetPosition(Stack[-2]); Obj=7 // @poff=180
0x1bd: Pop(0)
...
0x1c7: @@ GetPosition(Stack[-8]); Obj=20 // @poff=180
0x1c8: Pop(0)
0x1c9: @@ GetEyesHeight(Stack[-9]); Obj=20 // @poff=192
0x1ca: Pop(0)
...
0x228: Push("Can't find lsh animation : ") // @poff=206
...
0x237: Push("Can't find lsh animation : ") // @poff=206
...
0x251: Push((float)9.999999974752427e-07)
...
0x25f: Pop(0)
0x260: @@ HasItem(Stack[-2], Stack[-1]); Obj=6 // @poff=262
0x261: Pop(0)
...
0x266: Pop(0)
0x267: Pop(0); PushNull((bool) Stack[-1] == 0)
...
0x274: Stack[-1] = "ui/NPC_Citizen2.png" // @poff=270
0x275: Return(); Pop(0)

0x276: Stack[-1] = "ui/NPC_Citizen2_b.png" // @poff=310
0x277: Return(); Pop(0)
...
0x29b: Stack[-2] = "quest_d11_06" // @poff=368
0x29c: Stack[-1] = "place_maria" // @poff=394
0x29d: Call2 0x264
...
0x2a4: Stack[-5] = Stack[-2]
0x2a5: Stack[-1] = "d11q06KapellaBeads" // @poff=418
0x2a6: Call2 0x25d
...
0x2ab: Stack[-1] = "d11q06KlaraRing" // @poff=456
0x2ac: Call2 0x25d
...
0x2ce: Push("Can't retrieve diary root") // @poff=488
0x2cf: @ Trace(Stack[-1])
0x2d0: Pop(1)
...
0x2da: Pop(1)
0x2db: @@ Find(Stack[-7], Stack[-2]); Obj=3 // @poff=540
0x2dc: Pop(0)
0x2dd: Pop(0); Push((bool) Stack[-2] == 0)
0x2de: IF (Stack[-1] == 0) GOTO 0x2e5; Pop(1)

0x2df: Push("Can't find diary parent with id: ") // @poff=545
...
0x2e5: @@ AddChild(Stack[-8]); Obj=2 // @poff=613
0x2e6: Pop(0)
0x2e7: Push((int) 7)
0x2e8: @ SendWorldWndMessage(Stack[-1])
0x2e9: Pop(1)
0x2ea: @@ GetCategory(Stack[-1]); Obj=8 // @poff=622
0x2eb: Pop(0)
