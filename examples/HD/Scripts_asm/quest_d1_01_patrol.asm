...
Strings:
	...
	A:SetMapParams
	W:branch
// @pool_raw:
// @pool_encoding:utf8
...
0x269: Pop(0)
0x26a: @@ GetPosition(Stack[-2]); Obj=7 // @poff=196
0x26b: Pop(0)
...
0x275: @@ GetPosition(Stack[-8]); Obj=20 // @poff=196
0x276: Pop(0)
0x277: @@ GetEyesHeight(Stack[-9]); Obj=20 // @poff=208
0x278: Pop(0)
...
0x2a1: Pop(1)
0x2a2: Push(CvectorIndex(Stack[-4], 0))
0x2a3: Push(CvectorIndex(Stack[-5], 2))
...
0x2d6: Push("Can't find lsh animation : ") // @poff=222
0x2d7: Pop(1); Push(Stack[-1] + Stack[-8]);
0x2d8: @ Trace(Stack[-1])
0x2d9: Pop(1)
0x2da: Return(); Pop(6)
...
0x2e5: Push("Can't find lsh animation : ") // @poff=222
0x2e6: Pop(1); Push(Stack[-1] + Stack[-9]);
0x2e7: @ Trace(Stack[-1])
0x2e8: Pop(1)
0x2e9: Return(); Pop(6)

0x2ea: PushEmpty(float, cvector, float, cvector)
0x2eb: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=208
0x2ec: Pop(0)
0x2ed: Stack[-1] = CVector(0.0, 0.0, 0.0)
0x2ee: Push(CvectorIndex(Stack[-1], 1))
0x2ef: Stack[-3] = Stack[-1]
0x2f0: CvectorIndex(Stack[-2], 1) = Stack[-1];
0x2f1: Push("head") // @poff=186
...
0x303: Pop(0); Push(Stack[-3] | Stack[-3]);
0x304: Stack[-2] = Sqrt(Stack[-1]); Pop(1);
0x305: Push((float)9.999999974752427e-07)
...
0x30a: Stack[-4] = Stack[-3] / Stack[-1]; Pop(0);
0x30b: Return(); Pop(2)
...
0x317: Stack[-1] = "idle" // @poff=146
0x318: Push(Stack[-3])
0x319: IF (Stack[-1] == 0) GOTO 0x31b; Pop(1)

0x31a: Stack[-1] = Stack[-1] + Stack[-3]; Pop(0);
...
0x329: Push((int) 1)
0x32a: Stack[-3] = Stack[-3] + Stack[-1]; Pop(1);
0x32b: GOTO 0x31f
...
0x332: Stack[-1] = "ui/NPC_Citizen2.png" // @poff=278
0x333: Return(); Pop(0)

0x334: Stack[-1] = "ui/NPC_Citizen2_b.png" // @poff=318
0x335: Return(); Pop(0)
...
0x33f: Stack[-1] = Stack[-2]
0x340: Pop(1)
0x341: Push("d1q01PatrolGotoAlexandr") // @poff=374
0x342: Push("pt_map_alexandr") // @poff=422
0x343: Push((int) 1)
...
0x347: Pop(0)
0x348: @@ AddMark(Stack[-5], Stack[-4], Stack[-3], Stack[-2], Stack[-1]); Obj=6 // @poff=454
0x349: Pop(5)
0x34a: PushEmpty()
0x34b: Call2 0x36b
...
0x353: Stack[-1] = Stack[-4]
0x354: Pop(1)
0x355: Stack[-2] = "pt_map_alexandr" // @poff=422
0x356: Stack[-1] = (int) 2
0x357: Call2 0x3b2
...
0x35b: Pop(0)
0x35c: @@ ShowMap(Stack[-1]); Obj=2 // @poff=462
0x35d: Pop(1)
0x35e: Return(); Pop(0)

0x35f: PushEmpty()
0x360: PushEmpty(int, string)
0x361: Stack[-1] = "d1q01" // @poff=362
0x362: Call2 0x30c
...
0x37d: Push("Can't retrieve diary root") // @poff=470
0x37e: @ Trace(Stack[-1])
0x37f: Pop(1)
0x380: Stack[-3] = (bool) 0
0x381: Return(); Pop(2)

0x382: Stack[-1] = Stack[-3]
0x383: Return(); Pop(2)
...
0x389: Pop(1)
0x38a: @@ Find(Stack[-7], Stack[-2]); Obj=3 // @poff=522
0x38b: Pop(0)
0x38c: Pop(0); Push((bool) Stack[-2] == 0)
0x38d: IF (Stack[-1] == 0) GOTO 0x394; Pop(1)

0x38e: Push("Can't find diary parent with id: ") // @poff=527
0x38f: Pop(1); Push(Stack[-1] + Stack[-8]);
0x390: @ Trace(Stack[-1])
0x391: Pop(1)
0x392: Stack[-9] = (bool) 0
0x393: Return(); Pop(6)

0x394: @@ AddChild(Stack[-8]); Obj=2 // @poff=595
0x395: Pop(0)
0x396: Push((int) 7)
0x397: @ SendWorldWndMessage(Stack[-1])
0x398: Pop(1)
0x399: @@ GetCategory(Stack[-1]); Obj=8 // @poff=604
0x39a: Pop(0)
...
0x3a3: Pop(0)
0x3a4: Pop(0); PushNull((bool) Stack[-2] == 0)
0x3a5: IF (Stack[-1] == 0) GOTO 0x3ac; Pop(1)

0x3a6: Push("Can't find main outdoor scene") // @poff=616
0x3a7: @ Trace(Stack[-1])
0x3a8: Pop(1)
0x3a9: Stack[-1] = 0
0x3aa: Stack[-1] = Stack[-5]
0x3ab: Return(); Pop(4)

0x3ac: @@ GetMap(Stack[-1]); Obj=2 // @poff=676
0x3ad: Pop(0)
0x3ae: Stack[-1] = Stack[-5]
0x3af: Return(); Pop(4)
...
0x3b4: Pop(0)
0x3b5: Pop(0); PushNull((bool) Stack[-2] == 0)
0x3b6: IF (Stack[-1] == 0) GOTO 0x3bb; Pop(1)

0x3b7: Push("Can't find main outdoor scene") // @poff=616
0x3b8: @ Trace(Stack[-1])
0x3b9: Pop(1)
0x3ba: Return(); Pop(8)

0x3bb: @@ GetLocator(Stack[-10], Stack[-1], Stack[-4], Stack[-3]); Obj=2 // @poff=683
0x3bc: Pop(0)
0x3bd: Pop(0); Push((bool) Stack[-1] == 0)
0x3be: IF (Stack[-1] == 0) GOTO 0x3c5; Pop(1)

0x3bf: Push("Warning: outdoor scene locator ") // @poff=694
0x3c0: Pop(1); Push(Stack[-1] + Stack[-11]);
0x3c1: Push(" doesnt exist") // @poff=758
0x3c2: Pop(2); Push(Stack[-2] + Stack[-1]);
0x3c3: @ Trace(Stack[-1])
0x3c4: Pop(1)
0x3c5: @@ GetMap(Stack[-11]); Obj=2 // @poff=676
0x3c6: Pop(0)
0x3c7: Pop(0); PushNull((bool) Stack[-11] == 0)
0x3c8: IF (Stack[-1] == 0) GOTO 0x3cd; Pop(1)

0x3c9: Push("Can't find map") // @poff=786
0x3ca: @ Trace(Stack[-1])
0x3cb: Pop(1)
0x3cc: Return(); Pop(8)
...
0x3cf: @@ SetMapParams(Stack[-2], Stack[-1], Stack[-11]); Obj=13 // @poff=816
0x3d0: Pop(2)
0x3d1: Return(); Pop(8)

0x3d2: Stack[-2] = 0
0x3d3: PushEmpty(int, int)
0x3d4: Push("branch") // @poff=829
