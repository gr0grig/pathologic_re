...
0x2f: @@@ Remove(); Obj=0 // @poff=148
0x30: Pop(0)
0x31: GOTO 0x3f

0x32: Push("fail") // @poff=155
...
0x92: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=185
0x93: Pop(0)
0x94: Pop(0); Push((bool) Stack[-4] == 0)
0x95: IF (Stack[-1] == 0) GOTO 0x9e; Pop(1)

0x96: Push("Locator ") // @poff=196
0x97: Pop(1); Push(Stack[-1] + Stack[-12]);
0x98: Push(" doesn't exist") // @poff=214
...
0x9e: @@ AddStationaryActor(Stack[-1], Stack[-3], Stack[-2], Stack[-10], Stack[-9]); Obj=12 // @poff=244
0x9f: Pop(0)
