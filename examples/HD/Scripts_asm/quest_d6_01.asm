...
0x4e: Push("cleanup") // @poff=202
0x4f: @ Trigger(Stack[-2]T, Stack[-1])
0x50: Pop(1)
0x51: GOTO 0x73

0x52: Push("cleanup") // @poff=202
0x53: Pop(1); Push((bool) Stack[-10] == Stack[-1])
0x54: IF (Stack[-1] == 0) GOTO 0x66; Pop(1)

0x55: Push("d6q01") // @poff=392
...
0xa7: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=434
0xa8: Pop(0)
0xa9: Pop(0); Push((bool) Stack[-4] == 0)
0xaa: IF (Stack[-1] == 0) GOTO 0xb3; Pop(1)

0xab: Push("Locator ") // @poff=445
0xac: Pop(1); Push(Stack[-1] + Stack[-12]);
0xad: Push(" doesn't exist") // @poff=463
...
0xbd: Push("Door ") // @poff=493
0xbe: Pop(1); Push(Stack[-1] + Stack[-5]);
0xbf: Push(" not found") // @poff=505
0xc0: Pop(2); Push(Stack[-2] + Stack[-1]);
0xc1: @ Trace(Stack[-1])
0xc2: Pop(1)
0xc3: GOTO 0xc7

0xc4: Push("locked") // @poff=527
0xc5: @@ SetProperty(Stack[-1], Stack[-4]); Obj=2 // @poff=541
0xc6: Pop(1)
0xc7: Return(); Pop(2)
