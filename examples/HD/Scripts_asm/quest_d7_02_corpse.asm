...
Strings:
	...
	A:AddChild
	A:GetCategory
// @pool_raw:
...
0x1: Push("d7q02BirdBalahon") // @poff=0
0x2: Push((int) 1)
0x3: @ SetVariable(Stack[-2], Stack[-1])
0x4: Pop(2)
0x5: Push("d7q02") // @poff=34
...
0xf: Stack[-2] = "quest_d7_02" // @poff=46
0x10: Stack[-1] = "completed" // @poff=70
0x11: Call2 0x7d
...
0x22: Push("d7q02BirdBalahon") // @poff=0
...
0x7f: Pop(0)
0x80: Pop(0); PushNull((bool) Stack[-1] == 0)
...
0xa8: Push("Can't retrieve diary root") // @poff=122
0xa9: @ Trace(Stack[-1])
0xaa: Pop(1)
0xab: Stack[-3] = (bool) 0
0xac: Return(); Pop(2)
...
0xb4: Pop(1)
0xb5: @@ Find(Stack[-7], Stack[-2]); Obj=3 // @poff=174
0xb6: Pop(0)
0xb7: Pop(0); Push((bool) Stack[-2] == 0)
0xb8: IF (Stack[-1] == 0) GOTO 0xbf; Pop(1)

0xb9: Push("Can't find diary parent with id: ") // @poff=179
...
0xbf: @@ AddChild(Stack[-8]); Obj=2 // @poff=247
0xc0: Pop(0)
0xc1: Push((int) 7)
0xc2: @ SendWorldWndMessage(Stack[-1])
0xc3: Pop(1)
0xc4: @@ GetCategory(Stack[-1]); Obj=8 // @poff=256
0xc5: Pop(0)
