...
Strings:
	W:cot_eva
	W:pt_d8q02_doberman1
	W:pers_doberman
	W:d8q02_doberman.xml
	W:pt_d8q02_doberman2
	W:pt_d8q02_doberman3
	W:d8q02 dobermans are placed
	W:fail
	W:completed
	W:d8q02
	W:cleanup
	...
	W:Locator 
	W: doesn't exist
// @pool_raw:
...
0xc: Pop(2)
0xd: Push("cot_eva") // @poff=0
0xe: @ GetSceneByName(Stack[-2], Stack[-1])
0xf: Pop(1)
0x10: PushEmpty(object, object, string, string, string)
0x11: Stack[-6] = Stack[-4]
0x12: Stack[-3] = "pt_d8q02_doberman1" // @poff=16
0x13: Stack[-2] = "pers_doberman" // @poff=54
0x14: Stack[-1] = "d8q02_doberman.xml" // @poff=82
0x15: Call2 0x91
...
0x1a: Stack[-3] = "pt_d8q02_doberman2" // @poff=120
0x1b: Stack[-2] = "pers_doberman" // @poff=54
0x1c: Stack[-1] = "d8q02_doberman.xml" // @poff=82
0x1d: Call2 0x91
...
0x22: Stack[-3] = "pt_d8q02_doberman3" // @poff=158
0x23: Stack[-2] = "pers_doberman" // @poff=54
0x24: Stack[-1] = "d8q02_doberman.xml" // @poff=82
0x25: Call2 0x91

0x26: Stack[2 + Tasks[-1].StackPointer] = Stack[-5]
0x27: Pop(5)
0x28: Push("d8q02 dobermans are placed") // @poff=196
0x29: @ Trace(Stack[-1])
0x2a: Pop(1)
0x2b: PushEmpty(int)
0x2c: Stack[-1] = (int) 8
...
0x60: Push("cleanup") // @poff=292
0x61: @ Trigger(Stack[-0]T, Stack[-1])
0x62: Pop(1)
0x63: Push( Stack[1 + Tasks[-1].StackPointer] )
0x64: IF (Stack[-1] == 0) GOTO 0x68; Pop(1)

0x65: Push("cleanup") // @poff=292
0x66: @ Trigger(Stack[-1]T, Stack[-1])
0x67: Pop(1)
0x68: Push( Stack[2 + Tasks[-1].StackPointer] )
0x69: IF (Stack[-1] == 0) GOTO 0x6d; Pop(1)

0x6a: Push("cleanup") // @poff=292
0x6b: @ Trigger(Stack[-2]T, Stack[-1])
0x6c: Pop(1)
...
0x92: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=308
0x93: Pop(0)
0x94: Pop(0); Push((bool) Stack[-4] == 0)
0x95: IF (Stack[-1] == 0) GOTO 0x9e; Pop(1)

0x96: Push("Locator ") // @poff=319
0x97: Pop(1); Push(Stack[-1] + Stack[-12]);
0x98: Push(" doesn't exist") // @poff=337
...
0xb0: Stack[-5] = Stack[-1]
0xb1: Call2 0xb5

0xb2: Stack[-4] = Stack[-6]
0xb3: Pop(4)
0xb4: Return(); Pop(0)
