...
0x16: Pop(1)
0x17: @@@ clear(); Obj=0 // @poff=58
0x18: Pop(0)
0x19: Push("termitnik") // @poff=64
...
0x20: Stack[-7] = Stack[-4]
0x21: Push("pt_d8q04_morlok") // @poff=84
0x22: Push((int) 1)
0x23: Pop(1); Push(Stack[-8] + Stack[-1]);
0x24: Stack[-5] = Stack[-2] + Stack[-1]; Pop(2);
0x25: Stack[-2] = "pers_morlok" // @poff=116
0x26: Stack[-1] = "d8q04_morlok.xml" // @poff=140
0x27: Call2 0x95

0x28: Pop(4)
0x29: @@@ add(Stack[-1]); Obj=0 // @poff=174
0x2a: Pop(1)
0x2b: Push((int) 1)
...
0x3b: Stack[-1] = "attack" // @poff=190
0x3c: Call2 0xcd

0x3d: Pop(2)
0x3e: GOTO 0x4c

0x3f: Push("fail") // @poff=204
...
0x96: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=246
0x97: Pop(0)
0x98: Pop(0); Push((bool) Stack[-4] == 0)
0x99: IF (Stack[-1] == 0) GOTO 0xa2; Pop(1)

0x9a: Push("Locator ") // @poff=257
0x9b: Pop(1); Push(Stack[-1] + Stack[-12]);
0x9c: Push(" doesn't exist") // @poff=275
...
0xac: Push("Door ") // @poff=305
0xad: Pop(1); Push(Stack[-1] + Stack[-5]);
0xae: Push(" not found") // @poff=317
0xaf: Pop(2); Push(Stack[-2] + Stack[-1]);
0xb0: @ Trace(Stack[-1])
0xb1: Pop(1)
0xb2: GOTO 0xb6

0xb3: Push("locked") // @poff=339
0xb4: @@ SetProperty(Stack[-1], Stack[-4]); Obj=2 // @poff=353
0xb5: Pop(1)
0xb6: Return(); Pop(2)
...
0xbb: @@ size(Stack[-3]); Obj=7 // @poff=365
0xbc: Pop(0)
0xbd: Stack[-2] = (int) 0
0xbe: Pop(0); Push((bool) Stack[-2] < Stack[-3])
0xbf: IF (Stack[-1] == 0) GOTO 0xca; Pop(1)

0xc0: @@ get(Stack[-1], Stack[-2]); Obj=7 // @poff=370
0xc1: Pop(0)
0xc2: Push(Stack[-1])
...
0xd0: @@ size(Stack[-3]); Obj=8 // @poff=365
0xd1: Pop(0)
0xd2: Stack[-2] = (int) 0
0xd3: Pop(0); Push((bool) Stack[-2] < Stack[-3])
0xd4: IF (Stack[-1] == 0) GOTO 0xdf; Pop(1)

0xd5: @@ get(Stack[-1], Stack[-2]); Obj=8 // @poff=370
0xd6: Pop(0)
0xd7: Push(Stack[-1])
