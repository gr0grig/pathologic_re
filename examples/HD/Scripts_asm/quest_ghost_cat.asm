...
Strings:
	...
	W:ACHIEVEMENT_NOTICE
	A:GetPosition
	W:player
// @pool_raw:64003100470068006f00730074004300610074004800440050006100740068006f006c006f00670069006300000041004300480049004500560045004d0045004e0054005f004e004f0054004900430045000000476574506f736974696f6e0070006c0061007900650072000000
...
0x25: Pop(0); PushNull((bool) Stack[-6] == 0)
0x26: IF (Stack[-1] == 0) GOTO 0x28; Pop(1)

0x27: Return(); Pop(12)

0x28: PushEmpty(cvector, object)
0x29: Stack[-8] = Stack[-1]
0x2a: Call2 0x6b

0x2b: Stack[-2] = Stack[-5]
0x2c: Pop(2)
...
0x36: Stack[-1] = (int) 0
0x37: Push("d1GhostCatHDPathologic") // @poff=0
...
0x49: Stack[-1] = (bool) 1
0x4a: IF (Stack[-1] == 1) GOTO 0x56; Pop(1)
...
0x53: Stack[-1] = (bool) 1
0x54: IF (Stack[-1] == 1) GOTO 0x56; Pop(1)
...
0x61: Push("ACHIEVEMENT_NOTICE") // @poff=46
...
0x6c: @@ GetPosition(Stack[-1]); Obj=3 // @poff=84
0x6d: Pop(0)
0x6e: Stack[-1] = Stack[-4]
0x6f: Return(); Pop(2)

0x70: PushEmpty(object, object)
0x71: Push("player") // @poff=96
