...
// @pool_raw:
// @pool_encoding:utf8
...
0x20e: Stack[-2] = Stack[-5]
0x20f: Pop(2)
0x210: Return(); Pop(2)

0x211: Stack[-1] = 0
0x212: Push(CvectorIndex(Stack[0 + Tasks[-1].StackPointer], 0))
0x213: Push(CvectorIndex(Stack[0 + Tasks[-1].StackPointer], 2))
0x214: @ RotateAsync(Stack[-2], Stack[-1])
0x215: Pop(2)
0x216: Return(); Pop(0)

0x217: PushEmpty(object, bool, object, bool)
0x218: Push("player") // @poff=214
...
0x2f3: Pop(0)
0x2f4: @@ GetPosition(Stack[-2]); Obj=7 // @poff=228
0x2f5: Pop(0)
...
0x300: Pop(2)
0x301: Stack[-1] = Stack[-8]
0x302: Return(); Pop(6)

0x303: PushEmpty(cvector, cvector)
0x304: @@ GetPosition(Stack[-1]); Obj=3 // @poff=228
0x305: Pop(0)
0x306: PushEmpty(bool, cvector)
0x307: Stack[-3] = Stack[-1]
0x308: Call2 0x2f9

0x309: Stack[-2] = Stack[-6]
0x30a: Pop(2)
0x30b: Return(); Pop(2)
...
0x312: @@ GetPosition(Stack[-8]); Obj=20 // @poff=228
0x313: Pop(0)
0x314: @@ GetEyesHeight(Stack[-9]); Obj=20 // @poff=240
0x315: Pop(0)
...
0x381: Push("voice_common") // @poff=264
0x382: Push((int) 1)
...
0x389: Push("voice_common") // @poff=264
0x38a: Push((int) 0)
...
0x39c: Push("voice_common") // @poff=264
0x39d: Push((int) 1)
...
0x3aa: @@ HasProperty(Stack[-1], Stack[-4]); Obj=12 // @poff=294
0x3ab: Pop(1)
...
0x3d5: @@ HasProperty(Stack[-1], Stack[-4]); Obj=12 // @poff=294
0x3d6: Pop(1)
...
0x3ea: @@ GetProperty(Stack[-1], Stack[-2]); Obj=12 // @poff=306
0x3eb: Pop(1)
0x3ec: PushEmpty(bool, string)
0x3ed: Stack[-3] = Stack[-1]
0x3ee: Call2 0x411

0x3ef: Stack[-2] = Stack[-14]
0x3f0: Pop(2)
0x3f1: Return(); Pop(10)
...
0x3fd: Push("Can't find lsh animation : ") // @poff=322
...
0x40c: Push("Can't find lsh animation : ") // @poff=322
...
0x43e: Pop(0)
0x43f: Pop(0); PushNull((bool) Stack[-1] == 0)
...
0x467: Stack[-2] = Stack[-5]
0x468: Return(); Pop(4)

0x469: PushEmpty()
0x46a: Push("k11q01") // @poff=378
0x46b: Push((int) 7)
...
0x477: Stack[-2] = "quest_k11_01" // @poff=392
0x478: Stack[-1] = "fail" // @poff=418
0x479: Call2 0x43c
...
0x4a7: Push("Can't retrieve diary root") // @poff=428
0x4a8: @ Trace(Stack[-1])
0x4a9: Pop(1)
...
0x4b3: Pop(1)
0x4b4: @@ Find(Stack[-7], Stack[-2]); Obj=3 // @poff=480
0x4b5: Pop(0)
0x4b6: Pop(0); Push((bool) Stack[-2] == 0)
0x4b7: IF (Stack[-1] == 0) GOTO 0x4be; Pop(1)

0x4b8: Push("Can't find diary parent with id: ") // @poff=485
...
0x4be: @@ AddChild(Stack[-8]); Obj=2 // @poff=553
0x4bf: Pop(0)
0x4c0: Push((int) 7)
0x4c1: @ SendWorldWndMessage(Stack[-1])
0x4c2: Pop(1)
0x4c3: @@ GetCategory(Stack[-1]); Obj=8 // @poff=562
0x4c4: Pop(0)
...
0x4e0: Stack[-1] = "ui/NPC_Burah.png" // @poff=588
0x4e1: Return(); Pop(0)

0x4e2: Stack[-1] = "ui/NPC_Burah_b.png" // @poff=622
0x4e3: Return(); Pop(0)
