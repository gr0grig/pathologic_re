...
// @pool_raw:
// @pool_encoding:utf8
...
0x16a: Stack[-8] = Stack[-2]
0x16b: Stack[-7] = Stack[-1]
0x16c: Call2 0x1a4
...
0x24b: Pop(0)
0x24c: @@ GetPosition(Stack[-2]); Obj=7 // @poff=196
0x24d: Pop(0)
...
0x257: @@ GetPosition(Stack[-8]); Obj=20 // @poff=196
0x258: Pop(0)
0x259: @@ GetEyesHeight(Stack[-9]); Obj=20 // @poff=208
0x25a: Pop(0)
...
0x2b8: Push("Can't find lsh animation : ") // @poff=222
...
0x2c7: Push("Can't find lsh animation : ") // @poff=222
...
0x2cd: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=208
0x2ce: Pop(0)
...
0x2e3: Stack[-1] = 0
0x2e4: PushEmpty(float, float)
0x2e5: Pop(0); Push(Stack[-3] | Stack[-3]);
0x2e6: Stack[-2] = Sqrt(Stack[-1]); Pop(1);
0x2e7: Push((float)9.999999974752427e-07)
0x2e8: Pop(1); Push((bool) Stack[-2] < Stack[-1])
0x2e9: IF (Stack[-1] == 0) GOTO 0x2ec; Pop(1)

0x2ea: Stack[-4] = CVector(0.0, 0.0, 0.0)
0x2eb: Return(); Pop(2)

0x2ec: Stack[-4] = Stack[-3] / Stack[-1]; Pop(0);
0x2ed: Return(); Pop(2)

0x2ee: PushEmpty(int, int)
0x2ef: @ GetVariable(Stack[-3], Stack[-1])
0x2f0: Pop(0)
0x2f1: Stack[-1] = Stack[-4]
0x2f2: Return(); Pop(2)

0x2f3: PushEmpty(string, string)
0x2f4: Stack[-1] = "idle" // @poff=146
0x2f5: Push(Stack[-3])
0x2f6: IF (Stack[-1] == 0) GOTO 0x2f8; Pop(1)

0x2f7: Stack[-1] = Stack[-1] + Stack[-3]; Pop(0);
0x2f8: Stack[-1] = Stack[-4]
0x2f9: Return(); Pop(2)

0x2fa: PushEmpty(int, bool, int, bool)
0x2fb: Stack[-2] = (int) 0
0x2fc: Push("all") // @poff=138
0x2fd: PushEmpty(string, int)
0x2fe: Stack[-5] = Stack[-1]
0x2ff: Call2 0x2f3

0x300: Pop(1)
0x301: @ HasAnimation(Stack[-3], Stack[-2], Stack[-1])
0x302: Pop(2)
...
0x306: Push((int) 1)
0x307: Stack[-3] = Stack[-3] + Stack[-1]; Pop(1);
0x308: GOTO 0x2fc
...
0x30b: PushEmpty()
0x30c: Push("ook11KabakSoldier1") // @poff=278
0x30d: Push((int) 1)
...
0x31f: Stack[-1] = "ook11KabakSoldier1" // @poff=278
0x320: Call2 0x2ee
...
0x32a: Push("branch") // @poff=330
0x32b: @ GetVariable(Stack[-1], Stack[-2])
0x32c: Pop(1)
0x32d: Push((int) 0)
...
0x33a: Stack[-1] = (int) 515597
0x33b: Return(); Pop(0)

0x33c: Stack[-1] = (int) 514841
0x33d: Return(); Pop(0)

0x33e: Stack[-1] = "ui/NPC_Citizen2.png" // @poff=344
0x33f: Return(); Pop(0)

0x340: Stack[-1] = "ui/NPC_Citizen2_b.png" // @poff=384
0x341: Return(); Pop(0)
