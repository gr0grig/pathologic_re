...
Strings:
	...
	W:health
	W:cleanup
// @pool_raw:
...
0x39: Push("agony") // @poff=62
0x3a: Push(CVector(0.0, 40.0, 0.0))
0x3b: @ PlayGlobalSound(Stack[-2], Stack[-1])
0x3c: Pop(2)
0x3d: Push("all") // @poff=74
0x3e: Push("agony") // @poff=62
...
0x59: Stack[-8] = Stack[-2]
0x5a: Stack[-1] = (float) -0.30000001192092896
0x5b: Call2 0x10a

0x5c: Pop(3)
0x5d: Push("death") // @poff=82
0x5e: Push(CVector(0.0, 40.0, 0.0))
0x5f: @ PlayGlobalSound(Stack[-2], Stack[-1])
0x60: Pop(2)
0x61: Push("all") // @poff=74
0x62: Push("agony_die") // @poff=94
0x63: @ PlayAnimation(Stack[-2], Stack[-1])
0x64: Pop(2)
0x65: @ WaitForAnimEnd()
0x66: Pop(0)
0x67: Push("all") // @poff=74
0x68: Push("agony_die") // @poff=94
...
0x94: Push("HasProperty") // @poff=114
0x95: Push((int) 2)
...
0x9b: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=138
0x9c: Pop(0)
0x9d: Stack[-1] = Stack[-5]
0x9e: Return(); Pop(2)

0x9f: PushEmpty(float, float)
0xa0: PushEmpty(bool, object, string)
0xa1: Stack[-10] = Stack[-2]
0xa2: Stack[-9] = Stack[-1]
0xa3: Call2 0x93
...
0xc4: @@ GetPosition(Stack[-5]); Obj=15 // @poff=174
0xc5: Pop(0)
...
0xd5: Stack[-1] = "fhit" // @poff=186
0xd6: GOTO 0xd8

0xd7: Stack[-1] = "bhit" // @poff=196
0xd8: Push("hit_react") // @poff=206
0xd9: Push("1") // @poff=32
0xda: Pop(1); Push(Stack[-3] + Stack[-1]);
0xdb: Push("2") // @poff=226
...
0x124: Stack[-8] = Stack[-5]
0x125: Stack[-4] = "reputation" // @poff=276
...
0x12f: Pop(0)
0x130: @@ add(Stack[-3]); Obj=1 // @poff=298
0x131: Pop(0)
0x132: Push((int) 16)
...
0x139: Pop(0)
0x13a: Pop(0); PushNull((bool) Stack[-1] == 0)
...
0x152: Stack[-11] = Stack[-5]
0x153: Stack[-10] = Stack[-4]
0x154: Stack[-9] = Stack[-3]
0x155: Stack[-7] = Stack[-2]
