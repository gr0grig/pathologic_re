...
// @pool_raw:
// @pool_encoding:utf8
...
0x2e6: Pop(0)
0x2e7: Push((bool) 1)
0x2e8: @ CameraTransit(Stack[-4], Stack[-6], Stack[-1])
0x2e9: Pop(1)
0x2ea: Push(CvectorIndex(Stack[-4], 0))
0x2eb: Push(CvectorIndex(Stack[-5], 2))
0x2ec: @ Rotate(Stack[-2], Stack[-1])
0x2ed: Pop(2)
...
0x2f9: @ LookAsyncCamera(Stack[-1])
0x2fa: Pop(1)
0x2fb: @ CameraWaitForPlayFinish()
0x2fc: Pop(0)
0x2fd: @ ResumeWorld()
0x2fe: Pop(0)
0x2ff: Stack[-21] = (bool) 1
0x300: Return(); Pop(18)

0x301: PushEmpty(bool, bool)
0x302: Push((bool) 1)
0x303: @ CameraSwitchToNormal(Stack[-1])
0x304: Pop(1)
...
0x31e: Push("Can't find lsh animation : ") // @poff=222
...
0x32d: Push("Can't find lsh animation : ") // @poff=222
0x32e: Pop(1); Push(Stack[-1] + Stack[-9]);
0x32f: @ Trace(Stack[-1])
0x330: Pop(1)
0x331: Return(); Pop(6)

0x332: PushEmpty(float, cvector, float, cvector)
0x333: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=208
0x334: Pop(0)
0x335: Stack[-1] = CVector(0.0, 0.0, 0.0)
0x336: Push(CvectorIndex(Stack[-1], 1))
0x337: Stack[-3] = Stack[-1]
0x338: CvectorIndex(Stack[-2], 1) = Stack[-1];
0x339: Push("head") // @poff=186
0x33a: @ LookAsync(Stack[-6], Stack[-1], Stack[-2])
0x33b: Pop(1)
0x33c: Return(); Pop(4)
...
0x34b: Pop(0); Push(Stack[-3] | Stack[-3]);
0x34c: Stack[-2] = Sqrt(Stack[-1]); Pop(1);
0x34d: Push((float)9.999999974752427e-07)
0x34e: Pop(1); Push((bool) Stack[-2] < Stack[-1])
0x34f: IF (Stack[-1] == 0) GOTO 0x352; Pop(1)

0x350: Stack[-4] = CVector(0.0, 0.0, 0.0)
0x351: Return(); Pop(2)

0x352: Stack[-4] = Stack[-3] / Stack[-1]; Pop(0);
0x353: Return(); Pop(2)

0x354: PushEmpty(int, int)
0x355: @ GetVariable(Stack[-3], Stack[-1])
0x356: Pop(0)
0x357: Stack[-1] = Stack[-4]
0x358: Return(); Pop(2)

0x359: PushEmpty()
0x35a: Push("ook12Littleboy1") // @poff=278
0x35b: Push((int) 1)
...
0x361: Stack[-1] = "ook12Littleboy1" // @poff=278
0x362: Call2 0x354
...
0x36c: Push("branch") // @poff=310
0x36d: @ GetVariable(Stack[-1], Stack[-2])
0x36e: Pop(1)
0x36f: Push((int) 0)
...
0x37c: Stack[-1] = (int) 515560
0x37d: Return(); Pop(0)

0x37e: Stack[-1] = (int) 503345
0x37f: Return(); Pop(0)

0x380: Stack[-1] = "ui/NPC_Citizen3.png" // @poff=324
0x381: Return(); Pop(0)

0x382: Stack[-1] = "ui/NPC_Citizen3_b.png" // @poff=364
0x383: Return(); Pop(0)

0x384: Stack[-1] = (bool) 0
0x385: Return(); Pop(0)

0x386: PushEmpty(string, string)
0x387: Stack[-1] = "idle" // @poff=146
0x388: Push(Stack[-3])
0x389: IF (Stack[-1] == 0) GOTO 0x38b; Pop(1)

0x38a: Stack[-1] = Stack[-1] + Stack[-3]; Pop(0);
0x38b: Stack[-1] = Stack[-4]
0x38c: Return(); Pop(2)

0x38d: PushEmpty(int, bool, int, bool)
0x38e: Stack[-2] = (int) 0
0x38f: Push("all") // @poff=138
0x390: PushEmpty(string, int)
0x391: Stack[-5] = Stack[-1]
0x392: Call2 0x386

0x393: Pop(1)
0x394: @ HasAnimation(Stack[-3], Stack[-2], Stack[-1])
0x395: Pop(2)
0x396: Pop(0); Push((bool) Stack[-1] == 0)
0x397: IF (Stack[-1] == 0) GOTO 0x399; Pop(1)

0x398: GOTO 0x39c

0x399: Push((int) 1)
0x39a: Stack[-3] = Stack[-3] + Stack[-1]; Pop(1);
0x39b: GOTO 0x38f

0x39c: Stack[-2] = Stack[-5]
0x39d: Return(); Pop(4)
