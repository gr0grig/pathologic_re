...
0x11: Stack[-8] = Stack[-2]
0x12: Stack[-7] = Stack[-1]
0x13: Call2 0x4b
...
0x52: Push("player") // @poff=16
0x53: @ FindActor(Stack[-4]T, Stack[-1])
0x54: Pop(1)
...
0x67: Pop(0); PushNull((bool) Stack[4 + Tasks[-1].StackPointer] == 0)
...
0xbc: Pop(0)
0xbd: Push("all") // @poff=40
0xbe: PushEmpty(string, int)
0xbf: Stack[-7] = Stack[-1]
0xc0: Call2 0x10e
...
0xf2: Pop(0)
0xf3: @@ GetPosition(Stack[-2]); Obj=7 // @poff=48
0xf4: Pop(0)
...
0xfe: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=60
0xff: Pop(0)
...
0x116: Stack[-2] = (int) 0
0x117: Push("all") // @poff=40
0x118: PushEmpty(string, int)
0x119: Stack[-5] = Stack[-1]
0x11a: Call2 0x10e
