...
0x31: @ Trace(Stack[-1])
0x32: Pop(0)
0x33: Push("abinos_unload") // @poff=106
...
0x45: Push("spawning albinos") // @poff=154
0x46: @ Trace(Stack[-1])
0x47: Pop(1)
0x48: PushEmpty()
0x49: Call2 0x52
...
0x59: Push("pt_k6q03_albinos_path") // @poff=188
0x5a: Push((int) -1)
0x5b: @ SetVariable(Stack[-2], Stack[-1])
0x5c: Pop(2)
0x5d: PushEmpty(object, object, string, string, string)
0x5e: Stack[-8] = Stack[-4]
0x5f: Push("pt_k6q03_albinos_path") // @poff=188
0x60: PushEmpty(int, string)
0x61: Stack[-1] = "pt_k6q03_albinos_path" // @poff=188
0x62: Call2 0x15f
...
0x67: Stack[-2] = "pers_albinos" // @poff=232
0x68: Stack[-1] = "k6q03_albinos.xml" // @poff=258
0x69: Call2 0xd2
...
0x6e: Stack[-1] = "pt_k6q03_albinos_path" // @poff=188
0x6f: Call2 0x134
...
0x72: @ Trace(Stack[-1])
0x73: Pop(0)
0x74: Push("pt_k6q03_albinos_path") // @poff=188
0x75: @ SetVariable(Stack[-1], Stack[-2])
0x76: Pop(1)
0x77: PushEmpty(object, object, string, string, string)
0x78: Stack[-8] = Stack[-4]
0x79: Push("pt_k6q03_albinos_path") // @poff=188
0x7a: Push((int) 1)
0x7b: Pop(1); Push(Stack[-8] + Stack[-1]);
0x7c: Stack[-5] = Stack[-2] + Stack[-1]; Pop(2);
0x7d: Stack[-2] = "pers_albinos" // @poff=232
0x7e: Stack[-1] = "k6q03_albinos.xml" // @poff=258
0x7f: Call2 0xe4
...
0xc8: @@ GetPosition(Stack[-1]); Obj=3 // @poff=301
0xc9: Pop(0)
...
0xd3: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=313
0xd4: Pop(0)
0xd5: Pop(0); Push((bool) Stack[-4] == 0)
0xd6: IF (Stack[-1] == 0) GOTO 0xdf; Pop(1)

0xd7: Push("Locator ") // @poff=324
0xd8: Pop(1); Push(Stack[-1] + Stack[-12]);
0xd9: Push(" doesn't exist") // @poff=342
...
0xdf: @@ AddStationaryActor(Stack[-1], Stack[-3], Stack[-2], Stack[-10], Stack[-9]); Obj=12 // @poff=372
0xe0: Pop(0)
...
0xe5: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=313
0xe6: Pop(0)
0xe7: Pop(0); Push((bool) Stack[-4] == 0)
0xe8: IF (Stack[-1] == 0) GOTO 0xf1; Pop(1)

0xe9: Push("Locator ") // @poff=324
0xea: Pop(1); Push(Stack[-1] + Stack[-12]);
0xeb: Push(" doesn't exist") // @poff=342
...
0xf1: Push((bool) 1)
0xf2: @@ AddStationaryActor(Stack[-2], Stack[-4], Stack[-3], Stack[-11], Stack[-10], Stack[-1]); Obj=13 // @poff=372
0xf3: Pop(1)
...
0x13e: @@ GetLocator(Stack[-1], Stack[-4], Stack[-3], Stack[-2]); Obj=6 // @poff=313
0x13f: Pop(1)
...
0x153: Stack[-4] = Stack[-6]
0x154: Stack[-2] = Stack[-7]
0x155: GOTO 0x139
...
0x166: @@ GetLocator(Stack[-1], Stack[-4], Stack[-3], Stack[-2]); Obj=6 // @poff=313
0x167: Pop(1)
