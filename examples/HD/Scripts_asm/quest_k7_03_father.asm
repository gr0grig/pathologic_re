GlobalVarCount = 1
	G_VAR_0 object 

Strings:
	A:SetNPCName
	A:SetNPCDescription
	A:SetPhoto
	A:SetPhoto2
	A:SetPlayerName
	A:IsDialogEnd
	A:GetReturnValue
	W:Neutral
	A:SetMessage
	A:ClearReplies
	A:AddReply
	W:all
	W:idle
	W:cleanup
	W:player
	W:head
	A:GetPosition
	W:HasProperty
	A:HasProperty
	A:GetProperty
	A:SetProperty
	A:GetEyesHeight
	W:Can't find lsh animation : 
	W:reputation
	A:add
	W:k7q03
	W:tvirin
	A:RemoveItemByType
	W:quest_k7_03
	W:fail
	W:playsound
	W:giveitem
	A:GetItemCountOfType
	W:Can't retrieve diary root
	A:Find
	W:Can't find diary parent with id: 
	A:AddChild
	A:GetCategory
	W:branch
	W:ui/NPC_Citizen2.png
	W:ui/NPC_Citizen2_b.png
// @pool_raw:
// @pool_encoding:utf8

Import:
	CreateDialog (1 args)
	IsOverrideActive (1 args)
	DoDialog (1 args)
	sync (0 args)
	StopDialog (1 args)
	lshWaitForAnimEnd (0 args)
	PlayAnimation (2 args)
	WaitForAnimEnd (0 args)
	lshStopAnimation (0 args)
	StopAnimation (0 args)
	rand (2 args)
	Sleep (2 args)
	RemoveActor (1 args)
	FindActor (2 args)
	SetTimer (2 args)
	KillTimer (1 args)
	UnlookAsync (1 args)
	irand (2 args)
	WaitForAnimEnd (1 args)
	ResetAAS (0 args)
	StopGroup0 (0 args)
	GetPosition (1 args)
	IsLoaded (1 args)
	GetEyesHeight (1 args)
	StopWorld (0 args)
	CameraTransit (3 args)
	Rotate (2 args)
	HasAnimationTrack (2 args)
	LookAsyncCamera (1 args)
	CameraWaitForPlayFinish (0 args)
	ResumeWorld (0 args)
	CameraSwitchToNormal (1 args)
	lshHasAnimation (2 args)
	lshGetAnimTimes (3 args)
	lshPlayAnimation (3 args)
	Trace (1 args)
	LookAsync (3 args)
	lshStopSpeech (0 args)
	self (1 args)
	GetVariable (2 args)
	SendWorldWndMessage (1 args)
	CreateFloatVector (1 args)
	SendWorldWndMessage (2 args)
	Trigger (2 args)
	HasAnimation (3 args)
	SetVariable (2 args)
	TriggerWorld (2 args)
	CreateDiaryEntry (4 args)
	GetDiaryRoot (1 args)
	SetDiarySection (1 args)

RunOp = 0x144
RunTask = 2

GlobalTasks: 
	GTASK_0 Vars = (object) Params = 2
	GTASK_1 Vars = (object, object, string, bool) Params = 2
		EVENT_11 Op = 0xad Vars = (int, int)
	GTASK_2 Vars = (int, int, bool, bool, object, bool, bool) Params = 0
		EVENT_0 Op = 0x14a Vars = (object)
		EVENT_26 Op = 0x17b Vars = (string)
		EVENT_6 Op = 0x183 Vars = ()
		EVENT_7 Op = 0x1c4 Vars = (int)


0x0: PushEmpty(object, bool, int, bool, object, bool, int, bool)
0x1: Stack[0 + Tasks[-1].StackPointer] = Stack[-9]
0x2: PushEmpty(bool, object, float)
0x3: Stack[-12] = Stack[-2]
0x4: Stack[-1] = (float) 70.0
0x5: Call2 0x26e

0x6: Pop(2)
0x7: Pop(1); Push((bool) Stack[-1] == 0)
0x8: IF (Stack[-1] == 0) GOTO 0xb; Pop(1)

0x9: Stack[-10] = (int) -2
0xa: Return(); Pop(8)

0xb: @ CreateDialog(Stack[-4])
0xc: Pop(0)
0xd: PushEmpty(int)
0xe: Call2 0x402

0xf: Pop(0)
0x10: @@ SetNPCName(Stack[-1]); Obj=5 // @poff=0
0x11: Pop(1)
0x12: PushEmpty(int)
0x13: Call2 0x400

0x14: Pop(0)
0x15: @@ SetNPCDescription(Stack[-1]); Obj=5 // @poff=11
0x16: Pop(1)
0x17: PushEmpty(string)
0x18: Call2 0x404

0x19: Pop(0)
0x1a: @@ SetPhoto(Stack[-1]); Obj=5 // @poff=29
0x1b: Pop(1)
0x1c: PushEmpty(string)
0x1d: Call2 0x406

0x1e: Pop(0)
0x1f: @@ SetPhoto2(Stack[-1]); Obj=5 // @poff=38
0x20: Pop(1)
0x21: PushEmpty(int)
0x22: Call2 0x3ef

0x23: Pop(0)
0x24: @@ SetPlayerName(Stack[-1]); Obj=5 // @poff=48
0x25: Pop(1)
0x26: Stack[-2] = (int) -1
0x27: @ IsOverrideActive(Stack[-3])
0x28: Pop(0)
0x29: Push(Stack[-3])
0x2a: IF (Stack[-1] == 0) GOTO 0x2d; Pop(1)

0x2b: Stack[-10] = (int) -2
0x2c: Return(); Pop(8)

0x2d: @ DoDialog(Stack[-4])
0x2e: Pop(0)
0x2f: PushEmpty(object, object)
0x30: Stack[-11] = Stack[-2]
0x31: Stack[-6] = Stack[-1]
0x32: Push(-2, 4); TaskCall(1)
0x33: Call2 0x4a

0x34: Pop(-2, 4); TaskReturn
0x35: Pop(2)
0x36: @@ IsDialogEnd(Stack[-1]); Obj=4 // @poff=62
0x37: Pop(0)
0x38: Pop(0); Push((bool) Stack[-1] == 0)
0x39: IF (Stack[-1] == 0) GOTO 0x3f; Pop(1)

0x3a: @ sync()
0x3b: Pop(0)
0x3c: @@ IsDialogEnd(Stack[-1]); Obj=4 // @poff=62
0x3d: Pop(0)
0x3e: GOTO 0x38

0x3f: PushEmpty(object)
0x40: Stack[-10] = Stack[-1]
0x41: Call2 0x2b3

0x42: Pop(1)
0x43: @ StopDialog(Stack[-4])
0x44: Pop(0)
0x45: @@ GetReturnValue(Stack[-2]); Obj=4 // @poff=74
0x46: Pop(0)
0x47: Stack[-2] = Stack[-10]
0x48: Return(); Pop(8)

0x49: Stack[-4] = 0
0x4a: PushEmpty()
0x4b: Stack[0 + Tasks[-1].StackPointer] = Stack[-1]
0x4c: Stack[1 + Tasks[-1].StackPointer] = Stack[-2]
0x4d: Stack[3 + Tasks[-1].StackPointer] = (bool)0
0x4e: Push((int) 1)
0x4f: IF (Stack[-1] == 0) GOTO 0x79; Pop(1)

0x50: PushEmpty(string)
0x51: Stack[-1] = "Neutral" // @poff=89
0x52: Call2 0x97

0x53: Pop(1)
0x54: Push((int) 526382)
0x55: @@@ SetMessage(Stack[-1]); Obj=0 // @poff=105
0x56: Pop(1)
0x57: @@@ ClearReplies(); Obj=0 // @poff=116
0x58: Pop(0)
0x59: PushEmpty(bool)
0x5a: Stack[-1] = (bool) 0
0x5b: PushEmpty(bool, object)
0x5c: Stack[-1] = Stack[1 + Tasks[-1].StackPointer]
0x5d: Call2 0x38b

0x5e: Pop(1)
0x5f: IF (Stack[-1] == 0) GOTO 0x66; Pop(1)

0x60: PushEmpty(bool, object)
0x61: Stack[-1] = Stack[1 + Tasks[-1].StackPointer]
0x62: Call2 0x397

0x63: Pop(1)
0x64: IF (Stack[-1] == 0) GOTO 0x66; Pop(1)

0x65: Stack[-1] = (bool) 1
0x66: IF (Stack[-1] == 0) GOTO 0x6c; Pop(1)

0x67: Push((int) 526383)
0x68: Push((int) 27664)
0x69: Push((int) 27663)
0x6a: @@@ AddReply(Stack[-3], Stack[-2], Stack[-1]); Obj=0 // @poff=129
0x6b: Pop(3)
0x6c: Push((int) 526386)
0x6d: Push((int) -1)
0x6e: Push((int) 27666)
0x6f: @@@ AddReply(Stack[-3], Stack[-2], Stack[-1]); Obj=0 // @poff=129
0x70: Pop(3)
0x71: Push((int) 528664)
0x72: Push((int) -1)
0x73: Push((int) 30075)
0x74: @@@ AddReply(Stack[-3], Stack[-2], Stack[-1]); Obj=0 // @poff=129
0x75: Pop(3)
0x76: GOTO 0x79

0x77: Return(); Pop(0)

0x78: GOTO 0x4e

0x79: PushEmpty(bool)
0x7a: Call2 0x408

0x7b: Pop(0)
0x7c: IF (Stack[-1] == 0) GOTO 0x88; Pop(1)

0x7d: @ lshWaitForAnimEnd()
0x7e: Pop(0)
0x7f: Push( Stack[3 + Tasks[-1].StackPointer] )
0x80: IF (Stack[-1] == 0) GOTO 0x82; Pop(1)

0x81: GOTO 0x87

0x82: PushEmpty(string)
0x83: Stack[-1] = Stack[2 + Tasks[-1].StackPointer]
0x84: Call2 0x2c5

0x85: Pop(1)
0x86: GOTO 0x7d

0x87: GOTO 0x96

0x88: Push("all") // @poff=138
0x89: Push("idle") // @poff=146
0x8a: @ PlayAnimation(Stack[-2], Stack[-1])
0x8b: Pop(2)
0x8c: @ WaitForAnimEnd()
0x8d: Pop(0)
0x8e: Push( Stack[3 + Tasks[-1].StackPointer] )
0x8f: IF (Stack[-1] == 0) GOTO 0x91; Pop(1)

0x90: GOTO 0x96

0x91: Push("all") // @poff=138
0x92: Push("idle") // @poff=146
0x93: @ PlayAnimation(Stack[-2], Stack[-1])
0x94: Pop(2)
0x95: GOTO 0x8c

0x96: Return(); Pop(0)

0x97: PushEmpty()
0x98: PushEmpty(bool)
0x99: Call2 0x408

0x9a: Pop(0)
0x9b: Pop(1); Push((bool) Stack[-1] == 0)
0x9c: IF (Stack[-1] == 0) GOTO 0x9e; Pop(1)

0x9d: Return(); Pop(0)

0x9e: Pop(0); Push((bool) Stack[-1] == Stack[2 + Tasks[-1].StackPointer])
0x9f: IF (Stack[-1] == 0) GOTO 0xa1; Pop(1)

0xa0: Return(); Pop(0)

0xa1: PushEmpty(string, bool)
0xa2: Stack[-3] = Stack[-2]
0xa3: Push("") // @poff=102
0xa4: Pop(1); Push((bool) Stack[-4] == Stack[-1])
0xa5: IF (Stack[-1] == 0) GOTO 0xa8; Pop(1)

0xa6: Stack[-1] = (bool) 0
0xa7: GOTO 0xa9

0xa8: Stack[-1] = (bool) 1
0xa9: Call2 0x2d5

0xaa: Pop(2)
0xab: Stack[2 + Tasks[-1].StackPointer] = Stack[-1]
0xac: Return(); Pop(0)

0xad: PushEmpty()
0xae: Push((int) 1)
0xaf: IF (Stack[-1] == 0) GOTO 0x143; Pop(1)

0xb0: PushEmpty()
0xb1: Call2 0x2ef

0xb2: Pop(0)
0xb3: Push((int) 27665)
0xb4: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xb5: IF (Stack[-1] == 0) GOTO 0xc0; Pop(1)

0xb6: PushEmpty(object, object)
0xb7: Stack[-2] = Stack[1 + Tasks[-1].StackPointer]
0xb8: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0xb9: Call2 0x36e

0xba: Pop(2)
0xbb: PushEmpty(object, object)
0xbc: Stack[-2] = Stack[1 + Tasks[-1].StackPointer]
0xbd: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0xbe: Call2 0x385

0xbf: Pop(2)
0xc0: Push((int) 30081)
0xc1: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0xc2: IF (Stack[-1] == 0) GOTO 0xcd; Pop(1)

0xc3: PushEmpty(object, object)
0xc4: Stack[-2] = Stack[1 + Tasks[-1].StackPointer]
0xc5: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0xc6: Call2 0x37b

0xc7: Pop(2)
0xc8: PushEmpty(object, object)
0xc9: Stack[-2] = Stack[1 + Tasks[-1].StackPointer]
0xca: Stack[-1] = Stack[0 + Tasks[-1].StackPointer]
0xcb: Call2 0x367

0xcc: Pop(2)
0xcd: Push((int) 27662)
0xce: Pop(1); Push((bool) Stack[-3] == Stack[-1])
0xcf: IF (Stack[-1] == 0) GOTO 0xf7; Pop(1)

0xd0: PushEmpty(string)
0xd1: Stack[-1] = "Neutral" // @poff=89
0xd2: Call2 0x97

0xd3: Pop(1)
0xd4: Push((int) 526382)
0xd5: @@@ SetMessage(Stack[-1]); Obj=0 // @poff=105
0xd6: Pop(1)
0xd7: @@@ ClearReplies(); Obj=0 // @poff=116
0xd8: Pop(0)
0xd9: PushEmpty(bool)
0xda: Stack[-1] = (bool) 0
0xdb: PushEmpty(bool, object)
0xdc: Stack[-1] = Stack[1 + Tasks[-1].StackPointer]
0xdd: Call2 0x38b

0xde: Pop(1)
0xdf: IF (Stack[-1] == 0) GOTO 0xe6; Pop(1)

0xe0: PushEmpty(bool, object)
0xe1: Stack[-1] = Stack[1 + Tasks[-1].StackPointer]
0xe2: Call2 0x397

0xe3: Pop(1)
0xe4: IF (Stack[-1] == 0) GOTO 0xe6; Pop(1)

0xe5: Stack[-1] = (bool) 1
0xe6: IF (Stack[-1] == 0) GOTO 0xec; Pop(1)

0xe7: Push((int) 526383)
0xe8: Push((int) 27664)
0xe9: Push((int) 27663)
0xea: @@@ AddReply(Stack[-3], Stack[-2], Stack[-1]); Obj=0 // @poff=129
0xeb: Pop(3)
0xec: Push((int) 526386)
0xed: Push((int) -1)
0xee: Push((int) 27666)
0xef: @@@ AddReply(Stack[-3], Stack[-2], Stack[-1]); Obj=0 // @poff=129
0xf0: Pop(3)
0xf1: Push((int) 528664)
0xf2: Push((int) -1)
0xf3: Push((int) 30075)
0xf4: @@@ AddReply(Stack[-3], Stack[-2], Stack[-1]); Obj=0 // @poff=129
0xf5: Pop(3)
0xf6: Return(); Pop(0)

0xf7: Push((int) 27664)
0xf8: Pop(1); Push((bool) Stack[-3] == Stack[-1])
0xf9: IF (Stack[-1] == 0) GOTO 0x10e; Pop(1)

0xfa: PushEmpty(string)
0xfb: Stack[-1] = "Neutral" // @poff=89
0xfc: Call2 0x97

0xfd: Pop(1)
0xfe: Push((int) 526384)
0xff: @@@ SetMessage(Stack[-1]); Obj=0 // @poff=105
0x100: Pop(1)
0x101: @@@ ClearReplies(); Obj=0 // @poff=116
0x102: Pop(0)
0x103: Push((int) 528665)
0x104: Push((int) 30078)
0x105: Push((int) 30076)
0x106: @@@ AddReply(Stack[-3], Stack[-2], Stack[-1]); Obj=0 // @poff=129
0x107: Pop(3)
0x108: Push((int) 528666)
0x109: Push((int) 30078)
0x10a: Push((int) 30077)
0x10b: @@@ AddReply(Stack[-3], Stack[-2], Stack[-1]); Obj=0 // @poff=129
0x10c: Pop(3)
0x10d: Return(); Pop(0)

0x10e: Push((int) 30078)
0x10f: Pop(1); Push((bool) Stack[-3] == Stack[-1])
0x110: IF (Stack[-1] == 0) GOTO 0x125; Pop(1)

0x111: PushEmpty(string)
0x112: Stack[-1] = "Neutral" // @poff=89
0x113: Call2 0x97

0x114: Pop(1)
0x115: Push((int) 528667)
0x116: @@@ SetMessage(Stack[-1]); Obj=0 // @poff=105
0x117: Pop(1)
0x118: @@@ ClearReplies(); Obj=0 // @poff=116
0x119: Pop(0)
0x11a: Push((int) 528668)
0x11b: Push((int) 30082)
0x11c: Push((int) 30079)
0x11d: @@@ AddReply(Stack[-3], Stack[-2], Stack[-1]); Obj=0 // @poff=129
0x11e: Pop(3)
0x11f: Push((int) 528669)
0x120: Push((int) -1)
0x121: Push((int) 30081)
0x122: @@@ AddReply(Stack[-3], Stack[-2], Stack[-1]); Obj=0 // @poff=129
0x123: Pop(3)
0x124: Return(); Pop(0)

0x125: Push((int) 30082)
0x126: Pop(1); Push((bool) Stack[-3] == Stack[-1])
0x127: IF (Stack[-1] == 0) GOTO 0x137; Pop(1)

0x128: PushEmpty(string)
0x129: Stack[-1] = "Neutral" // @poff=89
0x12a: Call2 0x97

0x12b: Pop(1)
0x12c: Push((int) 528670)
0x12d: @@@ SetMessage(Stack[-1]); Obj=0 // @poff=105
0x12e: Pop(1)
0x12f: @@@ ClearReplies(); Obj=0 // @poff=116
0x130: Pop(0)
0x131: Push((int) 526385)
0x132: Push((int) -1)
0x133: Push((int) 27665)
0x134: @@@ AddReply(Stack[-3], Stack[-2], Stack[-1]); Obj=0 // @poff=129
0x135: Pop(3)
0x136: Return(); Pop(0)

0x137: Stack[3 + Tasks[-1].StackPointer] = (bool)1
0x138: PushEmpty(bool)
0x139: Call2 0x408

0x13a: Pop(0)
0x13b: IF (Stack[-1] == 0) GOTO 0x13f; Pop(1)

0x13c: @ lshStopAnimation()
0x13d: Pop(0)
0x13e: GOTO 0x141

0x13f: @ StopAnimation()
0x140: Pop(0)
0x141: Return(); Pop(0)

0x142: GOTO 0xae

0x143: Return(); Pop(0)

0x144: PushEmpty(float, float)
0x145: Stack[-2] = (int) 300
0x146: Stack[-1] = (int) 100
0x147: Call2 0x155

0x148: Pop(2)
0x149: Return(); Pop(0)

0x14a: PushEmpty()
0x14b: PushEmpty()
0x14c: Call2 0x1db

0x14d: Pop(0)
0x14e: PushEmpty(int, object)
0x14f: Stack[-3] = Stack[-1]
0x150: Push(-2, 1); TaskCall(0)
0x151: Call2 0x0

0x152: Pop(-2, 1); TaskReturn
0x153: Pop(2)
0x154: Return(); Pop(0)

0x155: PushEmpty(float, bool, float, bool)
0x156: Stack[6 + Tasks[-1].StackPointer] = (bool)0
0x157: Push((int) 3)
0x158: @ rand(Stack[-3], Stack[-1])
0x159: Pop(1)
0x15a: Push((int) 3)
0x15b: Pop(1); Push(Stack[-3] + Stack[-1]);
0x15c: @ Sleep(Stack[-1], Stack[-2])
0x15d: Pop(1)
0x15e: Stack[6 + Tasks[-1].StackPointer] = (bool)1
0x15f: PushEmpty(float, float)
0x160: Stack[-8] = Stack[-2]
0x161: Stack[-7] = Stack[-1]
0x162: Call2 0x19a

0x163: Pop(2)
0x164: Stack[6 + Tasks[-1].StackPointer] = (bool)0
0x165: GOTO 0x157

0x166: Return(); Pop(4)

0x167: Stack[5 + Tasks[-1].StackPointer] = (bool)1
0x168: PushEmpty(bool)
0x169: Stack[-1] = (bool) 0
0x16a: PushEmpty(bool)
0x16b: Call2 0x269

0x16c: Pop(0)
0x16d: Pop(1); Push((bool) Stack[-1] == 0)
0x16e: IF (Stack[-1] == 0) GOTO 0x174; Pop(1)

0x16f: PushEmpty(bool)
0x170: Call2 0x198

0x171: Pop(0)
0x172: IF (Stack[-1] == 0) GOTO 0x174; Pop(1)

0x173: Stack[-1] = (bool) 1
0x174: IF (Stack[-1] == 0) GOTO 0x17a; Pop(1)

0x175: PushEmpty(object)
0x176: Call2 0x2f6

0x177: Pop(0)
0x178: @ RemoveActor(Stack[-1])
0x179: Pop(1)
0x17a: Return(); Pop(0)

0x17b: PushEmpty()
0x17c: Push("cleanup") // @poff=156
0x17d: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0x17e: IF (Stack[-1] == 0) GOTO 0x182; Pop(1)

0x17f: PushEmpty()
0x180: Call2 0x167

0x181: Pop(0)
0x182: Return(); Pop(0)

0x183: Push( Stack[6 + Tasks[-1].StackPointer] )
0x184: IF (Stack[-1] == 0) GOTO 0x188; Pop(1)

0x185: PushEmpty()
0x186: Call2 0x1db

0x187: Pop(0)
0x188: PushEmpty(bool)
0x189: Stack[-1] = (bool) 0
0x18a: Push( Stack[5 + Tasks[-1].StackPointer] )
0x18b: IF (Stack[-1] == 0) GOTO 0x191; Pop(1)

0x18c: PushEmpty(bool)
0x18d: Call2 0x198

0x18e: Pop(0)
0x18f: IF (Stack[-1] == 0) GOTO 0x191; Pop(1)

0x190: Stack[-1] = (bool) 1
0x191: IF (Stack[-1] == 0) GOTO 0x197; Pop(1)

0x192: PushEmpty(object)
0x193: Call2 0x2f6

0x194: Pop(0)
0x195: @ RemoveActor(Stack[-1])
0x196: Pop(1)
0x197: Return(); Pop(0)

0x198: Stack[-1] = (bool) 1
0x199: Return(); Pop(0)

0x19a: PushEmpty()
0x19b: PushEmpty(bool)
0x19c: Call2 0x269

0x19d: Pop(0)
0x19e: Pop(1); Push((bool) Stack[-1] == 0)
0x19f: IF (Stack[-1] == 0) GOTO 0x1a1; Pop(1)

0x1a0: Return(); Pop(0)

0x1a1: Push("player") // @poff=172
0x1a2: @ FindActor(Stack[-4]T, Stack[-1])
0x1a3: Pop(1)
0x1a4: Stack[2 + Tasks[-1].StackPointer] = (bool)0
0x1a5: Stack[3 + Tasks[-1].StackPointer] = (bool)0
0x1a6: Stack[0 + Tasks[-1].StackPointer] = Stack[-2]
0x1a7: Stack[1 + Tasks[-1].StackPointer] = Stack[-1]
0x1a8: Push((int) 10)
0x1a9: Push((float)1.0)
0x1aa: @ SetTimer(Stack[-2], Stack[-1])
0x1ab: Pop(2)
0x1ac: PushEmpty()
0x1ad: Call2 0x1e9

0x1ae: Pop(0)
0x1af: Pop(0); Push((bool) Stack[3 + Tasks[-1].StackPointer] == 0)
0x1b0: IF (Stack[-1] == 0) GOTO 0x1b4; Pop(1)

0x1b1: Push((int) 10)
0x1b2: @ KillTimer(Stack[-1])
0x1b3: Pop(1)
0x1b4: Return(); Pop(0)

0x1b5: PushEmpty(float, float)
0x1b6: Pop(0); PushNull((bool) Stack[4 + Tasks[-1].StackPointer] == 0)
0x1b7: IF (Stack[-1] == 0) GOTO 0x1ba; Pop(1)

0x1b8: Stack[-3] = (bool) 0
0x1b9: Return(); Pop(2)

0x1ba: PushEmpty(float, object)
0x1bb: Stack[-1] = Stack[4 + Tasks[-1].StackPointer]
0x1bc: Call2 0x23f

0x1bd: Pop(1)
0x1be: Stack[-2] = Sqrt(Stack[-1]); Pop(1);
0x1bf: Push( Stack[2 + Tasks[-1].StackPointer] )
0x1c0: IF (Stack[-1] == 0) GOTO 0x1c2; Pop(1)

0x1c1: Stack[-1] = Stack[-1] - Stack[1 + Tasks[-1].StackPointer]; Pop(0);
0x1c2: Stack[-3] = Stack[-1] < Stack[0 + Tasks[-1].StackPointer]; Pop(0);
0x1c3: Return(); Pop(2)

0x1c4: PushEmpty()
0x1c5: Push((int) 10)
0x1c6: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0x1c7: IF (Stack[-1] == 0) GOTO 0x1da; Pop(1)

0x1c8: PushEmpty(bool)
0x1c9: Call2 0x1b5

0x1ca: Pop(0)
0x1cb: IF (Stack[-1] == 0) GOTO 0x1d4; Pop(1)

0x1cc: Pop(0); Push((bool) Stack[2 + Tasks[-1].StackPointer] == 0)
0x1cd: IF (Stack[-1] == 0) GOTO 0x1d3; Pop(1)

0x1ce: PushEmpty(object)
0x1cf: Stack[-1] = Stack[4 + Tasks[-1].StackPointer]
0x1d0: Call2 0x2e4

0x1d1: Pop(1)
0x1d2: Stack[2 + Tasks[-1].StackPointer] = (bool)1
0x1d3: GOTO 0x1da

0x1d4: Push( Stack[2 + Tasks[-1].StackPointer] )
0x1d5: IF (Stack[-1] == 0) GOTO 0x1da; Pop(1)

0x1d6: Push("head") // @poff=186
0x1d7: @ UnlookAsync(Stack[-1])
0x1d8: Pop(1)
0x1d9: Stack[2 + Tasks[-1].StackPointer] = (bool)0
0x1da: Return(); Pop(0)

0x1db: PushEmpty()
0x1dc: Call2 0x23a

0x1dd: Pop(0)
0x1de: Push((int) 10)
0x1df: @ KillTimer(Stack[-1])
0x1e0: Pop(1)
0x1e1: Push( Stack[2 + Tasks[-1].StackPointer] )
0x1e2: IF (Stack[-1] == 0) GOTO 0x1e7; Pop(1)

0x1e3: Push("head") // @poff=186
0x1e4: @ UnlookAsync(Stack[-1])
0x1e5: Pop(1)
0x1e6: Stack[2 + Tasks[-1].StackPointer] = (bool)0
0x1e7: Stack[3 + Tasks[-1].StackPointer] = (bool)1
0x1e8: Return(); Pop(0)

0x1e9: PushEmpty(int, int, int, int, bool, float, bool, int, int, int, int, bool, float, bool)
0x1ea: @ WaitForAnimEnd()
0x1eb: Pop(0)
0x1ec: PushEmpty(bool)
0x1ed: Call2 0x269

0x1ee: Pop(0)
0x1ef: Pop(1); Push((bool) Stack[-1] == 0)
0x1f0: IF (Stack[-1] == 0) GOTO 0x1f2; Pop(1)

0x1f1: Return(); Pop(14)

0x1f2: PushEmpty(int)
0x1f3: Call2 0x356

0x1f4: Stack[-1] = Stack[-8]
0x1f5: Pop(1)
0x1f6: Stack[-6] = (int) 0
0x1f7: PushEmpty(bool)
0x1f8: Stack[-1] = (bool) 0
0x1f9: Push((int) 5)
0x1fa: Pop(1); Push((bool) Stack[-8] < Stack[-1])
0x1fb: IF (Stack[-1] == 0) GOTO 0x201; Pop(1)

0x1fc: PushEmpty(bool)
0x1fd: Call2 0x269

0x1fe: Pop(0)
0x1ff: IF (Stack[-1] == 0) GOTO 0x201; Pop(1)

0x200: Stack[-1] = (bool) 1
0x201: IF (Stack[-1] == 0) GOTO 0x235; Pop(1)

0x202: Push((int) 3)
0x203: @ irand(Stack[-6], Stack[-1])
0x204: Pop(1)
0x205: Push((int) 0)
0x206: Pop(1); Push((bool) Stack[-6] == Stack[-1])
0x207: IF (Stack[-1] == 0) GOTO 0x219; Pop(1)

0x208: Push(Stack[-7])
0x209: IF (Stack[-1] == 0) GOTO 0x218; Pop(1)

0x20a: @ irand(Stack[-4], Stack[-7])
0x20b: Pop(0)
0x20c: Push("all") // @poff=138
0x20d: PushEmpty(string, int)
0x20e: Stack[-7] = Stack[-1]
0x20f: Call2 0x34f

0x210: Pop(1)
0x211: @ PlayAnimation(Stack[-2], Stack[-1])
0x212: Pop(2)
0x213: @ WaitForAnimEnd(Stack[-3])
0x214: Pop(0)
0x215: Pop(0); Push((bool) Stack[-3] == 0)
0x216: IF (Stack[-1] == 0) GOTO 0x218; Pop(1)

0x217: GOTO 0x235

0x218: GOTO 0x22a

0x219: Push((int) 1)
0x21a: Pop(1); Push((bool) Stack[-6] == Stack[-1])
0x21b: IF (Stack[-1] == 0) GOTO 0x227; Pop(1)

0x21c: Push((int) 4)
0x21d: @ rand(Stack[-3], Stack[-1])
0x21e: Pop(1)
0x21f: Push((int) 1)
0x220: Pop(1); Push(Stack[-3] + Stack[-1]);
0x221: @ Sleep(Stack[-1], Stack[-2])
0x222: Pop(1)
0x223: Pop(0); Push((bool) Stack[-1] == 0)
0x224: IF (Stack[-1] == 0) GOTO 0x226; Pop(1)

0x225: GOTO 0x235

0x226: GOTO 0x22a

0x227: Push(Stack[-6])
0x228: IF (Stack[-1] == 0) GOTO 0x22a; Pop(1)

0x229: GOTO 0x235

0x22a: PushEmpty(bool)
0x22b: Call2 0x238

0x22c: Pop(0)
0x22d: Pop(1); Push((bool) Stack[-1] == 0)
0x22e: IF (Stack[-1] == 0) GOTO 0x230; Pop(1)

0x22f: GOTO 0x235

0x230: @ ResetAAS()
0x231: Pop(0)
0x232: Push((int) 1)
0x233: Stack[-7] = Stack[-7] + Stack[-1]; Pop(1);
0x234: GOTO 0x1f7

0x235: @ ResetAAS()
0x236: Pop(0)
0x237: Return(); Pop(14)

0x238: Stack[-1] = (bool) 1
0x239: Return(); Pop(0)

0x23a: @ StopAnimation()
0x23b: Pop(0)
0x23c: @ StopGroup0()
0x23d: Pop(0)
0x23e: Return(); Pop(0)

0x23f: PushEmpty(cvector, cvector, cvector, cvector, cvector, cvector)
0x240: @ GetPosition(Stack[-3])
0x241: Pop(0)
0x242: @@ GetPosition(Stack[-2]); Obj=7 // @poff=196
0x243: Pop(0)
0x244: Stack[-1] = Stack[-2] - Stack[-3]; Pop(0);
0x245: Stack[-8] = Stack[-1] | Stack[-1]; Pop(0);
0x246: Return(); Pop(6)

0x247: PushEmpty(bool, bool)
0x248: Push("HasProperty") // @poff=208
0x249: Push((int) 2)
0x24a: Pop(2); Push(IsFuncExist(Stack[-6], Stack[-2], Stack[-1]))
0x24b: Pop(1); Push((bool) Stack[-1] == 0)
0x24c: IF (Stack[-1] == 0) GOTO 0x24f; Pop(1)

0x24d: Stack[-5] = (bool) 0
0x24e: Return(); Pop(2)

0x24f: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=232
0x250: Pop(0)
0x251: Stack[-1] = Stack[-5]
0x252: Return(); Pop(2)

0x253: PushEmpty(float, float)
0x254: PushEmpty(bool, object, string)
0x255: Stack[-10] = Stack[-2]
0x256: Stack[-9] = Stack[-1]
0x257: Call2 0x247

0x258: Pop(2)
0x259: Pop(1); Push((bool) Stack[-1] == 0)
0x25a: IF (Stack[-1] == 0) GOTO 0x25d; Pop(1)

0x25b: Stack[-8] = (bool) 0
0x25c: Return(); Pop(2)

0x25d: @@ GetProperty(Stack[-6], Stack[-1]); Obj=7 // @poff=244
0x25e: Pop(0)
0x25f: PushEmpty(float, float, float, float)
0x260: Stack[-3] = Stack[-5] + Stack[-9]; Pop(0);
0x261: Stack[-8] = Stack[-2]
0x262: Stack[-7] = Stack[-1]
0x263: Call2 0x306

0x264: Pop(3)
0x265: @@ SetProperty(Stack[-7], Stack[-1]); Obj=8 // @poff=256
0x266: Pop(1)
0x267: Stack[-8] = (bool) 1
0x268: Return(); Pop(2)

0x269: PushEmpty(bool, bool)
0x26a: @ IsLoaded(Stack[-1])
0x26b: Pop(0)
0x26c: Stack[-1] = Stack[-3]
0x26d: Return(); Pop(2)

0x26e: PushEmpty(float, cvector, cvector, cvector, cvector, cvector, cvector, bool, bool, float, cvector, cvector, cvector, cvector, cvector, cvector, bool, bool)
0x26f: @@ GetPosition(Stack[-8]); Obj=20 // @poff=196
0x270: Pop(0)
0x271: @@ GetEyesHeight(Stack[-9]); Obj=20 // @poff=268
0x272: Pop(0)
0x273: Push(CvectorIndex(Stack[-8], 1))
0x274: Stack[-1] = Stack[-1] + Stack[-10]; Pop(0);
0x275: CvectorIndex(Stack[-9], 1) = Stack[-1];
0x276: @ GetPosition(Stack[-7])
0x277: Pop(0)
0x278: @ GetEyesHeight(Stack[-9])
0x279: Pop(0)
0x27a: Push(CvectorIndex(Stack[-7], 1))
0x27b: Stack[-1] = Stack[-1] + Stack[-10]; Pop(0);
0x27c: CvectorIndex(Stack[-8], 1) = Stack[-1];
0x27d: Stack[-6] = Stack[-8] - Stack[-7]; Pop(0);
0x27e: Push(CvectorIndex(Stack[-6], 1))
0x27f: Stack[-1] = (int) 0
0x280: CvectorIndex(Stack[-7], 1) = Stack[-1];
0x281: Pop(0); Push(Stack[-6] | Stack[-6]);
0x282: Pop(1); Push(Sqrt(Stack[-1]))
0x283: Stack[-7] = Stack[-7] / Stack[-1]; Pop(1);
0x284: Stack[-5] = -Stack[-6]; Pop(0);
0x285: Pop(0); Push(Stack[-6] * Stack[-19]);
0x286: PushEmpty(cvector, cvector)
0x287: Push(CVector(0.0, 1.0, 0.0))
0x288: Stack[-2] = Stack[-9] ^ Stack[-1]; Pop(1);
0x289: Call2 0x2fc

0x28a: Pop(1)
0x28b: Push((int) 25)
0x28c: Pop(2); Push(Stack[-2] * Stack[-1]);
0x28d: Pop(2); Push(Stack[-2] + Stack[-1]);
0x28e: Push(CVector(0.0, 10.0, 0.0))
0x28f: Stack[-6] = Stack[-2] - Stack[-1]; Pop(2);
0x290: Stack[-3] = Stack[-7] + Stack[-4]; Pop(0);
0x291: @ IsOverrideActive(Stack[-2])
0x292: Pop(0)
0x293: Push(Stack[-2])
0x294: IF (Stack[-1] == 0) GOTO 0x297; Pop(1)

0x295: Stack[-21] = (bool) 0
0x296: Return(); Pop(18)

0x297: @ StopWorld()
0x298: Pop(0)
0x299: Push((bool) 1)
0x29a: @ CameraTransit(Stack[-4], Stack[-6], Stack[-1])
0x29b: Pop(1)
0x29c: Push(CvectorIndex(Stack[-4], 0))
0x29d: Push(CvectorIndex(Stack[-5], 2))
0x29e: @ Rotate(Stack[-2], Stack[-1])
0x29f: Pop(2)
0x2a0: PushEmpty(bool)
0x2a1: Call2 0x408

0x2a2: Pop(0)
0x2a3: IF (Stack[-1] == 0) GOTO 0x2a5; Pop(1)

0x2a4: GOTO 0x2ad

0x2a5: Push("head") // @poff=186
0x2a6: @ HasAnimationTrack(Stack[-2], Stack[-1])
0x2a7: Pop(1)
0x2a8: Push(Stack[-1])
0x2a9: IF (Stack[-1] == 0) GOTO 0x2ad; Pop(1)

0x2aa: Push("head") // @poff=186
0x2ab: @ LookAsyncCamera(Stack[-1])
0x2ac: Pop(1)
0x2ad: @ CameraWaitForPlayFinish()
0x2ae: Pop(0)
0x2af: @ ResumeWorld()
0x2b0: Pop(0)
0x2b1: Stack[-21] = (bool) 1
0x2b2: Return(); Pop(18)

0x2b3: PushEmpty(bool, bool)
0x2b4: Push((bool) 1)
0x2b5: @ CameraSwitchToNormal(Stack[-1])
0x2b6: Pop(1)
0x2b7: PushEmpty(bool)
0x2b8: Call2 0x408

0x2b9: Pop(0)
0x2ba: IF (Stack[-1] == 0) GOTO 0x2bc; Pop(1)

0x2bb: GOTO 0x2c4

0x2bc: Push("head") // @poff=186
0x2bd: @ HasAnimationTrack(Stack[-2], Stack[-1])
0x2be: Pop(1)
0x2bf: Push(Stack[-1])
0x2c0: IF (Stack[-1] == 0) GOTO 0x2c4; Pop(1)

0x2c1: Push("head") // @poff=186
0x2c2: @ UnlookAsync(Stack[-1])
0x2c3: Pop(1)
0x2c4: Return(); Pop(2)

0x2c5: PushEmpty(bool, float, float, bool, float, float)
0x2c6: @ lshHasAnimation(Stack[-3], Stack[-7])
0x2c7: Pop(0)
0x2c8: Push(Stack[-3])
0x2c9: IF (Stack[-1] == 0) GOTO 0x2d0; Pop(1)

0x2ca: @ lshGetAnimTimes(Stack[-7], Stack[-2], Stack[-1])
0x2cb: Pop(0)
0x2cc: Push((bool) 0)
0x2cd: @ lshPlayAnimation(Stack[-3], Stack[-2], Stack[-1])
0x2ce: Pop(1)
0x2cf: GOTO 0x2d4

0x2d0: Push("Can't find lsh animation : ") // @poff=282
0x2d1: Pop(1); Push(Stack[-1] + Stack[-8]);
0x2d2: @ Trace(Stack[-1])
0x2d3: Pop(1)
0x2d4: Return(); Pop(6)

0x2d5: PushEmpty(bool, float, float, bool, float, float)
0x2d6: @ lshHasAnimation(Stack[-3], Stack[-8])
0x2d7: Pop(0)
0x2d8: Push(Stack[-3])
0x2d9: IF (Stack[-1] == 0) GOTO 0x2df; Pop(1)

0x2da: @ lshGetAnimTimes(Stack[-8], Stack[-2], Stack[-1])
0x2db: Pop(0)
0x2dc: @ lshPlayAnimation(Stack[-2], Stack[-1], Stack[-7])
0x2dd: Pop(0)
0x2de: GOTO 0x2e3

0x2df: Push("Can't find lsh animation : ") // @poff=282
0x2e0: Pop(1); Push(Stack[-1] + Stack[-9]);
0x2e1: @ Trace(Stack[-1])
0x2e2: Pop(1)
0x2e3: Return(); Pop(6)

0x2e4: PushEmpty(float, cvector, float, cvector)
0x2e5: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=268
0x2e6: Pop(0)
0x2e7: Stack[-1] = CVector(0.0, 0.0, 0.0)
0x2e8: Push(CvectorIndex(Stack[-1], 1))
0x2e9: Stack[-3] = Stack[-1]
0x2ea: CvectorIndex(Stack[-2], 1) = Stack[-1];
0x2eb: Push("head") // @poff=186
0x2ec: @ LookAsync(Stack[-6], Stack[-1], Stack[-2])
0x2ed: Pop(1)
0x2ee: Return(); Pop(4)

0x2ef: PushEmpty(bool)
0x2f0: Call2 0x408

0x2f1: Pop(0)
0x2f2: IF (Stack[-1] == 0) GOTO 0x2f5; Pop(1)

0x2f3: @ lshStopSpeech()
0x2f4: Pop(0)
0x2f5: Return(); Pop(0)

0x2f6: PushEmpty(object, object)
0x2f7: @ self(Stack[-1])
0x2f8: Pop(0)
0x2f9: Stack[-1] = Stack[-3]
0x2fa: Return(); Pop(2)

0x2fb: Stack[-1] = 0
0x2fc: PushEmpty(float, float)
0x2fd: Pop(0); Push(Stack[-3] | Stack[-3]);
0x2fe: Stack[-2] = Sqrt(Stack[-1]); Pop(1);
0x2ff: Push((float)9.999999974752427e-07)
0x300: Pop(1); Push((bool) Stack[-2] < Stack[-1])
0x301: IF (Stack[-1] == 0) GOTO 0x304; Pop(1)

0x302: Stack[-4] = CVector(0.0, 0.0, 0.0)
0x303: Return(); Pop(2)

0x304: Stack[-4] = Stack[-3] / Stack[-1]; Pop(0);
0x305: Return(); Pop(2)

0x306: PushEmpty()
0x307: Pop(0); Push((bool) Stack[-3] < Stack[-2])
0x308: IF (Stack[-1] == 0) GOTO 0x30b; Pop(1)

0x309: Stack[-2] = Stack[-4]
0x30a: Return(); Pop(0)

0x30b: Pop(0); Push((bool) Stack[-3] > Stack[-1])
0x30c: IF (Stack[-1] == 0) GOTO 0x30f; Pop(1)

0x30d: Stack[-1] = Stack[-4]
0x30e: Return(); Pop(0)

0x30f: Stack[-3] = Stack[-4]
0x310: Return(); Pop(0)

0x311: PushEmpty(int, int)
0x312: @ GetVariable(Stack[-3], Stack[-1])
0x313: Pop(0)
0x314: Stack[-1] = Stack[-4]
0x315: Return(); Pop(2)

0x316: PushEmpty()
0x317: Pop(0); Push((bool) Stack[-2] == 0)
0x318: IF (Stack[-1] == 0) GOTO 0x31b; Pop(1)

0x319: Stack[-3] = (bool) 0
0x31a: Return(); Pop(0)

0x31b: Push((int) 0)
0x31c: Pop(1); Push((bool) Stack[-2] > Stack[-1])
0x31d: IF (Stack[-1] == 0) GOTO 0x322; Pop(1)

0x31e: Push((int) 8)
0x31f: @ SendWorldWndMessage(Stack[-1])
0x320: Pop(1)
0x321: GOTO 0x32b

0x322: Push((int) 0)
0x323: Pop(1); Push((bool) Stack[-2] < Stack[-1])
0x324: IF (Stack[-1] == 0) GOTO 0x329; Pop(1)

0x325: Push((int) 9)
0x326: @ SendWorldWndMessage(Stack[-1])
0x327: Pop(1)
0x328: GOTO 0x32b

0x329: Stack[-3] = (bool) 0
0x32a: Return(); Pop(0)

0x32b: PushEmpty(float)
0x32c: Stack[-2] = Stack[-1]
0x32d: Call2 0x339

0x32e: Pop(1)
0x32f: PushEmpty(bool, object, string, float, float, float)
0x330: Stack[-8] = Stack[-5]
0x331: Stack[-4] = "reputation" // @poff=338
0x332: Stack[-7] = Stack[-3]
0x333: Stack[-2] = (int) 0
0x334: Stack[-1] = (int) 1
0x335: Call2 0x253

0x336: Pop(6)
0x337: Stack[-3] = (bool) 1
0x338: Return(); Pop(0)

0x339: PushEmpty(object, object)
0x33a: @ CreateFloatVector(Stack[-1])
0x33b: Pop(0)
0x33c: @@ add(Stack[-3]); Obj=1 // @poff=360
0x33d: Pop(0)
0x33e: Push((int) 16)
0x33f: @ SendWorldWndMessage(Stack[-1], Stack[-2])
0x340: Pop(1)
0x341: Return(); Pop(2)

0x342: Stack[-1] = 0
0x343: PushEmpty(object, object)
0x344: @ FindActor(Stack[-1], Stack[-4])
0x345: Pop(0)
0x346: Pop(0); PushNull((bool) Stack[-1] == 0)
0x347: IF (Stack[-1] == 0) GOTO 0x34a; Pop(1)

0x348: Stack[-5] = (bool) 0
0x349: Return(); Pop(2)

0x34a: @ Trigger(Stack[-1], Stack[-3])
0x34b: Pop(0)
0x34c: Stack[-5] = (bool) 1
0x34d: Return(); Pop(2)

0x34e: Stack[-1] = 0
0x34f: PushEmpty(string, string)
0x350: Stack[-1] = "idle" // @poff=146
0x351: Push(Stack[-3])
0x352: IF (Stack[-1] == 0) GOTO 0x354; Pop(1)

0x353: Stack[-1] = Stack[-1] + Stack[-3]; Pop(0);
0x354: Stack[-1] = Stack[-4]
0x355: Return(); Pop(2)

0x356: PushEmpty(int, bool, int, bool)
0x357: Stack[-2] = (int) 0
0x358: Push("all") // @poff=138
0x359: PushEmpty(string, int)
0x35a: Stack[-5] = Stack[-1]
0x35b: Call2 0x34f

0x35c: Pop(1)
0x35d: @ HasAnimation(Stack[-3], Stack[-2], Stack[-1])
0x35e: Pop(2)
0x35f: Pop(0); Push((bool) Stack[-1] == 0)
0x360: IF (Stack[-1] == 0) GOTO 0x362; Pop(1)

0x361: GOTO 0x365

0x362: Push((int) 1)
0x363: Stack[-3] = Stack[-3] + Stack[-1]; Pop(1);
0x364: GOTO 0x358

0x365: Stack[-2] = Stack[-5]
0x366: Return(); Pop(4)

0x367: PushEmpty()
0x368: PushEmpty(bool, object, float)
0x369: Stack[-5] = Stack[-2]
0x36a: Stack[-1] = (float) -0.05000000074505806
0x36b: Call2 0x316

0x36c: Pop(3)
0x36d: Return(); Pop(0)

0x36e: PushEmpty(int, int)
0x36f: Push("k7q03") // @poff=364
0x370: Push((int) 2)
0x371: @ SetVariable(Stack[-2], Stack[-1])
0x372: Pop(2)
0x373: PushEmpty()
0x374: Call2 0x3b9

0x375: Pop(0)
0x376: Push("tvirin") // @poff=376
0x377: Push((int) 5)
0x378: @@ RemoveItemByType(Stack[-3], Stack[-2], Stack[-1]); Obj=6 // @poff=390
0x379: Pop(2)
0x37a: Return(); Pop(2)

0x37b: PushEmpty()
0x37c: PushEmpty()
0x37d: Call2 0x3ac

0x37e: Pop(0)
0x37f: PushEmpty(bool, string, string)
0x380: Stack[-2] = "quest_k7_03" // @poff=407
0x381: Stack[-1] = "fail" // @poff=431
0x382: Call2 0x343

0x383: Pop(3)
0x384: Return(); Pop(0)

0x385: PushEmpty()
0x386: Push("playsound") // @poff=441
0x387: Push("giveitem") // @poff=461
0x388: @ TriggerWorld(Stack[-2], Stack[-1])
0x389: Pop(2)
0x38a: Return(); Pop(0)

0x38b: PushEmpty()
0x38c: PushEmpty(int, string)
0x38d: Stack[-1] = "k7q03" // @poff=364
0x38e: Call2 0x311

0x38f: Pop(1)
0x390: Push((int) 1)
0x391: Pop(2); Push((bool) Stack[-2] == Stack[-1])
0x392: IF (Stack[-1] == 0) GOTO 0x395; Pop(1)

0x393: Stack[-2] = (bool) 1
0x394: Return(); Pop(0)

0x395: Stack[-2] = (bool) 0
0x396: Return(); Pop(0)

0x397: PushEmpty()
0x398: PushEmpty(bool, object)
0x399: Stack[-3] = Stack[-1]
0x39a: Call2 0x3a1

0x39b: Pop(1)
0x39c: IF (Stack[-1] == 0) GOTO 0x39f; Pop(1)

0x39d: Stack[-2] = (bool) 1
0x39e: Return(); Pop(0)

0x39f: Stack[-2] = (bool) 0
0x3a0: Return(); Pop(0)

0x3a1: PushEmpty(int, int)
0x3a2: Push("tvirin") // @poff=376
0x3a3: @@ GetItemCountOfType(Stack[-2], Stack[-1]); Obj=4 // @poff=479
0x3a4: Pop(1)
0x3a5: Push((int) 5)
0x3a6: Pop(1); Push((bool) Stack[-2] >= Stack[-1])
0x3a7: IF (Stack[-1] == 0) GOTO 0x3aa; Pop(1)

0x3a8: Stack[-4] = (bool) 1
0x3a9: Return(); Pop(2)

0x3aa: Stack[-4] = (bool) 0
0x3ab: Return(); Pop(2)

0x3ac: PushEmpty(object, object)
0x3ad: Push((int) 779)
0x3ae: Push((int) 2)
0x3af: Push((int) 541759)
0x3b0: @ CreateDiaryEntry(Stack[-4], Stack[-3], Stack[-2], Stack[-1])
0x3b1: Pop(3)
0x3b2: PushEmpty(bool, object, int)
0x3b3: Stack[-4] = Stack[-2]
0x3b4: Stack[-1] = (int) 434
0x3b5: Call2 0x3d3

0x3b6: Pop(3)
0x3b7: Return(); Pop(2)

0x3b8: Stack[-1] = 0
0x3b9: PushEmpty(object, object)
0x3ba: Push((int) 436)
0x3bb: Push((int) 2)
0x3bc: Push((int) 526392)
0x3bd: @ CreateDiaryEntry(Stack[-4], Stack[-3], Stack[-2], Stack[-1])
0x3be: Pop(3)
0x3bf: PushEmpty(bool, object, int)
0x3c0: Stack[-4] = Stack[-2]
0x3c1: Stack[-1] = (int) 434
0x3c2: Call2 0x3d3

0x3c3: Pop(3)
0x3c4: Return(); Pop(2)

0x3c5: Stack[-1] = 0
0x3c6: PushEmpty(object, object)
0x3c7: @ GetDiaryRoot(Stack[-1])
0x3c8: Pop(0)
0x3c9: Pop(0); Push((bool) Stack[-1] == 0)
0x3ca: IF (Stack[-1] == 0) GOTO 0x3d0; Pop(1)

0x3cb: Push("Can't retrieve diary root") // @poff=498
0x3cc: @ Trace(Stack[-1])
0x3cd: Pop(1)
0x3ce: Stack[-3] = (bool) 0
0x3cf: Return(); Pop(2)

0x3d0: Stack[-1] = Stack[-3]
0x3d1: Return(); Pop(2)

0x3d2: Stack[-1] = 0
0x3d3: PushEmpty(object, object, int, object, object, int)
0x3d4: PushEmpty(object)
0x3d5: Call2 0x3c6

0x3d6: Stack[-1] = Stack[-4]
0x3d7: Pop(1)
0x3d8: @@ Find(Stack[-7], Stack[-2]); Obj=3 // @poff=550
0x3d9: Pop(0)
0x3da: Pop(0); Push((bool) Stack[-2] == 0)
0x3db: IF (Stack[-1] == 0) GOTO 0x3e2; Pop(1)

0x3dc: Push("Can't find diary parent with id: ") // @poff=555
0x3dd: Pop(1); Push(Stack[-1] + Stack[-8]);
0x3de: @ Trace(Stack[-1])
0x3df: Pop(1)
0x3e0: Stack[-9] = (bool) 0
0x3e1: Return(); Pop(6)

0x3e2: @@ AddChild(Stack[-8]); Obj=2 // @poff=623
0x3e3: Pop(0)
0x3e4: Push((int) 7)
0x3e5: @ SendWorldWndMessage(Stack[-1])
0x3e6: Pop(1)
0x3e7: @@ GetCategory(Stack[-1]); Obj=8 // @poff=632
0x3e8: Pop(0)
0x3e9: @ SetDiarySection(Stack[-1])
0x3ea: Pop(0)
0x3eb: Stack[-9] = (bool) 0
0x3ec: Return(); Pop(6)

0x3ed: Stack[-2] = 0
0x3ee: Stack[-3] = 0
0x3ef: PushEmpty(int, int)
0x3f0: Push("branch") // @poff=644
0x3f1: @ GetVariable(Stack[-1], Stack[-2])
0x3f2: Pop(1)
0x3f3: Push((int) 0)
0x3f4: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0x3f5: IF (Stack[-1] == 0) GOTO 0x3f9; Pop(1)

0x3f6: Stack[-3] = (int) 1
0x3f7: Return(); Pop(2)

0x3f8: GOTO 0x3fe

0x3f9: Push((int) 1)
0x3fa: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0x3fb: IF (Stack[-1] == 0) GOTO 0x3fe; Pop(1)

0x3fc: Stack[-3] = (int) 2
0x3fd: Return(); Pop(2)

0x3fe: Stack[-3] = (int) 3
0x3ff: Return(); Pop(2)

0x400: Stack[-1] = (int) 515564
0x401: Return(); Pop(0)

0x402: Stack[-1] = (int) 503349
0x403: Return(); Pop(0)

0x404: Stack[-1] = "ui/NPC_Citizen2.png" // @poff=658
0x405: Return(); Pop(0)

0x406: Stack[-1] = "ui/NPC_Citizen2_b.png" // @poff=698
0x407: Return(); Pop(0)

0x408: Stack[-1] = (bool) 0
0x409: Return(); Pop(0)

