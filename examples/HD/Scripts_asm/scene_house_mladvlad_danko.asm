...
Strings:
	...
	A:get
	A:clear
// @pool_raw:700074005f00720061007400000070006500720073005f0072006100740000007200610074005f0069006e0064006f006f0072002e0078006d006c0000006164640073697a650067657400636c65617200
...
0x14: PushEmpty()
0x15: Stack[-2] = "pers_rat" // @poff=14
0x16: Return(); Pop(0)

0x17: PushEmpty()
0x18: Stack[-2] = "rat_indoor.xml" // @poff=32
0x19: Return(); Pop(0)
...
0x30: Stack[-8] = Stack[-3]
0x31: Stack[-6] = Stack[-2]
0x32: Stack[-5] = Stack[-1]
0x33: Call2 0x3b

0x34: Pop(3)
0x35: @@@ add(Stack[-1]); Obj=0 // @poff=62
0x36: Pop(1)
0x37: Push((int) 1)
...
0x5b: @@ size(Stack[-3]); Obj=7 // @poff=66
0x5c: Pop(0)
0x5d: Stack[-2] = (int) 0
0x5e: Pop(0); Push((bool) Stack[-2] < Stack[-3])
0x5f: IF (Stack[-1] == 0) GOTO 0x6a; Pop(1)

0x60: @@ get(Stack[-1], Stack[-2]); Obj=7 // @poff=71
0x61: Pop(0)
0x62: Push(Stack[-1])
