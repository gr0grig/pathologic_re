...
Strings:
	...
	W:scrollbar
	W:diary
// @pool_raw:4368696c64436f756e74004368696c640047657443617465676f72790047657454657874494400640065006600610075006c00740000007300630072006f006c006c006200610072000000640069006100720079000000
// @pool_encoding:utf8
...
0x2: @ GetWindowSize(Stack[-3]T, Stack[-4]T)
0x3: Pop(0)
0x4: PushEmpty(bool)
0x5: Stack[-1] = (bool) 1
0x6: Call2 0x14
...
0x17: Pop(0)
0x18: @@ ChildCount(Stack[-4]); Obj=5 // @poff=0
0x19: Pop(0)
0x1a: Stack[-2] = (int) 0
0x1b: Pop(0); Push((bool) Stack[-2] < Stack[-4])
0x1c: IF (Stack[-1] == 0) GOTO 0x30; Pop(1)

0x1d: @@ Child(Stack[-1], Stack[-2]); Obj=5 // @poff=11
0x1e: Pop(0)
0x1f: @@ GetCategory(Stack[-3]); Obj=1 // @poff=17
0x20: Pop(0)
...
0x25: Stack[-4] = Stack[-3]
0x26: Stack[-2] = (int) 0
0x27: Stack[-14] = Stack[-1]
0x28: Call2 0x3c
...
0x3d: @@ GetCategory(Stack[-8]); Obj=19 // @poff=17
0x3e: Pop(0)
...
0x42: @@ GetTextID(Stack[-7]); Obj=19 // @poff=29
0x43: Pop(0)
...
0x48: Push("default") // @poff=39
0x49: Pop(0); Push(Stack[3 + Tasks[-1].StackPointer] - Stack[-19]);
0x4a: @ GetTextHeightInWidth(Stack[-7], Stack[-2], Stack[-1], Stack[-8])
0x4b: Pop(2)
0x4c: GOTO 0x55

0x4d: Push("default") // @poff=39
0x4e: Pop(0); Push(Stack[1 + Tasks[-1].StackPointer] + Stack[6 + Tasks[-1].StackPointer]);
0x4f: Pop(0); Push(Stack[3 + Tasks[-1].StackPointer] - Stack[-20]);
0x50: Push((float)1.0)
0x51: Push((float)1.0)
0x52: Push((float)1.0)
...
0x56: @@ ChildCount(Stack[-4]); Obj=19 // @poff=0
0x57: Pop(0)
0x58: Stack[-3] = (int) 0
0x59: Pop(0); Push((bool) Stack[-3] < Stack[-4])
0x5a: IF (Stack[-1] == 0) GOTO 0x6f; Pop(1)

0x5b: @@ Child(Stack[-2], Stack[-3]); Obj=19 // @poff=11
0x5c: Pop(0)
0x5d: @@ GetCategory(Stack[-1]); Obj=2 // @poff=17
0x5e: Pop(0)
...
0xd6: Pop(1); Push(Stack[-1] / Stack[2 + Tasks[-1].StackPointer]);
0xd7: Push("scrollbar") // @poff=55
