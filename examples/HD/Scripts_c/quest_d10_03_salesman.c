// @GLOBALS: 0:object:

task task_0
{
}


task task_1
{
	// @pe
	void event_11(object var_0_object, object var_1_object, object var_2_object, string var_3_string, bool var_4_bool, int var_5_int, int var_6_int, int var_7_int, int var_8_int, bool var_9_bool, bool var_10_bool, object var_11_object, bool var_12_bool, bool var_13_bool)
	{
		if(1 != 0) {
			func_698();
			if(var_13_bool == 35730) {
				object var_18_object; object var_19_object;
				var_18_object = var_1_object;
				var_19_object = var_0_object;
				func_756();
			}
			if(var_12_bool == 35729) {
				bool var_47_bool;
				func_765(var_1_object);
				if(var_47_bool != 0) {
					object var_55_object; object var_56_object;
					var_55_object = var_1_object;
					var_56_object = var_0_object;
					func_750();
					func_162(var_13_bool, "Neutral");
					var_0_object->SetMessage(534142); //@t
					var_0_object->ClearReplies(); //@t
					var_0_object->AddReply(534435, 36067, 36066); //@t
					var_0_object->AddReply(534437, 36067, 36068); //@t
					return 0;
				}
				func_162(var_13_bool, "Neutral");
				var_0_object->SetMessage(534144); //@t
				var_0_object->ClearReplies(); //@t
				var_0_object->AddReply(534145, -1, 35732); //@t
				var_0_object->AddReply(534443, -1, 36076); //@t
				return 0;
			}
			if(var_12_bool == 36067) {
				func_162(var_13_bool, "Neutral");
				var_0_object->SetMessage(534436); //@t
				var_0_object->ClearReplies(); //@t
				var_0_object->AddReply(534438, 36070, 36069); //@t
				var_0_object->AddReply(534442, 36070, 36074); //@t
				return 0;
			}
			if(var_12_bool == 36070) {
				func_162(var_13_bool, "Neutral");
				var_0_object->SetMessage(534439); //@t
				var_0_object->ClearReplies(); //@t
				var_0_object->AddReply(534143, -1, 35730); //@t
				return 0;
			}
			var_3_string = true;
			bool var_108_bool;
			func_856(var_108_bool);
			if(var_108_bool != 0)
				@lshStopAnimation();
			else
				@StopAnimation();
			return 0;
		}
		return 0;
	EMIT "GOTO 0xb9";
	
	}

}


maintask task_2
{
	void init(object var_0_object, object var_1_object, object var_2_object, string var_3_string, bool var_4_bool, int var_5_int, int var_6_int, bool var_7_bool, bool var_8_bool, object var_9_object, bool var_10_bool, bool var_11_bool)
	{
		func_322(var_11_bool, (float)300, (float)100);
	}

	// @pe
	void OnUse(object actor, object var_1_object, object var_2_object, string var_3_string, bool var_4_bool, int var_5_int, int var_6_int, bool var_7_bool, bool var_8_bool, object var_9_object, bool var_10_bool, bool var_11_bool, object var_12_object)
	{
		func_456(var_11_bool, var_12_object);
		int var_16_int; object var_17_object;
		var_12_object = var_17_object;
		TaskCall(0);
		func_0(var_18_object, var_16_int, var_17_object);
		TaskReturn();
	}

	// @pe
	void OnTrigger(object var_0_object, object var_1_object, object var_2_object, string var_3_string, bool var_4_bool, int var_5_int, int var_6_int, bool var_7_bool, bool var_8_bool, object var_9_object, bool var_10_bool, bool var_11_bool, string var_12_string)
	{
		if(var_12_string == "cleanup")
			func_340(var_12_string);
	}

	void OnUnload(object var_0_object, object var_1_object, object var_2_object, string var_3_string, bool var_4_bool, int var_5_int, int var_6_int, bool var_7_bool, bool var_8_bool, object var_9_object, bool var_10_bool, bool var_11_bool)
	{
		if(var_6_int != 0)
			func_456(var_10_bool, var_11_bool);
		bool var_16_bool = false;
		if(var_5_int != 0) {
			bool var_18_bool;
			func_389(var_18_bool);
			if(var_18_bool != 0)
				var_16_bool = true;
		}
		if(var_16_bool != 0) {
			object var_19_object;
			func_705(var_19_object);
			@RemoveActor(var_19_object);
		}
	}

	// @pe
	void OnTimer(object var_0_object, object var_1_object, object var_2_object, string var_3_string, bool var_4_bool, int var_5_int, int var_6_int, bool var_7_bool, bool var_8_bool, object var_9_object, bool var_10_bool, bool var_11_bool, int var_12_int)
	{
		if(var_12_int == 10) {
			bool var_15_bool;
			func_418(var_10_bool, var_11_bool, var_12_int, var_15_bool);
			if(var_15_bool != 0) {
				if(!var_2_object) { //@nz
					func_687(var_4_bool);
					var_2_object = true;
				}
			} else if(var_2_object != 0) {
				@UnlookAsync("head");
				var_2_object = false;
			}
		}
	
	}

}


void func_0(object var_0_object, int var_16_int, object var_17_object)
{
	var_0_object = var_17_object;
	bool var_27_bool; object var_28_object;
	var_17_object = var_28_object;
	func_569(var_27_bool, var_28_object, 70.0);
	if(!var_27_bool) { //@nz
		var_16_int = -2;
		return 8;
	}
	object var_23_object;
	@CreateDialog(var_23_object);
	int var_75_int;
	func_850(var_75_int);
	var_23_object->SetNPCName(var_75_int);
	int var_76_int;
	func_848(var_76_int);
	var_23_object->SetNPCDescription(var_76_int);
	string var_77_string;
	func_852(var_77_string);
	var_23_object->SetPhoto(var_77_string);
	string var_78_string;
	func_854(var_78_string);
	var_23_object->SetPhoto2(var_78_string);
	int var_79_int;
	func_831(var_79_int);
	var_23_object->SetPlayerName(var_79_int);
	bool var_24_bool;
	@IsOverrideActive(var_24_bool);
	if(var_24_bool != 0) {
		var_16_int = -2;
		return 8;
	}
	@DoDialog(var_23_object);
	object var_88_object; object var_89_object;
	var_17_object = var_88_object;
	var_23_object = var_89_object;
	TaskCall(1);
	func_74(var_90_object, var_91_object, var_92_string, var_93_bool, var_88_object, var_89_object);
	TaskReturn();
	bool var_26_bool;
	var_23_object->IsDialogEnd(var_26_bool);
	
	for(;;) {
		var_157_bool = !var_26_bool; //@nz
		if(var_157_bool == 0) goto Label_63;
		@sync();
		var_23_object->IsDialogEnd(var_26_bool);
	}
	
Label_63:
	object var_158_object;
	var_17_object = var_158_object;
	func_638();
	@StopDialog(var_23_object);
	var_23_object->GetReturnValue(-1);
	int var_25_int = var_16_int;
}
EMIT "Stack[-4] = 0";


void func_389(bool var_18_bool)
{
	var_18_bool = true;
}


// @pe
void func_391(object var_0_object, object var_1_object, object var_2_object, string var_3_string, bool var_4_bool, float var_21_float, float var_22_float)
{
	bool var_23_bool;
	func_564(var_23_bool);
	if(!var_23_bool) //@nz
		return 0;
	@FindActor(var_4_bool, "player");
	var_2_object = false;
	var_0_object = var_21_float;
	var_1_object = var_22_float;
	@SetTimer(10, 1.0);
	func_470();
	if(!false) //@nz
		@KillTimer(10);
}


void func_777(void)
{
	object var_23_object;
	@CreateDiaryEntry(var_23_object, 674, 2, 534159);
	bool var_27_bool; object var_28_object;
	var_23_object = var_28_object;
	func_803(var_27_bool, var_28_object, 672);
}
EMIT "Stack[-1] = 0";


void func_656(string var_133_string)
{
	bool var_137_bool; float var_138_float; float var_139_float;
	@lshHasAnimation(var_137_bool, var_133_string);
	if(var_137_bool != 0) {
		@lshGetAnimTimes(var_133_string, var_138_float, var_139_float);
		@lshPlayAnimation(var_138_float, var_139_float, false);
	} else {
		@Trace("Can't find lsh animation : " + var_133_string);
	}
	
}


void func_790(object var_36_object)
{
	object var_38_object;
	@GetDiaryRoot(var_38_object);
	if(!var_38_object) { //@nz
		@Trace("Can't retrieve diary root");
		var_36_object = false;
	}
	var_38_object = var_36_object;
}
EMIT "Stack[-1] = 0";


void func_672(string var_111_string, bool var_112_bool)
{
	bool var_118_bool; float var_119_float; float var_120_float;
	@lshHasAnimation(var_118_bool, var_111_string);
	if(var_118_bool != 0) {
		@lshGetAnimTimes(var_111_string, var_119_float, var_120_float);
		@lshPlayAnimation(var_119_float, var_120_float, var_112_bool);
	} else {
		@Trace("Can't find lsh animation : " + var_111_string);
	}
	
}


// @pe
void func_162(object var_2_object, string var_107_string)
{
	bool var_108_bool;
	func_856(var_108_bool);
	if(!var_108_bool) //@nz
		return 0;
	if(var_107_string == var_2_object)
		return 0;
	string var_111_string; bool var_112_bool;
	var_107_string = var_111_string;
	if(var_107_string == "")
		var_112_bool = false;
	else
		var_112_bool = true;
	func_672(var_111_string, var_112_bool);
	var_2_object = var_107_string;
	
}


void func_418(object var_0_object, object var_1_object, bool var_4_bool, bool var_15_bool)
{
	if(var_4_bool == null)
		var_15_bool = false;
	float var_19_float;
	func_556(var_19_float, var_4_bool);
	float var_17_float = sqrt(var_19_float);
	if(var_2_object != 0)
		var_17_float -= var_1_object;
	var_15_bool = var_17_float < var_0_object;
}


void func_803(bool var_27_bool, object var_28_object, int var_29_int)
{
	object var_36_object;
	func_790(var_36_object);
	object var_33_object;
	var_36_object = var_33_object;
	object var_34_object;
	var_33_object->Find(var_29_int, var_34_object);
	if(!var_34_object) { //@nz
		@Trace("Can't find diary parent with id: " + var_29_int);
		var_27_bool = false;
	}
	var_34_object->AddChild(var_28_object);
	@SendWorldWndMessage(7);
	int var_35_int;
	var_28_object->GetCategory(var_35_int);
	@SetDiarySection(var_35_int);
	var_27_bool = false;
}
EMIT "Stack[-2] = 0";
EMIT "Stack[-3] = 0";


void func_549(bool var_71_bool)
{
	var_71_bool = true;
}


void func_551(void)
{
	@StopAnimation();
	@StopGroup0();
}


void func_556(float var_19_float, object var_20_object)
{
	cvector var_24_cvector;
	@GetPosition(var_24_cvector);
	cvector var_25_cvector;
	var_20_object->GetPosition(var_25_cvector);
	var_19_float = (var_25_cvector - var_24_cvector) | (var_25_cvector - var_24_cvector);
}


void func_687(object var_29_object)
{
	float var_32_float;
	var_29_object->GetEyesHeight(var_32_float);
	cvector var_33_cvector = [0.0, 0.0, 0.0];
	var_34_float = GetByIndex(var_33_cvector, 1);
	var_32_float = var_34_float;
	SetByIndex(var_33_cvector, 1) = var_34_float;
	@LookAsync(var_29_object, "head", var_33_cvector);
}


void func_564(bool var_23_bool)
{
	bool var_25_bool;
	@IsLoaded(var_25_bool);
	var_25_bool = var_23_bool;
}


void func_569(bool var_27_bool, object var_28_object, float var_29_float)
{
	cvector var_40_cvector; bool var_47_bool;
	var_28_object->GetPosition(var_40_cvector);
	float var_39_float;
	var_28_object->GetEyesHeight(var_39_float);
	var_48_float = GetByIndex(var_40_cvector, 1);
	SetByIndex(var_40_cvector, 1) = (var_48_float + var_39_float);
	cvector var_41_cvector;
	@GetPosition(var_41_cvector);
	@GetEyesHeight(var_39_float);
	var_49_float = GetByIndex(var_41_cvector, 1);
	SetByIndex(var_41_cvector, 1) = (var_49_float + var_39_float);
	cvector var_42_cvector = var_40_cvector - var_41_cvector;
	var_50_float = GetByIndex(var_42_cvector, 1);
	SetByIndex(var_42_cvector, 1) = (float)0;
	var_52_float = sqrt(var_42_cvector | var_42_cvector);
	var_42_cvector /= var_52_float;
	cvector var_43_cvector = -var_42_cvector;
	cvector var_54_cvector;
	func_711(var_54_cvector, (var_43_cvector ^ [0.0, 1.0, 0.0]));
	cvector var_44_cvector = ((var_42_cvector * var_29_float) + (var_54_cvector * 25)) - [0.0, 10.0, 0.0];
	bool var_46_bool;
	@IsOverrideActive(var_46_bool);
	if(var_46_bool != 0)
		var_27_bool = false;
	@StopWorld();
	@CameraTransit((var_41_cvector + var_44_cvector), var_43_cvector, true);
	var_68_float = GetByIndex(var_44_cvector, 0);
	var_69_float = GetByIndex(var_44_cvector, 2);
	@Rotate(var_68_float, var_69_float);
	bool var_70_bool;
	func_856(var_70_bool);
	if(var_70_bool != 0) {
	} else {
		@HasAnimationTrack(var_47_bool, "head");
		if(var_47_bool == 0) goto Label_632;
		@LookAsyncCamera("head");
	}
Label_632:
	@CameraWaitForPlayFinish();
	@ResumeWorld();
	var_27_bool = true;
	
}


void func_698(void)
{
	bool var_15_bool;
	func_856(var_15_bool);
	if(var_15_bool != 0)
		@lshStopSpeech();
}


void func_831(int var_79_int)
{
	int var_81_int;
	@GetVariable("branch", var_81_int);
	if(var_81_int == 0) {
		var_79_int = 1;
		return 2;
	EMIT "GOTO 0x34e";
	}
	if(var_81_int == 1) {
		var_79_int = 2;
		return 2;
	}
	var_79_int = 3;
}


void func_705(object var_19_object)
{
	object var_21_object;
	@self(var_21_object);
	var_21_object = var_19_object;
}
EMIT "Stack[-1] = 0";


void func_322(int var_6_int, float var_12_float, float var_13_float)
{
	float var_14_float; bool var_15_bool; float var_16_float; bool var_17_bool;
	var_6_int = false;
	
	for(;;) {
		@rand(var_16_float, 3);
		@Sleep((var_16_float + 3), var_17_bool);
		var_6_int = true;
		float var_21_float; float var_22_float;
		var_12_float = var_21_float;
		func_391(var_22_float, var_14_float, var_15_bool, var_16_float, var_17_bool, var_21_float, var_22_float);
		var_6_int = false;
	}
}
EMIT "Return(); Pop(4)";


void func_711(cvector var_54_cvector, cvector var_55_cvector)
{
	float var_58_float = sqrt(var_55_cvector | var_55_cvector);
	if(var_58_float < 0.000001)
		var_54_cvector = [0.0, 0.0, 0.0];
	var_54_cvector = var_55_cvector / var_58_float;
}


// @pe
void func_456(object var_2_object, string var_3_string)
{
	func_551();
	@KillTimer(10);
	if(var_2_object != 0) {
		@UnlookAsync("head");
		var_2_object = false;
	}
	var_3_string = true;
}


// @pe
void func_74(object var_0_object, object var_1_object, object var_2_object, string var_3_string, object var_88_object, object var_89_object)
{
	var_0_object = var_89_object;
	var_1_object = var_88_object;
	var_3_string = false;
	if(1 != 0) {
		bool var_95_bool;
		func_765(var_1_object);
		if(var_95_bool != 0) {
			object var_103_object; object var_104_object;
			var_103_object = var_1_object;
			var_104_object = var_0_object;
			func_750();
			func_162(var_89_object, "Neutral");
			var_0_object->SetMessage(534142); //@t
			var_0_object->ClearReplies(); //@t
			var_0_object->AddReply(534435, 36067, 36066); //@t
			var_0_object->AddReply(534437, 36067, 36068); //@t
		} else {
					func_162(var_89_object, "Neutral");
					var_0_object->SetMessage(534144); //@t
					var_0_object->ClearReplies(); //@t
					var_0_object->AddReply(534145, -1, 35732); //@t
					var_0_object->AddReply(534443, -1, 36076); //@t
		}
	}
	for(;;) {
		bool var_131_bool;
		func_856(var_131_bool);
		if(var_131_bool != 0) {

			for(;;) {
				@lshWaitForAnimEnd();
				if(var_3_string != 0) {
				} else {
					func_656(var_2_object);
				}
		} else {
			@PlayAnimation("all", "idle");

			for(;;) {
				@WaitForAnimEnd();
				if(var_3_string != 0) {
					goto Label_161;
				}
				@PlayAnimation("all", "idle");
			}

		}
		Label_161:
			return 0;

		}

	}
}
EMIT "Return(); Pop(0)";
EMIT "GOTO 0x4e";


void func_848(int var_76_int)
{
	var_76_int = 541582;
}


void func_721(int var_97_int, string var_98_string)
{
	int var_100_int;
	@GetVariable(var_98_string, var_100_int);
	var_100_int = var_97_int;
}


void func_850(int var_75_int)
{
	var_75_int = 541581;
}


void func_852(string var_77_string)
{
	var_77_string = "ui/NPC_Citizen3.png";
}


void func_340(int var_5_int)
{
	var_5_int = true;
	bool var_15_bool = false;
	bool var_16_bool;
	func_564(var_16_bool);
	if(!var_16_bool) { //@nz
		bool var_20_bool;
		func_389(var_20_bool);
		if(var_20_bool != 0)
			var_15_bool = true;
	}
	if(var_15_bool != 0) {
		object var_21_object;
		func_705(var_21_object);
		@RemoveActor(var_21_object);
	}
}


void func_854(string var_78_string)
{
	var_78_string = "ui/NPC_Citizen3_b.png";
}


void func_470(void)
{
	int var_39_int; int var_40_int; bool var_41_bool; float var_42_float; bool var_43_bool;
	@WaitForAnimEnd();
	bool var_44_bool;
	func_564(var_44_bool);
	if(!var_44_bool) //@nz
		return 14;
	int var_46_int;
	func_733(var_46_int);
	int var_37_int;
	var_46_int = var_37_int;
	int var_38_int = 0;
	
	for(;;) {
		bool var_59_bool = false;
		if(var_38_int < 5) {
			bool var_62_bool;
			func_564(var_62_bool);
			if(var_62_bool != 0)
				var_59_bool = true;
		}
		if(var_59_bool != 0) {
			@irand(var_39_int, 3);
			if(var_39_int == 0) {
				if(var_37_int == 0) goto Label_517;
				@irand(var_40_int, var_37_int);
				string var_68_string; int var_69_int;
				var_40_int = var_69_int;
				func_726(var_68_string, var_69_int);
				@PlayAnimation("all", var_68_string);
				@WaitForAnimEnd(var_41_bool);
				if(!var_41_bool) { //@nz
				} else {
			} else {
			if(var_39_int == 1) {
				@rand(var_42_float, 4);
				@Sleep((var_42_float + 1), var_43_bool);
				if(!var_43_bool) { //@nz
					goto Label_546;
				}
			} else if(var_38_int != 0) {
				goto Label_546;
			}
			}
					bool var_71_bool;
					func_549(var_71_bool);
					var_72_bool = !var_71_bool; //@nz
					if(var_72_bool == 0) goto Label_541;
			}
		}
	Label_546:
		for(;;) {
			@ResetAAS();
			return 14;

		}

	Label_541:
		@ResetAAS();
		var_38_int += 1;
	}
	
}


void func_856(bool var_70_bool)
{
	var_70_bool = false;
}


void func_726(string var_52_string, int var_53_int)
{
	string var_55_string = "idle";
	if(var_53_int != 0)
		var_55_string += var_53_int;
	var_55_string = var_52_string;
}


void func_733(int var_46_int)
{
	int var_49_int; bool var_50_bool;
	var_49_int = 0;
	
	for(;;) {
		string var_52_string; int var_53_int;
		var_49_int = var_53_int;
		func_726(var_52_string, var_53_int);
		@HasAnimation(var_50_bool, "all", var_52_string);
		if(!var_50_bool) //@nz
			break;
		var_49_int += 1;
	}
	var_49_int = var_46_int;
}


// @pe
void func_750(void)
{
	@SetVariable("ood10Salesman1", 1);
}


// @pe
void func_756(void)
{
	@SetVariable("d10q03", 2);
	func_777();
}


// @pe
void func_765(bool var_95_bool)
{
	int var_97_int;
	func_721(var_97_int, "ood10Salesman1");
	if(var_97_int == 0) {
		var_95_bool = true;
		return 0;
	}
	var_95_bool = false;
}


void func_638(void)
{
	bool var_160_bool;
	@CameraSwitchToNormal(true);
	bool var_162_bool;
	func_856(var_162_bool);
	if(var_162_bool != 0) {
	} else {
		@HasAnimationTrack(var_160_bool, "head");
		if(var_160_bool == 0) goto Label_655;
		@UnlookAsync("head");
	}
Label_655:
	
}


