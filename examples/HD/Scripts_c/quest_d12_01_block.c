// @IMPORTS: CreateDialog/1,IsOverrideActive/1,DoDialog/1,sync/0,StopDialog/1,lshWaitForAnimEnd/0,PlayAnimation/2,WaitForAnimEnd/0,lshStopAnimation/0,StopAnimation/0,Hold/0,IsLoaded/1,RemoveActor/1,GetPosition/1,GetEyesHeight/1,StopWorld/0,CameraTransit/3,Rotate/2,HasAnimationTrack/2,LookAsyncCamera/1,CameraWaitForPlayFinish/0,ResumeWorld/0,CameraSwitchToNormal/1,UnlookAsync/1,GetVariable/2,irand/2,SetVariable/2,lshHasAnimation/2,lshGetAnimTimes/3,lshPlayAnimation/3,Trace/1,lshHasSpeech/2,lshPlaySpeech/1,lshStopSpeech/0,self/1,FindActor/2,GetGameTime/1,CreateDiaryEntry/4,GetDiaryRoot/1,SendWorldWndMessage/1,SetDiarySection/1
// @STRINGS: A:SetNPCName|A:SetNPCDescription|A:SetPhoto|A:SetPhoto2|A:SetPlayerName|A:IsDialogEnd|A:GetReturnValue|W:Neutral|A:SetMessage|A:ClearReplies|A:AddReply|W:all|W:idle|W:Doubt|W:cleanup|W:restore|A:GetPosition|A:GetEyesHeight|W:head|W:voice_common|W:c|A:HasProperty|A:GetProperty|W:m|W:Can't find lsh animation : |W:Door |W: not found|W:locked|A:SetProperty|W:game_final|W:isobor@door1|W:d12q01BurahInSobor|W:d12q01KlaraInSobor|W:Can't retrieve diary root|A:Find|W:Can't find diary parent with id: |A:AddChild|A:GetCategory|W:branch|W:ui/NPC_Block.png|W:ui/NPC_Block_b.png
// @GLOBALS: 0:object:
// @RUN_OP: 0x235
// @RUN_TASK: 2
// @TASK_0: vars=object params=2
// @TASK_1: vars=object,object,string,bool params=2
// @EVENT_11: op=0xcf vars=int,int
// @TASK_2: vars=bool params=0
// @EVENT_0: op=0x239 vars=object
// @EVENT_26: op=0x245 vars=string
// @EVENT_6: op=0x261 vars=
// @PE: 0x51,0xb9,0xcf,0x235,0x239,0x3b8,0x3be,0x3c4,0x3ca,0x3d1,0x3d7,0x3dc,0x3e1,0x3e6,0x3eb,0x3f7,0x403

task_1_event_11(var_0_object, var_1_object, var_2_object, var_3_string, var_4_bool, var_5_int, var_6_int, var_7_bool)
{
	if((int)1 != 0) {
		func_898();
		var_11_bool = var_7_bool == (int)37241;
		if(var_11_bool != 0) {
			var_12_object = Obj(); var_13_object = Obj();
			var_12_object = var_1_object;
			var_13_object = var_0_object;
			func_958();
			var_16_object = Obj(); var_17_object = Obj();
			var_16_object = var_1_object;
			var_17_object = var_0_object;
			func_988();
			var_41_object = Obj(); var_42_object = Obj();
			var_41_object = var_1_object;
			var_42_object = var_0_object;
			func_970();
		}
		var_54_bool = var_7_bool == (int)37236;
		if(var_54_bool != 0) {
			var_55_object = Obj(); var_56_object = Obj();
			var_55_object = var_1_object;
			var_56_object = var_0_object;
			func_952();
			var_59_object = Obj(); var_60_object = Obj();
			var_59_object = var_1_object;
			var_60_object = var_0_object;
			func_998();
			var_69_object = Obj(); var_70_object = Obj();
			var_69_object = var_1_object;
			var_70_object = var_0_object;
			func_970();
		}
		var_72_bool = var_7_bool == (int)37246;
		if(var_72_bool != 0) {
			var_73_object = Obj(); var_74_object = Obj();
			var_73_object = var_1_object;
			var_74_object = var_0_object;
			func_964();
			var_77_object = Obj(); var_78_object = Obj();
			var_77_object = var_1_object;
			var_78_object = var_0_object;
			func_993();
			var_87_object = Obj(); var_88_object = Obj();
			var_87_object = var_1_object;
			var_88_object = var_0_object;
			func_970();
		}
		var_90_bool = var_7_bool == (int)42258;
		if(var_90_bool != 0) {
			var_91_object = Obj(); var_92_object = Obj();
			var_91_object = var_1_object;
			var_92_object = var_0_object;
			func_977();
			var_95_object = Obj(); var_96_object = Obj();
			var_95_object = var_1_object;
			var_96_object = var_0_object;
			func_970();
			var_97_object = Obj(); var_98_object = Obj();
			var_97_object = var_1_object;
			var_98_object = var_0_object;
			func_983();
		}
		var_108_bool = var_6_int == (int)37224;
		if(var_108_bool != 0) {
			var_109_bool = 0; var_110_object = Obj();
			var_110_object = var_1_object;
			func_1003(var_110_object);
			var_117_bool = var_109_bool == 0; //@nz
			if(var_117_bool != 0) {
				var_118_string = "";
				func_185(var_7_bool, "Neutral");
				@@@var_0_object:SetMessage((int)535541);
				@@@var_0_object:ClearReplies();
				@@@var_0_object:AddReply((int)535542, (int)37226, (int)37225);
				@@@var_0_object:AddReply((int)540019, (int)41991, (int)41990);
				var_142_bool = 0; var_143_object = Obj();
				var_143_object = var_1_object;
				func_1015(var_143_object);
				if(var_142_bool != 0) {
					@@@var_0_object:AddReply((int)535545, (int)42317, (int)37228);
				}
				var_151_bool = 0; var_152_object = Obj();
				var_152_object = var_1_object;
				func_1027(var_152_object);
				if(var_151_bool != 0) {
					@@@var_0_object:AddReply((int)535559, (int)37243, (int)37242);
				}
				@@@var_0_object:AddReply((int)540276, (int)42323, (int)42256);
				return 0;
			}
			var_163_string = "";
			func_185(var_7_bool, "Neutral");
			@@@var_0_object:SetMessage((int)535564);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)535565, (int)-1, (int)37248);
			return 0;
		}
		var_169_bool = var_6_int == (int)42323;
		if(var_169_bool != 0) {
			var_170_string = "";
			func_185(var_7_bool, "Neutral");
			@@@var_0_object:SetMessage((int)540340);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)540278, (int)-1, (int)42258);
			return 0;
		}
		var_176_bool = var_6_int == (int)37243;
		if(var_176_bool != 0) {
			var_177_string = "";
			func_185(var_7_bool, "Neutral");
			@@@var_0_object:SetMessage((int)535560);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)535561, (int)37245, (int)37244);
			return 0;
		}
		var_183_bool = var_6_int == (int)37245;
		if(var_183_bool != 0) {
			var_184_string = "";
			func_185(var_7_bool, "Doubt");
			@@@var_0_object:SetMessage((int)535562);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)535563, (int)-1, (int)37246);
			return 0;
		}
		var_190_bool = var_6_int == (int)42317;
		if(var_190_bool != 0) {
			var_191_string = "";
			func_185(var_7_bool, "Neutral");
			@@@var_0_object:SetMessage((int)540334);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)540336, (int)42320, (int)42319);
			return 0;
		}
		var_197_bool = var_6_int == (int)42320;
		if(var_197_bool != 0) {
			var_198_string = "";
			func_185(var_7_bool, "Doubt");
			@@@var_0_object:SetMessage((int)540337);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)535553, (int)-1, (int)37236);
			return 0;
		}
		var_204_bool = var_6_int == (int)41991;
		if(var_204_bool != 0) {
			var_205_string = "";
			func_185(var_7_bool, "Neutral");
			@@@var_0_object:SetMessage((int)540020);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)540021, (int)41993, (int)41992);
			return 0;
		}
		var_211_bool = var_6_int == (int)41993;
		if(var_211_bool != 0) {
			var_212_string = "";
			func_185(var_7_bool, "Neutral");
			@@@var_0_object:SetMessage((int)540022);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)540023, (int)41995, (int)41994);
			return 0;
		}
		var_218_bool = var_6_int == (int)41995;
		if(var_218_bool != 0) {
			var_219_string = "";
			func_185(var_7_bool, "Neutral");
			@@@var_0_object:SetMessage((int)540024);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)540025, (int)37238, (int)41996);
			return 0;
		}
		var_225_bool = var_6_int == (int)37238;
		if(var_225_bool != 0) {
			var_226_string = "";
			func_185(var_7_bool, "Neutral");
			@@@var_0_object:SetMessage((int)535555);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)535556, (int)37240, (int)37239);
			return 0;
		}
		var_232_bool = var_6_int == (int)37240;
		if(var_232_bool != 0) {
			var_233_string = "";
			func_185(var_7_bool, "Neutral");
			@@@var_0_object:SetMessage((int)535557);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)535558, (int)-1, (int)37241);
			return 0;
		}
		var_239_bool = var_6_int == (int)37226;
		if(var_239_bool != 0) {
			var_240_string = "";
			func_185(var_7_bool, "Neutral");
			@@@var_0_object:SetMessage((int)535543);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)535544, (int)-1, (int)37227);
			return 0;
		}
		var_3_string = true;
		var_245_bool = 0;
		func_1157(var_245_bool);
		if(var_245_bool != 0) {
			lshStopAnimation();
		} else {
			StopAnimation();
	}
		return 0;
	}
	return 0;
EMIT "GOTO 0xd0";
	
}


task_2_event_0(var_0_object, var_1_object, var_2_object, var_3_string, var_4_bool, var_5_bool, var_6_object)
{
	var_7_int = 0; var_8_object = Obj();
	var_6_object = var_8_object;
	TaskCall(0);
	func_0(var_9_object, var_7_int, var_8_object);
	TaskReturn();
	return 0;
}


task_2_event_26(var_0_object, var_1_object, var_2_object, var_3_string, var_4_bool, var_5_bool, var_6_string)
{
	var_7_bool = 0; var_8_bool = 0;
	var_10_bool = var_6_string == "cleanup";
	if(var_10_bool != 0) {
		var_0_object = true;
		IsLoaded(var_8_bool);
		var_11_bool = 0;
		var_11_bool = 0;
		var_12_bool = var_8_bool == 0; //@nz
		if(var_12_bool != 0) {
			var_13_bool = 0;
			func_625(var_13_bool);
			if(var_13_bool != 0) {
				var_11_bool = 1;
			}
		}
		if(var_11_bool != 0) {
			var_14_object = Obj();
			func_905(var_14_object);
			RemoveActor(var_14_object);
		}
	} else {
		var_18_bool = var_6_string == "restore";
		if(var_18_bool == 0) goto Label_608;
		var_0_object = false;
	}
Label_608:
	return 2;
	
}


task_2_event_6(var_0_object, var_1_object, var_2_object, var_3_string, var_4_bool, var_5_bool)
{
	var_6_bool = 0;
	var_6_bool = 0;
	var_7_object = var_0_object;
	if(var_7_object != 0) {
		var_8_bool = 0;
		func_625(var_8_bool);
		if(var_8_bool != 0) {
			var_6_bool = 1;
		}
	}
	if(var_6_bool != 0) {
		var_9_object = Obj();
		func_905(var_9_object);
		RemoveActor(var_9_object);
	}
	return 0;
}


main(var_0_object, var_1_object, var_2_object, var_3_string, var_4_bool, var_5_bool)
{
	func_577();
	return 0;
}


func_0(var_0_object, var_7_int, var_8_object)
{
	var_10_object = Obj(); var_11_bool = 0; var_12_int = 0; var_13_bool = 0; var_14_object = Obj(); var_15_bool = 0; var_16_int = 0; var_17_bool = 0;
	var_0_object = var_8_object;
	var_18_bool = 0; var_19_object = Obj(); var_20_float = 0;
	var_8_object = var_19_object;
	func_627(var_18_bool, var_19_object, (float)70.0);
	var_65_bool = var_18_bool == 0; //@nz
	if(var_65_bool != 0) {
		var_7_int = -2;
		return 8;
	}
	CreateDialog(var_14_object);
	var_66_int = 0;
	func_1151(var_66_int);
	@@var_14_object:SetNPCName(var_66_int);
	var_67_int = 0;
	func_1149(var_67_int);
	@@var_14_object:SetNPCDescription(var_67_int);
	var_68_string = "";
	func_1153(var_68_string);
	@@var_14_object:SetPhoto(var_68_string);
	var_69_string = "";
	func_1155(var_69_string);
	@@var_14_object:SetPhoto2(var_69_string);
	var_70_int = 0;
	func_1132(var_70_int);
	@@var_14_object:SetPlayerName(var_70_int);
	IsOverrideActive(var_15_bool);
	var_78_bool = var_15_bool;
	if(var_78_bool != 0) {
		var_7_int = -2;
		return 8;
	}
	DoDialog(var_14_object);
	var_79_bool = 0; var_80_object = Obj();
	func_905(Obj());
	var_81_object = var_80_object;
	func_714(var_79_bool, var_80_object);
	var_174_object = Obj(); var_175_object = Obj();
	var_8_object = var_174_object;
	var_14_object = var_175_object;
	TaskCall(1);
	func_81(var_176_object, var_177_object, var_178_string, var_179_bool, var_174_object, var_175_object);
	TaskReturn();
	@@var_14_object:IsDialogEnd(var_17_bool);
	
Label_63:
	var_258_bool = var_17_bool == 0; //@nz
	if(var_258_bool != 0) {
		sync();
		@@var_14_object:IsDialogEnd(var_17_bool);
		goto Label_63;
	}
	var_8_object = Obj();
	func_696();
	StopDialog(var_14_object);
	@@var_14_object:GetReturnValue((int)-1);
	var_16_int = var_7_int;
	return 8;
}
EMIT "Stack[-4] = 0";


func_1153(var_68_string)
{
	var_68_string = "ui/NPC_Block.png";
	return 0;
}


func_898()
{
	var_9_bool = 0;
	func_1157(var_9_bool);
	if(var_9_bool != 0) {
		lshStopSpeech();
	}
	return 0;
}


func_1155(var_69_string)
{
	var_69_string = "ui/NPC_Block_b.png";
	return 0;
}


func_1027(var_223_bool)
{
	var_225_int = 0; var_226_string = "";
	func_921(var_225_int, "d12q01KlaraInSobor");
	var_228_bool = var_225_int != (int)0;
	if(var_228_bool != 0) {
		var_223_bool = 1;
		return 0;
	}
	var_223_bool = 0;
	return 0;
}


func_1157(var_61_bool)
{
	var_61_bool = 1;
	return 0;
}


func_772(var_90_bool, var_91_object)
{
	var_92_string = ""; var_93_int = 0; var_94_bool = 0; var_95_int = 0; var_96_string = ""; var_97_string = ""; var_98_int = 0; var_99_bool = 0; var_100_int = 0; var_101_string = "";
	var_97_string = "c";
	var_98_int = 0;
	
Label_775:
	if((int)1 != 0) {
		var_104_int = var_98_int + (int)1;
		var_105_int = var_97_string + var_104_int;
		@@var_91_object:HasProperty(var_105_int, var_99_bool);
		var_106_bool = var_99_bool == 0; //@nz
		if(var_106_bool != 0) {
		} else {
			var_98_int = var_98_int + (int)1;
			goto Label_775;
		}
	}
	var_107_bool = var_98_int == 0; //@nz
	if(var_107_bool != 0) {
		var_90_bool = 0;
		return 10;
	}
	var_100_int = 0;
	var_109_bool = var_98_int > (int)1;
	if(var_109_bool != 0) {
		irand(var_100_int, var_98_int);
	}
	var_111_int = var_100_int + (int)1;
	var_112_int = var_97_string + var_111_int;
	@@var_91_object:GetProperty(var_112_int, var_101_string);
	var_113_bool = 0; var_114_string = "";
	var_101_string = var_114_string;
	func_883(var_113_bool, var_114_string);
	var_113_bool = var_90_bool;
	return 10;
	
}


func_905(var_9_object)
{
	var_10_object = Obj(); var_11_object = Obj();
	self(var_11_object);
	var_11_object = var_9_object;
	return 2;
}
EMIT "Stack[-1] = 0";


func_911(var_45_cvector, var_46_cvector)
{
	var_48_float = 0; var_49_float = 0;
	var_50_int = var_46_cvector | var_46_cvector;
	var_49_float = sqrt(var_50_int);
	var_51_float = 9.999999974752427e-07;
	var_52_bool = var_49_float < var_51_float;
	if(var_52_bool != 0) {
		var_45_cvector = CVector(0.0, 0.0, 0.0);
		return 2;
	}
	var_45_cvector = var_46_cvector / var_49_float;
	return 2;
}


func_1039()
{
	var_99_object = Obj(); var_100_object = Obj();
	CreateDiaryEntry(var_100_object, (int)773, (int)1, (int)540274);
	var_104_bool = 0; var_105_object = Obj(); var_106_int = 0;
	var_100_object = var_105_object;
	func_1104(var_104_bool, var_105_object, (int)699);
	return 2;
}
EMIT "Stack[-1] = 0";


func_921(var_183_int, var_184_string)
{
	var_185_int = 0; var_186_int = 0;
	GetVariable(var_184_string, var_186_int);
	var_186_int = var_183_int;
	return 2;
}


func_1052()
{
	var_79_object = Obj(); var_80_object = Obj();
	CreateDiaryEntry(var_80_object, (int)704, (int)1, (int)535687);
	var_84_bool = 0; var_85_object = Obj(); var_86_int = 0;
	var_80_object = var_85_object;
	func_1104(var_84_bool, var_85_object, (int)699);
	return 2;
}
EMIT "Stack[-1] = 0";


func_926(var_43_string, var_44_bool)
{
	var_45_object = Obj(); var_46_object = Obj();
	FindActor(var_46_object, var_43_string);
	var_47_bool = var_46_object == 0; //@nz
	if(var_47_bool != 0) {
		var_49_int = "Door " + var_43_string;
		var_51_int = var_49_int + " not found";
		Trace(var_51_int);
	} else {
		@@var_46_object:SetProperty("locked", var_44_bool);
	}
	return 2;
	
}
EMIT "Stack[-1] = 0";


func_809(var_121_bool, var_122_object)
{
	var_123_string = ""; var_124_int = 0; var_125_bool = 0; var_126_int = 0; var_127_string = ""; var_128_string = ""; var_129_int = 0; var_130_bool = 0; var_131_int = 0; var_132_string = "";
	var_134_int = 0;
	func_943(var_134_int);
	var_140_int = "d" + var_134_int;
	var_128_string = var_140_int + "m";
	var_129_int = 0;
	
Label_818:
	if((int)1 != 0) {
		var_144_int = var_129_int + (int)1;
		var_145_int = var_128_string + var_144_int;
		@@var_122_object:HasProperty(var_145_int, var_130_bool);
		var_146_bool = var_130_bool == 0; //@nz
		if(var_146_bool != 0) {
		} else {
			var_129_int = var_129_int + (int)1;
			goto Label_818;
		}
	}
	var_147_bool = var_129_int == 0; //@nz
	if(var_147_bool != 0) {
		var_121_bool = 0;
		return 10;
	}
	var_131_int = 0;
	var_149_bool = var_129_int > (int)1;
	if(var_149_bool != 0) {
		irand(var_131_int, var_129_int);
	}
	var_151_int = var_131_int + (int)1;
	var_152_int = var_128_string + var_151_int;
	@@var_122_object:GetProperty(var_152_int, var_132_string);
	var_153_bool = 0; var_154_string = "";
	var_132_string = var_154_string;
	func_883(var_153_bool, var_154_string);
	var_153_bool = var_121_bool;
	return 10;
	
}


func_1065()
{
	var_61_object = Obj(); var_62_object = Obj();
	CreateDiaryEntry(var_62_object, (int)702, (int)1, (int)535685);
	var_66_bool = 0; var_67_object = Obj(); var_68_int = 0;
	var_62_object = var_67_object;
	func_1104(var_66_bool, var_67_object, (int)699);
	return 2;
}
EMIT "Stack[-1] = 0";


func_943(var_134_int)
{
	var_135_float = 0; var_136_float = 0;
	GetGameTime(var_136_float);
	var_138_int = 0;
	var_138_int = var_136_float / (int)24;
	var_134_int = (int)1 + var_138_int;
	return 2;
}


func_1078()
{
	var_18_object = Obj(); var_19_object = Obj();
	CreateDiaryEntry(var_19_object, (int)703, (int)1, (int)535686);
	var_23_bool = 0; var_24_object = Obj(); var_25_int = 0;
	var_19_object = var_24_object;
	func_1104(var_23_bool, var_24_object, (int)699);
	return 2;
}
EMIT "Stack[-1] = 0";


func_696()
{
	var_260_bool = 0; var_261_bool = 0;
	CameraSwitchToNormal((bool)1);
	var_263_bool = 0;
	func_1157(var_263_bool);
	if(var_263_bool != 0) {
	} else {
		HasAnimationTrack(var_261_bool, "head");
		var_265_bool = var_261_bool;
		if(var_265_bool == 0) goto Label_713;
		UnlookAsync("head");
	}
Label_713:
	return 2;
	
}


func_185(var_2_object, var_190_string)
{
	var_191_bool = 0;
	func_1157(var_191_bool);
	var_192_bool = var_191_bool == 0; //@nz
	if(var_192_bool != 0) {
		return 0;
	}
	var_193_bool = var_190_string == var_2_object;
	if(var_193_bool != 0) {
		return 0;
	}
	var_194_string = ""; var_195_bool = 0;
	var_190_string = var_194_string;
	var_197_bool = var_190_string == "";
	if(var_197_bool != 0) {
		var_195_bool = 0;
	} else {
		var_195_bool = 1;
	}
	func_868(var_194_string, var_195_bool);
	var_2_object = var_190_string;
	return 0;
	
}


func_952()
{
	SetVariable("game_final", (int)1);
	return 0;
}


func_958()
{
	SetVariable("game_final", (int)2);
	return 0;
}


func_577()
{
	
Label_577:
	Hold();
	goto Label_577;
}
EMIT "Return(); Pop(0)";


func_1091(var_32_object)
{
	var_33_object = Obj(); var_34_object = Obj();
	GetDiaryRoot(var_34_object);
	var_35_bool = var_34_object == 0; //@nz
	if(var_35_bool != 0) {
		Trace("Can't retrieve diary root");
		var_32_object = 0;
		return 2;
	}
	var_34_object = var_32_object;
	return 2;
}
EMIT "Stack[-1] = 0";


func_964()
{
	SetVariable("game_final", (int)3);
	return 0;
}


func_714(var_79_bool, var_80_object)
{
	var_84_int = 0; var_85_int = 0; var_86_int = 0; var_87_int = 0;
	GetVariable("voice_common", var_86_int);
	var_89_int = var_86_int;
	if(var_89_int != 0) {
		var_90_bool = 0; var_91_object = Obj();
		var_80_object = var_91_object;
		func_772(var_90_bool, var_91_object);
		var_120_bool = var_90_bool == 0; //@nz
		if(var_120_bool != 0) {
			var_121_bool = 0; var_122_object = Obj();
			var_80_object = var_122_object;
			func_809(var_121_bool, var_122_object);
			var_156_bool = var_121_bool == 0; //@nz
			if(var_156_bool != 0) {
				var_79_bool = 0;
				return 4;
			}
		}
		irand(var_87_int, (int)2);
		var_158_int = var_87_int;
		if(var_158_int != 0) {
			var_161_int = var_86_int + (int)1;
			var_163_int = var_161_int % (int)3;
			SetVariable("voice_common", var_163_int);
		} else {
			SetVariable("voice_common", (int)0);
	}
		var_166_bool = 0; var_167_object = Obj();
		var_80_object = var_167_object;
		func_809(var_166_bool, var_167_object);
		var_168_bool = var_166_bool == 0; //@nz
		if(var_168_bool != 0) {
			var_169_bool = 0; var_170_object = Obj();
			var_80_object = var_170_object;
			func_772(var_169_bool, var_170_object);
			var_171_bool = var_169_bool == 0; //@nz
			if(var_171_bool != 0) {
				var_79_bool = 0;
				return 4;
			}
		}
		SetVariable("voice_common", (int)1);
	}
	goto Label_770;
	
Label_770:
	var_79_bool = 1;
	return 4;
	
}


func_970()
{
	var_43_string = ""; var_44_bool = 0;
	func_926("isobor@door1", (bool)0);
	return 0;
}


func_1104(var_23_bool, var_24_object, var_25_int)
{
	var_26_object = Obj(); var_27_object = Obj(); var_28_int = 0; var_29_object = Obj(); var_30_object = Obj(); var_31_int = 0;
	func_1091(Obj());
	var_32_object = var_29_object;
	@@var_29_object:Find(var_25_int, var_30_object);
	var_37_bool = var_30_object == 0; //@nz
	if(var_37_bool != 0) {
		var_39_int = "Can't find diary parent with id: " + var_25_int;
		Trace(var_39_int);
		var_23_bool = 0;
		return 6;
	}
	@@var_30_object:AddChild(var_24_object);
	SendWorldWndMessage((int)7);
	@@var_24_object:GetCategory(var_31_int);
	SetDiarySection(var_31_int);
	var_23_bool = 0;
	return 6;
}
EMIT "Stack[-2] = 0";
EMIT "Stack[-3] = 0";


func_81(var_0_object, var_1_object, var_2_object, var_3_string, var_174_object, var_175_object)
{
	var_0_object = var_175_object;
	var_1_object = var_174_object;
	var_3_string = false;
	if((int)1 != 0) {
		var_181_bool = 0; var_182_object = Obj();
		var_182_object = var_1_object;
		func_1003(var_182_object);
		var_189_bool = var_181_bool == 0; //@nz
		if(var_189_bool != 0) {
			var_190_string = "";
			func_185(var_175_object, "Neutral");
			@@@var_0_object:SetMessage((int)535541);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)535542, (int)37226, (int)37225);
			@@@var_0_object:AddReply((int)540019, (int)41991, (int)41990);
			var_214_bool = 0; var_215_object = Obj();
			var_215_object = var_1_object;
			func_1015(var_215_object);
			if(var_214_bool != 0) {
				@@@var_0_object:AddReply((int)535545, (int)42317, (int)37228);
			}
			var_223_bool = 0; var_224_object = Obj();
			var_224_object = var_1_object;
			func_1027(var_224_object);
			if(var_223_bool != 0) {
				@@@var_0_object:AddReply((int)535559, (int)37243, (int)37242);
			}
			@@@var_0_object:AddReply((int)540276, (int)42323, (int)42256);
		} else {
				var_253_string = "";
				func_185(var_175_object, "Neutral");
				@@@var_0_object:SetMessage((int)535564);
				@@@var_0_object:ClearReplies();
				@@@var_0_object:AddReply((int)535565, (int)-1, (int)37248);
				goto Label_155;
		}
	}
Label_155:
	var_235_bool = 0;
	func_1157(var_235_bool);
	if(var_235_bool != 0) {

	Label_159:
		lshWaitForAnimEnd();
		var_236_string = var_3_string;
		if(var_236_string != 0) {
		} else {
			var_237_string = "";
			var_237_string = var_2_object;
			func_852(var_237_string);
			goto Label_159;
	}
		PlayAnimation("all", "idle");

	Label_174:
		WaitForAnimEnd();
		var_250_string = var_3_string;
		if(var_250_string != 0) {
			goto Label_184;
		}
		PlayAnimation("all", "idle");
		goto Label_174;

	}
	goto Label_184;
	
Label_184:
	return 0;
	
}
EMIT "Return(); Pop(0)";
EMIT "GOTO 0x55";


func_977()
{
	SetVariable("game_final", (int)4);
	return 0;
}


func_852(var_237_string)
{
	var_238_bool = 0; var_239_float = 0; var_240_float = 0; var_241_bool = 0; var_242_float = 0; var_243_float = 0;
	lshHasAnimation(var_241_bool, var_237_string);
	var_244_bool = var_241_bool;
	if(var_244_bool != 0) {
		lshGetAnimTimes(var_237_string, var_242_float, var_243_float);
		lshPlayAnimation(var_242_float, var_243_float, (bool)0);
	} else {
		var_247_int = "Can't find lsh animation : " + var_237_string;
		Trace(var_247_int);
	}
	return 6;
	
}


func_983()
{
	func_1039();
	return 0;
}


func_988()
{
	func_1078();
	return 0;
}


func_993()
{
	func_1052();
	return 0;
}


func_868(var_194_string, var_195_bool)
{
	var_198_bool = 0; var_199_float = 0; var_200_float = 0; var_201_bool = 0; var_202_float = 0; var_203_float = 0;
	lshHasAnimation(var_201_bool, var_194_string);
	var_204_bool = var_201_bool;
	if(var_204_bool != 0) {
		lshGetAnimTimes(var_194_string, var_202_float, var_203_float);
		lshPlayAnimation(var_202_float, var_203_float, var_195_bool);
	} else {
		var_206_int = "Can't find lsh animation : " + var_194_string;
		Trace(var_206_int);
	}
	return 6;
	
}


func_998()
{
	func_1065();
	return 0;
}


func_1003(var_181_bool)
{
	var_183_int = 0; var_184_string = "";
	func_921(var_183_int, "game_final");
	var_188_bool = var_183_int != (int)0;
	if(var_188_bool != 0) {
		var_181_bool = 1;
		return 0;
	}
	var_181_bool = 0;
	return 0;
}


func_1132(var_70_int)
{
	var_71_int = 0; var_72_int = 0;
	GetVariable("branch", var_72_int);
	var_75_bool = var_72_int == (int)0;
	if(var_75_bool != 0) {
		var_70_int = 1;
		return 2;
	EMIT "GOTO 0x47b";
	}
	var_77_bool = var_72_int == (int)1;
	if(var_77_bool != 0) {
		var_70_int = 2;
		return 2;
	}
	var_70_int = 3;
	return 2;
}


func_625(var_8_bool)
{
	var_8_bool = 1;
	return 0;
}


func_627(var_18_bool, var_19_object, var_20_float)
{
	var_21_float = 0; var_22_cvector = CVector(0,0,0); var_23_cvector = CVector(0,0,0); var_24_cvector = CVector(0,0,0); var_25_cvector = CVector(0,0,0); var_26_cvector = CVector(0,0,0); var_27_cvector = CVector(0,0,0); var_28_bool = 0; var_29_bool = 0; var_30_float = 0; var_31_cvector = CVector(0,0,0); var_32_cvector = CVector(0,0,0); var_33_cvector = CVector(0,0,0); var_34_cvector = CVector(0,0,0); var_35_cvector = CVector(0,0,0); var_36_cvector = CVector(0,0,0); var_37_bool = 0; var_38_bool = 0;
	@@var_19_object:GetPosition(var_31_cvector);
	@@var_19_object:GetEyesHeight(var_30_float);
	var_39_float = GetByIndex(var_31_cvector, 1);
	var_39_float = var_39_float + var_30_float;
	SetByIndex(var_31_cvector, 1) = var_39_float;
	GetPosition(var_32_cvector);
	GetEyesHeight(var_30_float);
	var_40_float = GetByIndex(var_32_cvector, 1);
	var_40_float = var_40_float + var_30_float;
	SetByIndex(var_32_cvector, 1) = var_40_float;
	var_33_cvector = var_31_cvector - var_32_cvector;
	var_41_float = GetByIndex(var_33_cvector, 1);
	SetByIndex(var_33_cvector, 1) = (float)0;
	var_42_int = var_33_cvector | var_33_cvector;
	var_43_float = sqrt(var_42_int);
	var_33_cvector = var_33_cvector / var_43_float;
	var_34_cvector = -var_33_cvector;
	var_44_float = var_33_cvector * var_20_float;
	var_45_cvector = CVector(0,0,0); var_46_cvector = CVector(0,0,0);
	var_46_cvector = var_34_cvector ^ CVector(0.0, 1.0, 0.0);
	func_911(var_45_cvector, var_46_cvector);
	var_54_float = var_45_cvector * (int)25;
	var_55_int = var_44_float + var_54_float;
	var_35_cvector = var_55_int - CVector(0.0, 10.0, 0.0);
	var_36_cvector = var_32_cvector + var_35_cvector;
	IsOverrideActive(var_37_bool);
	var_57_bool = var_37_bool;
	if(var_57_bool != 0) {
		var_18_bool = 0;
		return 18;
	}
	StopWorld();
	CameraTransit(var_36_cvector, var_34_cvector, (bool)1);
	var_59_float = GetByIndex(var_35_cvector, 0);
	var_60_float = GetByIndex(var_35_cvector, 2);
	Rotate(var_59_float, var_60_float);
	var_61_bool = 0;
	func_1157(var_61_bool);
	if(var_61_bool != 0) {
	} else {
		HasAnimationTrack(var_38_bool, "head");
		var_63_bool = var_38_bool;
		if(var_63_bool == 0) goto Label_690;
		LookAsyncCamera("head");
	}
Label_690:
	CameraWaitForPlayFinish();
	ResumeWorld();
	var_18_bool = 1;
	return 18;
	
}


func_883(var_113_bool, var_114_string)
{
	var_115_bool = 0; var_116_bool = 0;
	var_117_bool = 0;
	func_1157(var_117_bool);
	if(var_117_bool != 0) {
		lshHasSpeech(var_116_bool, var_114_string);
		var_118_bool = var_116_bool;
		if(var_118_bool != 0) {
			lshPlaySpeech(var_114_string);
			var_113_bool = 1;
			return 2;
		}
	}
	var_113_bool = 0;
	return 2;
}


func_1015(var_214_bool)
{
	var_216_int = 0; var_217_string = "";
	func_921(var_216_int, "d12q01BurahInSobor");
	var_219_bool = var_216_int != (int)0;
	if(var_219_bool != 0) {
		var_214_bool = 1;
		return 0;
	}
	var_214_bool = 0;
	return 0;
}


func_1149(var_67_int)
{
	var_67_int = 515532;
	return 0;
}


func_1151(var_66_int)
{
	var_66_int = 514840;
	return 0;
}


