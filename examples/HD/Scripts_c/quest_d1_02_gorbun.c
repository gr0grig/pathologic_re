// @IMPORTS: CreateDialog/1,IsOverrideActive/1,DoDialog/1,sync/0,StopDialog/1,lshWaitForAnimEnd/0,PlayAnimation/2,WaitForAnimEnd/0,lshStopAnimation/0,StopAnimation/0,rand/2,Sleep/2,RemoveActor/1,FindActor/2,SetTimer/2,KillTimer/1,UnlookAsync/1,irand/2,WaitForAnimEnd/1,ResetAAS/0,StopGroup0/0,GetPosition/1,IsLoaded/1,GetEyesHeight/1,StopWorld/0,CameraTransit/3,Rotate/2,HasAnimationTrack/2,LookAsyncCamera/1,CameraWaitForPlayFinish/0,ResumeWorld/0,CameraSwitchToNormal/1,lshHasAnimation/2,lshGetAnimTimes/3,lshPlayAnimation/3,Trace/1,LookAsync/3,lshStopSpeech/0,self/1,GetVariable/2,CreateIntVector/1,SendWorldWndMessage/2,GetInvItemByName/2,Trigger/2,HasAnimation/3,SetVariable/2,TriggerWorld/2,CreateDiaryEntry/4,GetDiaryRoot/1,SendWorldWndMessage/1,SetDiarySection/1,GetMainOutdoorScene/1
// @STRINGS: A:SetNPCName|A:SetNPCDescription|A:SetPhoto|A:SetPhoto2|A:SetPlayerName|A:IsDialogEnd|A:GetReturnValue|W:Neutral|A:SetMessage|A:ClearReplies|A:AddReply|W:all|W:idle|W:cleanup|W:player|W:head|A:GetPosition|A:GetProperty|A:SetProperty|A:GetEyesHeight|W:Can't find lsh animation : |A:add|W:money|W:Money|W:ui/NPC_Citizen3.png|W:ui/NPC_Citizen3_b.png|W:d1q02AnnaGotoLaska|A:FindMark|A:Remove|W:d1q02JuliaGotoAnna|W:quest_d1_02|W:completed|W:ood1GorbunLaska1|W:d1q02|W:money 2000 removed|W:ood1GorbunLaska2|W:playsound|W:givemoney|W:Can't retrieve diary root|A:Find|W:Can't find diary parent with id: |A:AddChild|A:GetCategory|W:Can't find main outdoor scene|A:GetMap|W:branch
// @GLOBALS: 0:object:
// @RUN_OP: 0x1fd
// @RUN_TASK: 2
// @TASK_0: vars=object params=2
// @TASK_1: vars=object,object,string,bool params=2
// @EVENT_11: op=0xc4 vars=int,int
// @TASK_2: vars=int,int,bool,bool,object,bool,bool params=0
// @EVENT_0: op=0x203 vars=object
// @EVENT_26: op=0x234 vars=string
// @EVENT_6: op=0x23c vars=
// @EVENT_7: op=0x27d vars=int
// @PE: 0x4a,0xae,0xc4,0x203,0x234,0x253,0x27d,0x294,0x414,0x41a,0x423,0x42d,0x433,0x439,0x443,0x44f,0x45b,0x467

task_1_event_11(var_0_object, var_1_object, var_2_object, var_3_string, var_4_bool, var_5_int, var_6_int, var_7_int, var_8_int, var_9_bool, var_10_bool, var_11_object, var_12_bool, var_13_bool)
{
	if((int)1 != 0) {
		func_909();
		var_17_bool = var_13_bool == (int)34192;
		if(var_17_bool != 0) {
			var_18_object = Obj(); var_19_object = Obj();
			var_18_object = var_1_object;
			var_19_object = var_0_object;
			func_1044();
		}
		var_23_bool = var_13_bool == (int)34194;
		if(var_23_bool != 0) {
			var_24_object = Obj(); var_25_object = Obj();
			var_24_object = var_1_object;
			var_25_object = var_0_object;
			func_1075();
			var_28_object = Obj(); var_29_object = Obj();
			var_28_object = var_1_object;
			var_29_object = var_0_object;
			func_1014();
			var_74_object = Obj(); var_75_object = Obj();
			var_74_object = var_1_object;
			var_75_object = var_0_object;
			func_1059(var_75_object);
		}
		var_96_bool = var_13_bool == (int)34195;
		if(var_96_bool != 0) {
			var_97_object = Obj(); var_98_object = Obj();
			var_97_object = var_1_object;
			var_98_object = var_0_object;
			func_1050();
		}
		var_110_bool = var_13_bool == (int)34196;
		if(var_110_bool != 0) {
			var_111_object = Obj(); var_112_object = Obj();
			var_111_object = var_1_object;
			var_112_object = var_0_object;
			func_1069();
		}
		var_116_bool = var_13_bool == (int)34198;
		if(var_116_bool != 0) {
			var_117_object = Obj(); var_118_object = Obj();
			var_117_object = var_1_object;
			var_118_object = var_0_object;
			func_1014();
			var_119_object = Obj(); var_120_object = Obj();
			var_119_object = var_1_object;
			var_120_object = var_0_object;
			func_1075();
			var_121_object = Obj(); var_122_object = Obj();
			var_121_object = var_1_object;
			var_122_object = var_0_object;
			func_1059(var_122_object);
		}
		var_124_bool = var_13_bool == (int)34591;
		if(var_124_bool != 0) {
			var_125_object = Obj(); var_126_object = Obj();
			var_125_object = var_1_object;
			var_126_object = var_0_object;
			func_1014();
			var_127_object = Obj(); var_128_object = Obj();
			var_127_object = var_1_object;
			var_128_object = var_0_object;
			func_1075();
			var_129_object = Obj(); var_130_object = Obj();
			var_129_object = var_1_object;
			var_130_object = var_0_object;
			func_1059(var_130_object);
		}
		var_132_bool = var_12_bool == (int)34191;
		if(var_132_bool != 0) {
			var_133_string = "";
			func_174(var_13_bool, "Neutral");
			@@@var_0_object:SetMessage((int)532716);
			@@@var_0_object:ClearReplies();
			var_151_bool = 0;
			var_151_bool = 0;
			var_152_bool = 0; var_153_object = Obj();
			var_153_object = var_1_object;
			func_1127(var_153_object);
			if(var_152_bool != 0) {
				var_160_bool = 0; var_161_object = Obj();
				var_161_object = var_1_object;
				func_1091(var_161_object);
				if(var_160_bool != 0) {
					var_151_bool = 1;
				}
			}
			if(var_151_bool != 0) {
				@@@var_0_object:AddReply((int)532717, (int)34571, (int)34192);
			}
			var_169_bool = 0;
			var_169_bool = 0;
			var_170_bool = 0;
			var_170_bool = 0;
			var_171_bool = 0; var_172_object = Obj();
			var_172_object = var_1_object;
			func_1081(var_171_bool, var_172_object);
			if(var_171_bool != 0) {
				var_180_bool = 0; var_181_object = Obj();
				var_181_object = var_1_object;
				func_1103(var_181_object);
				if(var_180_bool != 0) {
					var_170_bool = 1;
				}
			}
			if(var_170_bool != 0) {
				var_186_bool = 0; var_187_object = Obj();
				var_187_object = var_1_object;
				func_1115(var_187_object);
				if(var_186_bool != 0) {
					var_169_bool = 1;
				}
			}
			if(var_169_bool != 0) {
				@@@var_0_object:AddReply((int)532721, (int)34197, (int)34196);
			}
			@@@var_0_object:AddReply((int)532724, (int)-1, (int)34199);
			return 0;
		}
		var_199_bool = var_12_bool == (int)34197;
		if(var_199_bool != 0) {
			var_200_string = "";
			func_174(var_13_bool, "Neutral");
			@@@var_0_object:SetMessage((int)532722);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)532723, (int)-1, (int)34198);
			@@@var_0_object:AddReply((int)533075, (int)34590, (int)34589);
			return 0;
		}
		var_209_bool = var_12_bool == (int)34590;
		if(var_209_bool != 0) {
			var_210_string = "";
			func_174(var_13_bool, "Neutral");
			@@@var_0_object:SetMessage((int)533076);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)533077, (int)-1, (int)34591);
			return 0;
		}
		var_216_bool = var_12_bool == (int)34571;
		if(var_216_bool != 0) {
			var_217_string = "";
			func_174(var_13_bool, "Neutral");
			@@@var_0_object:SetMessage((int)533060);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)533061, (int)34573, (int)34572);
			@@@var_0_object:AddReply((int)533071, (int)34573, (int)34583);
			return 0;
		}
		var_226_bool = var_12_bool == (int)34573;
		if(var_226_bool != 0) {
			var_227_string = "";
			func_174(var_13_bool, "Neutral");
			@@@var_0_object:SetMessage((int)533062);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)533063, (int)34575, (int)34574);
			@@@var_0_object:AddReply((int)533072, (int)34193, (int)34585);
			return 0;
		}
		var_236_bool = var_12_bool == (int)34575;
		if(var_236_bool != 0) {
			var_237_string = "";
			func_174(var_13_bool, "Neutral");
			@@@var_0_object:SetMessage((int)533064);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)533065, (int)34193, (int)34576);
			return 0;
		}
		var_243_bool = var_12_bool == (int)34193;
		if(var_243_bool != 0) {
			var_244_string = "";
			func_174(var_13_bool, "Neutral");
			@@@var_0_object:SetMessage((int)532718);
			@@@var_0_object:ClearReplies();
			@@@var_0_object:AddReply((int)533073, (int)34588, (int)34587);
			return 0;
		}
		var_250_bool = var_12_bool == (int)34588;
		if(var_250_bool != 0) {
			var_251_string = "";
			func_174(var_13_bool, "Neutral");
			@@@var_0_object:SetMessage((int)533074);
			@@@var_0_object:ClearReplies();
			var_253_bool = 0; var_254_object = Obj();
			var_254_object = var_1_object;
			func_1081(var_253_bool, var_254_object);
			if(var_253_bool != 0) {
				@@@var_0_object:AddReply((int)532719, (int)-1, (int)34194);
			}
			@@@var_0_object:AddReply((int)532720, (int)-1, (int)34195);
			return 0;
		}
		var_3_string = true;
		var_261_bool = 0;
		func_988(var_261_bool);
		if(var_261_bool != 0) {
			lshStopAnimation();
		} else {
			StopAnimation();
	}
		return 0;
	}
	return 0;
EMIT "GOTO 0xc5";
	
}


task_2_event_0(var_0_object, var_1_object, var_2_object, var_3_string, var_4_bool, var_5_int, var_6_int, var_7_bool, var_8_bool, var_9_object, var_10_bool, var_11_bool, var_12_object)
{
	func_660(var_11_bool, var_12_object);
	var_16_int = 0; var_17_object = Obj();
	var_12_object = var_17_object;
	TaskCall(0);
	func_0(var_18_object, var_16_int, var_17_object);
	TaskReturn();
	return 0;
}


task_2_event_26(var_0_object, var_1_object, var_2_object, var_3_string, var_4_bool, var_5_int, var_6_int, var_7_bool, var_8_bool, var_9_object, var_10_bool, var_11_bool, var_12_string)
{
	var_14_bool = var_12_string == "cleanup";
	if(var_14_bool != 0) {
		func_544(var_12_string);
	}
	return 0;
}


task_2_event_6(var_0_object, var_1_object, var_2_object, var_3_string, var_4_bool, var_5_int, var_6_int, var_7_bool, var_8_bool, var_9_object, var_10_bool, var_11_bool)
{
	var_12_int = var_6_int;
	if(var_12_int != 0) {
		func_660(var_10_bool, var_11_bool);
	}
	var_16_bool = 0;
	var_16_bool = 0;
	var_17_int = var_5_int;
	if(var_17_int != 0) {
		var_18_bool = 0;
		func_593(var_18_bool);
		if(var_18_bool != 0) {
			var_16_bool = 1;
		}
	}
	if(var_16_bool != 0) {
		var_19_object = Obj();
		func_916(var_19_object);
		RemoveActor(var_19_object);
	}
	return 0;
}


task_2_event_7(var_0_object, var_1_object, var_2_object, var_3_string, var_4_bool, var_5_int, var_6_int, var_7_bool, var_8_bool, var_9_object, var_10_bool, var_11_bool, var_12_int)
{
	var_14_bool = var_12_int == (int)10;
	if(var_14_bool != 0) {
		var_15_bool = 0;
		func_622(var_10_bool, var_11_bool, var_12_int, var_15_bool);
		if(var_15_bool != 0) {
			var_28_bool = var_2_object == 0; //@nz
			if(var_28_bool != 0) {
				var_29_object = Obj();
				var_29_object = var_4_bool;
				func_898(var_29_object);
				var_2_object = true;
			}
		} else {
			var_36_object = var_2_object;
			if(var_36_object == 0) goto Label_659;
			UnlookAsync("head");
			var_2_object = false;
		}
	}
Label_659:
	return 0;
	
}


main(var_0_object, var_1_object, var_2_object, var_3_string, var_4_bool, var_5_int, var_6_int, var_7_bool, var_8_bool, var_9_object, var_10_bool, var_11_bool)
{
	var_12_float = 0; var_13_float = 0;
	func_526(var_11_bool, (float)300, (float)100);
	return 0;
}


func_0(var_0_object, var_16_int, var_17_object)
{
	var_19_object = Obj(); var_20_bool = 0; var_21_int = 0; var_22_bool = 0; var_23_object = Obj(); var_24_bool = 0; var_25_int = 0; var_26_bool = 0;
	var_0_object = var_17_object;
	var_27_bool = 0; var_28_object = Obj(); var_29_float = 0;
	var_17_object = var_28_object;
	func_780(var_27_bool, var_28_object, (float)100.0);
	var_74_bool = var_27_bool == 0; //@nz
	if(var_74_bool != 0) {
		var_16_int = -2;
		return 8;
	}
	CreateDialog(var_23_object);
	var_75_int = 0;
	func_982(var_75_int);
	@@var_23_object:SetNPCName(var_75_int);
	var_76_int = 0;
	func_980(var_76_int);
	@@var_23_object:SetNPCDescription(var_76_int);
	var_77_string = "";
	func_984(var_77_string);
	@@var_23_object:SetPhoto(var_77_string);
	var_78_string = "";
	func_986(var_78_string);
	@@var_23_object:SetPhoto2(var_78_string);
	var_79_int = 0;
	func_1234(var_79_int);
	@@var_23_object:SetPlayerName(var_79_int);
	IsOverrideActive(var_24_bool);
	var_87_bool = var_24_bool;
	if(var_87_bool != 0) {
		var_16_int = -2;
		return 8;
	}
	DoDialog(var_23_object);
	var_88_object = Obj(); var_89_object = Obj();
	var_17_object = var_88_object;
	var_23_object = var_89_object;
	TaskCall(1);
	func_74(var_90_object, var_91_object, var_92_string, var_93_bool, var_88_object, var_89_object);
	TaskReturn();
	@@var_23_object:IsDialogEnd(var_26_bool);
	
Label_56:
	var_178_bool = var_26_bool == 0; //@nz
	if(var_178_bool != 0) {
		sync();
		@@var_23_object:IsDialogEnd(var_26_bool);
		goto Label_56;
	}
	var_17_object = Obj();
	func_849();
	StopDialog(var_23_object);
	@@var_23_object:GetReturnValue((int)-1);
	var_25_int = var_16_int;
	return 8;
}
EMIT "Stack[-4] = 0";


func_768(var_81_object, var_82_string, var_83_int)
{
	var_84_int = 0; var_85_int = 0;
	@@var_81_object:GetProperty(var_82_string, var_85_int);
	var_86_int = var_85_int + var_83_int;
	@@var_81_object:SetProperty(var_82_string, var_86_int);
	return 2;
}


func_898(var_29_object)
{
	var_30_float = 0; var_31_cvector = CVector(0,0,0); var_32_float = 0; var_33_cvector = CVector(0,0,0);
	@@var_29_object:GetEyesHeight(var_32_float);
	var_33_cvector = CVector(0.0, 0.0, 0.0);
	var_34_float = GetByIndex(var_33_cvector, 1);
	var_32_float = var_34_float;
	SetByIndex(var_33_cvector, 1) = var_34_float;
	LookAsync(var_29_object, "head", var_33_cvector);
	return 4;
}


func_775(var_23_bool)
{
	var_24_bool = 0; var_25_bool = 0;
	IsLoaded(var_25_bool);
	var_25_bool = var_23_bool;
	return 2;
}


func_1163()
{
	var_101_object = Obj(); var_102_object = Obj();
	CreateDiaryEntry(var_102_object, (int)625, (int)2, (int)532728);
	var_106_bool = 0; var_107_object = Obj(); var_108_int = 0;
	var_102_object = var_107_object;
	func_1189(var_106_bool, var_107_object, (int)7);
	return 2;
}
EMIT "Stack[-1] = 0";


func_780(var_27_bool, var_28_object, var_29_float)
{
	var_30_float = 0; var_31_cvector = CVector(0,0,0); var_32_cvector = CVector(0,0,0); var_33_cvector = CVector(0,0,0); var_34_cvector = CVector(0,0,0); var_35_cvector = CVector(0,0,0); var_36_cvector = CVector(0,0,0); var_37_bool = 0; var_38_bool = 0; var_39_float = 0; var_40_cvector = CVector(0,0,0); var_41_cvector = CVector(0,0,0); var_42_cvector = CVector(0,0,0); var_43_cvector = CVector(0,0,0); var_44_cvector = CVector(0,0,0); var_45_cvector = CVector(0,0,0); var_46_bool = 0; var_47_bool = 0;
	@@var_28_object:GetPosition(var_40_cvector);
	@@var_28_object:GetEyesHeight(var_39_float);
	var_48_float = GetByIndex(var_40_cvector, 1);
	var_48_float = var_48_float + var_39_float;
	SetByIndex(var_40_cvector, 1) = var_48_float;
	GetPosition(var_41_cvector);
	GetEyesHeight(var_39_float);
	var_49_float = GetByIndex(var_41_cvector, 1);
	var_49_float = var_49_float + var_39_float;
	SetByIndex(var_41_cvector, 1) = var_49_float;
	var_42_cvector = var_40_cvector - var_41_cvector;
	var_50_float = GetByIndex(var_42_cvector, 1);
	SetByIndex(var_42_cvector, 1) = (float)0;
	var_51_int = var_42_cvector | var_42_cvector;
	var_52_float = sqrt(var_51_int);
	var_42_cvector = var_42_cvector / var_52_float;
	var_43_cvector = -var_42_cvector;
	var_53_float = var_42_cvector * var_29_float;
	var_54_cvector = CVector(0,0,0); var_55_cvector = CVector(0,0,0);
	var_55_cvector = var_43_cvector ^ CVector(0.0, 1.0, 0.0);
	func_922(var_54_cvector, var_55_cvector);
	var_63_float = var_54_cvector * (int)25;
	var_64_int = var_53_float + var_63_float;
	var_44_cvector = var_64_int - CVector(0.0, 10.0, 0.0);
	var_45_cvector = var_41_cvector + var_44_cvector;
	IsOverrideActive(var_46_bool);
	var_66_bool = var_46_bool;
	if(var_66_bool != 0) {
		var_27_bool = 0;
		return 18;
	}
	StopWorld();
	CameraTransit(var_45_cvector, var_43_cvector, (bool)1);
	var_68_float = GetByIndex(var_44_cvector, 0);
	var_69_float = GetByIndex(var_44_cvector, 2);
	Rotate(var_68_float, var_69_float);
	var_70_bool = 0;
	func_988(var_70_bool);
	if(var_70_bool != 0) {
	} else {
		HasAnimationTrack(var_47_bool, "head");
		var_72_bool = var_47_bool;
		if(var_72_bool == 0) goto Label_843;
		LookAsyncCamera("head");
	}
Label_843:
	CameraWaitForPlayFinish();
	ResumeWorld();
	var_27_bool = 1;
	return 18;
	
}


func_909()
{
	var_15_bool = 0;
	func_988(var_15_bool);
	if(var_15_bool != 0) {
		lshStopSpeech();
	}
	return 0;
}


func_526(var_6_int, var_12_float, var_13_float)
{
	var_14_float = 0; var_15_bool = 0; var_16_float = 0; var_17_bool = 0;
	var_6_int = false;
	
Label_528:
	rand(var_16_float, (int)3);
	var_20_int = var_16_float + (int)3;
	Sleep(var_20_int, var_17_bool);
	var_6_int = true;
	var_21_float = 0; var_22_float = 0;
	var_12_float = var_21_float;
	var_13_float = var_22_float;
	func_595(var_13_float, var_14_float, var_15_bool, var_16_float, var_17_bool, var_21_float, var_22_float);
	var_6_int = false;
	goto Label_528;
}
EMIT "Return(); Pop(4)";


func_1044()
{
	SetVariable("ood1GorbunLaska1", (int)1);
	return 0;
}


func_660(var_2_object, var_3_string)
{
	func_755();
	KillTimer((int)10);
	var_14_object = var_2_object;
	if(var_14_object != 0) {
		UnlookAsync("head");
		var_2_object = false;
	}
	var_3_string = true;
	return 0;
}


func_916(var_19_object)
{
	var_20_object = Obj(); var_21_object = Obj();
	self(var_21_object);
	var_21_object = var_19_object;
	return 2;
}
EMIT "Stack[-1] = 0";


func_1176(var_59_object)
{
	var_60_object = Obj(); var_61_object = Obj();
	GetDiaryRoot(var_61_object);
	var_62_bool = var_61_object == 0; //@nz
	if(var_62_bool != 0) {
		Trace("Can't retrieve diary root");
		var_59_object = 0;
		return 2;
	}
	var_61_object = var_59_object;
	return 2;
}
EMIT "Stack[-1] = 0";


func_1050()
{
	SetVariable("d1q02", (int)4);
	func_1163();
	return 0;
}


func_922(var_54_cvector, var_55_cvector)
{
	var_57_float = 0; var_58_float = 0;
	var_59_int = var_55_cvector | var_55_cvector;
	var_58_float = sqrt(var_59_int);
	var_60_float = 9.999999974752427e-07;
	var_61_bool = var_58_float < var_60_float;
	if(var_61_bool != 0) {
		var_54_cvector = CVector(0.0, 0.0, 0.0);
		return 2;
	}
	var_54_cvector = var_55_cvector / var_58_float;
	return 2;
}


func_544(var_5_int)
{
	var_5_int = true;
	var_15_bool = 0;
	var_15_bool = 0;
	var_16_bool = 0;
	func_775(var_16_bool);
	var_19_bool = var_16_bool == 0; //@nz
	if(var_19_bool != 0) {
		var_20_bool = 0;
		func_593(var_20_bool);
		if(var_20_bool != 0) {
			var_15_bool = 1;
		}
	}
	if(var_15_bool != 0) {
		var_21_object = Obj();
		func_916(var_21_object);
		RemoveActor(var_21_object);
	}
	return 0;
}


func_674()
{
	var_30_int = 0; var_31_int = 0; var_32_int = 0; var_33_int = 0; var_34_bool = 0; var_35_float = 0; var_36_bool = 0; var_37_int = 0; var_38_int = 0; var_39_int = 0; var_40_int = 0; var_41_bool = 0; var_42_float = 0; var_43_bool = 0;
	WaitForAnimEnd();
	var_44_bool = 0;
	func_775(var_44_bool);
	var_45_bool = var_44_bool == 0; //@nz
	if(var_45_bool != 0) {
		return 14;
	}
	func_997((int)0);
	var_46_int = var_37_int;
	var_38_int = 0;
	
Label_688:
	var_59_bool = 0;
	var_59_bool = 0;
	var_61_bool = var_38_int < (int)5;
	if(var_61_bool != 0) {
		var_62_bool = 0;
		func_775(var_62_bool);
		if(var_62_bool != 0) {
			var_59_bool = 1;
		}
	}
	if(var_59_bool != 0) {
		irand(var_39_int, (int)3);
		var_65_bool = var_39_int == (int)0;
		if(var_65_bool != 0) {
			var_66_int = var_37_int;
			if(var_66_int == 0) goto Label_721;
			irand(var_40_int, var_37_int);
			var_68_string = ""; var_69_int = 0;
			var_40_int = var_69_int;
			func_990(var_68_string, var_69_int);
			PlayAnimation("all", var_68_string);
			WaitForAnimEnd(var_41_bool);
			var_70_bool = var_41_bool == 0; //@nz
			if(var_70_bool != 0) {
			} else {
		} else {
				var_75_bool = var_39_int == (int)1;
				if(var_75_bool != 0) {
					rand(var_42_float, (int)4);
					var_78_int = var_42_float + (int)1;
					Sleep(var_78_int, var_43_bool);
					var_79_bool = var_43_bool == 0; //@nz
					if(var_79_bool != 0) {
						goto Label_750;
					}
					goto Label_739;
				}
				var_80_int = var_38_int;
				if(var_80_int == 0) goto Label_739;
				goto Label_750;
		}
		Label_739:
			var_71_bool = 0;
			func_753(var_71_bool);
			var_72_bool = var_71_bool == 0; //@nz
			if(var_72_bool != 0) {
				goto Label_750;
			}
			ResetAAS();
			var_38_int = var_38_int + (int)1;
			goto Label_688;

		}
	}
Label_750:
	ResetAAS();
	return 14;
	
}


func_1059(var_74_object)
{
	Trace("money 2000 removed");
	var_77_object = Obj(); var_78_int = 0;
	var_74_object = var_77_object;
	func_949(var_77_object, (int)-2000);
	return 0;
}


func_932(var_116_int, var_117_string)
{
	var_118_int = 0; var_119_int = 0;
	GetVariable(var_117_string, var_119_int);
	var_119_int = var_116_int;
	return 2;
}


func_1189(var_50_bool, var_51_object, var_52_int)
{
	var_53_object = Obj(); var_54_object = Obj(); var_55_int = 0; var_56_object = Obj(); var_57_object = Obj(); var_58_int = 0;
	func_1176(Obj());
	var_59_object = var_56_object;
	@@var_56_object:Find(var_52_int, var_57_object);
	var_64_bool = var_57_object == 0; //@nz
	if(var_64_bool != 0) {
		var_66_int = "Can't find diary parent with id: " + var_52_int;
		Trace(var_66_int);
		var_50_bool = 0;
		return 6;
	}
	@@var_57_object:AddChild(var_51_object);
	SendWorldWndMessage((int)7);
	@@var_51_object:GetCategory(var_58_int);
	SetDiarySection(var_58_int);
	var_50_bool = 0;
	return 6;
}
EMIT "Stack[-2] = 0";
EMIT "Stack[-3] = 0";


func_937(var_90_int, var_91_int)
{
	var_92_object = Obj(); var_93_object = Obj();
	CreateIntVector(var_93_object);
	@@var_93_object:add(var_90_int);
	@@var_93_object:add(var_91_int);
	SendWorldWndMessage((int)3, var_93_object);
	return 2;
}
EMIT "Stack[-1] = 0";


func_1069()
{
	SetVariable("ood1GorbunLaska2", (int)1);
	return 0;
}


func_174(var_2_object, var_95_string)
{
	var_96_bool = 0;
	func_988(var_96_bool);
	var_97_bool = var_96_bool == 0; //@nz
	if(var_97_bool != 0) {
		return 0;
	}
	var_98_bool = var_95_string == var_2_object;
	if(var_98_bool != 0) {
		return 0;
	}
	var_99_string = ""; var_100_bool = 0;
	var_95_string = var_99_string;
	var_102_bool = var_95_string == "";
	if(var_102_bool != 0) {
		var_100_bool = 0;
	} else {
		var_100_bool = 1;
	}
	func_883(var_99_string, var_100_bool);
	var_2_object = var_95_string;
	return 0;
	
}


func_1075()
{
	TriggerWorld("playsound", "givemoney");
	return 0;
}


func_949(var_77_object, var_78_int)
{
	var_79_int = 0; var_80_int = 0;
	var_81_object = Obj(); var_82_string = ""; var_83_int = 0;
	var_77_object = var_81_object;
	var_78_int = var_83_int;
	func_768(var_81_object, "money", var_83_int);
	var_88_bool = var_78_int > (int)0;
	if(var_88_bool != 0) {
		GetInvItemByName(var_80_int, "Money");
		var_90_int = 0; var_91_int = 0;
		var_80_int = var_90_int;
		var_78_int = var_91_int;
		func_937(var_90_int, var_91_int);
	}
	return 2;
}


func_1081(var_133_bool, var_134_object)
{
	var_135_bool = 0; var_136_object = Obj();
	var_134_object = var_136_object;
	func_1139(var_135_bool, var_136_object);
	if(var_135_bool != 0) {
		var_133_bool = 1;
		return 0;
	}
	var_133_bool = 0;
	return 0;
}


func_1217(var_34_object)
{
	var_35_object = Obj(); var_36_object = Obj(); var_37_object = Obj(); var_38_object = Obj();
	GetMainOutdoorScene(var_37_object);
	var_39_bool = var_37_object == 0; //@ne
	if(var_39_bool != 0) {
		Trace("Can't find main outdoor scene");
		var_38_object = 0;
		var_38_object = var_34_object;
		return 4;
	}
	@@var_37_object:GetMap(var_38_object);
	var_38_object = var_34_object;
	return 4;
}
EMIT "Stack[-1] = 0";
EMIT "Stack[-2] = 0";


func_1091(var_122_bool)
{
	var_124_int = 0; var_125_string = "";
	func_932(var_124_int, "ood1GorbunLaska1");
	var_127_bool = var_124_int == (int)0;
	if(var_127_bool != 0) {
		var_122_bool = 1;
		return 0;
	}
	var_122_bool = 0;
	return 0;
}


func_968(var_68_bool, var_69_string, var_70_string)
{
	var_71_object = Obj(); var_72_object = Obj();
	FindActor(var_72_object, var_69_string);
	var_73_bool = var_72_object == 0; //@ne
	if(var_73_bool != 0) {
		var_68_bool = 0;
		return 2;
	}
	Trigger(var_72_object, var_70_string);
	var_68_bool = 1;
	return 2;
}
EMIT "Stack[-1] = 0";


func_74(var_0_object, var_1_object, var_2_object, var_3_string, var_88_object, var_89_object)
{
	var_0_object = var_89_object;
	var_1_object = var_88_object;
	var_3_string = false;
	if((int)1 != 0) {
		var_95_string = "";
		func_174(var_89_object, "Neutral");
		@@@var_0_object:SetMessage((int)532716);
		@@@var_0_object:ClearReplies();
		var_113_bool = 0;
		var_113_bool = 0;
		var_114_bool = 0; var_115_object = Obj();
		var_115_object = var_1_object;
		func_1127(var_115_object);
		if(var_114_bool != 0) {
			var_122_bool = 0; var_123_object = Obj();
			var_123_object = var_1_object;
			func_1091(var_123_object);
			if(var_122_bool != 0) {
				var_113_bool = 1;
			}
		}
		if(var_113_bool != 0) {
			@@@var_0_object:AddReply((int)532717, (int)34571, (int)34192);
		}
		var_131_bool = 0;
		var_131_bool = 0;
		var_132_bool = 0;
		var_132_bool = 0;
		var_133_bool = 0; var_134_object = Obj();
		var_134_object = var_1_object;
		func_1081(var_133_bool, var_134_object);
		if(var_133_bool != 0) {
			var_142_bool = 0; var_143_object = Obj();
			var_143_object = var_1_object;
			func_1103(var_143_object);
			if(var_142_bool != 0) {
				var_132_bool = 1;
			}
		}
		if(var_132_bool != 0) {
			var_148_bool = 0; var_149_object = Obj();
			var_149_object = var_1_object;
			func_1115(var_149_object);
			if(var_148_bool != 0) {
				var_131_bool = 1;
			}
		}
		if(var_131_bool != 0) {
			@@@var_0_object:AddReply((int)532721, (int)34197, (int)34196);
		}
		@@@var_0_object:AddReply((int)532724, (int)-1, (int)34199);
		goto Label_144;
	EMIT "Return(); Pop(0)";
	EMIT "GOTO 0x4e";
	}
Label_144:
	var_160_bool = 0;
	func_988(var_160_bool);
	if(var_160_bool != 0) {

	Label_148:
		lshWaitForAnimEnd();
		var_161_string = var_3_string;
		if(var_161_string != 0) {
		} else {
			var_162_string = "";
			var_162_string = var_2_object;
			func_867(var_162_string);
			goto Label_148;
	}
		PlayAnimation("all", "idle");

	Label_163:
		WaitForAnimEnd();
		var_175_string = var_3_string;
		if(var_175_string != 0) {
			goto Label_173;
		}
		PlayAnimation("all", "idle");
		goto Label_163;
	}
	goto Label_173;
	
Label_173:
	return 0;
	
}


func_1103(var_142_bool)
{
	var_144_int = 0; var_145_string = "";
	func_932(var_144_int, "ood1GorbunLaska2");
	var_147_bool = var_144_int == (int)0;
	if(var_147_bool != 0) {
		var_142_bool = 1;
		return 0;
	}
	var_142_bool = 0;
	return 0;
}


func_849()
{
	var_180_bool = 0; var_181_bool = 0;
	CameraSwitchToNormal((bool)1);
	var_183_bool = 0;
	func_988(var_183_bool);
	if(var_183_bool != 0) {
	} else {
		HasAnimationTrack(var_181_bool, "head");
		var_185_bool = var_181_bool;
		if(var_185_bool == 0) goto Label_866;
		UnlookAsync("head");
	}
Label_866:
	return 2;
	
}


func_1234(var_79_int)
{
	var_80_int = 0; var_81_int = 0;
	GetVariable("branch", var_81_int);
	var_84_bool = var_81_int == (int)0;
	if(var_84_bool != 0) {
		var_79_int = 1;
		return 2;
	EMIT "GOTO 0x4e1";
	}
	var_86_bool = var_81_int == (int)1;
	if(var_86_bool != 0) {
		var_79_int = 2;
		return 2;
	}
	var_79_int = 3;
	return 2;
}


func_595(var_0_object, var_1_object, var_2_object, var_3_string, var_4_bool, var_21_float, var_22_float)
{
	var_23_bool = 0;
	func_775(var_23_bool);
	var_26_bool = var_23_bool == 0; //@nz
	if(var_26_bool != 0) {
		return 0;
	}
	FindActor(var_4_bool, "player");
	var_2_object = false;
	var_3_string = false;
	var_0_object = var_21_float;
	var_1_object = var_22_float;
	SetTimer((int)10, (float)1.0);
	func_674();
	var_81_bool = var_3_string == 0; //@nz
	if(var_81_bool != 0) {
		KillTimer((int)10);
	}
	return 0;
}


func_980(var_76_int)
{
	var_76_int = 515594;
	return 0;
}


func_593(var_18_bool)
{
	var_18_bool = 1;
	return 0;
}


func_982(var_75_int)
{
	var_75_int = 512583;
	return 0;
}


func_984(var_77_string)
{
	var_77_string = "ui/NPC_Citizen3.png";
	return 0;
}


func_986(var_78_string)
{
	var_78_string = "ui/NPC_Citizen3_b.png";
	return 0;
}


func_1115(var_148_bool)
{
	var_150_int = 0; var_151_string = "";
	func_932(var_150_int, "d1q02");
	var_153_bool = var_150_int == (int)4;
	if(var_153_bool != 0) {
		var_148_bool = 1;
		return 0;
	}
	var_148_bool = 0;
	return 0;
}


func_988(var_70_bool)
{
	var_70_bool = 0;
	return 0;
}


func_990(var_52_string, var_53_int)
{
	var_54_string = ""; var_55_string = "";
	var_56_int = var_53_int;
	if(var_56_int != 0) {
		"idle" = "idle" + var_53_int;
	}
	var_55_string = var_52_string;
	return 2;
}


func_867(var_162_string)
{
	var_163_bool = 0; var_164_float = 0; var_165_float = 0; var_166_bool = 0; var_167_float = 0; var_168_float = 0;
	lshHasAnimation(var_166_bool, var_162_string);
	var_169_bool = var_166_bool;
	if(var_169_bool != 0) {
		lshGetAnimTimes(var_162_string, var_167_float, var_168_float);
		lshPlayAnimation(var_167_float, var_168_float, (bool)0);
	} else {
		var_172_int = "Can't find lsh animation : " + var_162_string;
		Trace(var_172_int);
	}
	return 6;
	
}


func_997(var_46_int)
{
	var_47_int = 0; var_48_bool = 0; var_49_int = 0; var_50_bool = 0;
	var_49_int = 0;
	
Label_999:
	var_52_string = ""; var_53_int = 0;
	var_49_int = var_53_int;
	func_990(var_52_string, var_53_int);
	HasAnimation(var_50_bool, "all", var_52_string);
	var_57_bool = var_50_bool == 0; //@nz
	if(var_57_bool != 0) {
	} else {
		var_49_int = var_49_int + (int)1;
		goto Label_999;
	}
	var_49_int = var_46_int;
	return 4;
	
}


func_1127(var_114_bool)
{
	var_116_int = 0; var_117_string = "";
	func_932(var_116_int, "d1q02");
	var_121_bool = var_116_int == (int)2;
	if(var_121_bool != 0) {
		var_114_bool = 1;
		return 0;
	}
	var_114_bool = 0;
	return 0;
}


func_622(var_0_object, var_1_object, var_4_bool, var_15_bool)
{
	var_16_float = 0; var_17_float = 0;
	var_18_bool = var_4_bool == 0; //@ne
	if(var_18_bool != 0) {
		var_15_bool = 0;
		return 2;
	}
	var_19_float = 0; var_20_object = Obj();
	var_20_object = var_4_bool;
	func_760(var_19_float, var_20_object);
	var_17_float = sqrt(var_19_float);
	var_27_object = var_2_object;
	if(var_27_object != 0) {
		var_17_float = var_17_float - var_1_object;
	}
	var_15_bool = var_17_float < var_0_object;
	return 2;
}


func_753(var_71_bool)
{
	var_71_bool = 1;
	return 0;
}


func_883(var_99_string, var_100_bool)
{
	var_103_bool = 0; var_104_float = 0; var_105_float = 0; var_106_bool = 0; var_107_float = 0; var_108_float = 0;
	lshHasAnimation(var_106_bool, var_99_string);
	var_109_bool = var_106_bool;
	if(var_109_bool != 0) {
		lshGetAnimTimes(var_99_string, var_107_float, var_108_float);
		lshPlayAnimation(var_107_float, var_108_float, var_100_bool);
	} else {
		var_111_int = "Can't find lsh animation : " + var_99_string;
		Trace(var_111_int);
	}
	return 6;
	
}


func_755()
{
	StopAnimation();
	StopGroup0();
	return 0;
}


func_1139(var_135_bool, var_136_object)
{
	var_137_float = 0; var_138_float = 0;
	@@var_136_object:GetProperty("money", var_138_float);
	var_141_bool = var_138_float >= (int)2000;
	if(var_141_bool != 0) {
		var_135_bool = 1;
		return 2;
	}
	var_135_bool = 0;
	return 2;
}


func_1014()
{
	var_30_object = Obj(); var_31_object = Obj(); var_32_object = Obj(); var_33_object = Obj();
	func_1217(Obj());
	var_34_object = var_32_object;
	@@var_32_object:FindMark(var_33_object, "d1q02AnnaGotoLaska");
	var_42_object = var_33_object;
	if(var_42_object != 0) {
		@@var_33_object:Remove();
	}
	@@var_32_object:FindMark(var_33_object, "d1q02JuliaGotoAnna");
	var_44_object = var_33_object;
	if(var_44_object != 0) {
		@@var_33_object:Remove();
	}
	func_1150();
	var_68_bool = 0; var_69_string = ""; var_70_string = "";
	func_968(var_68_bool, "quest_d1_02", "completed");
	return 4;
}
EMIT "Stack[-1] = 0";
EMIT "Stack[-2] = 0";


func_760(var_19_float, var_20_object)
{
	var_21_cvector = CVector(0,0,0); var_22_cvector = CVector(0,0,0); var_23_cvector = CVector(0,0,0); var_24_cvector = CVector(0,0,0); var_25_cvector = CVector(0,0,0); var_26_cvector = CVector(0,0,0);
	GetPosition(var_24_cvector);
	@@var_20_object:GetPosition(var_25_cvector);
	var_26_cvector = var_25_cvector - var_24_cvector;
	var_19_float = var_26_cvector | var_26_cvector;
	return 6;
}


func_1150()
{
	var_45_object = Obj(); var_46_object = Obj();
	CreateDiaryEntry(var_46_object, (int)36, (int)2, (int)512118);
	var_50_bool = 0; var_51_object = Obj(); var_52_int = 0;
	var_46_object = var_51_object;
	func_1189(var_50_bool, var_51_object, (int)7);
	return 2;
}
EMIT "Stack[-1] = 0";


