...
{
	if (f_6af_a1_b(a0)) {
		@ReportReputationChange(a0, f_736_a0_o(), -0.05000000074505806);
	}
}
...
void f_902_a1_v(bool a0)
{
	...
	L2 = f_777_a0_i();
	if (a0) {
		f_727_a4_v("alpha_pills", 1, 2, 4);
		f_727_a4_v("meradorm", 2, 3, 3);
		if (L2 >= 3) {
			f_71c_a3_v("beta_pills", 1, 4);
		}
		if (L2 >= 8) {
			f_727_a4_v("monomicin", 1, 2, 2);
		} else {
			if (L2 >= 4) {
				f_71c_a3_v("monomicin", 1, 2);
			}
		}
	} else {
		f_71c_a3_v("lockpick", 1, 4);
		f_71c_a3_v("rifle_ammo", 1, 2);
		f_71c_a3_v("revolver_ammo", 1, 2);
		f_727_a4_v("samopal_ammo", 1, 2, 2);
		if (L2 >= 8) {
			f_727_a4_v("monomicin", 1, 2, 2);
		} else {
			if (L2 >= 4) {
...
bool f_97c_a0_b(void)
{
	return f_8e7_a0_f() > 0.6000000238418579;
}

bool f_982_a3_b(object a0, object a1, bool a2)
{
