		...
		for (L1 = 0; L1 < 5 && f_96f_a0_b(); L1++) {
			...
			if (L2 == 0) {
				if (L0) {
					...
					bool L4;
					@irand(L3, L0);
					@PlayAnimation("all", f_aaf_a1_s(L3));
					@WaitForAnimEnd(L4);
					if (!L4) {
						break;
					}
...
int f_ab6_a0_i(void)
{
	...
	for (L0 = 0; ; L0++) {
		bool L1;
		@HasAnimation(L1, "all", f_aaf_a1_s(L0));
		if (!L1) {
			break;
		}
	}
...
Vector f_ac7_a3_V(Vector a0, Vector a1, float a2)
{
	Vector L0;
	float L1;
	float L2;
...
{
	return f_b03_a1_b(a0);
}

void f_b37_a1_v(object a0)
{
	if (t7{a0}) {
		t6{a0};
	}
...
bool f_b89_a2_b(object a0, object a1)
{
	if (f_aea_a1_b(a1)) {
		bool L0;
		float L1;
...
void f_bf4_a2_v(int a0, int a1)
{
	...
	@AddItem(L1, f_c5a_a1_i("Money"), 0, L0);
}

void f_c13_a1_v(string a0)
{
	object L0;
	int L1;
	bool L2;
...
void f_c3c_a0_v(void)
{
	...
	@AddItem(L1, f_c5a_a1_i("grabitel_mark"), 0, 1);
}

int f_c5a_a1_i(string a0)
{
	int L0;
