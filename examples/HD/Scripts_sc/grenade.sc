...
maintask t0
{
	void OnCollision(object a0, object a1, Vector a2, Vector a3)
	{
		...
		while (L1->Next(L2, L3), L2) {
			...
			L6 = L6 * DamageAmount;
			L7 = f_10f_a3_f(L3, L6, DamageType);
			if (Owner != L3) {
	...
	void init(void)
	{
		...
		object L1;
		object L2;
		@CreateRigidBody(L0, [0.0, 0.0, 0.0], 10, StartVelocity, [2.0, 2.0, 2.0], Owner);
		L0->Enable(true);
		g5 = L0;
...
bool f_103_a2_b(object a0, string a1)
{
	bool L0;
...
float f_10f_a3_f(object a0, float a1, int a2)
{
	int L0;
	string L1;
	int L2;
	...
	float L4;
	float L5;
	if (!f_103_a2_b(a0, "health")) {
		return 0.0;
	}
	if (!f_103_a2_b(a0, "armor")) {
		L0 = 0;
	} else {
		a0->GetProperty("armor", L0);
	}
	L1 = "armor_" + f_e6_a1_s(a2);
	if (!f_103_a2_b(a0, L1)) {
		L2 = 0;
	} else {
		a0->GetProperty(L1, L2);
	}
	L3 = f_163_a2_f((L0 + L2) / 100.0, 1);
	a0->GetProperty("health", L4);
	L5 = a1 * (1 - L3);
	a0->SetProperty("health", f_16a_a3_f(L4 - L5, 0, 1));
	if (f_fe_a1_b(a0)) {
		f_175_a1_v(-L5);
	}
	return L5;
...
float f_15e_a2_f(Vector a0, Vector a1)
{
	Vector L0;
	L0 = a1 - a0;
	return sqrt(L0 | L0);
...
float f_163_a2_f(float a0, float a1)
{
	return a0 < a1 ? a0 : a1;
}

float f_16a_a3_f(float a0, float a1, float a2)
{
	if (a0 < a1) {
		return a1;
	}
