...
task t0
{
	...
	int init(object a0)
	{
		...
		@CreateDialog(L0);
		L0->SetNPCName(f_8c2_a0_i());
		L0->SetNPCDescription(f_8c0_a0_i());
		L0->SetPhoto(f_8c4_a0_s());
		L0->SetPhoto2(f_8c6_a0_s());
		L0->SetPlayerName(f_873_a0_i());
		L2 = -1;
		@IsOverrideActive(L1);
		if (L1) {
			return -2;
...
maintask t2
{
	...
	void OnUse(object a0)
	{
		...
		t0{a0};
		f_83b_a2_b("quest_b1_05", "attack");
	}

	void OnSee(object a0)
	{
		if (f_8ca_a0_b() && f_62e_a1_b(a0)) {
			f_83b_a2_b("quest_b1_05", "attack");
		}
	}
		...
		for (L1 = 0; L1 < 5 && f_752_a0_b(); L1++) {
			...
			if (L2 == 0) {
				if (L0) {
					...
					bool L4;
					@irand(L3, L0);
					@PlayAnimation("all", f_85b_a1_s(L3));
					@WaitForAnimEnd(L4);
					if (!L4) {
						break;
					}
...
float f_63f_a3_f(object a0, float a1, int a2)
{
	...
	L3 = f_818_a2_f((L0 + L2) / 100.0, 1);
	a0->GetProperty("health", L4);
	L5 = a1 * (1 - L3);
	a0->SetProperty("health", f_81f_a3_f(L4 - L5, 0, 1));
	if (f_62e_a1_b(a0)) {
		f_82a_a1_v(-L5);
	}
	return L5;
...
bool f_757_a2_b(object a0, float a1)
{
	...
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * a1 + f_80e_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
...
void f_7be_a2_v(string a0, bool a1)
{
	bool L0;
	@lshHasAnimation(L0, a0);
	if (L0) {
...
float f_818_a2_f(float a0, float a1)
{
	return a0 < a1 ? a0 : a1;
}

float f_81f_a3_f(float a0, float a1, float a2)
{
	if (a0 < a1) {
		return a1;
	}
...
bool f_83b_a2_b(string a0, string a1)
{
	object L0;
	@FindActor(L0, a0);
	if (L0 == null) {
...
void f_850_a0_v(void)
{
	object L0;
	@GetScene(L0);
	@BroadcastMessage("battle", f_808_a0_o(), L0);
}

string f_85b_a1_s(int a0)
{
	string L0;
	L0 = "idle";
	if (a0) {
...
int f_862_a0_i(void)
{
	...
	for (L0 = 0; ; L0++) {
		bool L1;
		@HasAnimation(L1, "all", f_85b_a1_s(L0));
		if (!L1) {
			break;
		}
	}
...
void f_8d6_a1_v(object a0)
{
	object L0;
	@GetScene(L0);
	L0->RemoveStationaryActor(f_808_a0_o());
	f_8a8_a0_v();
	t5{a0};
}
