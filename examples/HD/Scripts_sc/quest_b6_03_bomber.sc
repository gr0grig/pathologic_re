		...
		for (L1 = 0; L1 < 5 && f_5b8_a0_b(); L1++) {
			...
			if (L2 == 0) {
				if (L0) {
					...
					bool L4;
					@irand(L3, L0);
					@PlayAnimation("all", f_6be_a1_s(L3));
					@WaitForAnimEnd(L4);
					if (!L4) {
						break;
					}
...
task t4
{
	...
	void init(object a0)
	{
		...
		while (f_50f_a1_b(tv0)) {
			...
			L7->SetScriptProperty("Owner", f_662_a0_o());
			if (L3 == 1) {
...
bool f_688_a2_b(object a0, float a1)
{
	if (!a0) {
		return false;
	}
...
int f_6c5_a0_i(void)
{
	...
	for (L0 = 0; ; L0++) {
		bool L1;
		@HasAnimation(L1, "all", f_6be_a1_s(L0));
		if (!L1) {
			break;
		}
	}
...
void f_6e9_a2_v(int a0, int a1)
{
	...
	@AddItem(L1, f_756_a1_i("Money"), 0, L0);
}
...
void f_731_a0_v(void)
{
	...
	@AddItem(L1, f_756_a1_i("bomber_mark"), 0, 1);
}

int f_756_a1_i(string a0)
{
	int L0;
...
float f_76d_a3_f(Vector a0, Vector a1, Vector a2)
{
	float L0;
	float L1;
	float L2;
...
float f_7af_a2_f(int a0, float a1)
{
	if (a1 < 10) {
		return 10;
	} else {
...
void f_7fd_a1_v(object a0)
{
	f_88b_a1_v(a0);
	if (f_4bf_a1_b(a0)) {
		f_688_a2_b(a0, 0.05000000074505806);
	}
	t1{a0};
}
...
bool f_826_a2_b(object a0, object a1)
{
	if (f_75b_a1_b(a1)) {
		bool L0;
		@CanSee(L0, a0);
...
void f_83f_a0_v(void)
{
	g0 = f_682_a0_o();
}

bool f_846_a1_b(object a0)
{
	bool L0;
	float L1;
	bool L2;
