...
task t0
{
	...
	int init(object a0)
	{
		...
		@CreateDialog(L0);
		L0->SetNPCName(f_439_a0_i());
		L0->SetNPCDescription(f_437_a0_i());
		L0->SetPhoto(f_43b_a0_s());
		L0->SetPhoto2(f_43d_a0_s());
		L0->SetPlayerName(f_453_a0_i());
		L2 = -1;
		@IsOverrideActive(L1);
		if (L1) {
			return -2;
...
void f_27f_a0_v(void)
{
	...
	for (L1 = 0; L1 < 5 && f_2e6_a0_b(); L1++) {
		if (!L0) {
			...
			@PlayAnimation("all", f_41f_a1_s(L3));
			@WaitForAnimEnd(L5);
			if (!L5) {
				break;
			}
...
bool f_2eb_a2_b(object a0, float a1)
{
	...
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * a1 + f_407_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
...
void f_3dc_a2_v(string a0, bool a1)
{
	bool L0;
	@lshHasAnimation(L0, a0);
	if (L0) {
...
int f_426_a0_i(void)
{
	...
	for (L0 = 0; ; L0++) {
		bool L1;
		@HasAnimation(L1, "all", f_41f_a1_s(L0));
		if (!L1) {
			break;
		}
	}
...
string f_43b_a0_s(void)
{
	return "ui/NPC_Spi4ka.png";
}

string f_43d_a0_s(void)
{
...
{
	if (f_411_a1_i("oob9Spi4ka_Kapella1") == 0) {
		return true;
	}
	return false;
}
