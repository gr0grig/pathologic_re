...
task t0
{
	...
	int init(object a0)
	{
		...
		@CreateDialog(L0);
		L0->SetNPCName(f_24c_a0_i());
		L0->SetNPCDescription(f_24a_a0_i());
		L0->SetPhoto(f_24e_a0_s());
		L0->SetPhoto2(f_250_a0_s());
		L0->SetPlayerName(f_254_a0_i());
		L2 = -1;
		@IsOverrideActive(L1);
		if (L1) {
			return -2;
...
maintask t2
{
	...
	void OnUse(object a0)
	{
		if (!tv0) {
			...
			if (L0) {
				f_226_a2_b("quest_d1_01", "remove_masks");
			}
		}
	}
}
	...
	for (L1 = 0; L1 < 5 && f_19a_a0_b(); L1++) {
		...
		if (L2 == 0) {
			if (L0) {
				...
				bool L4;
				@irand(L3, L0);
				@PlayAnimation("all", f_232_a1_s(L3));
				@WaitForAnimEnd(L4);
				if (!L4) {
					break;
				}
...
bool f_19f_a2_b(object a0, float a1)
{
	...
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * a1 + f_21c_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
...
void f_206_a2_v(string a0, bool a1)
{
	bool L0;
	@lshHasAnimation(L0, a0);
	if (L0) {
...
bool f_226_a2_b(string a0, string a1)
{
	object L0;
	@FindActor(L0, a0);
	if (L0 == null) {
...
int f_239_a0_i(void)
{
	...
	for (L0 = 0; ; L0++) {
		bool L1;
		@HasAnimation(L1, "all", f_232_a1_s(L0));
		if (!L1) {
			break;
		}
	}
