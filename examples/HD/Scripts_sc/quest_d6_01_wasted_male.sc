...
task t0
{
	...
	int init(object a0)
	{
		...
		@CreateDialog(L0);
		L0->SetNPCName(f_2bb_a0_i());
		L0->SetNPCDescription(f_2b9_a0_i());
		L0->SetPhoto(f_2bd_a0_s());
		L0->SetPhoto2(f_2bf_a0_s());
	...
	for (L1 = 0; L1 < 5 && f_205_a0_b(); L1++) {
		...
		if (L2 == 0) {
			if (L0) {
				...
				bool L4;
				@irand(L3, L0);
				@PlayAnimation("all", f_2a1_a1_s(L3));
				@WaitForAnimEnd(L4);
				if (!L4) {
					break;
				}
...
int f_2a8_a0_i(void)
{
	...
	for (L0 = 0; ; L0++) {
		bool L1;
		@HasAnimation(L1, "all", f_2a1_a1_s(L0));
		if (!L1) {
			break;
		}
	}
...
void f_2c3_a2_v(object a0, object a1)
{
...
{
	if (f_29c_a1_i("ood6WastedMale1") == 0) {
		return true;
	}
	return false;
}
