...
maintask t0
{
	...
	void init(void)
	{
		...
		for (L0 = 0; L0 < 4; L0++) {
			...
			L1->add(L2);
			L1->add(L3);
			@SendMessage(-1, f_30b_a1_s(L0), L1);
		}
		@SendMessage(-6, f_30b_a1_s(0));
		f_183_a0_v();
		f_1a0_a0_v();
		f_17a_a0_v();
		@ProcessEvents();
	...
	{
		@SendMessage(100 * f_165_a0_f(), "level");
	}
	...
	void f_1a0_a0_v(void)
	{
		...
		for (; L1 < 4 + tv7; L1++) {
			if (L1 < L0) {
				...
				tv4->get(L4, L1);
				if (L3) {
					@SendMessage(16384, f_30b_a1_s(L1 - tv7), L2);
				} else {
					@SendMessage(0, f_30b_a1_s(L1 - tv7), L2);
				}
				@SendMessage(L3 | 65536, f_30b_a1_s(L1 - tv7));
			} else {
				@SendMessage(32768, f_30b_a1_s(L1 - tv7));
			}
		}
	}
		...
		if (a1 == "ok") {
			...
			if (a1 == "cancel") {
				...
				if (a1 == "scrollbar") {
					int L1;
					tv0->size(L1);
					tv7 = 0.009999999776482582 * (L1 - 4) * a0 + 0.5;
					if (tv7 < 0) {
						tv7 = 0;
					}
					f_183_a0_v();
...
int f_320_a2_i(int a0, string a1)
{
	int L0;
	for (L0 = 0; L0 < 4; L0++) {
		if (a1 == f_30b_a1_s(L0)) {
