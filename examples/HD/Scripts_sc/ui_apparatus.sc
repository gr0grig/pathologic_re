...
maintask t0
{
	...
	void f_24c_a1_v(int a0)
	{
		...
		for (L0 = 0; L0 < 12; L0++) {
			...
			L1->add(f_48a_a1_s((L0 + 12 - 1) % 12));
			L1->add(f_48a_a1_s((L0 + 12 + 1) % 12));
			@SendMessage(-1, f_48a_a1_s(L0), L1);
		}
		f_296_a0_v();
		@SetCursor("default");
		@ShowCursor();
	...
	void f_303_a1_v(int a0)
	{
		if (a0 == 269) {
			int L0;
			for (L0 = 0; L0 < 12; L0++) {
				@SendMessage(-2, f_48a_a1_s(L0));
			}
		}
		if (a0 == 270) {
			int L1;
			for (L1 = 0; L1 < 12; L1++) {
				@SendMessage(-3, f_48a_a1_s(L1));
			}
		}
	}
	...
	{
		@SendMessage(f_3e3_a0_i(), "money");
	}

	void f_3f5_a1_v(int a0)
	{
		if (a0 != 0) {
	...
	void f_433_a1_v(int a0)
	{
		...
		for (; L2 < 12; L2++) {
			if (L2 < L1) {
				...
				if (L5) {
					if (L6) {
						@SendMessage(0, f_48a_a1_s(L2), L3);
					} else {
						@SendMessage(16384, f_48a_a1_s(L2), L3);
					}
				} else {
					if (L6) {
						@SendMessage(131072, f_48a_a1_s(L2), L3);
					} else {
						@SendMessage(0, f_48a_a1_s(L2), L3);
					}
				}
				@SendMessage(L4 | 65536, f_48a_a1_s(L2));
			} else {
				@SendMessage(32768, f_48a_a1_s(L2));
			}
		}
	}
}
