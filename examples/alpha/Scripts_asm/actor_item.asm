...
0x33: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=45
0x34: Pop(0)
0x35: Stack[-1] = Stack[-5]
0x36: Return(); Pop(2)

0x37: PushEmpty(bool, bool)
0x38: @@ IsDead(Stack[-1]); Obj=3 // @poff=57
0x39: Pop(0)
0x3a: Stack[-1] = Stack[-4]
0x3b: Return(); Pop(2)
...
0x51: Pop(0)
0x52: Pop(0); PushNull((bool) Stack[-2] == 0)
0x53: IF (Stack[-1] == 0) GOTO 0x56; Pop(1)

0x54: Stack[-6] = (bool) 0
0x55: Return(); Pop(4)

0x56: @@ GetScene(Stack[-1]); Obj=5 // @poff=78
0x57: Pop(0)
...
0x6a: Stack[-6] = Stack[-2]
0x6b: Stack[-1] = "noaccess" // @poff=87
0x6c: Call 0x2b
...
0x72: Push("noaccess") // @poff=87
0x73: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=105
0x74: Pop(1)
0x75: Push((int) 0)
...
0x79: @@ GetItemID(Stack[-3]); Obj=8 // @poff=117
0x7a: Pop(0)
0x7b: Push("Category") // @poff=127
0x7c: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0x7d: Pop(1)
0x7e: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7]); Obj=9 // @poff=145
0x7f: Pop(0)
0x80: Stack[-1] = Stack[-10]
0x81: Return(); Pop(6)
