...
Strings:
	...
	W:Category
	A:AddItem
// @pool_raw:
...
0xa: Push("object") // @poff=0
0xb: @ FindGeometry(Stack[-1], Stack[-2])
0xc: Pop(1)
0xd: Push((bool) 1)
0xe: @@ Enable(Stack[-1]); Obj=2 // @poff=14
0xf: Pop(1)
0x10: @ Hold()
0x11: Pop(0)
0x12: GOTO 0x10
...
0x24: Stack[-9] = Stack[-3]
0x25: Stack[-6] = Stack[-2]
...
0x3a: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=45
0x3b: Pop(0)
0x3c: Stack[-1] = Stack[-5]
0x3d: Return(); Pop(2)

0x3e: PushEmpty(bool, bool)
0x3f: @@ IsDead(Stack[-1]); Obj=3 // @poff=57
0x40: Pop(0)
...
0x58: Pop(0)
0x59: Pop(0); PushNull((bool) Stack[-2] == 0)
0x5a: IF (Stack[-1] == 0) GOTO 0x5d; Pop(1)

0x5b: Stack[-6] = (bool) 0
0x5c: Return(); Pop(4)

0x5d: @@ GetScene(Stack[-1]); Obj=5 // @poff=78
0x5e: Pop(0)
...
0x71: Stack[-6] = Stack[-2]
0x72: Stack[-1] = "noaccess" // @poff=87
0x73: Call 0x32
...
0x79: Push("noaccess") // @poff=87
0x7a: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=105
0x7b: Pop(1)
0x7c: Push((int) 0)
...
0x80: @@ GetItemID(Stack[-3]); Obj=8 // @poff=117
0x81: Pop(0)
0x82: Push("Category") // @poff=127
0x83: @ GetInvItemProperty(Stack[-3], Stack[-4], Stack[-1])
0x84: Pop(1)
0x85: @@ AddItem(Stack[-1], Stack[-8], Stack[-2], Stack[-7]); Obj=9 // @poff=145
0x86: Pop(0)
0x87: Stack[-1] = Stack[-10]
0x88: Return(); Pop(6)
