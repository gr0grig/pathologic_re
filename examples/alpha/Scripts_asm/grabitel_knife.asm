...
Strings:
	...
	A:GetScene
	W:noaccess
// @pool_raw:
// @pool_encoding:utf8
...
0x24: Push("Knife hit: ") // @poff=31
0x25: Pop(1); Push(Stack[-1] + Stack[-6]);
0x26: @ Trace(Stack[-1])
0x27: Pop(1)
0x28: Push(GlobalVars[2])
0x29: Push((bool) 0)
0x2a: @@ Enable(Stack[-1]); Obj=2 // @poff=0
0x2b: Pop(2)
...
0x36: Stack[-9] = Stack[-3]
0x37: Stack[-2] = (float) 0.8999999761581421
0x38: Stack[-1] = (int) 0
0x39: Call 0x6b

0x3a: Stack[-4] = Stack[-5]
0x3b: Pop(4)
...
0x49: Push((int) 1)
0x4a: Push((float)0.8999999761581421)
...
0x5b: Stack[-2] = "fire" // @poff=65
0x5c: Return(); Pop(0)

0x5d: Stack[-2] = "phys" // @poff=75
0x5e: Return(); Pop(0)

0x5f: PushEmpty(bool, bool)
0x60: Push("HasProperty") // @poff=85
0x61: Push((int) 2)
...
0x67: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=109
0x68: Pop(0)
...
0x6d: Stack[-18] = Stack[-2]
0x6e: Stack[-1] = "health" // @poff=121
0x6f: Call 0x5f
...
0x76: Stack[-18] = Stack[-2]
0x77: Stack[-1] = "armor" // @poff=135
0x78: Call 0x5f
...
0x7e: Push("armor") // @poff=135
0x7f: @@ GetProperty(Stack[-1], Stack[-7]); Obj=16 // @poff=147
0x80: Pop(1)
0x81: Push("armor_") // @poff=159
0x82: PushEmpty(string, int)
0x83: Stack[-16] = Stack[-1]
0x84: Call 0x57

0x85: Pop(1)
0x86: Stack[-7] = Stack[-2] + Stack[-1]; Pop(2);
0x87: PushEmpty(bool, object, string)
0x88: Stack[-18] = Stack[-2]
0x89: Stack[-8] = Stack[-1]
0x8a: Call 0x5f
...
0x90: @@ GetProperty(Stack[-5], Stack[-4]); Obj=15 // @poff=147
0x91: Pop(0)
...
0x98: Stack[-3] = Stack[-6]
0x99: Pop(3)
0x9a: Push("health") // @poff=121
0x9b: @@ GetProperty(Stack[-1], Stack[-3]); Obj=16 // @poff=147
0x9c: Pop(1)
0x9d: Push((int) 1)
0x9e: Pop(1); Push(Stack[-1] - Stack[-4]);
0x9f: Stack[-2] = Stack[-15] * Stack[-1]; Pop(1);
0xa0: Push("health") // @poff=121
...
0xa6: Pop(3)
0xa7: @@ SetProperty(Stack[-2], Stack[-1]); Obj=17 // @poff=173
0xa8: Pop(2)
0xa9: Stack[-1] = Stack[-16]
0xaa: Return(); Pop(12)

0xab: PushEmpty(float, float)
0xac: PushEmpty(bool, object, string)
0xad: Stack[-6] = Stack[-2]
0xae: Stack[-1] = "health" // @poff=121
0xaf: Call 0x5f
...
0xb5: Push("health") // @poff=121
0xb6: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=147
0xb7: Pop(1)
0xb8: Push((float)0.0)
...
0xbc: @@ IsDead(Stack[-1]); Obj=3 // @poff=185
0xbd: Pop(0)
0xbe: Stack[-1] = Stack[-4]
0xbf: Return(); Pop(2)

0xc0: PushEmpty(object, object, object, object)
0xc1: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0xd5: Pop(0)
0xd6: Pop(0); PushNull((bool) Stack[-2] == 0)
0xd7: IF (Stack[-1] == 0) GOTO 0xda; Pop(1)

0xd8: Stack[-6] = (bool) 0
0xd9: Return(); Pop(4)

0xda: @@ GetScene(Stack[-1]); Obj=5 // @poff=206
0xdb: Pop(0)
...
0xef: Stack[-1] = "noaccess" // @poff=215
0xf0: Call 0x5f
...
0xf6: Push("noaccess") // @poff=215
0xf7: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=147
0xf8: Pop(1)
0xf9: Push((int) 0)
