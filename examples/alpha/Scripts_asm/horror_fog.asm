...
Strings:
	...
	A:GetScene
	W:noaccess
// @pool_raw:
...
0x73: Stack[-6] = Stack[-8]
0x74: Pop(6)
0x75: Return(); Pop(0)

0x76: PushEmpty()
0x77: PushEmpty(int)
0x78: Stack[-2] = Stack[-1]
0x79: Call 0xe6
...
0x83: Stack[-17] = Stack[-7]
0x84: PushEmpty(bool, object)
0x85: Stack[-23] = Stack[-1]
0x86: Call 0x10c
...
0x8c: @@ GetPosition(Stack[-5]); Obj=21 // @poff=56
0x8d: Pop(0)
...
0xa3: @@ GetPFPosition(Stack[-5]); Obj=21 // @poff=14
0xa4: Pop(0)
...
0xb5: Push((int) 0)
0xb6: Push((float)0.30000001192092896)
...
0xd8: Push((int) 0)
0xd9: Push((float)0.30000001192092896)
...
0x110: Stack[-2] = Stack[-4]
0x111: Pop(2)
0x112: Return(); Pop(0)

0x113: Stack[-1] = "walk" // @poff=68
0x114: Return(); Pop(0)

0x115: Stack[-1] = "run" // @poff=78
0x116: Return(); Pop(0)
...
0x11d: @@ GetPosition(Stack[-1]); Obj=3 // @poff=56
0x11e: Pop(0)
0x11f: Stack[-1] = Stack[-4]
0x120: Return(); Pop(2)

0x121: PushEmpty(bool, bool)
0x122: Push("HasProperty") // @poff=86
0x123: Push((int) 2)
...
0x129: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=110
0x12a: Pop(0)
...
0x12f: Stack[-10] = Stack[-2]
0x130: Stack[-9] = Stack[-1]
0x131: Call 0x121
...
0x137: @@ GetProperty(Stack[-6], Stack[-1]); Obj=7 // @poff=122
0x138: Pop(0)
0x139: PushEmpty(float, float, float, float)
0x13a: Stack[-3] = Stack[-5] + Stack[-9]; Pop(0);
0x13b: Stack[-8] = Stack[-2]
0x13c: Stack[-7] = Stack[-1]
0x13d: Call 0x1cd

0x13e: Pop(3)
0x13f: @@ SetProperty(Stack[-7], Stack[-1]); Obj=8 // @poff=134
0x140: Pop(1)
...
0x145: Stack[-11] = Stack[-2]
0x146: Stack[-1] = "disease" // @poff=146
0x147: Call 0x121
...
0x14e: Stack[-11] = Stack[-2]
0x14f: Stack[-1] = "armor_disease" // @poff=162
0x150: Call 0x121

0x151: Pop(2)
0x152: IF (Stack[-1] == 0) GOTO 0x161; Pop(1)

0x153: Push("armor_disease") // @poff=162
0x154: @@ GetProperty(Stack[-1], Stack[-4]); Obj=9 // @poff=122
0x155: Pop(1)
0x156: Push((int) 100)
...
0x163: Stack[-1] = "immunity" // @poff=190
0x164: Call 0x121

0x165: Pop(2)
0x166: IF (Stack[-1] == 0) GOTO 0x178; Pop(1)

0x167: Push("immunity") // @poff=190
0x168: @@ GetProperty(Stack[-1], Stack[-2]); Obj=9 // @poff=122
0x169: Pop(1)
0x16a: Pop(0); Push((bool) Stack[-1] < Stack[-7])
0x16b: IF (Stack[-1] == 0) GOTO 0x172; Pop(1)

0x16c: Push("immunity") // @poff=190
0x16d: Push((int) 0)
0x16e: @@ SetProperty(Stack[-2], Stack[-1]); Obj=10 // @poff=134
0x16f: Pop(2)
0x170: Stack[-2] = Stack[-7] - Stack[-1]; Pop(0);
0x171: GOTO 0x178

0x172: Push("immunity") // @poff=190
0x173: Pop(0); Push(Stack[-2] - Stack[-8]);
0x174: @@ SetProperty(Stack[-2], Stack[-1]); Obj=10 // @poff=134
0x175: Pop(2)
0x176: Stack[-7] = Stack[-9]
0x177: Return(); Pop(6)

0x178: PushEmpty(bool, object, string, float, float, float)
0x179: Stack[-14] = Stack[-5]
0x17a: Stack[-4] = "disease" // @poff=146
0x17b: Stack[-8] = Stack[-3]
0x17c: Stack[-2] = (int) 0
0x17d: Stack[-1] = (int) 1
0x17e: Call 0x12d

0x17f: Pop(6)
0x180: Stack[-7] = Stack[-9]
0x181: Return(); Pop(6)

0x182: PushEmpty(bool, bool)
0x183: @@ IsDead(Stack[-1]); Obj=3 // @poff=208
0x184: Pop(0)
0x185: Stack[-1] = Stack[-4]
0x186: Return(); Pop(2)

0x187: PushEmpty(object, object, object, object)
0x188: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x19c: Pop(0)
0x19d: Pop(0); PushNull((bool) Stack[-2] == 0)
0x19e: IF (Stack[-1] == 0) GOTO 0x1a1; Pop(1)

0x19f: Stack[-6] = (bool) 0
0x1a0: Return(); Pop(4)

0x1a1: @@ GetScene(Stack[-1]); Obj=5 // @poff=229
0x1a2: Pop(0)
...
0x1b6: Stack[-1] = "noaccess" // @poff=238
0x1b7: Call 0x121
...
0x1bd: Push("noaccess") // @poff=238
0x1be: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=122
0x1bf: Pop(1)
0x1c0: Push((int) 0)
