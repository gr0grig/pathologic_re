...
Strings:
	...
	A:GetProperty
	A:RemoveItem
// @pool_raw:
...
0x26: Push("door_closed") // @poff=0
0x27: @ PlaySound(Stack[-1])
0x28: Pop(1)
0x29: Push("Door is locked") // @poff=24
0x2a: @ Trace(Stack[-1])
0x2b: Pop(1)
0x2c: Return(); Pop(16)
...
0x31: Stack[-1] = "@" // @poff=54
0x32: Call 0x69

0x33: Stack[-3] = Stack[-10]
0x34: Pop(3)
0x35: Push((int) -1)
0x36: Pop(1); Push((bool) Stack[-8] == Stack[-1])
0x37: IF (Stack[-1] == 0) GOTO 0x3c; Pop(1)

0x38: Push("Wrong door name") // @poff=58
0x39: @ Trace(Stack[-1])
0x3a: Pop(1)
0x3b: Return(); Pop(16)

0x3c: PushEmpty(string, string, int, int)
0x3d: Stack[-12] = Stack[-3]
0x3e: Stack[-2] = (int) 0
0x3f: Stack[-11] = Stack[-1]
0x40: Call 0x73

0x41: Stack[-4] = Stack[-10]
0x42: Pop(4)
0x43: Push("pt_") // @poff=90
0x44: PushEmpty(string, string, int)
0x45: Stack[-12] = Stack[-2]
0x46: Push((int) 1)
...
0x4c: Pop(0)
0x4d: Pop(0); PushNull((bool) Stack[-4] == 0)
0x4e: IF (Stack[-1] == 0) GOTO 0x56; Pop(1)

0x4f: Push("Destination scene '") // @poff=98
0x50: Pop(1); Push(Stack[-1] + Stack[-7]);
0x51: Push("' not found") // @poff=138
0x52: Pop(2); Push(Stack[-2] + Stack[-1]);
0x53: @ Trace(Stack[-1])
0x54: Pop(1)
0x55: Return(); Pop(16)

0x56: @@ GetLocator(Stack[-5], Stack[-3], Stack[-2], Stack[-1]); Obj=4 // @poff=162
0x57: Pop(0)
0x58: Push(Stack[-3])
0x59: IF (Stack[-1] == 0) GOTO 0x60; Pop(1)

0x5a: Push("door_open") // @poff=173
...
0x60: Push("Locator ") // @poff=193
0x61: Pop(1); Push(Stack[-1] + Stack[-6]);
0x62: Push(" not found in destination scene ") // @poff=211
...
0x88: Stack[-5] = Stack[-2]
0x89: Stack[-1] = "@" // @poff=54
0x8a: Call 0x69

0x8b: Stack[-3] = Stack[-4]
0x8c: Pop(3)
0x8d: Push((int) -1)
0x8e: Pop(1); Push((bool) Stack[-2] == Stack[-1])
0x8f: IF (Stack[-1] == 0) GOTO 0x95; Pop(1)

0x90: Push("Wrong door name") // @poff=58
0x91: @ Trace(Stack[-1])
0x92: Pop(1)
0x93: Stack[-5] = "" // @poff=21
0x94: Return(); Pop(4)

0x95: PushEmpty(string, string, int, int)
0x96: Stack[-6] = Stack[-3]
0x97: Stack[-2] = (int) 0
0x98: Stack[-5] = Stack[-1]
0x99: Call 0x73

0x9a: Stack[-4] = Stack[-9]
0x9b: Pop(4)
0x9c: Return(); Pop(4)

0x9d: PushEmpty(int, int, int, int, int, object, int, bool, string, int, int, int, int, int, object, int, bool, string)
0x9e: Push("uni_key") // @poff=277
0x9f: @ GetInvItemByName(Stack[-10], Stack[-1])
0xa0: Pop(1)
0xa1: @@ GetSubContainerCount(Stack[-8]); Obj=21 // @poff=293
0xa2: Pop(0)
0xa3: Stack[-7] = (int) 0
0xa4: Pop(0); Push((bool) Stack[-7] < Stack[-8])
0xa5: IF (Stack[-1] == 0) GOTO 0xcb; Pop(1)

0xa6: @@ GetItemCount(Stack[-6], Stack[-7]); Obj=21 // @poff=314
0xa7: Pop(0)
0xa8: Stack[-5] = (int) 0
0xa9: Pop(0); Push((bool) Stack[-5] < Stack[-6])
0xaa: IF (Stack[-1] == 0) GOTO 0xc8; Pop(1)

0xab: @@ GetItem(Stack[-4], Stack[-5], Stack[-7]); Obj=21 // @poff=327
0xac: Pop(0)
0xad: @@ GetItemID(Stack[-3]); Obj=4 // @poff=335
0xae: Pop(0)
0xaf: Pop(0); Push((bool) Stack[-3] == Stack[-9])
0xb0: IF (Stack[-1] == 0) GOTO 0xc4; Pop(1)

0xb1: Push("key_scene_name") // @poff=345
0xb2: @@ HasProperty(Stack[-3], Stack[-1]); Obj=5 // @poff=375
0xb3: Pop(1)
0xb4: Push(Stack[-2])
0xb5: IF (Stack[-1] == 0) GOTO 0xc4; Pop(1)

0xb6: Push("key_scene_name") // @poff=345
0xb7: @@ GetProperty(Stack[-2], Stack[-1]); Obj=5 // @poff=387
0xb8: Pop(1)
0xb9: Push(" ") // @poff=207
...
0xc0: @@ RemoveItem(Stack[-5], Stack[-7]); Obj=21 // @poff=399
0xc1: Pop(0)
