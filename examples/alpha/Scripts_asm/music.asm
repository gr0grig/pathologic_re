...
Strings:
	...
	W:loop_stop
	W:music1.ogg
// @pool_raw:
...
0x4: @ IsExistingLoop(Stack[-1]T, Stack[-1])
0x5: Pop(0)
0x6: Pop(0); Push((bool) Stack[-1] == 0)
0x7: IF (Stack[-1] == 0) GOTO 0x16; Pop(1)

0x8: @ IsExistingLoop(Stack[-2]T, Stack[-1])
0x9: Pop(0)
...
0x20: Push("Playing music: ") // @poff=0
0x21: Pop(1); Push(Stack[-1] + Stack[0 + Tasks[-1].StackPointer]);
0x22: @ Trace(Stack[-1])
0x23: Pop(1)
0x24: @ PlayMusic1(Stack[-0]T)
0x25: Pop(0)
0x26: @ WaitForMusicEnd1()
0x27: Pop(0)
0x28: GOTO 0x2e

0x29: Push("Nothing to play") // @poff=32
0x2a: @ Trace(Stack[-1])
0x2b: Pop(1)
0x2c: @ Hold()
0x2d: Pop(0)
...
0x3e: Pop(0)
0x3f: @ GetRegionLoop(Stack[-1], Stack[-2]T)
0x40: Pop(0)
0x41: Push("Next loop: ") // @poff=64
0x42: Pop(1); Push(Stack[-1] + Stack[2 + Tasks[-1].StackPointer]);
0x43: @ Trace(Stack[-1])
0x44: Pop(1)
0x45: Push((int) 0)
...
0x56: Push("Playing loop: ") // @poff=88
0x57: Pop(1); Push(Stack[-1] + Stack[2 + Tasks[-1].StackPointer]);
0x58: @ Trace(Stack[-1])
0x59: Pop(1)
0x5a: @ PlayMusic2(Stack[-2]T)
0x5b: Pop(0)
...
0x64: @ IsExistingMusic(Stack[-0]T, Stack[-1])
0x65: Pop(0)
...
0x70: @ StopGroup0()
0x71: Pop(0)
0x72: Push("Music time limit reached") // @poff=118
0x73: @ Trace(Stack[-1])
0x74: Pop(1)
0x75: Return(); Pop(4)

0x76: PushEmpty()
0x77: Push("loop") // @poff=168
0x78: Pop(1); Push((bool) Stack[-3] == Stack[-1])
0x79: IF (Stack[-1] == 0) GOTO 0x82; Pop(1)

0x7a: Push("Playing loop: ") // @poff=88
...
0x89: Stack[-2] = "play" // @poff=54
0x8a: Push(Stack[-2])
0x8b: IF (Stack[-1] == 0) GOTO 0x8f; Pop(1)

0x8c: Push("music1.ogg") // @poff=198
