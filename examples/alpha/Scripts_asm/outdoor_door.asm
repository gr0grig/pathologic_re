...
0x16: Stack[-11] = Stack[-2]
0x17: Stack[-1] = "@" // @poff=54
0x18: Call 0x4f

0x19: Stack[-3] = Stack[-10]
0x1a: Pop(3)
0x1b: Push((int) -1)
...
0x23: Stack[-12] = Stack[-3]
0x24: Stack[-2] = (int) 0
0x25: Stack[-11] = Stack[-1]
0x26: Call 0x59

0x27: Stack[-4] = Stack[-10]
0x28: Pop(4)
0x29: Push("pt_") // @poff=90
...
0x32: Pop(0)
0x33: Pop(0); PushNull((bool) Stack[-4] == 0)
0x34: IF (Stack[-1] == 0) GOTO 0x3c; Pop(1)

0x35: Push("Destination scene '") // @poff=98
0x36: Pop(1); Push(Stack[-1] + Stack[-7]);
0x37: Push("' not found") // @poff=138
0x38: Pop(2); Push(Stack[-2] + Stack[-1]);
0x39: @ Trace(Stack[-1])
0x3a: Pop(1)
0x3b: Return(); Pop(16)

0x3c: @@ GetLocator(Stack[-5], Stack[-3], Stack[-2], Stack[-1]); Obj=4 // @poff=162
0x3d: Pop(0)
0x3e: Push(Stack[-3])
0x3f: IF (Stack[-1] == 0) GOTO 0x46; Pop(1)

0x40: Push("door_open") // @poff=173
...
0x46: Push("Locator ") // @poff=193
0x47: Pop(1); Push(Stack[-1] + Stack[-6]);
0x48: Push(" not found in destination scene ") // @poff=211
