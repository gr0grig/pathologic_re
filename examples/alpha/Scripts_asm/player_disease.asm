...
// @pool_raw:6400690073006500610073006500000044006900730065006100730065002000650066006600650063007400200073007400610072007400650064000000440069007300650061007300650020006500660066006500630074002000730074006f0070007000650064000000
...
0x14: Push("Disease effect started") // @poff=16
0x15: @ Trace(Stack[-1])
0x16: Pop(1)
0x17: Push("disease") // @poff=0
...
0x5b: Push("Disease effect stopped") // @poff=62
0x5c: @ Trace(Stack[-1])
0x5d: Pop(1)
0x5e: Return(); Pop(4)
...
0x66: Pop(1)
0x67: Push((float)0.6283185482025146)
0x68: Push((float)0.5)
...
0x7d: Stack[-5] = Stack[-2]
0x7e: Stack[-9] = Stack[-1]
0x7f: Call 0xb6
...
0x84: Stack[-5] = Stack[-2]
0x85: Stack[-8] = Stack[-1]
0x86: Call 0xca
...
0x8b: IF (Stack[-1] == 1) GOTO 0x76; Pop(1)

0x8c: Stack[-4] = Stack[-3]
0x8d: @ sync(Stack[-1])
0x8e: Pop(0)
...
0x96: Stack[-5] = Stack[-2]
0x97: Stack[-9] = Stack[-1]
0x98: Call 0xb6
...
0xa5: IF (Stack[-1] == 1) GOTO 0x8d; Pop(1)
...
0xe6: Push((float)9.999999974752427e-07)
...
0xf6: Stack[-3] = Stack[-4]
0xf7: Return(); Pop(0)

0xf8: PushEmpty(float, float)
0xf9: Push("disease") // @poff=0
0xfa: @ GetProperty(Stack[-1], Stack[-2])
0xfb: Pop(1)
0xfc: Push((float)0.009999999776482582)
