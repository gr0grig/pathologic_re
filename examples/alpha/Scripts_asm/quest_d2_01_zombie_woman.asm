...
Strings:
	...
	W:health
	W:idle
// @pool_raw:
...
0x3a9: Pop(0)
0x3aa: @@ GetPosition(Stack[-2]); Obj=7 // @poff=10
0x3ab: Pop(0)
...
0x3b0: Push("HasProperty") // @poff=264
0x3b1: Push((int) 2)
...
0x3b7: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=288
0x3b8: Pop(0)
...
0x3bd: Stack[-10] = Stack[-2]
0x3be: Stack[-9] = Stack[-1]
0x3bf: Call 0x3af
...
0x3c5: @@ GetProperty(Stack[-6], Stack[-1]); Obj=7 // @poff=300
0x3c6: Pop(0)
0x3c7: PushEmpty(float, float, float, float)
0x3c8: Stack[-3] = Stack[-5] + Stack[-9]; Pop(0);
0x3c9: Stack[-8] = Stack[-2]
0x3ca: Stack[-7] = Stack[-1]
0x3cb: Call 0x49f

0x3cc: Pop(3)
0x3cd: @@ SetProperty(Stack[-7], Stack[-1]); Obj=8 // @poff=312
0x3ce: Pop(1)
...
0x3d3: Stack[-11] = Stack[-2]
0x3d4: Stack[-1] = "disease" // @poff=324
0x3d5: Call 0x3af
...
0x3dc: Stack[-11] = Stack[-2]
0x3dd: Stack[-1] = "armor_disease" // @poff=340
0x3de: Call 0x3af

0x3df: Pop(2)
0x3e0: IF (Stack[-1] == 0) GOTO 0x3ef; Pop(1)

0x3e1: Push("armor_disease") // @poff=340
0x3e2: @@ GetProperty(Stack[-1], Stack[-4]); Obj=9 // @poff=300
0x3e3: Pop(1)
0x3e4: Push((int) 100)
...
0x3f0: Stack[-11] = Stack[-2]
0x3f1: Stack[-1] = "immunity" // @poff=368
0x3f2: Call 0x3af

0x3f3: Pop(2)
0x3f4: IF (Stack[-1] == 0) GOTO 0x406; Pop(1)

0x3f5: Push("immunity") // @poff=368
0x3f6: @@ GetProperty(Stack[-1], Stack[-2]); Obj=9 // @poff=300
0x3f7: Pop(1)
0x3f8: Pop(0); Push((bool) Stack[-1] < Stack[-7])
0x3f9: IF (Stack[-1] == 0) GOTO 0x400; Pop(1)

0x3fa: Push("immunity") // @poff=368
0x3fb: Push((int) 0)
0x3fc: @@ SetProperty(Stack[-2], Stack[-1]); Obj=10 // @poff=312
0x3fd: Pop(2)
0x3fe: Stack[-2] = Stack[-7] - Stack[-1]; Pop(0);
0x3ff: GOTO 0x406

0x400: Push("immunity") // @poff=368
0x401: Pop(0); Push(Stack[-2] - Stack[-8]);
0x402: @@ SetProperty(Stack[-2], Stack[-1]); Obj=10 // @poff=312
0x403: Pop(2)
0x404: Stack[-7] = Stack[-9]
0x405: Return(); Pop(6)

0x406: PushEmpty(bool, object, string, float, float, float)
0x407: Stack[-14] = Stack[-5]
0x408: Stack[-4] = "disease" // @poff=324
0x409: Stack[-8] = Stack[-3]
0x40a: Stack[-2] = (int) 0
0x40b: Stack[-1] = (int) 1
0x40c: Call 0x3bb

0x40d: Pop(6)
0x40e: Stack[-7] = Stack[-9]
0x40f: Return(); Pop(6)

0x410: PushEmpty(bool, bool)
0x411: @@ IsDead(Stack[-1]); Obj=3 // @poff=386
0x412: Pop(0)
...
0x42f: @@ GetScene(Stack[-1]); Obj=5 // @poff=407
0x430: Pop(0)
...
0x444: Stack[-1] = "noaccess" // @poff=416
0x445: Call 0x3af
...
0x44b: Push("noaccess") // @poff=416
0x44c: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=300
0x44d: Pop(1)
0x44e: Push((int) 0)
...
0x467: Push("scripted") // @poff=434
0x468: Push(CVector(0.0, 0.0, 1.0))
0x469: Push("blood.xml") // @poff=452
...
0x488: Stack[-1] = "fhit" // @poff=472
0x489: GOTO 0x48b

0x48a: Stack[-1] = "bhit" // @poff=482
0x48b: Push("hit_react") // @poff=492
0x48c: Push("1") // @poff=512
0x48d: Pop(1); Push(Stack[-3] + Stack[-1]);
0x48e: Push("2") // @poff=516
...
0x4c7: Stack[-7] = Stack[-3]
0x4c8: Stack[-6] = Stack[-2]
0x4c9: Stack[-5] = Stack[-1]
0x4ca: Call 0x451
...
0x51c: Push(Stack[-2])
0x51d: IF (Stack[-1] == 1) GOTO 0x526; Pop(1)
...
0x524: IF (Stack[-1] == 1) GOTO 0x526; Pop(1)

0x525: Stack[-1] = (bool) 0
0x526: IF (Stack[-1] == 0) GOTO 0x52c; Pop(1)

0x527: Push((float)-0.30000001192092896)
