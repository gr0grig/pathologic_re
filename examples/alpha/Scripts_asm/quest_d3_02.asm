...
Strings:
	...
	W:Locator 
	W: doesn't exist
// @pool_raw:
...
0xb: Stack[-6] = Stack[-4]
0xc: Stack[-3] = "pt_d3q02_butcher" // @poff=38
0xd: Stack[-2] = "pers_butcher" // @poff=72
0xe: Stack[-1] = "d3q02_butcher.xml" // @poff=98
0xf: Call 0xa3

0x10: Stack[0 + Tasks[-1].StackPointer] = Stack[-5]
0x11: Pop(5)
0x12: Push("d3q02_corpse") // @poff=134
0x13: Push("actor_disp.bin") // @poff=160
0x14: Push(CVector(0.0, 0.0, 0.0))
0x15: @ AddScriptedActor(Stack[-1]T, Stack[-3], Stack[-2], Stack[-4], Stack[-1])
0x16: Pop(3)
0x17: Push("d3q02_btrigger") // @poff=190
0x18: Push("quest_d3_02_btrigger.bin") // @poff=220
0x19: Push(CVector(0.0, 0.0, 0.0))
0x1a: @ AddScriptedActor(Stack[-2]T, Stack[-3], Stack[-2], Stack[-4], Stack[-1])
0x1b: Pop(3)
0x1c: PushEmpty(int)
0x1d: Stack[-1] = (int) 3
0x1e: Call 0x7f
...
0x23: @ Trace(Stack[-1])
0x24: Pop(0)
0x25: Push("butcher_trigger") // @poff=270
...
0x2a: Push("trigger") // @poff=204
0x2b: @ Trigger(Stack[-0]T, Stack[-1])
0x2c: Pop(1)
0x2d: PushEmpty()
0x2e: Call 0x65

0x2f: Pop(0)
0x30: Push("d3q02SeeCorpse") // @poff=302
0x31: Push((int) 1)
0x32: @ SetVariable(Stack[-2], Stack[-1])
0x33: Pop(2)
0x34: GOTO 0x3b

0x35: Push("completed") // @poff=332
...
0x67: Push("cleanup") // @poff=352
0x68: @ Trigger(Stack[-0]T, Stack[-1])
0x69: Pop(1)
0x6a: Push( Stack[1 + Tasks[-1].StackPointer] )
0x6b: IF (Stack[-1] == 0) GOTO 0x6f; Pop(1)

0x6c: Push("cleanup") // @poff=352
0x6d: @ Trigger(Stack[-1]T, Stack[-1])
0x6e: Pop(1)
0x6f: Push( Stack[2 + Tasks[-1].StackPointer] )
0x70: IF (Stack[-1] == 0) GOTO 0x74; Pop(1)

0x71: Push("cleanup") // @poff=352
0x72: @ Trigger(Stack[-2]T, Stack[-1])
0x73: Pop(1)
0x74: Return(); Pop(0)
...
0xa4: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=368
0xa5: Pop(0)
0xa6: Pop(0); Push((bool) Stack[-4] == 0)
0xa7: IF (Stack[-1] == 0) GOTO 0xb0; Pop(1)

0xa8: Push("Locator ") // @poff=379
0xa9: Pop(1); Push(Stack[-1] + Stack[-12]);
0xaa: Push(" doesn't exist") // @poff=397
