...
Strings:
	...
	W:player
	W:ui/NPC_Black.png
// @pool_raw:
// @pool_encoding:utf8
...
0x6f: Push("all") // @poff=110
0x70: Push("idle") // @poff=118
...
0x78: Push("all") // @poff=110
0x79: Push("idle") // @poff=118
...
0x14a: Stack[-2] = "quest_d4_01" // @poff=128
0x14b: Stack[-1] = "birdmask_done" // @poff=152
0x14c: Call 0x1fd
...
0x171: Pop(0)
0x172: Push("all") // @poff=110
0x173: PushEmpty(string, int)
0x174: Stack[-7] = Stack[-1]
0x175: Call 0x20e
...
0x19f: @@ GetPosition(Stack[-7]); Obj=17 // @poff=180
0x1a0: Pop(0)
0x1a1: @@ GetEyesHeight(Stack[-8]); Obj=17 // @poff=192
0x1a2: Pop(0)
...
0x1e2: Pop(0)
0x1e3: Push("start: ") // @poff=224
0x1e4: Pop(1); Push(Stack[-1] + Stack[-3]);
0x1e5: @ Trace(Stack[-1])
0x1e6: Pop(1)
0x1e7: Push("end: ") // @poff=240
...
0x1f6: Push((float)9.999999974752427e-07)
...
0x1ff: Pop(0)
0x200: Pop(0); PushNull((bool) Stack[-1] == 0)
...
0x22a: Pop(1)
0x22b: Push("d4q01Whitemask") // @poff=252
0x22c: Push("pt_d4q01_key1") // @poff=282
0x22d: Push((int) 1)
...
0x231: Pop(0)
0x232: @@ AddMark(Stack[-5], Stack[-4], Stack[-3], Stack[-2], Stack[-1]); Obj=6 // @poff=310
0x233: Pop(5)
0x234: Return(); Pop(2)
...
0x23b: Pop(1)
0x23c: Stack[-2] = "pt_d4q01_whitemask" // @poff=318
0x23d: Stack[-1] = (int) 2
0x23e: Call 0x257
...
0x242: Pop(0)
0x243: @@ ShowMap(Stack[-1]); Obj=2 // @poff=356
0x244: Pop(1)
0x245: Return(); Pop(0)
...
0x248: Pop(0)
0x249: Pop(0); PushNull((bool) Stack[-2] == 0)
0x24a: IF (Stack[-1] == 0) GOTO 0x251; Pop(1)

0x24b: Push("Can't find main outdoor scene") // @poff=364
0x24c: @ Trace(Stack[-1])
0x24d: Pop(1)
0x24e: Stack[-1] = 0
0x24f: Stack[-1] = Stack[-5]
0x250: Return(); Pop(4)

0x251: @@ GetMap(Stack[-1]); Obj=2 // @poff=424
0x252: Pop(0)
...
0x25c: Push("Can't find main outdoor scene") // @poff=364
0x25d: @ Trace(Stack[-1])
0x25e: Pop(1)
0x25f: Return(); Pop(8)

0x260: @@ GetLocator(Stack[-10], Stack[-1], Stack[-4], Stack[-3]); Obj=2 // @poff=431
0x261: Pop(0)
0x262: Pop(0); Push((bool) Stack[-1] == 0)
0x263: IF (Stack[-1] == 0) GOTO 0x26a; Pop(1)

0x264: Push("Warning: outdoor scene locator ") // @poff=442
0x265: Pop(1); Push(Stack[-1] + Stack[-11]);
0x266: Push(" doesnt exist") // @poff=506
0x267: Pop(2); Push(Stack[-2] + Stack[-1]);
0x268: @ Trace(Stack[-1])
0x269: Pop(1)
0x26a: @@ GetMap(Stack[-11]); Obj=2 // @poff=424
0x26b: Pop(0)
0x26c: Pop(0); PushNull((bool) Stack[-11] == 0)
0x26d: IF (Stack[-1] == 0) GOTO 0x272; Pop(1)

0x26e: Push("Can't find map") // @poff=534
0x26f: @ Trace(Stack[-1])
0x270: Pop(1)
0x271: Return(); Pop(8)
...
0x274: @@ SetMapParams(Stack[-2], Stack[-1], Stack[-11]); Obj=13 // @poff=564
0x275: Pop(2)
0x276: Return(); Pop(8)

0x277: Stack[-2] = 0
0x278: PushEmpty(int, int)
0x279: Push("player") // @poff=577
...
0x28b: Stack[-1] = "ui/NPC_Black.png" // @poff=591
0x28c: Return(); Pop(0)
