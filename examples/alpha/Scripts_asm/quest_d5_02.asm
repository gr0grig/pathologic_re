...
Strings:
	...
	W:Locator 
	W: doesn't exist
// @pool_raw:
...
0x15: Push("shouse1_kabak") // @poff=68
0x16: @ GetSceneByName(Stack[-3], Stack[-1])
0x17: Pop(1)
0x18: PushEmpty(object, object, string, string, string)
0x19: Stack[-7] = Stack[-4]
0x1a: Stack[-3] = "pt_d5q02_girl" // @poff=96
0x1b: Stack[-2] = "pers_nudegirl" // @poff=124
0x1c: Stack[-1] = "d5q02_girl.xml" // @poff=152
0x1d: Call 0xb9
...
0x22: Push("restore") // @poff=182
0x23: @ Trigger(Stack[-0]T, Stack[-1])
0x24: Pop(1)
0x25: GOTO 0x60

0x26: Push("remove_girl") // @poff=198
0x27: Pop(1); Push((bool) Stack[-6] == Stack[-1])
0x28: IF (Stack[-1] == 0) GOTO 0x33; Pop(1)

0x29: Push("d5q01NudeInKabak") // @poff=34
0x2a: Push((int) 0)
...
0x2f: Push("cleanup") // @poff=222
0x30: @ Trigger(Stack[-0]T, Stack[-1])
0x31: Pop(1)
0x32: GOTO 0x60

0x33: Push("place_gorbun") // @poff=238
0x34: Pop(1); Push((bool) Stack[-6] == Stack[-1])
0x35: IF (Stack[-1] == 0) GOTO 0x4d; Pop(1)

0x36: Push("d5q01Gorbun") // @poff=264
0x37: Push((int) 1)
...
0x3c: Push("lc_House6_05") // @poff=288
0x3d: @ GetSceneByName(Stack[-2], Stack[-1])
0x3e: Pop(1)
0x3f: PushEmpty(object, object, string, string, string)
0x40: Stack[-6] = Stack[-4]
0x41: Stack[-3] = "pt_gorbun" // @poff=314
0x42: Stack[-2] = "pers_bomber" // @poff=334
0x43: Stack[-1] = "d5q02_gorbun.xml" // @poff=358
0x44: Call 0xb9
...
0x49: Push("restore") // @poff=182
0x4a: @ Trigger(Stack[-1]T, Stack[-1])
0x4b: Pop(1)
0x4c: GOTO 0x60

0x4d: Push("remove_gorbun") // @poff=392
0x4e: Pop(1); Push((bool) Stack[-6] == Stack[-1])
0x4f: IF (Stack[-1] == 0) GOTO 0x5a; Pop(1)

0x50: Push("d5q01Gorbun") // @poff=264
0x51: Push((int) 0)
...
0x56: Push("cleanup") // @poff=222
0x57: @ Trigger(Stack[-1]T, Stack[-1])
0x58: Pop(1)
0x59: GOTO 0x60

0x5a: Push("completed") // @poff=420
...
0x8d: Push("cleanup") // @poff=222
0x8e: @ Trigger(Stack[-0]T, Stack[-1])
0x8f: Pop(1)
0x90: Push( Stack[1 + Tasks[-1].StackPointer] )
0x91: IF (Stack[-1] == 0) GOTO 0x95; Pop(1)

0x92: Push("cleanup") // @poff=222
0x93: @ Trigger(Stack[-1]T, Stack[-1])
0x94: Pop(1)
...
0xba: @@ GetLocator(Stack[-11], Stack[-4], Stack[-3], Stack[-2]); Obj=12 // @poff=440
0xbb: Pop(0)
0xbc: Pop(0); Push((bool) Stack[-4] == 0)
0xbd: IF (Stack[-1] == 0) GOTO 0xc6; Pop(1)

0xbe: Push("Locator ") // @poff=451
0xbf: Pop(1); Push(Stack[-1] + Stack[-12]);
0xc0: Push(" doesn't exist") // @poff=469
