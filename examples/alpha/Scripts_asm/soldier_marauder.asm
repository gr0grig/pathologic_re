...
Strings:
	...
	W:rusk
	W:health
// @pool_raw:
...
0x93: Push((int) 110)
0x94: Push((float)0.30000001192092896)
...
0x13b: Pop(0)
0x13c: @@@ GetPosition(Stack[-5]); Obj=0 // @poff=10
0x13d: Pop(0)
...
0x15a: Stack[-1] = (float) 1.7453293800354004
0x15b: Call 0x139

0x15c: Stack[-2] = Stack[-7]
0x15d: Pop(2)
...
0x162: Push("Can't retreat, distance: ") // @poff=178
...
0x178: Push((int) 1)
0x179: @ MovePoint(Stack[-1]T, Stack[-1], Stack[-4])
0x17a: Pop(1)
0x17b: Push(Stack[-3])
0x17c: IF (Stack[-1] == 0) GOTO 0x194; Pop(1)

0x17d: Pop(0); PushNull((bool) Stack[0 + Tasks[-1].StackPointer] == 0)
...
0x195: IF (Stack[-1] == 1) GOTO 0x178; Pop(1)
...
0x19c: Pop(0); PushNull((bool) Stack[0 + Tasks[-1].StackPointer] == 0)
...
0x1ab: Stack[-1] = (float) 1.7453293800354004
0x1ac: Call 0x139

0x1ad: Stack[-2] = Stack[-4]
0x1ae: Pop(2)
...
0x1e1: Stack[-2] = Stack[-3]
0x1e2: Pop(2)
0x1e3: PushEmpty(float, cvector, cvector)
0x1e4: Stack[-5] = Stack[-2]
0x1e5: Stack[-4] = Stack[-1]
0x1e6: Call 0x291

0x1e7: Pop(2)
0x1e8: Push((float)-0.3420201241970062)
...
0x1f2: Pop(0)
0x1f3: @@ GetPosition(Stack[-1]); Obj=5 // @poff=10
0x1f4: Pop(0)
0x1f5: Stack[-6] = Stack[-1] - Stack[-2]; Pop(0);
0x1f6: Return(); Pop(4)

0x1f7: PushEmpty(bool, bool)
0x1f8: Push("HasProperty") // @poff=230
0x1f9: Push((int) 2)
...
0x1ff: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=254
0x200: Pop(0)
0x201: Stack[-1] = Stack[-5]
0x202: Return(); Pop(2)

0x203: PushEmpty(bool, bool)
0x204: @@ IsDead(Stack[-1]); Obj=3 // @poff=266
0x205: Pop(0)
0x206: Stack[-1] = Stack[-4]
0x207: Return(); Pop(2)

0x208: PushEmpty(object, object, object, object)
0x209: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x21d: Pop(0)
0x21e: Pop(0); PushNull((bool) Stack[-2] == 0)
0x21f: IF (Stack[-1] == 0) GOTO 0x222; Pop(1)

0x220: Stack[-6] = (bool) 0
0x221: Return(); Pop(4)

0x222: @@ GetScene(Stack[-1]); Obj=5 // @poff=287
0x223: Pop(0)
...
0x237: Stack[-1] = "noaccess" // @poff=296
0x238: Call 0x1f7
...
0x23e: Push("noaccess") // @poff=296
0x23f: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=314
0x240: Pop(1)
0x241: Push((int) 0)
...
0x26a: @@ GetPosition(Stack[-5]); Obj=23 // @poff=10
0x26b: Pop(0)
...
0x27b: Stack[-1] = "fhit" // @poff=364
0x27c: GOTO 0x27e

0x27d: Stack[-1] = "bhit" // @poff=374
0x27e: Push("hit_react") // @poff=384
0x27f: Push("1") // @poff=404
0x280: Pop(1); Push(Stack[-3] + Stack[-1]);
0x281: Push("2") // @poff=408
...
0x29c: Push("GenerateMoney: iMin > iMax") // @poff=412
0x29d: @ Trace(Stack[-1])
0x29e: Pop(1)
0x29f: Return(); Pop(8)
...
0x2c7: Stack[-1] = "rifle_ammo" // @poff=478
0x2c8: Call 0x2dc
...
0x2eb: Stack[-7] = Stack[-3]
0x2ec: Stack[-6] = Stack[-2]
