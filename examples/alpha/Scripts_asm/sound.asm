...
0x1d: Push("distance") // @poff=0
0x1e: @ GetProperty(Stack[-1], Stack[-1]T)
0x1f: Pop(1)
...
0x25: Pop(1)
0x26: Stack[-1] = "mindistance" // @poff=18
0x27: Call 0x9b
...
0x2d: Push("mindistance") // @poff=18
0x2e: @ GetProperty(Stack[-1], Stack[-2]T)
0x2f: Pop(1)
0x30: Push((int) 101)
0x31: Push((float)1.0)
...
0x3b: Stack[-1] = Stack[-6]
0x3c: Pop(1)
0x3d: Pop(0); PushNull((bool) Stack[-5] == 0)
0x3e: IF (Stack[-1] == 0) GOTO 0x43; Pop(1)

0x3f: Push("Null player") // @poff=42
0x40: @ Trace(Stack[-1])
0x41: Pop(1)
0x42: Return(); Pop(10)

0x43: PushEmpty(cvector, object)
0x44: Stack[-7] = Stack[-1]
0x45: Call 0x8f

0x46: Stack[-2] = Stack[-6]
0x47: Pop(2)
...
0x56: Push("stopped") // @poff=66
0x57: @ Trace(Stack[-1])
0x58: Pop(1)
0x59: @@@ Stop(); Obj=0 // @poff=82
0x5a: Pop(0)
...
0x65: EventDisable(7)
0x66: Push("started") // @poff=87
0x67: @ Trace(Stack[-1])
0x68: Pop(1)
0x69: Push("envsound") // @poff=103
0x6a: @ WaitFor3DSoundToLoad(Stack[-1])
0x6b: Pop(1)
0x6c: Push("envsound") // @poff=103
0x6d: Push(CVector(0.0, 0.0, 0.0))
0x6e: Push((float)0.0)
0x6f: @ PlayLoopedGlobalSound(Stack[-0]T, Stack[-3], Stack[-2], Stack[-1], Stack[-2]T)
0x70: Pop(3)
0x71: Pop(0); PushNull((bool) Stack[0 + Tasks[-1].StackPointer] == 0)
0x72: IF (Stack[-1] == 0) GOTO 0x77; Pop(1)

0x73: Push("Sound was not found") // @poff=121
0x74: @ Trace(Stack[-1])
0x75: Pop(1)
0x76: GOTO 0x7a

0x77: Push((float)0.5)
0x78: @@@ FadeIn(Stack[-1]); Obj=0 // @poff=161
0x79: Pop(1)
0x7a: EventEnable(7)
0x7b: Return(); Pop(10)
...
0x90: @@ GetPosition(Stack[-1]); Obj=3 // @poff=168
0x91: Pop(0)
0x92: Stack[-1] = Stack[-4]
0x93: Return(); Pop(2)

0x94: PushEmpty(object, object)
0x95: Push("player") // @poff=52
...
0x9c: Push("HasProperty") // @poff=180
0x9d: Push((int) 2)
...
0xa3: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=204
0xa4: Pop(0)
