...
Strings:
	...
	W:armor_
	A:SetProperty
// @pool_raw:
// @pool_encoding:utf8
...
0xb: @@ iterator(Stack[-5]); Obj=1 // @poff=0
0xc: Pop(1)
0xd: @@ Next(Stack[-3], Stack[-2]); Obj=4 // @poff=9
0xe: Pop(0)
0xf: Push(Stack[-3])
0x10: IF (Stack[-1] == 0) GOTO 0x24; Pop(1)

0x11: Push("Damaging: ") // @poff=14
0x12: Pop(1); Push(Stack[-1] + Stack[-3]);
0x13: @ Trace(Stack[-1])
0x14: Pop(1)
0x15: PushEmpty(float, object, float, int)
0x16: Stack[-6] = Stack[-3]
0x17: Stack[-2] = (float) 0.800000011920929
0x18: Stack[-1] = (int) 1
0x19: Call 0x82

0x1a: Stack[-4] = Stack[-5]
0x1b: Pop(4)
...
0x1e: Pop(0)
0x1f: Push((int) 4)
0x20: Push((float)0.800000011920929)
...
0x2b: Stack[-1] = "health" // @poff=36
0x2c: Call 0x76

0x2d: Pop(2)
0x2e: IF (Stack[-1] == 0) GOTO 0x36; Pop(1)

0x2f: Push("New damage target: ") // @poff=50
0x30: Pop(1); Push(Stack[-1] + Stack[-2]);
0x31: @ Trace(Stack[-1])
0x32: Pop(1)
0x33: Push(GlobalVars[0])
0x34: @@ add(Stack[-2]); Obj=1 // @poff=90
0x35: Pop(1)
0x36: Return(); Pop(0)

0x37: PushEmpty()
0x38: Push(GlobalVars[0])
0x39: @@ erase(Stack[-2]); Obj=1 // @poff=94
0x3a: Pop(1)
0x3b: Push("Removed damage target: ") // @poff=100
...
0x72: Stack[-2] = "fire" // @poff=180
0x73: Return(); Pop(0)

0x74: Stack[-2] = "phys" // @poff=190
0x75: Return(); Pop(0)

0x76: PushEmpty(bool, bool)
0x77: Push("HasProperty") // @poff=200
0x78: Push((int) 2)
...
0x7e: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=224
0x7f: Pop(0)
...
0x84: Stack[-18] = Stack[-2]
0x85: Stack[-1] = "health" // @poff=36
0x86: Call 0x76
...
0x8d: Stack[-18] = Stack[-2]
0x8e: Stack[-1] = "armor" // @poff=236
0x8f: Call 0x76
...
0x95: Push("armor") // @poff=236
0x96: @@ GetProperty(Stack[-1], Stack[-7]); Obj=16 // @poff=248
0x97: Pop(1)
0x98: Push("armor_") // @poff=260
0x99: PushEmpty(string, int)
0x9a: Stack[-16] = Stack[-1]
0x9b: Call 0x6e

0x9c: Pop(1)
0x9d: Stack[-7] = Stack[-2] + Stack[-1]; Pop(2);
0x9e: PushEmpty(bool, object, string)
0x9f: Stack[-18] = Stack[-2]
0xa0: Stack[-8] = Stack[-1]
0xa1: Call 0x76
...
0xa7: @@ GetProperty(Stack[-5], Stack[-4]); Obj=15 // @poff=248
0xa8: Pop(0)
...
0xaf: Stack[-3] = Stack[-6]
0xb0: Pop(3)
0xb1: Push("health") // @poff=36
0xb2: @@ GetProperty(Stack[-1], Stack[-3]); Obj=16 // @poff=248
0xb3: Pop(1)
0xb4: Push((int) 1)
0xb5: Pop(1); Push(Stack[-1] - Stack[-4]);
0xb6: Stack[-2] = Stack[-15] * Stack[-1]; Pop(1);
0xb7: Push("health") // @poff=36
...
0xbd: Pop(3)
0xbe: @@ SetProperty(Stack[-2], Stack[-1]); Obj=17 // @poff=274
0xbf: Pop(2)
0xc0: Stack[-1] = Stack[-16]
0xc1: Return(); Pop(12)
