...
Strings:
	...
	W:default
	W:scrollbar
// @pool_raw:4368696c64436f756e74004368696c640047657443617465676f72790047657454657874494400640065006600610075006c00740000007300630072006f006c006c006200610072000000
// @pool_encoding:utf8
...
0x11: @@ ChildCount(Stack[-3]); Obj=4 // @poff=0
0x12: Pop(0)
0x13: Stack[-2] = (int) 0
0x14: Pop(0); Push((bool) Stack[-2] < Stack[-3])
0x15: IF (Stack[-1] == 0) GOTO 0x23; Pop(1)

0x16: @@ Child(Stack[-1], Stack[-2]); Obj=4 // @poff=11
0x17: Pop(0)
...
0x2b: @@ GetCategory(Stack[-8]); Obj=18 // @poff=17
0x2c: Pop(0)
...
0x30: @@ GetTextID(Stack[-7]); Obj=18 // @poff=29
0x31: Pop(0)
0x32: @ GetStringByID(Stack[-6], Stack[-7])
0x33: Pop(0)
0x34: Push("default") // @poff=39
0x35: Pop(0); Push(Stack[1 + Tasks[-1].StackPointer] + Stack[6 + Tasks[-1].StackPointer]);
0x36: Pop(0); Push(Stack[3 + Tasks[-1].StackPointer] + Stack[-19]);
0x37: Push((float)1.0)
0x38: Push((float)1.0)
0x39: Push((float)1.0)
...
0x3d: @@ ChildCount(Stack[-4]); Obj=18 // @poff=0
0x3e: Pop(0)
0x3f: Stack[-3] = (int) 0
0x40: Pop(0); Push((bool) Stack[-3] < Stack[-4])
0x41: IF (Stack[-1] == 0) GOTO 0x55; Pop(1)

0x42: @@ Child(Stack[-2], Stack[-3]); Obj=18 // @poff=11
0x43: Pop(0)
0x44: @@ GetCategory(Stack[-1]); Obj=2 // @poff=17
0x45: Pop(0)
