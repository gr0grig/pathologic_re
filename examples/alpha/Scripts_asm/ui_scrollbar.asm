...
Strings:
	...
	W:scroll_button_up
	W:scroll up 
	W:scroll_button_down
	W:scroll down 
// @pool_raw:
...
0xb: Push("iMouseMax ") // @poff=0
0xc: Pop(1); Push(Stack[-1] + Stack[3 + Tasks[-1].StackPointer]);
0xd: @ Trace(Stack[-1])
0xe: Pop(1)
0xf: @ ProcessEvents()
0x10: Pop(0)
...
0x17: Push("slider_pressed") // @poff=22
0x18: Push((int) 0)
0x19: @ Blit(Stack[-2], Stack[-1], Stack[-2]T)
0x1a: Pop(2)
0x1b: GOTO 0x20

0x1c: Push("slider") // @poff=52
0x1d: Push((int) 0)
0x1e: @ Blit(Stack[-2], Stack[-1], Stack[-2]T)
0x1f: Pop(2)
0x20: Return(); Pop(0)

0x21: PushEmpty(int, int)
0x22: Push((int) 100)
0x23: Pop(1); Push(Stack[2 + Tasks[-1].StackPointer] * Stack[-1]);
...
0x2d: Stack[-11] = Stack[-2]
0x2e: Stack[-10] = Stack[-1]
0x2f: Call 0xb2
...
0x4a: Push((int) 27)
0x4b: Pop(1); Push(Stack[-1] * Stack[3 + Tasks[-1].StackPointer]);
0x4c: Push((int) 100)
...
0x59: Push((int) 27)
0x5a: Pop(1); Push(Stack[-1] * Stack[3 + Tasks[-1].StackPointer]);
0x5b: Push((int) 100)
...
0x6a: PushEmpty()
0x6b: Push("scroll_button_up") // @poff=66
0x6c: Pop(1); Push((bool) Stack[-3] == Stack[-1])
0x6d: IF (Stack[-1] == 0) GOTO 0x7f; Pop(1)

0x6e: Push((int) 27)
0x6f: Pop(1); Push(Stack[-1] * Stack[3 + Tasks[-1].StackPointer]);
0x70: Push((int) 100)
...
0x77: Push("scroll up ") // @poff=100
0x78: Pop(1); Push(Stack[-1] + Stack[2 + Tasks[-1].StackPointer]);
0x79: @ Trace(Stack[-1])
0x7a: Pop(1)
0x7b: PushEmpty()
0x7c: Call 0x21
...
0x7f: Push("scroll_button_down") // @poff=122
...
0x87: Push("scroll down ") // @poff=160
0x88: Pop(1); Push(Stack[-1] + Stack[2 + Tasks[-1].StackPointer]);
0x89: Push(" ") // @poff=18
0x8a: Pop(2); Push(Stack[-2] + Stack[-1]);
0x8b: Pop(1); Push(Stack[-1] + Stack[3 + Tasks[-1].StackPointer]);
0x8c: @ Trace(Stack[-1])
0x8d: Pop(1)
0x8e: PushEmpty()
0x8f: Call 0x21
...
0xb7: Push((int) 14)
0xb8: Pop(1); Push(Stack[2 + Tasks[-1].StackPointer] + Stack[-1]);
