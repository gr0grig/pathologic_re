...
Strings:
	...
	W:health
	W:idle
// @pool_raw:
...
0x28: Pop(0); PushNull((bool) Stack[-21] == 0)
0x29: IF (Stack[-1] == 0) GOTO 0x2f; Pop(1)

0x2a: PushEmpty(string)
0x2b: Stack[-1] = "fdie" // @poff=0
0x2c: Call 0x7e

0x2d: Pop(1)
0x2e: GOTO 0x7d

0x2f: @@ GetPosition(Stack[-10]); Obj=21 // @poff=10
0x30: Pop(0)
...
0x40: Stack[-6] = "fdie" // @poff=0
0x41: GOTO 0x43

0x42: Stack[-6] = "bdie" // @poff=22
...
0x49: @ StopAsync()
0x4a: Pop(0)
0x4b: Stack[-21] = Stack[-5]
0x4c: Push("GetScriptProperty") // @poff=32
0x4d: Push((int) 2)
0x4e: Pop(2); Push(IsFuncExist(Stack[-23], Stack[-2], Stack[-1]))
0x4f: IF (Stack[-1] == 0) GOTO 0x5b; Pop(1)

0x50: Push("Owner") // @poff=68
0x51: @@ HasScriptProperty(Stack[-5], Stack[-1]); Obj=22 // @poff=80
0x52: Pop(1)
0x53: Push(Stack[-4])
0x54: IF (Stack[-1] == 0) GOTO 0x5b; Pop(1)

0x55: Push("Owner") // @poff=68
0x56: @@ GetScriptProperty(Stack[-6], Stack[-1]); Obj=22 // @poff=98
0x57: Pop(1)
0x58: Pop(0); PushNull((bool) Stack[-5] == 0)
0x59: IF (Stack[-1] == 0) GOTO 0x5b; Pop(1)

0x5a: Stack[-21] = Stack[-5]
0x5b: Push("@GetEyesHeight") // @poff=116
0x5c: Push((int) 1)
0x5d: Pop(2); Push(IsFuncExist(Stack[-7], Stack[-2], Stack[-1]))
0x5e: IF (Stack[-1] == 0) GOTO 0x6a; Pop(1)

0x5f: @@ GetEyesHeight(Stack[-2]); Obj=5 // @poff=146
0x60: Pop(0)
0x61: Stack[-1] = CVector(0.0, 0.0, 0.0)
0x62: Push(CvectorIndex(Stack[-1], 1))
0x63: Stack[-3] = Stack[-1]
0x64: CvectorIndex(Stack[-2], 1) = Stack[-1];
0x65: Push("head") // @poff=160
...
0x72: @ StopAsync()
0x73: Pop(0)
0x74: Push("head") // @poff=160
0x75: @ UnlookAsync(Stack[-1])
0x76: Pop(1)
0x77: Push("all") // @poff=170
...
0x88: Pop(0)
0x89: Push("all") // @poff=170
0x8a: @ PlayAnimation(Stack[-1], Stack[-2])
0x8b: Pop(1)
0x8c: @ WaitForAnimEnd()
0x8d: Pop(0)
0x8e: Push("all") // @poff=170
...
0x178: Stack[-5] = Stack[-1]
0x179: Call 0x4b0

0x17a: Stack[-2] = Stack[-3]
0x17b: Pop(2)
0x17c: Push((int) 0)
...
0x18c: Stack[-5] = Stack[-1]
0x18d: Call 0x48c

0x18e: Stack[-2] = Stack[-3]
0x18f: Pop(2)
0x190: Push((int) 0)
...
0x1a6: Stack[-6] = Stack[-1]
0x1a7: Call 0x49e

0x1a8: Stack[-2] = Stack[-3]
0x1a9: Pop(2)
0x1aa: Push((int) 0)
...
0x1ba: Stack[-5] = Stack[-1]
0x1bb: Call 0x4bd

0x1bc: Stack[-2] = Stack[-3]
0x1bd: Pop(2)
0x1be: Push((int) 0)
...
0x1ce: Stack[-6] = Stack[-2]
0x1cf: Stack[-7] = Stack[-1]
0x1d0: Call 0x4c2
...
0x1dd: Stack[-5] = Stack[-2]
0x1de: Stack[-6] = Stack[-1]
0x1df: Call 0x4c5
...
0x20b: Pop(0)
0x20c: Push("all") // @poff=170
0x20d: PushEmpty(string, int)
0x20e: Stack[-7] = Stack[-1]
0x20f: Call 0x474
...
0x23c: Pop(0)
0x23d: @@@ GetPosition(Stack[-5]); Obj=0 // @poff=10
0x23e: Pop(0)
...
0x263: Push("Can't retreat, distance: ") // @poff=178
...
0x279: Push((int) 1)
0x27a: @ MovePoint(Stack[-1]T, Stack[-1], Stack[-4])
0x27b: Pop(1)
0x27c: Push(Stack[-3])
0x27d: IF (Stack[-1] == 0) GOTO 0x295; Pop(1)

0x27e: Pop(0); PushNull((bool) Stack[0 + Tasks[-1].StackPointer] == 0)
...
0x283: Stack[-1] = (float) 2.6179938316345215
0x284: Call 0x23a

0x285: Stack[-2] = Stack[-4]
0x286: Pop(2)
...
0x296: IF (Stack[-1] == 1) GOTO 0x279; Pop(1)
...
0x29d: Pop(0); PushNull((bool) Stack[0 + Tasks[-1].StackPointer] == 0)
...
0x2ac: Stack[-1] = (float) 1.7453293800354004
0x2ad: Call 0x23a
...
0x2bc: IF (Stack[-1] == 1) GOTO 0x2c2; Pop(1)

0x2bd: PushEmpty(bool)
0x2be: Call 0x2dc

0x2bf: Pop(0)
0x2c0: IF (Stack[-1] == 1) GOTO 0x2c2; Pop(1)
...
0x2e8: Pop(2)
0x2e9: Push((float)-0.3420201241970062)
...
0x2ee: Pop(0)
0x2ef: @@@ GetPosition(Stack[-5]); Obj=0 // @poff=10
0x2f0: Pop(0)
...
0x30d: Stack[-1] = (float) 1.7453293800354004
0x30e: Call 0x2ec

0x30f: Stack[-2] = Stack[-7]
0x310: Pop(2)
...
0x315: Push("Can't retreat, distance: ") // @poff=178
...
0x32b: Push((int) 1)
0x32c: @ MovePoint(Stack[-1]T, Stack[-1], Stack[-4])
0x32d: Pop(1)
0x32e: Push(Stack[-3])
...
0x335: Stack[-1] = (float) 2.6179938316345215
0x336: Call 0x2ec
...
0x348: IF (Stack[-1] == 1) GOTO 0x32b; Pop(1)
...
0x34f: Pop(0); PushNull((bool) Stack[0 + Tasks[-1].StackPointer] == 0)
...
0x35e: Stack[-1] = (float) 1.7453293800354004
0x35f: Call 0x2ec

0x360: Stack[-2] = Stack[-4]
0x361: Pop(2)
...
0x36e: IF (Stack[-1] == 1) GOTO 0x374; Pop(1)
...
0x39a: Pop(2)
0x39b: Push((float)-0.3420201241970062)
...
0x3a5: Pop(0)
0x3a6: @@ GetPosition(Stack[-1]); Obj=5 // @poff=10
0x3a7: Pop(0)
0x3a8: Stack[-6] = Stack[-1] - Stack[-2]; Pop(0);
0x3a9: Return(); Pop(4)

0x3aa: PushEmpty(bool, bool)
0x3ab: Push("HasProperty") // @poff=230
0x3ac: Push((int) 2)
...
0x3b2: @@ HasProperty(Stack[-3], Stack[-1]); Obj=4 // @poff=254
0x3b3: Pop(0)
0x3b4: Stack[-1] = Stack[-5]
0x3b5: Return(); Pop(2)

0x3b6: PushEmpty(bool, bool)
0x3b7: @@ IsDead(Stack[-1]); Obj=3 // @poff=266
0x3b8: Pop(0)
0x3b9: Stack[-1] = Stack[-4]
0x3ba: Return(); Pop(2)

0x3bb: PushEmpty(object, object, object, object)
0x3bc: Pop(0); PushNull((bool) Stack[-5] == 0)
...
0x3d0: Pop(0)
0x3d1: Pop(0); PushNull((bool) Stack[-2] == 0)
0x3d2: IF (Stack[-1] == 0) GOTO 0x3d5; Pop(1)

0x3d3: Stack[-6] = (bool) 0
0x3d4: Return(); Pop(4)

0x3d5: @@ GetScene(Stack[-1]); Obj=5 // @poff=287
0x3d6: Pop(0)
...
0x3e9: Stack[-6] = Stack[-2]
0x3ea: Stack[-1] = "noaccess" // @poff=296
0x3eb: Call 0x3aa
...
0x3f1: Push("noaccess") // @poff=296
0x3f2: @@ GetProperty(Stack[-1], Stack[-2]); Obj=4 // @poff=314
0x3f3: Pop(1)
0x3f4: Push((int) 0)
...
0x41d: @@ GetPosition(Stack[-5]); Obj=23 // @poff=10
0x41e: Pop(0)
...
0x42e: Stack[-1] = "fhit" // @poff=364
0x42f: GOTO 0x431

0x430: Stack[-1] = "bhit" // @poff=374
0x431: Push("hit_react") // @poff=384
0x432: Push("1") // @poff=404
0x433: Pop(1); Push(Stack[-3] + Stack[-1]);
0x434: Push("2") // @poff=408
...
0x46e: Stack[-7] = Stack[-3]
0x46f: Stack[-6] = Stack[-2]
