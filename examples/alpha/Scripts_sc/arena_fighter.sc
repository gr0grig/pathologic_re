...
maintask t1
{
	...
	void init(void)
	{
		...
		@Sleep(1.5);
		@FindActor(L0, "player");
		f_9b_a3_v(L0, true, 155.0);
	}

	float f_8e_a2_f(object a0, int a1)
	{
		return 0.10000000149011612;
	}

	int f_91_a2_i(object a0, int a1)
	{
		return 0;
	}

	bool f_94_a1_b(object a0)
	{
		return f_376_a1_b(a0);
	}

	void f_9b_a3_v(object a0, bool a1, float a2)
	{
		for (tv1 = 0; ; tv1++) {
			bool L0;
		...
		for (; f_94_a1_b(tv0) && !tv3; ) {
			...
			if (L8 >= (300.0 + L2) * (300.0 + L2)) {
				...
				if (L8 >= a2 * a2) {
					...
					@rand(L9);
					if (L9 < 0.6000000238418579 || f_253_a0_b()) {
						@Face(tv0);
	...
	void f_168_a2_v(float a0, int a1)
	{
		object L0;
		...
		if (L0 == tv0) {
			...
			float L2;
			L1 = f_8e_a2_f(L0, a1);
			L2 = f_331_a3_f(L0, L1, f_91_a2_i(L0, a1));
			@ReportHit(tv0, f_27e_a0_i(), L2, L1);
			f_280_a2_v(L0, L2);
		}
	}

	bool f_18f_a1_b(float a0)
	{
		int L0;
		bool L1;
		@irand(L0, tv1);
	...
	void f_280_a2_v(object a0, float a1)
	{
	}
		...
		for (; ; ) {
			...
			if (L6 > a1 * a1) {
				...
				if (L2 != null) {
					...
					@FollowPath(L2, a3, L0, f_319_a0_s(), f_31b_a0_s());
					if (!L0) {
						if (tv0) {
							L2 = null;
							break;
...
bool f_325_a2_b(object a0, string a1)
{
	bool L0;
...
float f_331_a3_f(object a0, float a1, int a2)
{
	int L0;
	string L1;
	int L2;
	...
	float L4;
	float L5;
	if (!f_325_a2_b(a0, "health")) {
		return 0.0;
	}
	if (!f_325_a2_b(a0, "armor")) {
		L0 = 0;
	} else {
		a0->GetProperty("armor", L0);
	}
	L1 = "armor_" + f_31d_a1_s(a2);
	if (!f_325_a2_b(a0, L1)) {
		L2 = 0;
	} else {
		a0->GetProperty(L1, L2);
	}
	L3 = f_3e8_a2_f((L0 + L2) / 100.0, 1);
	a0->GetProperty("health", L4);
	L5 = a1 * (1 - L3);
	a0->SetProperty("health", f_3ef_a3_f(L4 - L5, 0, 1));
	return L5;
}
...
bool f_376_a1_b(object a0)
{
	...
	if (a0->FuncExist("IsDead", 1) && f_371_a1_b(a0)) {
		return false;
	}
	@GetScene(L0);
	if (L0 == null) {
...
float f_3e8_a2_f(float a0, float a1)
{
	return a0 < a1 ? a0 : a1;
}

float f_3ef_a3_f(float a0, float a1, float a2)
{
	if (a0 < a1) {
		return a1;
	}
