		...
		for (; L1 < 5 && f_4f8_a0_b(); L1++) {
			...
			if (L2 == 0) {
				if (L0) {
					...
					bool L4;
					@irand(L3, L0);
					@PlayAnimation("all", f_50e_a1_s(L3));
					@WaitForAnimEnd(L4);
					if (!L4) {
						break;
					}
...
task t4
{
	...
	void init(object a0)
	{
		...
		while (f_491_a1_b(tv0)) {
			...
			tv0->GetPosition(L2);
			int L3;
			L3 = f_59a_a1_i(f_503_a2_f(L2, L1));
			...
			float L10;
			L10 = f_56e_a3_f(L1, L2, L5);
			if (L10 < 0) {
				L10 = f_5a8_a1_f(L3);
			} else {
				L10 = f_5b0_a2_f(L3, L10);
			}
...
int f_515_a0_i(void)
{
	...
	for (L0 = 0; ; L0++) {
		bool L1;
		@HasAnimation(L1, "all", f_50e_a1_s(L0));
		if (!L1) {
			break;
		}
	}
...
void f_544_a0_v(void)
{
	...
	@irand(L0, 4);
	if (L0 != 0) {
		@AddItem(L1, f_569_a1_i("rifle_ammo"), 0, L0);
	}
	@irand(L0, 3);
	if (L0 == 0) {
		@AddItem(L1, f_569_a1_i("rusk"), 0, 1);
	}
}

int f_569_a1_i(string a0)
{
	int L0;
...
float f_56e_a3_f(Vector a0, Vector a1, Vector a2)
{
	float L0;
	float L1;
	float L2;
...
float f_5b0_a2_f(int a0, float a1)
{
	if (a1 < 10) {
		return 10;
	} else {
...
bool f_613_a2_b(object a0, object a1)
{
	string L0;
	bool L1;
