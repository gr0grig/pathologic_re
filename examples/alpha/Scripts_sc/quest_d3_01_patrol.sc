...
task t0
{
	...
	void f_e2_a2_v(float a0, int a1)
	{
		...
		if (L0 == tv0) {
			...
			float L2;
			L1 = f_f_a2_f(L0, a1);
			L2 = f_4ea_a3_f(L0, L1, f_12_a2_i(L0, a1));
...
task t4
{
	...
	void init(object a0, object a1)
	{
		...
		while (1) {
			...
			tv0->SetMessage(9102);
			tv0->ClearReplies();
			tv0->AddReply(9103, 9989, 9982);
			tv0->AddReply(9104, 9985, 9983);
			break;
			return;
		}
		if (f_670_a0_b()) {
	...
	void evt_11(int a0, int a1)
	{
		while (1) {
			...
			if (a0 == 9992) {
				...
				tv0->SetMessage(9113);
				tv0->ClearReplies();
				tv0->AddReply(9114, 9994, 9993);
				tv0->AddReply(9116, -1, 9995);
				return;
			}
			if (a0 == 9994) {
				f_398_a1_v("Neutral");
				tv0->SetMessage(9115);
				tv0->ClearReplies();
				tv0->AddReply(9117, -1, 9997);
				tv0->AddReply(9118, 9999, 9998);
				return;
			}
			if (a0 == 9999) {
				f_398_a1_v("Neutral");
...
bool f_4de_a2_b(object a0, string a1)
{
	bool L0;
...
float f_4ea_a3_f(object a0, float a1, int a2)
{
	int L0;
	string L1;
	int L2;
	...
	float L4;
	float L5;
	if (!f_4de_a2_b(a0, "health")) {
		return 0.0;
	}
	if (!f_4de_a2_b(a0, "armor")) {
		L0 = 0;
	} else {
		a0->GetProperty("armor", L0);
	}
	L1 = "armor_" + f_4d6_a1_s(a2);
	if (!f_4de_a2_b(a0, L1)) {
		L2 = 0;
	} else {
		a0->GetProperty(L1, L2);
	}
	L3 = f_61c_a2_f((L0 + L2) / 100.0, 1);
	a0->GetProperty("health", L4);
	L5 = a1 * (1 - L3);
	a0->SetProperty("health", f_623_a3_f(L4 - L5, 0, 1));
	return L5;
}
...
void f_56b_a3_v(object a0, int a1, float a2)
{
	if (a1 != 4 && a1 != 5) {
		object L0;
		object L1;
...
bool f_5b9_a1_b(object a0)
{
	...
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * 70 + f_60e_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
...
float f_618_a2_f(Vector a0, Vector a1)
{
	Vector L0;
	L0 = a1 - a0;
	return L0 | L0;
...
float f_61c_a2_f(float a0, float a1)
{
	return a0 < a1 ? a0 : a1;
}

float f_623_a3_f(float a0, float a1, float a2)
{
	if (a0 < a1) {
		return a1;
	}
