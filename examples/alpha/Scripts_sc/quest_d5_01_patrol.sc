...
task t0
{
	...
	void f_e2_a2_v(float a0, int a1)
	{
		...
		if (L0 == tv0) {
			...
			float L2;
			L1 = f_f_a2_f(L0, a1);
			L2 = f_416_a3_f(L0, L1, f_12_a2_i(L0, a1));
...
task t3
{
	...
	int init(object a0)
	{
		...
		@CreateDialog(L0);
		L0->SetNPCName(f_594_a0_i());
		L0->SetPhoto(f_596_a0_s());
...
task t4
{
	...
	void init(object a0, object a1)
	{
		...
		while (1) {
			f_3a0_a1_v("Neutral");
			tv0->SetMessage(12541);
			tv0->ClearReplies();
			tv0->AddReply(12542, -1, 13711);
			break;
			return;
		}
		if (f_598_a0_b()) {
...
maintask t5
{
	...
	void OnAttacked(object a0)
	{
		...
		@IsPlayerActor(a0, L0);
		if (L0) {
			f_55c_a2_b("quest_d5_01", "factory_fight");
		}
	}
...
bool f_40a_a2_b(object a0, string a1)
{
	bool L0;
...
float f_416_a3_f(object a0, float a1, int a2)
{
	int L0;
	string L1;
	int L2;
	...
	float L4;
	float L5;
	if (!f_40a_a2_b(a0, "health")) {
		return 0.0;
	}
	if (!f_40a_a2_b(a0, "armor")) {
		L0 = 0;
	} else {
		a0->GetProperty("armor", L0);
	}
	L1 = "armor_" + f_402_a1_s(a2);
	if (!f_40a_a2_b(a0, L1)) {
		L2 = 0;
	} else {
		a0->GetProperty(L1, L2);
	}
	L3 = f_54a_a2_f((L0 + L2) / 100.0, 1);
	a0->GetProperty("health", L4);
	L5 = a1 * (1 - L3);
	a0->SetProperty("health", f_551_a3_f(L4 - L5, 0, 1));
	return L5;
}
...
bool f_45b_a1_b(object a0)
{
	...
	if (a0->FuncExist("IsDead", 1) && f_456_a1_b(a0)) {
		return false;
	}
	@GetScene(L0);
	if (L0 == null) {
...
void f_497_a3_v(object a0, int a1, float a2)
{
	if (a1 != 4 && a1 != 5) {
		object L0;
		object L1;
...
bool f_4e5_a1_b(object a0)
{
	...
	L3 = L3 / sqrt(L3 | L3);
	L4 = -L3;
	L5 = L3 * 70 + f_540_a1_V(L4 ^ [0.0, 1.0, 0.0]) * 25 - [0.0, 10.0, 0.0];
	L6 = L2 + L5;
	@IsOverrideActive(L7);
	if (L7) {
		return false;
...
float f_54a_a2_f(float a0, float a1)
{
	return a0 < a1 ? a0 : a1;
}
...
bool f_55c_a2_b(string a0, string a1)
{
	object L0;
	@FindActor(L0, a0);
	if (L0 == null) {
