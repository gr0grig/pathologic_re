...
def assemble_instruction(text, string_offsets, import_table, task_info, func_name_offsets=None):
    ...
    # ── CString (HD-only): CString(V1); Pop(P) ──
    m = re.match(r'CString\((.+?)\); Pop\((\d+)\)$', text)
    if m:
        ...
        if tv_in:
            task_byte |= 0x80
        return 0x8000 | 0x4D, w_uint32(var_in) + w_int8(task_byte if task_byte < 128 else task_byte - 256)

    # ── CString2 (HD-only): VarOut = CString2(V1); Pop(P) ──
    m = re.match(r'(.+?) = (?:CString2|NEW1)\((.+?)\); Pop\((\d+)\)$', text)
    if m:
        vout_str = m.group(1).strip()
        v_str = m.group(2).strip()
        pop_n = int(m.group(3))
...
def assemble(parsed, is_alpha=True):
    ...
    for addr, text in instructions:
        ...
        # HD version inserts 2 new opcodes (CString, CString2) at 0x4D-0x4E,
        # shifting Call and everything after by +2.
        # Opcodes with 0x8000 marker are HD-direct (use as-is, no shift).
        if opcode & 0x8000:
            opcode &= 0x7FFF
        elif not is_alpha and opcode >= 0x4D:
            opcode += 2
...
def main():
    ...
    parser = _ap.ArgumentParser(description='Assemble Pathologic script .asm to .bin')
    ...
    if args.folder:
        ...
        assemble_folder(args.folder[0], args.folder[1], is_alpha=is_alpha)
        return
    ...
    if args.batch or os.path.isdir(args.input):
        ...
        output_dir = args.output or input_dir.rstrip('/\\').replace('_compiled', '_assembled')
        if output_dir == input_dir:
            output_dir = input_dir.rstrip('/\\') + '_assembled'
        ok = fail = 0
        import glob as _glob
        for f in sorted(_glob.glob(os.path.join(input_dir, '**', '*.asm'), recursive=True)):
            rel = os.path.relpath(f, input_dir)
            out = os.path.join(output_dir, os.path.splitext(rel)[0] + '.bin')
            try:
                assemble_file(f, out, is_alpha=is_alpha)
                ok += 1
            except Exception as e:
                print(f"ERROR: {rel}: {e}")
                fail += 1
        ...
        out = args.output or args.input.replace('.asm', '.bin')
        try:
            assemble_file(args.input, out, is_alpha=is_alpha)
            print(f'OK: {out}')
        except Exception as e:
            print(f'ERROR: {e}')
