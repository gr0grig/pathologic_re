...
class CInstructionCString:
	"""HD-only: convert variable to char-string (opcode 0x4D in HD)."""
	def __init__(self, r):
		self.OpCode = 'CString'
		self.VarIn = r.uint32()
		self.TaskVar = r.int8()
	...
	def __repr__(self):
		...
		return f'CString({var_1}); Pop({self.PopCount})'


class CInstructionCString2:
	"""HD-only: convert variable to char-string with destination (opcode 0x4E in HD)."""
	def __init__(self, r):
		self.OpCode = 'CString2'
		self.VarIn = r.uint32()
		self.VarOut = r.uint32()
		self.TaskVar = r.int8()
		self.PopCount = self.TaskVar & 0x3F
