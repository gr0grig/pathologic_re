...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print('Usage:')
        print('  python compile.py input.c [output.asm]')
        print('  python compile.py --folder input_dir output_dir')
        sys.exit(1)

    if sys.argv[1] == '--folder':
        if len(sys.argv) < 4:
            print('Usage: python compile.py --folder input_dir output_dir')
            sys.exit(1)
        compile_folder(sys.argv[2], sys.argv[3])
    else:
        in_path = sys.argv[1]
        out_path = sys.argv[2] if len(sys.argv) > 2 else in_path.replace('.c', '.compiled.asm')
        try:
            compile_file(in_path, out_path)
            print(f'OK: {out_path}')
        except Exception as e:
